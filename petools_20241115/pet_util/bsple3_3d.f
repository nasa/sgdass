      FUNCTION   VAL_3D_BSPLE3 ( ARGS, DIMS, INDS, ARG_ARR_1, &
     &                           ARG_ARR_2, ARG_ARR_3, BCF )
! ************************************************************************
! *                                                                      *
! *   Routine VAL_3D_BSPLE  computes the value of the 3D spline          *
...
! *                                                                      *
! * <VAL_3D_BSPLE3> ( REAL*8    ) -- Value of the 3D spline at the point *
! *                            with coordinates (ARG_1, ARG_2, ARG_3 ).  *
! *       DER ( REAL*8    ) -- Vector of partial derivatives at the      *
! *                            point with coordinates                    *
! *                            (ARG_1, ARG_2, ARG_3).                    *
! *                                                                      *
! * ### 01-FEB-2008    VAL_3D_BSPLE3  v1.1 (c) L. Petrov 12-MAY-2014 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  INDS(3), DIMS(3)
      REAL*8     VAL_3D_BSPLE3 
      INTEGER*4  DEG
      PARAMETER  ( DEG = 3 )
      REAL*8     ARGS(3), &
     &           ARG_ARR_1(1-DEG:DIMS(1)), ARG_ARR_2(1-DEG:DIMS(2)), &
     &           ARG_ARR_3(1-DEG:DIMS(3)), &
     &           BCF(1-DEG:DIMS(1),1-DEG:DIMS(2),1-DEG:DIMS(3))
      REAL*8     VAL
      REAL*8     BSPL(-DEG:0,3), DSPL(-DEG:0,3)
...
!
      RETURN
      END  FUNCTION  VAL_3D_BSPLE3  !#!#
!
! ------------------------------------------------------------------------
!
      SUBROUTINE VAL_DER_3D_BSPLE3 ( ARGS, DIMS, INDS, ARG_ARR_1, &
     &                               ARG_ARR_2, ARG_ARR_3, BCF, &
     &                               VAL, DER )
! ************************************************************************
! *                                                                      *
! *   Routine VAL_DER_3D_BSPLE  computes the value of the 3D spline      *
! *   of the 3rd degree at the point with coordinates                    *
...
! *                            (ARG_1, ARG_2, ARG_3).                    *
! *                                                                      *
! * ### 01-FEB-2008 VAL_DER_3D_BSPLE3 v1.1 (c) L. Petrov 12-MAY-2014 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  INDS(3), DIMS(3)
...
!
      RETURN
      END  SUBROUTINE  VAL_DER_3D_BSPLE3  !#!#
!
! ------------------------------------------------------------------------
!
      SUBROUTINE VAL_DER_DR2_3D_BSPLE3 ( ARGS, DIMS, INDS, ARG_ARR_1, &
     &                                    ARG_ARR_2, ARG_ARR_3, BCF, &
     &                                    VAL, DER, DR2 )
! ************************************************************************
! *                                                                      *
! *   Routine VAL_DER_DR2_3D_BSPLE  computes the value of the 3D spline  *
! *   of the 3rd degree at the point with coordinates                    *
! *   (ARG_1, ARG_2, ARG_3) and a vector or partial derivatives over all *
! *                                                                      *
! *   arguments.                                                         *
...
! *   at the end is recommended to be eps*(arg[n] - arg[n-1]).           *
! *                                                                      *
! *   NB: VAL_DER_DR2_3D_BSPLE3 does not make a check of validity of     *
! *   extended knots. If the step is too big, the result will be wrong.  *
! *   If the step is too short, VAL_3D_BSPLE3_R4 will crash.             *
! *                                                                      *
! *   It is recommended to use routine BSPLE3_EXTEND for computing       *
...
! *                            point with coordinates                    *
! *                            (ARG_1, ARG_2, ARG_3).                    *
! *       DR2 ( REAL*8    ) -- Matrix of second partial derivatives at   *
! *                            the point with coordinates                *
! *                            (ARG_1, ARG_2, ARG_3).                    *
! *                                                                      *
! * ### 08-AUG-2016 VAL_DER_3D_BSPLE3 v1.0 (c) L. Petrov 08-AUG-2016 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  INDS(3), DIMS(3)
     ...
     &           ARG_ARR_3(1-DEG:DIMS(3)), &
     &           BCF(1-DEG:DIMS(1),1-DEG:DIMS(2),1-DEG:DIMS(3))
      REAL*8     VAL, DER(3), DR2(3,3)
      REAL*8     BSPL(-DEG:0,3), DSPL(-DEG:0,3), D2SPL(-DEG:0,3)
      INTEGER*4  J1, J2, J3, J4, J5, J6
...
!
      DO 410 J1=-DEG,0
         BSPL(J1,1)  = BSPLE3_VAL ( DIMS(1), ARG_ARR_1, INDS(1)+J1, ARGS(1) )
         DSPL(J1,1)  = BSPLE3_DER ( DIMS(1), ARG_ARR_1, INDS(1)+J1, ARGS(1) )
         D2SPL(J1,1) = BSPLE3_DR2 ( DIMS(1), ARG_ARR_1, INDS(1)+J1, ARGS(1) )
 410  CONTINUE 
      DO 420 J2=-DEG,0
         BSPL(J2,2)  = BSPLE3_VAL ( DIMS(2), ARG_ARR_2, INDS(2)+J2, ARGS(2) )
         DSPL(J2,2)  = BSPLE3_DER ( DIMS(2), ARG_ARR_2, INDS(2)+J2, ARGS(2) )
         D2SPL(J2,2) = BSPLE3_DR2 ( DIMS(2), ARG_ARR_2, INDS(1)+J2, ARGS(2) )
 420  CONTINUE 
      DO 430 J3=-DEG,0
         BSPL(J3,3)  = BSPLE3_VAL ( DIMS(3), ARG_ARR_3, INDS(3)+J3, ARGS(3) )
         DSPL(J3,3)  = BSPLE3_DER ( DIMS(3), ARG_ARR_3, INDS(3)+J3, ARGS(3) )
         D2SPL(J3,3) = BSPLE3_DR2 ( DIMS(3), ARG_ARR_3, INDS(3)+J3, ARGS(3) )
 430  CONTINUE 
!
! --- Compute the tensor product of B-splines
!
...
!!
               DR2(1,1) = DR2(1,1) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    D2SPL(J6,1)*BSPL(J5,2)*BSPL(J4,3)
               DR2(1,2) = DR2(1,2) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    DSPL(J6,1)*DSPL(J5,2)*BSPL(J4,3)
               DR2(1,3) = DR2(1,3) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    DSPL(J6,1)*BSPL(J5,2)*DSPL(J4,3)
!
               DR2(2,1) = DR2(2,1) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    DSPL(J6,1)*DSPL(J5,2)*BSPL(J4,3)
               DR2(2,2) = DR2(2,2) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    BSPL(J6,1)*D2SPL(J5,2)*BSPL(J4,3)
               DR2(2,3) = DR2(2,3) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    BSPL(J6,1)*DSPL(J5,2)*DSPL(J4,3)
!
               DR2(3,1) = DR2(3,1) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    DSPL(J6,1)*BSPL(J5,2)*DSPL(J4,3)
     ...
     &                    BSPL(J6,1)*DSPL(J5,2)*DSPL(J4,3)
               DR2(3,3) = DR2(3,3) + &
     &                    BCF(INDS(1)+J6,INDS(2)+J5,INDS(3)+J4)* &
     &                    BSPL(J6,1)*BSPL(J5,2)*D2SPL(J4,3)
 460        CONTINUE 
 450     CONTINUE 
 440  CONTINUE 
!
      RETURN
      END  SUBROUTINE  VAL_DER_DR2_3D_BSPLE3  !#!#
