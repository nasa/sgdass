      FUNCTION   DIAGI_BST ( DIAGI_S )
! ************************************************************************
! *                                                                      *
! *   Routine  DIAGI_BST  bids user to change the error bar style        *
! *   representation of the plot of the current colour. Field            *
! *   DIAGI_S.IBST(ICLR) is updated.                                     *
! *                                                                      *
...
! *                              internal parameters.                    *
! *                                                                      *
! *  ###  15-OCT-97    DIAGI_BST   v1.3  (c) L. Petrov  08-AUG-2002 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'diagi.i'
      ...
      INTEGER*4  DIAGI_BST
      INTEGER*4  ICLR, IBST, ILST, IPST, IWST, MP, NSP, NPTS
...
!
      IF ( IBST .GT. MBST-1 ) IBST = MBST-1
      IF ( IBST .LT. 0      ) IBST = 0
!
      CALL CLRCH ( MES )
      MES = 'Select error bar style'
      CALL PGSAVE ! 1
!
...
!
      XC = BOX(IBST+1)%XLB + ( BOX(IBST+1)%XTU - BOX(IBST+1)%XLB )/3.
      YC = BOX(IBST+1)%YLB + ( BOX(IBST+1)%YTU - BOX(IBST+1)%YLB )/3.
!
...
!
      IF ( CH .EQ. '0'  .OR.  CH .EQ. '1'  .OR. &
     &     CH .EQ. '2'  .OR.  CH .EQ. '3'  .OR.  CH .EQ. '4' ) THEN
           CALL CHIN ( CH, IBST )
           DIAGI_BST = 1
         ELSE IF ( CH .NE. 'X' ) THEN
           IDI = DIAGI_INBOX ( MBST, BOX, XC, YC )
           IF ( IDI .GT. 0 ) THEN
                IBST = IDI - 1
                DIAGI_BST = 1
           END IF
