...
! *                                                                      *
! * ### 12-JAN-2007  SUR_ASTRO_SEQ  v1.27 (d) L. Petrov  28-APR-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           SLEW_SRC(SUR__M_SOU), IND_SCORE(SUR__M_SOU), SLEW_TIME_MAX
      REAL*8     TAI_OBS, AZ, EL, HA, AZ_END, EL_END, HA_END, ALP, DEL, A, B, &
     ...
     &           DIST_MIN_RECENT, TIM_LAS2, A_LAST, B_LAST, DIF_A, DIF_B, &
     &           DIF_EL, DIF_DEL, DIF_AZ, TIM_DIF_BEST, TIM_DIF_NRML
      REAL*8     AZ_STA(SUR__M_STA), EL_STA(SUR__M_STA), HA_STA(SUR__M_STA), &
     &           EL_SCAN_MIN, DIF_HA, PAR1, PAR2, PAR3, PAR4, ARG, &
     &           ARC_MIN, ARC_MAX, DUR_SES, DUR_SCA
      REAL*8     PRI, DUR, TIME_FROM_START, ARC, SLEW_TIME_STA(SUR__M_STA), &
     &           GAP_MIN, GAP_NOR, SUR_SOU_DUR, TEMP_VAR(2), TC, TW
      REAL*8     TAPE_CHANGE_MIN_TIME, GNSS_FULL_SBR
      PARAMETER  ( TAPE_CHANGE_MIN_TIME = 120.001D0   ) ! Mimimum change tape time
      PARAMETER  ( GNSS_FULL_SBR        = 1.0D0/1.5D0 ) ! Downweight factor for subnetworking in GNSS_03 mode
     ...
     &           IND_SRC_CAL, M_TRY, SPL_STATUS, IND_SOU_SPC, IVAL, NSCA_RECENT, &
     &           IER
...
!
      ...
      IF ( STR == 'YES' .OR. STR == 'yes' ) THEN
           FL_LAST_SCA_EXT = .TRUE.
           IF ( IVRB > 0 ) THEN
                WRITE ( 6, * ) 'SUR_ASTRO_SEQ: last scan extension is prohibited'
           END IF
      END IF
      CALL GETENVAR ( 'SUR_SKED_NSCA_RECENT', STR )
      IF ( ILEN(STR) > 0 ) THEN
           CALL CHIN ( STR, NSCA_RECENT )
      END IF
!
...
!
 ...
 110          FORMAT ( A, I5,' ',A,$ )
     ...
     &          SUR%ALGORITHM == 'ASTROMET_07'      ) .AND. J1 == 2 ) THEN
              DO 420 J2=1,SUR%L_SOU
                 SUR%NOBS_SRC(J2) = SUR%NOBS_SRC(J2) + SUR%SOU(J2)%NOBS
 420          CONTINUE 
              DO 430 J3=1,SUR%L_SO2
                 SUR%NOBS_SO2(J3) = SUR%NOBS_SO2(J3) + SUR%SO2(J3)%NOBS
 430          CONTINUE 
...
!
     ...
     &                      SUR%ALGORITHM == 'GNSS_03'            ) THEN
                    IF ( SUR%NOBS_SRC(J11) > 0 ) THEN
                         ...
                         END IF
                    END IF
                    IF ( SUR%ALGORITHM == 'GNSS_03' ) THEN
!
! ---------------------- Downweight this scan since it is at the incomplete network
!
                         IF ( SUR%K_STA < SUR%L_STA ) THEN
                              SCORE(J11) = GNSS_FULL_SBR*SCORE(J11)
                         END IF 
                    END IF
!
! ----------------- Determine DIST_MIN -- minimal distance between the current J11-th sources
! ----------------- and every source that was observed in this session.
!
                    DIST_MIN = PI2
                    DIST_MIN_RECENT = PI2
     ...
     &                      SUR%TROPO_BURST_INTERVAL > 0.0D0          ) GOTO 4150
                       ...
                       END IF
                       IF ( CUR_TYP == SUR__TYP_TAG ) THEN
                            IF ( J15 .GT. SUR%L_SCN - NSCA_RECENT ) THEN
                                 IF ( DIST < DIST_MIN_RECENT ) THEN
                                      DIST_MIN_RECENT = DIST
                                 END IF
                            END IF
                         ELSE
                            IF ( J15 .GT. SUR%L_SCN - NSCA_RECENT/2 ) THEN
                                 IF ( DIST < DIST_MIN_RECENT ) THEN
                                      DIST_MIN_RECENT = DIST
                                 END IF
                            END IF
 ...
 4150               CONTINUE 
                    ...
                    IF ( SUR%ALGORITHM == 'GEODETIC_01' ) THEN
                       ...
                       ELSE IF ( SUR%ALGORITHM == 'GEODETIC_03' ) THEN
                         ARC_MAX = 1.05 ! 60.0  deg
                       ELSE IF ( SUR%ALGORITHM == 'GEODETIC_04' ) THEN
                         ARC_MAX = 1.05 ! 60.0  deg
                       ELSE IF ( SUR%ALGORITHM == 'GNSS_01' ) THEN
                         ARC_MAX = 0.52D0 ! 30.0  deg
                       ELSE IF ( SUR%ALGORITHM == 'GNSS_02' ) THEN
                         ARC_MAX = 0.26D0 ! 10.0  deg
                       ELSE IF ( SUR%ALGORITHM == 'GNSS_03' ) THEN
                         ARC_MAX = 0.10D0 ! 5.7 deg
                    END IF
                    IF ( FL_GEO_TEST ) THEN
                         ARC_MAX = 10.0D0
                    ...
                    END IF
                    IF ( SUR%ALGORITHM == 'GNSS_03'            ) THEN
                         IF ( DIST_MIN_RECENT < ARC_MAX ) THEN             
                              SCORE(J11) = 0.0D0
                         END IF
                         SCORE(J11) = SCORE(J11)*((DIST_MIN_RECENT + 0.01D0)/ARC_MAX)**0.25
                      ELSE
                         IF ( DIST_MIN < ARC_MAX ) THEN
                              SCORE(J11) = SCORE(J11)*((DIST_MIN + 0.01D0)/ARC_MAX)**4
                         END IF
                         IF ( SUR%ALGORITHM == 'GEODETIC_04' ) THEN
                              SCORE(J11) = SCORE(J11)*((DIST_MIN_RECENT + 0.01D0)/ARC_MAX)**4
                         END IF
                    END IF
                  ELSE IF ( ( SUR%ALGORITHM == 'ASTROMET_03' .OR. &
     ...
     &                               SUR%SOU(J11)%EL_MIN/DEG__TO__RAD, SLEW_TIME_MAX, DIST_MIN_RECENT
 212                FORMAT ( 8X, ' Source: ',A, ' Score: ', 1PD11.4, &
     &                           ' Tim_last: ', 0PF6.2, ' Fut: ', 0PF5.2, &
     &                           ' Ons: ', 0PF5.2, ' PRI: ', 0PF9.1, ' # ', I1, &
     &                           ' J11= ',I5, 1X, A, 1X,' Sou_el_min: ', F5.1, &
     &                           ' deg Slew: ', F5.1, ' Dist_min= ', F5.3, ' rad'  )
     ...
     &                               SUR%SO2(J11)%EL_MIN/DEG__TO__RAD, SLEW_TIME_MAX, DIST_MIN
               END IF
 4110       CONTINUE
!
...
!
 ...
 216             FORMAT ( 'Scan: ', I4,' Nobs: ', 4(I4,1X), '  ', A, &
...
!
            IF ( CUR_TYP == SUR__TYP_TAG ) THEN
                 IF ( EL < SUR%SOU(IND_SRC)%EL_MIN ) GOTO 4190
              ELSE IF ( CUR_TYP == SUR__TYP_SEC ) THEN
                 IF ( EL < SUR%SO2(IND_SRC)%EL_MIN ) GOTO 4190
              ELSE IF ( CUR_TYP == SUR__TYP_CAL ) THEN
                 IF ( EL < SUR%CAL(IND_SRC)%EL_MIN ) GOTO 4190
            END IF
!
! --------- Check visibility of a source at the J19th station at
! --------- scan start
!
     ...
     &                                 HA, IER ) ) THEN
            ...
            END IF
            IF ( CUR_TYP == SUR__TYP_TAG ) THEN
                 DUR_SCA = SUR%SOU(IND_SRC)%DUR
               ELSE IF ( CUR_TYP == SUR__TYP_SEC ) THEN
                 DUR_SCA = SUR%SO2(IND_SRC)%DUR
               ELSE
                 DUR_SCA = 0.0
            END IF
!
...
! --------- scan end
!
            CALL SUR_AZEL ( SUR, VTD, CUR_TYP, MJD_OBS, TAI_OBS + SUR%STA(J19)%PREOB + DUR_SCA, &
     &                      J19, IND_SRC, AZ_END, EL_END, HA_END, IER )
            IF ( .NOT. SUR_CHECK_VIS ( SUR, J19, CUR_TYP, IND_SRC, AZ_END, EL_END, &
     &                                 HA_END, IER ) ) THEN
                 GOTO 4190          
            END IF
!
...
!
     ...
     &                      SLEW_TIME_MAX, SUR%NOBS_SRC(IND_SRC)
              WRITE ( 6, * ) ' '
         END IF
         IF ( IVRB .GE. 5 ) THEN
 ...
 180          FORMAT ( 'SCAN ', I4, ' Obs_beg: ', A, ' Obs_end: ', A, ' Tag_type: ', A )
     ...
     &                    'in search for  phase calibrator' )
         ...
         END IF
!
         CALL GETENVAR ( 'SUR_SKED_NOFILL', STR )
         CALL TRAN     ( 11, STR, STR )
         IF ( SUR%ALGORITHM == 'GNSS_03' .AND. &
     &        K_STA .LE. SUR%L_STA -2    .AND. &
     &        STR(1:1) .NE. 'Y'                ) THEN
              CALL ERR_PASS      ( IUER, IER )
              CALL SUR_ASTRO_FILL ( SUR, VTD, IVRB, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 1736, IUER, 'SUR_ASTRO_SEQ', 'Error in '// &
     &                 'attempt to fill the scan' )
                   RETURN
              END IF
         END IF
 410  CONTINUE
 810  CONTINUE
      SUR%SCAN_TYPE(SUR%L_SCN) = SUR__LAST
!
      CALL GETENVAR ( 'SUR_SKED_NOSORT', STR )
      CALL TRAN     ( 11, STR, STR )
      IF ( SUR%ALGORITHM == 'GNSS_03' .AND. STR(1:1) .NE.  'Y' ) THEN
!
! -------- Sort the schedule in time order
!
           CALL ERR_PASS ( IUER, IER )
           CALL SUR_SORT ( SUR, IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 1737, IUER, 'SUR_ASTRO_SEQ', 'Failure in '// &
     &              'an attempt to sort the schedule' )
                RETURN
           END IF
      END IF
      IF ( IVRB > 0 ) THEN
