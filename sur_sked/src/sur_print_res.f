...
! *                                                                      *
! * ### 11-OCT-2005  SUR_PRINT_RES v2.41 (d) L. Petrov  31-MAR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           HDS_PAT*16, HDS_PAT_DEF*16, STA_NAM*8, STA_LST*8, STA_LIST_STR*4096
...
!
     ...
     &           NVEX_HEAD, NKEY_HEAD, LAST_SCA(SUR__M_STA), ID, &
...
!
     ...
     &                       BUF_VEX_HEAD(J1)(IP+ILEN('__@DURATION@__'):)
         END IF
!
         HDS_PAT_DEF = '__@HDS@__'
         IP = INDEX ( BUF_VEX_HEAD(J1), TRIM(HDS_PAT_DEF) )
         IF ( IP > 0 ) THEN
              BUF_VEX_HEAD(J1) = BUF_VEX_HEAD(J1)(1:IP-1)// &
     &                           TRIM(SUR%HARDWARE_SETUP_NAMS(K1))// &
     &                           BUF_VEX_HEAD(J1)(IP+ILEN(HDS_PAT_DEF):)
         END IF
...
!
      STA_LIST_STR = ' '
      DO 590 J9=1,SUR%L_STA
         STA_LIST_STR = TRIM(STA_LIST_STR)//', '//SUR%STA(J9)%NAME 
 590  CONTINUE 
      STA_LIST_STR = STA_LIST_STR(2:)
!
...
!
         ...
         END IF
!
         IP = INDEX ( BUF_KEY_HEAD(J2), '__@STA_LIST@__' )
         IF ( IP > 0 ) THEN
              BUF_KEY_HEAD(J2) = BUF_KEY_HEAD(J2)(1:IP-1)//TRIM(STA_LIST_STR)// &
     &                           BUF_KEY_HEAD(J2)(IP+LEN('__@STA_LIST@__'):)
         END IF
...
!
      DO 430 J3=1,SUR%L_STA
         IF ( SUR%STA(J3)%NAME == 'ATCA-104' .OR. &
     &        SUR%STA(J3)%NAME == 'ATCAP104' .OR. &
     &        SUR%STA(J3)%NAME == 'CEDUNA  ' .OR. &
     &        SUR%STA(J3)%NAME == 'MOPRA   ' .OR. &
     &        SUR%STA(J3)%NAME == 'WARK12M ' .OR. &
     &        SUR%STA(J3)%NAME == 'ASKAP   '      ) THEN
!
              FL_LBA_KEY = .TRUE.
         END IF
         IF ( SUR%STA(J3)%NAME == 'TORUN   ' .OR. &
     &        SUR%STA(J3)%NAME == 'YEBES40M' .OR. &
     &        SUR%STA(J3)%NAME == 'NOTO    '      ) THEN
!
              FL_EVN_KEY = .TRUE.
         END IF
         IF ( INDEX ( SUR%STA(J3)%NAME, 'VLBA'    ) > 0 .OR. &
     &        INDEX ( SUR%STA(J3)%NAME, 'PIETOWN' ) > 0      ) THEN
              FL_VLBA_KEY = .TRUE.
         END IF
...
!
         IF ( SUR%STA(J3)%MOUNT_TYPE == MT__ALTAZ ) THEN
            ...
            ELSE IF ( SUR%STA(J3)%MOUNT_TYPE == MT__XY_N  .OR. &
     &                SUR%STA(J3)%MOUNT_TYPE == MT__XY_E       ) THEN
              AXIS_NAME(1) = 'X-axis'
              AXIS_NAME(2) = 'Y-axis'
         END IF
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A)' ) '#'
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A)' ) 'Station_parameters:         '//SUR%STA(J3)%NAME// &
     &                            '  Short_name:    '//SUR%STA(J3)%SHORT_NAME
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,A)' ) &
     &                    '  Last_update:              ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%LAST_UPDATE
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,1X,F13.3,1X,F13.3,1X,F13.3,A)' ) &
     &                    '  Coordinates:              ', SUR%STA(J3)%NAME, SUR%STA(J3)%COO_TRS, ' meter'
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,A)' )                         &
     &                    '  Mount:                    ', SUR%STA(J3)%NAME, TRIM(SUR%STA(J3)%MOUNT_TYPE)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,2X,4(F6.1,1X),1X,A,2X,A,A)' ) &
     &                    '  1st_axis_range:           ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%AZ_RANGE/DEG__TO__RAD, &
     &                                                  'deg', 'Axis: ', AXIS_NAME(1)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,2X,2(F6.1,1X),1X,A,16X,A,A)' ) &
     &                    '  2nd_axis_range:           ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%EL_MIN/DEG__TO__RAD, &
     &                                                  SUR%STA(J3)%EL_MAX/DEG__TO__RAD, &
     &                                                  'deg', 'Axis: ', AXIS_NAME(2)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,17X,A,A)' ) &
     &                    '  1st_axis_slewing_rate:    ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%SLEW_RATE_AZ/DEG__TO__RAD, &
     &                                                  'deg/sec', 'Axis: ', AXIS_NAME(1)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,17X,A,A)' ) &
     &                    '  2nd_axis_slewing_rate:    ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%SLEW_RATE_EL/DEG__TO__RAD, &
     &                                                  'deg/sec', 'Axis: ', AXIS_NAME(2)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,15X,A,A)' ) &
     &                    '  1st_axis_slewing_accl:    ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%SLEW_ACCL_AZ/DEG__TO__RAD, &
     &                                                  'deg/sec^2', 'Axis: ', AXIS_NAME(1)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,15X,A,A)' ) &
     &                    '  2nd_axis_slewing_accl:    ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%SLEW_ACCL_EL/DEG__TO__RAD, &
     &                                                  'deg/sec^2', 'Axis: ', AXIS_NAME(2)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,17X,A,A)' ) &
     &                    '  1st_axis_settle_time:     ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%TIME_SETTLE_AZ, &
     &                                                  'sec', '    Axis: ', AXIS_NAME(1)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.2,4X,A,17X,A,A)' ) &
     &                    '  2nd_axis_settle_time:     ', SUR%STA(J3)%NAME, &
     &                                                  SUR%STA(J3)%TIME_SETTLE_EL, &
     &                                                  'sec', '    Axis: ', AXIS_NAME(2)
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.1,4X,A)' ) '  Preob_proc_duration:      ', SUR%STA(J3)%NAME, SUR%STA(J3)%PREOB,           'sec'
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,F5.1,4X,A)' ) '  Postob_proc_duration:     ', SUR%STA(J3)%NAME, SUR%STA(J3)%POSTOB,          'sec'
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,3X,A)' )         '  Recorder:                 ', SUR%STA(J3)%NAME, SUR%STA(J3)%RECORDER
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,2X,F6.3,4X,A)' ) '  Recording_rate:           ', SUR%STA(J3)%NAME, SUR%RECORDING_RATE/1.D9,     'Gbps'
         NA = NA + 1 ; WRITE ( UNIT=AST_BUF(NA), FMT='(A,A,2X,F6.1,4X,A)' ) '  Setmode_duration:         ', SUR%STA(J3)%NAME, SUR%STA(J3)%SETMODE,         ' '
 430  CONTINUE
!
      IS_TAG = 1
...
!
     ...
     &     SUR%ALGORITHM == 'GNSS_03'          ) THEN
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) " "
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "! =========================================================="
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "! ====================  Satellite orbit definitions ========"
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "! =========================================================="
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) " "
!
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "satinit /"
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "   kerfile = @@@@"
           DO 440 J4=1,SUR%L_SAT
              ID = 1
              WRITE ( UNIT=LUN_KEY, FMT=208 ) SUR%C_SAT_ABBR(J4), SUR%C_SAT_NORAD(J4), &
     &                                        TRIM(SUR%C_TLE_FILE(J4)(ID:))
 208          FORMAT ( "   satname='",A,"'  satnum= ", A5, " tlefile= ", A, " /" )
 440       CONTINUE 
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) "endsat /"
           WRITE ( UNIT=LUN_KEY, FMT='(A)' ) " "
      END IF
...
!              END IF
     ...
     &                          MODE_FLUX_CALIB(1:I_LEN(MODE_FLUX_CALIB))//';'
                     ELSE IF ( SUR%SCAN_TYPE(J5) == SUR__FIRST .OR. &
     &                         J5 == 1 ) THEN
                        WRITE ( UNIT=LUN_VEX, FMT='(A)' ) '    mode='// &
     &                          MODE_START(1:I_LEN(MODE_START))//';'
                      ELSE IF ( SUR%SCAN_TYPE(J5) == SUR__SHORT ) THEN
...
!
     ...
     &                                        '  GBT_COLD'
                         ELSE 
!@                           IF ( SUR%STA(J13)%NAME(4:4) .NE. '_' .AND. ILEN(SUR%STA(J13)%NAME) > 4 ) THEN
!@                                STR_KEY_STATIONS = STR_KEY_STATIONS(1:I_LEN(STR_KEY_STATIONS))// &
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 24-NOV-2012 SUR_SOU_OUT   v1.2 (d)  L. Petrov  13-JUN-2016 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
!
     ...
     &         SOU(J3)%DELTA_STR(5:6)//":"// &
     &         SOU(J3)%DELTA_STR(8:12)// &
     &         "0  EQUINOX='J2000' // "//SUR__TYP_STR(I_TYP)//'  '//SOU(J3)%J2000_NAME
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 09-JAN-2018    UPROUND    v1.0 (d)  L. Petrov  09-JAN-2018 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      REAL*8     UPROUND
