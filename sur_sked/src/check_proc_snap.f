...
! *                                                                      *
! * ### 26-JAN-2022  CHECK_PROC_SNAP  v1.2 (d) L. Petrov 25-FEB-2024 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  MBUF
...
!
      IUER = -1 
      CALL CHECK_SNAP ( NERS, NS, BUFS, FILS, IVRB, IUER )
      IF ( IVRB .GE. 1 ) THEN
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 26-JAN-2022  CHECK_VERS   v1.1 (d)  L. Petrov  12-FEB-2024 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 26-JAN-2022 CHECK_PROC_DEF v1.3 (d)  L. Petrov 25-NOV-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INTEGER*4  NP, NS, IVRB, IUER 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 26-JAN-2022  CHECK_WAIT  v1.0 (d)  L. Petrov  26-JAN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      CHARACTER  COM*(*), MESS*(*)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 28-JAN-2022   FS_TO_DATE  v1.0 (d)  L. Petrov  28-JAN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      CHARACTER  STR*(*)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 27-JAN-2022   CHECK_AZEL  v1.0 (d)  L. Petrov  27-JAN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 28-JAN-2022  HOR_MASK_OK  v1.0 (d)  L. Petrov  28-JAN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 28-JAN-2022    AZLIM_OK   v1.0 (d)  L. Petrov  28-JAN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 28-JAN-2022  GET_SLEW_TIME  v1.0 (d) L. Petrov  28-JAN-2022 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
! ------------------------------------------------------------------------
!
      SUBROUTINE CHECK_SNAP ( NERS, NS, BUFS, FILS, IVRB, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine CHECK_SNAP checks consitensy of a VLBI schedule in snap    *
! *   format.                                                            *
! *                                                                      *
! *   Copyright (c) 1975-2025 United States Government as represented by *
! *   the Administrator of the National Aeronautics and Space            *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 22-APR-2026  CHECK_SNAP   v1.0 (d)  L. Petrov  22-APR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
      ...
      INTEGER*4  MIND
      PARAMETER  ( MIND = 64 )
      CHARACTER  SOU_NAM*8, SCAN_NAM*10, GEN_SNAP_PROG*32, STR*128
      INTEGER*4  IND_SETMODE, IND_SETSCAN, IND_PREOB, IND_MIDOB, &
     &           IND_POSTOB, IND_POSTSES, IND_FIRST_SETMODE, &
     &           IND_SCANNAME
      REAL*8     UTC_M_TAI
      INTEGER*4  LIND, IND(2,MIND), N_SCA, MJD_PREOB, MJD_POSTOB, &
     &           IS, J1, J2, J3, J4, IER
...
!
      ...
      END IF
      UTC_M_TAI = 0.0D0
      CALL CLRCH ( GEN_SNAP_PROG )
      N_SCA = 0
      IND_FIRST_SETMODE = 0
      DO 410 J1=1,NS
         CALL EXWORD ( BUFS(J1), MIND, LIND, IND, ' ', IER )
         IF ( LIND > 3 ) THEN
              IF ( BUFS(J1)(IND(1,1):IND(2,1)) == '"'         .AND. &
     &             BUFS(J1)(IND(1,2):IND(2,2)) == "Generated" .AND. &
     &             BUFS(J1)(IND(1,3):IND(2,3)) == "with"            ) THEN
                   GEN_SNAP_PROG = BUFS(J1)(IND(1,4):IND(2,4))
              END IF
         END IF
         IF ( BUFS(J1)(IND(1,1):IND(2,1)) .NE. '"' .AND. &
     &        ILEN(GEN_SNAP_PROG) == 0                   ) THEN
              CALL ERR_LOG ( 1411, IUER, 'CHECK_SNAP', 'Did not find '// &
     &            'a generator comment in the header of snap file '// &
     &             FILS )
              RETURN 
         END IF
         IF ( GEN_SNAP_PROG == 'vex_to_snap.py' .OR. &
     &        GEN_SNAP_PROG == 'sds_to_snap'         ) THEN
              CONTINUE 
            ELSE IF ( ILEN(GEN_SNAP_PROG) > 0 ) THEN
              CALL ERR_LOG ( 1412, IUER, 'CHECK_SNAP', 'Header of '// &
     &            'snap file '//TRIM(FILS)//' has an unsupported '// &
     &            'generator '//GEN_SNAP_PROG )
              RETURN 
         END IF
!
         IF ( BUFS(J1)(1:10) == 'scan_name=' ) THEN
              IF ( N_SCA > 0 .AND. GEN_SNAP_PROG == 'vex_to_snap.py' ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   IF ( IND_FIRST_SETMODE == 0  ) THEN
                        CALL ERR_LOG ( 1413, IUER, 'CHECK_SNAP', 'setmode '// &
     &                      'procedure was not defined in the first '// &
     &                      'scan of snap file '//FILS )
                        RETURN 
                   END IF
                   IF ( IND_SETSCAN == 0 ) THEN
                        CALL ERR_LOG ( 1414, IUER, 'CHECK_SNAP', 'Error in '// &
     &                      'line '//TRIM(STR)//' of snap file '//TRIM(FILS)// &
     &                      ' -- setscan command was not used for this scan' )
                        RETURN 
                  END IF
                  IF ( IND_PREOB == 0 ) THEN
                        CALL ERR_LOG ( 1415, IUER, 'CHECK_SNAP', 'Error in '// &
     &                      'line '//TRIM(STR)//' of snap file '//TRIM(FILS)// &
     &                      ' -- preob command was not used for this scan' )
                        RETURN 
                  END IF
                  IF ( IND_MIDOB == 0 ) THEN
                        CALL ERR_LOG ( 1416, IUER, 'CHECK_SNAP', 'Error in '// &
     &                      'line '//TRIM(STR)//' of snap file '//TRIM(FILS)// &
     &                      ' -- midob command was not used for this scan' )
                        RETURN 
                  END IF
                  IF ( IND_POSTOB == 0 ) THEN
                        CALL ERR_LOG ( 1417, IUER, 'CHECK_SNAP', 'Error in '// &
     &                      'line '//TRIM(STR)//' of snap file '//TRIM(FILS)// &
     &                      ' -- postob command was not used for this scan' )
                        RETURN 
                  END IF
              END IF
              IND_SETMODE  = 0 
              ...
              IND_SCANNAME = J1
              N_SCA = N_SCA + 1
           ELSE IF ( BUFS(J1)(1:8) == 'setmode_' ) THEN
              IND_SETMODE = J1
              IF ( IND_FIRST_SETMODE == 0 ) THEN
                   IND_FIRST_SETMODE = J1
              END IF 
           ELSE  IF ( BUFS(J1)(1:8) == 'setscan_' ) THEN
              IND_SETSCAN = J1
           ELSE  IF ( BUFS(J1)(1:6) == 'preob_' ) THEN
              IND_PREOB   = J1
           ELSE  IF ( BUFS(J1)(1:6) == 'midob_' ) THEN
              IND_MIDOB   = J1
           ELSE  IF ( BUFS(J1)(1:7) == 'postob_' ) THEN
              IND_POSTOB  = J1
           ELSE  IF ( BUFS(J1)(1:8) == 'postses_' ) THEN
              IND_POSTSES = J1
         END IF
 410  CONTINUE 
      IF ( IND_POSTSES == 0 .AND. GEN_SNAP_PROG == 'vex_to_snap.py' ) THEN
           CALL ERR_LOG ( 1418, IUER, 'CHECK_SNAP', 'postses command '// &
     &         'was not found at the end of snap file '//FILS )
           RETURN 
      END IF
!
      CALL ERR_LOG ( 0, IUER ) 
      RETURN
      END  SUBROUTINE   CHECK_SNAP  !#!  
