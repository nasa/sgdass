...
! *                                                                      *
! * ### 10-OCT-2005   SUR_SKED_CONF   v3.7 (d) L. Petrov 06-JUL-2023 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
     ...
     &              'to load apriori EOP data using NERS' )
                RETURN
           END IF
      END IF
!
...
!
      ...
      CALL ERR_PASS   ( IUER, IER )
      IF ( IVRB .GE. 6 ) THEN
           WRITE ( 6, * ) 'SUR_SKED: processing the primary source list '//TRIM(SOURCE_FILE)
      END IF
      SUR%L_SAT = 0
...
!
     ...
     &        SUR%ALGORITHM .NE. 'GEODETIC_03' .AND. &
     &        SUR%ALGORITHM .NE. 'GEODETIC_04'       ) THEN
!
...
!
      IF ( IVRB .GE. 6 ) THEN
           WRITE ( 6, * ) 'SUR_SKED: processing the secondary source list '//TRIM(SEC_SOURCE_FILE)
      END IF
      CALL ERR_PASS   ( IUER, IER )
