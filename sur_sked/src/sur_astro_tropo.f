...
! *                                                                      *
! * ### 15-JAN-2007  SUR_ASTRO_TROPO  v2.8 (d) L. Petrov 31-MAR-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           TAI_0, SLEW_TIME_MIN, EL_MIN, EL_MAX
      REAL*8     OBS_STA_SAVE(SUR__M_STA,SUR__M_SCN)
      ...
      IF ( SUR%TROPO_RANGE == 0 ) THEN
        ...
        ELSE IF ( SUR%TROPO_RANGE == 1 ) THEN
         ...
         ELSE IF ( SUR%TROPO_RANGE == 19 ) THEN
           ...
           EL_MAX_TROPO  = 88.0D0*DEG__TO__RAD
           L_RANGE = 4
         ELSE IF ( SUR%TROPO_RANGE == 20 ) THEN
           EL_RANGE(1,1) = 10.0D0*DEG__TO__RAD; EL_RANGE(2,1) = 50.0D0*DEG__TO__RAD
           EL_RANGE(1,2) = 50.0D0*DEG__TO__RAD; EL_RANGE(2,2) = 87.0D0*DEG__TO__RAD
           EL_RANGE(1,3) = 10.0D0*DEG__TO__RAD; EL_RANGE(2,3) = 50.0D0*DEG__TO__RAD
           EL_RANGE(1,4) = 50.0D0*DEG__TO__RAD; EL_RANGE(2,4) = 87.0D0*DEG__TO__RAD
           TROPO_LEN(1)  = SUR%TROPO_SCAN_LEN
           TROPO_LEN(2)  = SUR%TROPO_SCAN_LEN
           TROPO_LEN(3)  = SUR%TROPO_SCAN_LEN
           TROPO_LEN(4)  = SUR%TROPO_SCAN_LEN
           EL_MIN_TROPO  = 10.0D0*DEG__TO__RAD
           EL_MAX_TROPO  = 87.0D0*DEG__TO__RAD
           L_RANGE = 2
...
!
     ...
     &                        MAX(0.0D0, SUR%PREOBS_LONG)
         ...
         IF ( TAI_TROPO(J2) > 86400.0D0 ) THEN
              ...
              MJD_TROPO(J2) = MJD_TROPO(J2) + 1
         END IF
         STR = MJDSEC_TO_DATE ( MJD_TROPO(J2), TAI_TROPO(J2), IER )
         IF ( IVRB .GE. 5 ) THEN
...
!@!
!@           IF ( IVRB .GE. 6 ) THEN
!@                STR1 = MJDSEC_TO_DATE ( SUR%MJD_CUR,  SUR%TAI_CUR,  IER )
!@                STR2 = MJDSEC_TO_DATE ( SUR%MJD_STOP, SUR%TAI_STOP, IER)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 15-JAN-2007  SUR_TROPO_GET_SLEW v1.1 (d) L. Petrov 19-JAN-2008 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
!
                   IF ( INDEX ( SUR%STA(J2)%NAME, 'VLBA'     ) > 0 .OR. &
     &                  INDEX ( SUR%STA(J2)%NAME, 'PIETOWN ' ) > 0      ) THEN
!
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * # 16-JAN-2007 SUR_TROPO_SCAN_UPDATE v1.3 (d) L. Petrov 22-JAN-2018 # *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 17-JAN-2007  SUR_CUR_SAVE  v1.0 (d)  L. Petrov 17-JAN-2007 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 17-JAN-2007   SUR_CUR_RESTORE  v1.1 (d)  L. Petrov 12-DEC-2016 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 15-JAN-2007  SUR_POCAL_GBT_4HR v2.3 (d) L. Petrov 27-NOV-2010 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 23-NOV-2012   SUR_POCAL_KVN   v1.0 (d) L. Petrov 27-NOV-2012 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 13-SEP-2014  SUR_POCAL_ATCA   v1.0 (d) L. Petrov 27-NOV-2012 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
