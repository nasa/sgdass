...
! *                                                                      *
! * ### 12-JAN-2007 SUR_ASTRO_FIRST  v1.10 (d) L. Petrov 20-FEB-2024 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &             SUR__EL_OPT_FIRST, SUR__AZ_OPT_FIRST
      PARAMETER  ( SUR__EL_MIN_FIRST =  12.0D0*DEG__TO__RAD )
...
!
     ...
     &             SUR%STA(1)%NAME(1:8) == 'HOBART13'       ) THEN
           ...
           SUR__AZ_MAX_FIRST =  170.0D0*DEG__TO__RAD
           SUR__AZ_OPT_FIRST =   30.0D0*DEG__TO__RAD
         ELSE IF ( SUR%STA(1)%NAME(1:8) == 'KOKEE  ' ) THEN
           SUR__AZ_MIN_FIRST =  -40.0D0*DEG__TO__RAD
           SUR__AZ_MAX_FIRST =   50.0D0*DEG__TO__RAD
           SUR__AZ_OPT_FIRST =   30.0D0*DEG__TO__RAD
         ELSE
           SUR__AZ_MIN_FIRST =  40.0D0*DEG__TO__RAD
...
!
     ...
     &        SUR%ALGORITHM == 'GNSS_03'      ) THEN
              IF ( IVRB .GE. 6 ) THEN
                   WRITE ( 6, * ) 'SUR_ASTRO_FIRST: ', SUR%SOU(J1)%J2000_NAME,  SNGL((SUR%MJD_CUR - SUR%SOU(J1)%MJD_EPOCH)*86400.0D0 + &
     &                                                 (SUR%TAI_CUR + SUR%SOU(J1)%DUR/2.0D0 - SUR%SOU(J1)%TAI_EPOCH)) 
              END IF
              IF ( DABS( (SUR%MJD_CUR - SUR%SOU(J1)%MJD_EPOCH)*86400.0D0 + &
     &                   (SUR%TAI_CUR + SUR%SOU(J1)%DUR/2.0D0 - &
