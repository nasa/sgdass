...
! *                                                                      *
! * ### 10-OCT-2005  SUR_SKED_MAIN v2.10 (d) L. Petrov  03-OCT-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
      ...
      PARAMETER  ( STACK_SIZE_IN_BYTES = STACK_SIZE_IN_GIGABYTES * GB )
      INTEGER*8, EXTERNAL :: SET_STACKSIZE 
      INTEGER*4, EXTERNAL :: IARGS, ILEN, I_LEN, GETPID
...
!
     ...
     &         ', but linked against '//TRIM(ANS)//'. Please recompile sur_sked' )
      ...
      END IF
!
      ALLOCATE ( SUR(1), STAT=IUER )
      IF ( IUER .NE. 0 ) THEN
           CALL CLRCH  ( STR )
           CALL IINCH8 ( SIZEOF(SUR(1)), STR )
           IUER = -1
           CALL ERR_LOG ( 1682, IUER, 'SUR_SKED_MAIN', 'Trap of internal '// &
     &         'control: cannot allocate '//TRIM(STR)//' bytes of dynamic '// &
     &         'memory for object SUR' )
           CALL EXIT ( 1 )
      END IF
      ALLOCATE ( VTD(1), STAT=IUER )
      IF ( IUER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( SIZEOF(VTD(1)), STR )
           IUER = -1
           CALL ERR_LOG ( 1683, IUER, 'SUR_SKED_MAIN', 'Trap of internal '// &
     &         'control: cannot allocate '//TRIM(STR)//' bytes of dynamic '// &
     &         'memory for object SUR' )
           CALL EXIT ( 1 )
      END IF
!
      ...
      IF ( IVRB > 0 ) THEN
           CALL CLRCH  ( STR )
           CALL INCH   ( GETPID(), STR(1:8) )
           CALL CHASHR ( STR(1:8) )
           WRITE ( 6, '(A)' ) SUR__LABEL//'  Pid: '//STR(1:8)
      END IF
!
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
           IUER = -1
           CALL ERR_LOG ( 1684, IUER, 'SUR_SKED_MAIN', 'Error in SUR_SKED_CONF '// &
     &         'while parsing control file '//EXP_FIL )
           CALL EXIT ( 1 )
      END IF
!
...
!
     ...
     &             SUR(1)%ALGORITHM == 'GEODETIC_04'   .OR.  &
     &             SUR(1)%ALGORITHM == 'GNSS_01'       .OR.  &
     &             SUR(1)%ALGORITHM == 'GNSS_02'       .OR.  &
     &             SUR(1)%ALGORITHM == 'GNSS_03'       .OR.  &
...
!
      IF ( IVRB > 0 ) THEN
           ...
           WRITE ( 6, * ) 'sur_sked: successful termination. It is '//GET_CDATE()
      END IF
      CALL EXIT ( 0 )
