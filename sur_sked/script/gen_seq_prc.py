...
# *                                                                      *
# *   gen_seq_prc.py -s /vlbi/y2/y2_m02.seq -o /vlbi/y4045a              *
# *                                                                      *
...
# *                                                                      *
# * ### 09-FEB-2024  gen_seq_prc.py v 1.12 (d) L. Petrov 18-AUG-2026 ### *
# *                                                                      *
# ************************************************************************
...
#
# A case of Fortaleza as of 2026.08.10
#
mark5b_geo6 = { \
               1: {"bbc_ind":  9, "sub_band": 'U', "chan": 11, "band_ind":  1 }, \
               2: {"bbc_ind": 10, "sub_band": 'U', "chan": 12, "band_ind":  2 }, \
...
#
    ...
    if ( not buf ):
         print ( "%s: cannot read file %s" % ( where_error(), fil_seq ) )
         exit  ( 1 )
    
    if ( buf[0] != prc_magic ):
         print ( "%s: wrong format of sequence file %s" % \
                  ( where_error(), fil_seq ) )
    ...
    if ( not frd ):
         print ( "%s: cannot read frequency defintion file %s" % \
                 ( where_error(), frq_def ) )
         exit  ( 1 )
    
    if ( frd[0] != frd_magic ):
         print ( "%s: wrong format of the frequency defintion file %s" % \
                 ( where_error(), frq_def ) )
...
#
             ...
             if ( sta_name in seq["station"].keys() ):
                  print ( "%s: station %s is defined more than once in %s" % \
                          ( where_info(), sta_name, fil_seq ) )
...
#
             ...
             if ( wir_def == {} ):
                  print ( "%s: did not find wiring %s for station %s in the frequency definition file %s" % \
                          ( where_error(), wir_name, sta_name, frq_def ) )
                  exit ( 1 )
#
...
#
             ...
             if ( len(line.split()) >= 5 ):
                  if ( line.split()[4].lower() == "dir" or \
                      ...
                      print ( "%s: wrong 5th word in line %s -- dir or inv were expected" % \
                              ( where_error(), line ) )
    ...
    if ( ivrb > 2 ):
         print ( where_info(), "num_band= ", len(seq["band"]) )
         print ( where_info(), "seq= ", seq )
...
#
                               ...
                               fl_match = True
                               if ( ivrb >= 2 ):
                                    print ( where_info(), "band= ", band, ' lo_band= ', lo_band,  'band_frq_min= ', band_frq_min ) 
                 ...
                 if ( not fl_match ):
                      print ( "%s sta: %s band: %s -- did not find a matching dynamic range" % \
                              ( where_error(), sta, band ) )
                      exit  ( 1 )
                 if ( ivrb >= 3 ):
                      print ( where_info(), "DYN sta= ", sta, " band= ", band, " frq= ", sb_band[band], " sb= ", sb_band[band] )
                 continue
            elif ( not band in seq["station"][sta]["wiring"] ):
                 print ( "%s: did not find band %s in wiring %s for station %s in the frequency definition file: %s" % \
                         ( where_error(), band, seq["station"][sta]["wiring"]["name"], sta, frq_def ) )
                 exit ( 1 )
            mode_dbbc3_v = False
            ...
            if   ( seq["station"][sta]["wiring"][band]["lo_typ"] == "a" ):
                   ...
                   udc_band[band] = None
                   if ( ivrb >= 3 ):
                        print ( where_info(), "STT sta= ", sta, " band= ", band, " frq= ", sb_band[band], " sb= ", sb_band[band] )
...
#
                   ...
                   for i in range(0,len(seq["band"][band]["if_frq"])):
                       ...
                       if ( seq["station"][sta]["wiring"][band]["das"] == "rdbe" ):
                            ind = (frq_band[band] - seq["band"][band]["if_frq"][k])/seq["band"][band]["if_bwd"][k]
                            dbe_band[band] = dbe_band[band] + "%d:" % ind
                       elif ( seq["station"][sta]["wiring"][band]["das"] == "r2dbe" ):
                            ind = 32 - (frq_band[band] - seq["band"][band]["if_frq"][k])/seq["band"][band]["if_bwd"][k]
                            dbe_band[band] = dbe_band[band] + "%d," % ind
                   if ( seq["station"][sta]["wiring"][band]["das"] == "rdbe" ):
...
#
                 ...
                 print ( "%s sta: %s band: %s -- unsupported lo type %s" % \
                         ( where_error(), sta, band, seq["station"][sta]["wiring"]["lo_typ"] ) )
                 exit  ( 1 )
            if ( not fl_match ):
                 print ( "%s sta: %s band: %s -- did not find matching lo type" % \
                         ( where_error(), sta, band ) )
                 exit  ( 1 )
...
# ----- Get wirinig name
#
        wir_name = seq["station"][sta]["wiring"]["name"] 
       
#
# ----- Read a template for procedure 
#
        prc_tmpl = prc_dir + "/" + wir_name + "_template.prc"
        if ( not os.path.isfile ( prc_tmpl ) ):
#
# ---------- Try a generic name without wiring name
#
             prc_tmpl = prc_dir + "/" + sta + "_template.prc"
             if ( not os.path.isfile ( prc_tmpl ) ):
                  print ( "%s: cannot find procedure template file %s" % \
                          ( where_error(), prc_tmpl ) )
                  exit ( 1 )
        
        buf = read_file ( prc_tmpl )
        if ( not buf ):
             print ( "%s in reading the procedure template file %s" % \
                      ( where_error(), prc_tmpl ) )
             exit ( 1 )
...
#
                 ...
                 ind_lo_name= line.find("@lo@") - 3
                 lo_band_name = line[ind_lo_name:ind_lo_name+1].lower()
...
#
                 ...
                 if ( lo_band_name == None ):
                      ...
                      print ( "%s in parsing line %s of template file %s -- %s" % \
                              ( where_error(), line, prc_tmpl, err_msg ) )
                      exit ( 1 )
...
#
                 ...
                 elif ( line[0:12] == "pcal_offset=" ):
                      pcal_band_name = line[12:13].lower()
                 elif ( line[6:18] == "pcal_offset=" ):
                      pcal_band_name = line[18:19].lower()
                 else:
                      print ( "%s in pcal option line %s of template file %s -- %s" % \
                              ( where_error(), line, prc_tmpl, 'no pcal defintion was found' ) )
                      exit  ( 1 ) 
                 if ( pcal_band_name.lower() in band_list ):
                      if ( seq["station"][sta]["wiring"][pcal_band_name.upper()]["das"] == 'rdbe' ):
                           line = line.replace( '@pcal_step_5@', "%5.3fe6" % math.fmod( lo_band[pcal_band_name.upper()]+1024.0,5.0 ) )
                      elif ( seq["station"][sta]["wiring"][pcal_band_name.upper()]["das"] == 'r2dbe' ):                      
                           line = line.replace( '@pcal_step_5@', "%5.3fe6" % (5.0 - math.fmod( lo_band[pcal_band_name.upper()],5.0 ) ) )
                      prc.append ( line )
                 continue
...
#
                 if ( line[5:11] == "=pcal=" ):
                      ...
                      print ( "%s in pcal optiion line %s of template file %s -- %s" % \
                              ( where_error(), line, prc_tmpl, err_msg ) )
                      exit  ( 1 ) 
                 if ( pcal_band_name.lower() in band_list ):
                      if ( seq["station"][sta]["wiring"][pcal_band_name.upper()]["das"] == 'rdbe' ):
                           ...
                           line = line.replace( '@pcal_step_10@', "%5.3fe6" % (10.0 - math.fmod( lo_band[pcal_band_name.upper()],10.0 ) ) )
                      prc.append ( line )
                 ...
                 elif ( seq["station"][sta]["wiring"][if_band_name]["das"] == "mark5b_geo5" ):
                        for bbc in mark5b_geo5:
                            if ( mark5b_geo5[bbc]["bbc_ind"] == bbc_ind ):
                                 bbc_def_ind = bbc
                                 bbc_def_chan = mark5b_geo5[bbc]["chan"]
                                 bbc_sub_band = mark5b_geo5[bbc]["sub_band"]
                                 bbc_band_ind = mark5b_geo5[bbc]["band_ind"]
                        
                 elif ( seq["station"][sta]["wiring"][if_band_name]["das"] == "mark5b_geo6" ):
                        for bbc in mark5b_geo6:
                            if ( mark5b_geo6[bbc]["bbc_ind"] == bbc_ind ):
                                 bbc_def_ind = bbc
                                 bbc_def_chan = mark5b_geo6[bbc]["chan"]
                                 bbc_sub_band = mark5b_geo6[bbc]["sub_band"]
                                 bbc_band_ind = mark5b_geo6[bbc]["band_ind"]
...
#
                 ...
                 if ( ivrb >= 3 ):
                      print ( where_info(), "sta= ", sta, "band_core= ", band_core, "ind_core=", ind_core, "sb_band= ", sb_band )
#                 print ( where_info(), "band_core_name= ", band_core ) # %%%%%%%%
#                 print ( where_info(), "wir=", seq["station"][sta]["wiring"].keys() ) # %%%%%%%%
#                 print ( where_info(), "cor=", seq["station"][sta]["wiring"][band_core] ) # %%%%%%%%
                 if ( band_core != None ):
                      if ( "dynamic" in seq["station"][sta]["wiring"].keys() ):
                           if ( ivrb >= 3 ):
                                print ( where_info(), "sta= ", sta, "band_core=", band_core, 'sb_band[band_core]= ', sb_band[band_core] )
#
...
#
                           ...
                           if ( ivrb > 1 ):
                                print ( where_info(), "sta= ", sta, "if_typ= ", seq["station"][sta]["wiring"][band_core]["if_typ"], " sb= ", sb_band[band_core] )
#
...
#
                 if ( ivrb >= 3 ):
                      print ( where_info(), "sta= ", sta, "side_band= ", side_band )
                 if ( side_band == "lsb" ):
...
#
                     ...
                     break
                 if ( '@if_mask@' in line ):
                      if ( seq["band"][band]["if_bwd"][0] == 16.0 or seq["band"][band]["if_bwd"][0] == 8.0 ):
...
#
        ...
        if ( ret != 0 ):
             print ( "%s in writing output procedure file %s" % \
                     ( where_error(), prc_filout ) )
             exit ( 1 )
                 
        print ( "PRC: ", prc_fileout ) # %%%%%%%%
        if ( fl_vex_created ):
             continue
#
...
#
                     ...
                     sample_rate = 2*seq["band"][band]["if_bwd"][k] 
            elif ( seq["station"][sta]["wiring"][band]["das"] == 'mark5b_geo5' ):
#
# -------------- Mark5b in geodetic wiring mode, Kokee (old) falvor
#
                 for i in range(0,len(seq["band"][band]["if_frq"])):
                     ind_chn = ind_chn + 1
                     ind_bbc = ind_bbc + 1
...
# ------------------ FIXME!! We need to check here subband 
#
                     sbd = mark5b_geo5[ind_bbc]["sub_band"]
                     if ( sbd == "U" ):
                          vex.append ( '    chan_def = &%s : %9.2f MHz : %s : %6.2f MHz : &CH%03d : &BBC%03d : &L_cal;' % \
                                       ( band_name, seq["band"][band]["if_frq"][i], \
                                         sbd, seq["band"][band]["if_bwd"][k], \
                                         mark5b_geo5[ind_chn]["chan"], mark5b_geo5[ind_chn]["bbc_ind"] \
                                       ) \
                                     )
                     elif ( sbd == "L" ):
                          vex.append ( '    chan_def = &%s : %9.2f MHz : %s : %6.2f MHz : &CH%03d : &BBC%03d : &L_cal;' % \
                     ...
                     sample_rate = 2*seq["band"][band]["if_bwd"][k] 
            elif ( seq["station"][sta]["wiring"][band]["das"] == 'mark5b_geo6' ):
#
# -------------- Mark5b in geodetic wiring mode, Kokee (old) falvor
#
                 for i in range(0,len(seq["band"][band]["if_frq"])):
...
# ------------------ FIXME!! We need to check here subband 
#
                     sbd = mark5b_geo6[ind_bbc]["sub_band"]
                     if ( sbd == "U" ):
                          vex.append ( '    chan_def = &%s : %9.2f MHz : %s : %6.2f MHz : &CH%03d : &BBC%03d : &L_cal;' % \
                                       ( band_name, seq["band"][band]["if_frq"][i], \
                                         sbd, seq["band"][band]["if_bwd"][k], \
                                         mark5b_geo6[ind_chn]["chan"], mark5b_geo6[ind_chn]["bbc_ind"] \
                                       ) \
                                     )
                     elif ( sbd == "L" ):
                          vex.append ( '    chan_def = &%s : %9.2f MHz : %s : %6.2f MHz : &CH%03d : &BBC%03d : &L_cal;' % \
        ...
        vex_fileout = dir_out + "/" + seq["mod_name"] + "_" + seq["hds_name"] + "_vex.frq"
        (ret,err) = write_file ( vex, vex_fileout )
        if ( ret != 0 ):
             print ( "%s in writing output vex template file %s" % \
                     ( where_error(), vex_filout ) )
             exit ( 1 )
