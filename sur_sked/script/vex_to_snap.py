...
# *                                                                      *
# * ### 25-JUN-2018 vex_to_snap.py v 2.4 (d)  L. Petrov 30-MAR-2026 ###  *
# *                                                                      *
# ************************************************************************
"""
...
def gen_vex_to_snap ( vex, sta_name, filout, check_schedule, gain_macros_file, mode ):
    ...
    if ( vex.sta[sta_name]["num_sca"] == 0 ):
         print ( "Station %s has no scans in experiment %s" % \
                 ( sta_name, vex.exper_name ) )
         print ( "List of participating stations: %s" % sorted(list(vex.sta.keys())) )
         exit  ( 1 )
...
#
                  ...
                  if ( vex.sta[sta_name]["slew_accel"][0] == 0.0 ): 
                       print ( "vex_to_snap.pt ERROR antenna acceleration over the 1st axis of station %s is zero" %  sta_name )
                       exit  ( 1 )
                  if ( vex.sta[sta_name]["slew_accel"][1] == 0.0 ):
                       print ( "vex_to_snap.pt ERROR antenna acceleration over the 2nd axis of station %s is zero" %  sta_name )
                       exit  ( 1 )
#
...
#
             if ( vex.sta[sta_name]["das_name"] == "mark6_recorder"    or \
                  ...
                  vex.sta[sta_name]["das_name"] == "mark5b_recorder"   or \
                  vex.sta[sta_name]["das_name"] == "mark5c_recorder"      ):
                  ...
                  print ( "vex_to_snap.py ERROR: unsupported DAS recorder: %s for station %s \n" %
                          ( vex.sta[sta_name]["das_name"], sta_name ) )
                  print ( "vex_to_snap.py list of suported recorders: " + \
                          "mark5_recorder, mark5b_recorder, mark5c_recorder, flexbuff_recorder, mark6_recorder" )
                  exit ( 1 )             
