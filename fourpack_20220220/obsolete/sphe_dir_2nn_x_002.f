...
! --------- P: p(j4,0)
!
            CALL AYBZ ( FSH%F1(J4,0)*Z, PM1_X, -FSH%F2(J4,0), PM2_X, P_X )
            PM2_X = PM1_X
            PM1_X = P_X
            SPH(1,J4,0) = SPH(1,J4,0) + P_X%F* FFC_EVEN(1)
            UM(J4) = U* UM(J4-1)
...
! --------- P: p(j4+1,0)
!
            CALL AYBZ ( FSH%F1(J4+1,0)*Z, PM1_X, -FSH%F2(J4+1,0), PM2_X, P_X )
            PM2_X = PM1_X
            PM1_X = P_X
            SPH(1,J4+1,0) = SPH(1,J4+1,0) + P_X%F* FFC_ODD(1)
            UM(J4+1) = U* UM(J4)
...
!
              CALL AYBZ ( FSH%F1(DEG,0)*Z, PM1_X, -FSH%F2(DEG,0), PM2_X, P_X )
              PM2_X = PM1_X
              PM1_X = P_X
              SPH(1,DEG,0) = SPH(1,DEG,0) + P_X%F* FFC_EVEN(1)
              UM(DEG) = U* UM(DEG-1)
...
! ------------ P: p(j6,j5)
!
               CALL AYBZ ( FSH%F1(J6,J5)*Z, PM1_X, -FSH%F2(J6,J5), PM2_X, P_X )
!+
!#               ID = PM1_X%E - PM2_X%E
!#               IF ( ID == 0 ) THEN
!#                    P_X%F = FSH%F1(J6,J5)*Z*PM1_X%F - FSH%F2(J6,J5)*PM2_X%F
!#                    P_X%E = PM1_X%E
!#                 ELSE IF ( ID ==  1 ) THEN
!#                    P_X%F = FSH%F1(J6,J5)*Z*PM1_X%F - FSH%F2(J6,J5)*(PM2_X%F*BIGU_FF)
!#                    P_X%E = PM1_X%E
!#                 ELSE IF ( ID == -1 ) THEN
!#                    P_X%F = FSH%F1(J6,J5)*Z*(PM1_X%F*BIGU_FF) - FSH%F2(J6,J5)*PM2_X%F
!#                    P_X%E = PM2_X%E
!#                 ELSE IF ( ID  >  1 ) THEN
!#                    P_X%F = FSH%F1(J6,J5)*Z*PM1_X%F 
!#                    P_X%E = PM1_X%E
!#                 ELSE
!#                    P_X%F = -FSH%F2(J6,J5)*PM2_X%F
!#                    P_X%E = PM2_X%E
!#               END IF
!#               W = ABS(P_X%F)
...
!-
               ...
               SPH(1:2,J6,J5) = SPH(1:2,J6,J5) + PUM* FFC_ODD
!+
!#               PUM_X%F = P_X%F * UM_X(J5)%F 
!#               PUM_X%E = P_X%E + UM_X(J5)%E 
!#               W = ABS(PUM_X%F)
!#               IF ( W .GE. BIGHO_FF ) THEN
!#                    PUM_X%F = PUM_X%F*BIGU_FF
!#                    PUM_X%E = PUM_X%E + 1
!#                 ELSE IF ( W .LT. BIGHU_FF ) THEN
!#                    PUM_X%F = PUM_X%F*BIGO_FF
!#                    PUM_X%E = PUM_X%E - 1
!#               END IF
!#               IF ( PUM_X%E == 0 ) THEN
!#                    SPH(1:2,J6,J5) = SPH(1:2,J6,J5) + PUM_X%F* FFC_ODD
!#                  ELSE IF ( PUM_X%E > 0 ) THEN
!#                    SPH(1:2,J6,J5) = SPH(1:2,J6,J5) + (PUM_X%F*BIGO_FF)* FFC_ODD
!#               END IF
!-
#ifdef ALF_SQ_COMP
...
! ------------ P: p(j6+1,j5)
!
               CALL AYBZ ( FSH%F1(J6+1,J5)*Z, PM1_X, -FSH%F2(J6+1,J5), PM2_X, P_X )
!+
!#               ID = PM1_X%E - PM2_X%E
!#               IF ( ID == 0 ) THEN
!#                    P_X%F = FSH%F1(J6+1,J5)*Z*PM1_X%F - FSH%F2(J6+1,J5)*PM2_X%F
!#                    P_X%E = PM1_X%E
!#                 ELSE IF ( ID ==  1 ) THEN
!#                    P_X%F = FSH%F1(J6+1,J5)*Z*PM1_X%F - FSH%F2(J6+1,J5)*(PM2_X%F*BIGU_FF)
!#                    P_X%E = PM1_X%E
!#                 ELSE IF ( ID == -1 ) THEN
!#                    P_X%F = FSH%F1(J6+1,J5)*Z*(PM1_X%F*BIGU_FF) - FSH%F2(J6+1,J5)*PM2_X%F
!#                    P_X%E = PM2_X%E
!#                 ELSE IF ( ID  >  1 ) THEN
!#                    P_X%F = FSH%F1(J6+1,J5)*Z*PM1_X%F 
!#                    P_X%E = PM1_X%E
!#                 ELSE
!#                    P_X%F = -FSH%F2(J6+1,J5)*PM2_X%F
!#                    P_X%E = PM2_X%E
!#               END IF
!#               W = ABS(P_X%F)
!#               IF ( W .GE. BIGHO_FF ) THEN
...
!-
               ...
               PM1_X = P_X
               CALL YZ ( P_X, UM_X(J5), PUM_X, PUM )
               SPH(1:2,J6+1,J5) = SPH(1:2,J6+1,J5) + PUM* FFC_EVEN
!+
!#               PUM_X%F = P_X%F * UM_X(J5)%F 
!#               PUM_X%E = P_X%E + UM_X(J5)%E 
!#               W = ABS(PUM_X%F)
...
!#               END IF
!#               IF ( PUM_X%E == 0 ) THEN
!#                    SPH(1:2,J6+1,J5) = SPH(1:2,J6+1,J5) + PUM_X%F* FFC_EVEN
!#                  ELSE IF ( PUM_X%E > 0 ) THEN
!#                    SPH(1:2,J6+1,J5) = SPH(1:2,J6+1,J5) + (PUM_X%F*BIGO_FF)* FFC_EVEN
!#               END IF
!-
#ifdef ALF_SQ_COMP
               CALL XZZ    ( ALF_ORD_SQ_X(J5), P_X )
 ...
 460        CONTINUE 
            IF ( KD == DEG-2 ) THEN
                 CALL AYBZ ( FSH%F1(DEG,J5)*Z, PM1_X, -FSH%F2(DEG,J5), PM2_X, P_X )
!+
!                 ID = PM1_X%E - PM2_X%E
!                 IF ( ID == 0 ) THEN
!                      P_X%F = FSH%F1(DEG,J5)*Z*PM1_X%F - FSH%F2(DEG,J5)*PM2_X%F
!                      P_X%E = PM1_X%E
!                   ELSE IF ( ID ==  1 ) THEN
!                      P_X%F = FSH%F1(DEG,J5)*Z*PM1_X%F - FSH%F2(DEG,J5)*(PM2_X%F*BIGU_FF)
!                      P_X%E = PM1_X%E
!                   ELSE IF ( ID == -1 ) THEN
!                      P_X%F = FSH%F1(DEG,J5)*Z*(PM1_X%F*BIGU_FF) - FSH%F2(DEG,J5)*PM2_X%F
!                      P_X%E = PM2_X%E
!                   ELSE IF ( ID  >  1 ) THEN
!                      P_X%F = FSH%F1(DEG,J5)*Z*PM1_X%F 
!                      P_X%E = PM1_X%E
!                   ELSE
!                      P_X%F = -FSH%F2(DEG,J5)*PM2_X%F
!                      P_X%E = PM2_X%E
!                 END IF
!                 W = ABS(P_X%F)
!                 IF ( W .GE. BIGHO_FF ) THEN
...
!                 END IF
!                 IF ( PUM_X%E == 0 ) THEN
!                      SPH(1:2,DEG,J5) = SPH(1:2,DEG,J5) + PUM_X%F* FFC_ODD
!                   ELSE IF ( PUM_X%E > 0 ) THEN
!                      SPH(1:2,DEG,J5) = SPH(1:2,DEG,J5) + (PUM_X%F*BIGO_FF)* FFC_ODD
!                 END IF
!-
#ifdef ALF_SQ_COMP
                 CALL XZZ    ( ALF_ORD_SQ_X(J5), P_X )
