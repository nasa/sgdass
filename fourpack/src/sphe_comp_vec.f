...
! *   intitive expectation: the second dimension runs over order m       *
! *   and the third dimension rins over degree l.                        *
! *                                                                      *
...
! *   All rights reserved.                                               *
! *                                                                      *
! * # 06-NOV-2012 SPHE_COMP_VEC v2.0 modified by L. Petrov 19-OCT-2015 # *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'fourpack.i'
...
!
      IF ( NORM .NE. 1 ) THEN
           ...
           CALL INCH  ( NORM, STR )
           CALL ERR_LOG ( 6745, IUER, 'SPHE_COMP_VEC', 'Wrong value '// &
     &         'of NORM: '//STR(1:I_LEN(STR))//' -- presently, only value '// &
     &         '1 "geodesy" is supported' )
           RETURN 
      END IF
!
...
! --- F-version of X-version. This is only for debugging purposes!
!
...
!
      ...
      X = SIN(LAT_VAL)
      Z = COS(LAT_VAL)
      IF ( Z < Z_MIN ) Z = Z_MIN !  In order to avid overflow at poles
!
      RES = 0.0D0
!
...
!$OMP   PRIVATE ( J2, J3 ), REDUCTION (+: RES ), SCHEDULE ( DYNAMIC )
      DO 420 J2=DEG,0,-1
         RES(1) = RES(1) + SPH(1,0,J2,1)*FSH%PL(J2,0)
         RES(3) = RES(3) + SPH(1,0,J2,2)*FSH%DPLT(0,J2)
         DO 430 J3=1,J2
            RES(1) = RES(1) + &
     &                  (  SPH(1,J3,J2,1)*FSH%MCOS(J3) + SPH(2,J3,J2,1)*FSH%MSIN(J3) )* &
     &               FSH%PLT(J3,J2)
            RES(2) = RES(2) + &
     &               J3*( -SPH(1,J3,J2,2)*FSH%MSIN(J3) + SPH(2,J3,J2,2)*FSH%MCOS(J3) )* &
     &               FSH%PLT(J3,J2)
            RES(3) = RES(3) + &
     &                  (  SPH(1,J3,J2,2)*FSH%MCOS(J3) + SPH(2,J3,J2,2)*FSH%MSIN(J3) )* &
     &                  FSH%DPLT(J3,J2)
 430     CONTINUE 
 420  CONTINUE 
!$OMP END PARALLEL DO
