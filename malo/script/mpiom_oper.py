...
# *   displacements.                                                     *
# *                                                                      *
# * ### 04-NOV-2013   mpiom_oper.py v3.9 (c) L. Petrov  26-FEB-2026 ###  *
# *                                                                      *
# ************************************************************************
"""
...
date_range = "1978.01.01 2099.12.31"
...
# --- Important constants
#
mpiom_oper__label = "mpiom_oper.py  v 3.38 of 2025.02.01"
...
#
    ...
    for i in range (0,len(config.date_list)):
        ...
        im  = url.find ( "M%%" )
        if ( im > 1 ):
             url = url.replace ( url[im:im+3], \
                                 config.date_list[i][4:6] )

        id  = url.find ( "D%%" )
        if ( id > 1 ):
             url = url.replace ( url[id:id+3], \
...
#
        if ( "https" in url_list[i] ):
             list_file = "index.html_%08d" % os.getpid()
        else:
             list_file = ".listing"
        ...
        if ( "ftp://" in url_list[i] ):
              wget_command = "cd " + config.geos_temp_dir + " ; " + \
                       "wget --timeout=30 --tries=64 -nH --cut-dirs=8 -r -np --no-check-certificate --no-remove-listing -q -c --retry-connrefused --ftp-password=\"\" -A 'PODSTANOVKA' " + \
                       url_list[i][0:id] + "/" 
              wget_command = wget_command + url_list[i][ia+6:ia+10] 
        else:
              wget_command = "cd " + config.geos_temp_dir + " ; " + \
                       "wget -O " + list_file + " --timeout=30 --tries=64 -nH --no-check-certificate " + \
                        url_list[i][0:id] + "/" 
...
# ----- listing file
#
        listing_file = config.geos_temp_dir + "/" + list_file
        if ( ivrb > 4 ): print ( "Here is listing file: " + listing_file, ' exists: ', os.path.isfile(listing_file) )
...
#
                 if ( ivrb > 5 ): print ("LISTING_LINE: ", line, ' URL: ', url_list[i][id+1:] )
...
#
                      if ( "ftp://" in url_list[i] ):
                           ...
                           url_exists_list.append ( url_list[i][0:id] + "/" + url_list[i][id+1:] )
...
#
    ...
    if ( n_fil > 0 and proc_date != "n/a" ):
         ...
         for i in range (0,len(url_list)):
             error_message = "Error in processing input url %s" % url_list[i]
             print_err ( config, error_message )
             print_log ( config, "failed update" )
             return    ( 0, last_proc_date )

    print_log ( config, "successfully finished" )
    return ( n_fil, last_proc_date )
...
#
    ...
    for i in range (0,len(config.pivot_sds)):
        ...
        wget_com = "cd " + \
             ...
             " ; wget --timeout=30 --tries=64 --no-check-certificate --retry-connrefused -nH --cut-dirs=8 -c --ftp-password=\"\" " + \
             url
        if ( ivrb > 2 ):
...
#
    ...
    if ( opts.run_level == 10 ):
         ...
         if ( opts.file == None ):
              (n_fil, last_date) = get_mpiom_oper ( config, opts.run_level, opts.ivrb )
         else:
              log_buf = []
              (n_fil, last_date) = geos_process ( config, opts.file, opts.run_level, opts.ivrb, log_buf )
   
#
# --- Check for export of the stop file
#
...
#
         ...
         if ( opts.ivrb > 0 ):
              ...
              if ( ret == 0 ):
                   print ( "mpiom_oper %s finished updating sandbox directory %s" % \
                            ( time_str_now, config.load_bdsp_sandbox_dir ) )
              else:
                   print ( "mpiom_oper %s FAILURE in an attempt to update sandbox directory %s" % \
                            ( time_str_now, config.load_bdsp_sandbox_dir ) )
