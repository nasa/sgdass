...
! *                                                                      *
! *  ### 06-JAN-2005   AGRA_WRITE  v1.0 (d)  L. Petrov  06-JAN-2005 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'agra.i'
...
!
     ...
     &                    %REF(AGRA__D_RECORD_TEMPLATE), D_REC )
         ...
         WRITE ( UNIT=D_REC%TAI,       FMT='(F7.1)' ) SEC_EPC
         WRITE ( UNIT=D_REC%DATE,      FMT='(A19)'  ) STR_DATE(1:19)
         WRITE ( UNIT=D_REC%DEGREE,    FMT='(I3)'   ) 1
         WRITE ( UNIT=D_REC%ORDER,     FMT='(I3)'   ) 0
         WRITE ( UNIT=D_REC%STOKES_C,  FMT='(1PD12.5)' ) AGRA%STOKES_10
         WRITE ( UNIT=D_REC%STOKES_S,  FMT='(1PD12.5)' ) 0.0D0
         CALL LIB$MOVC3 ( LEN(AGRA__D_RECORD_TEMPLATE), D_REC, &
     &                     %REF(STR) )
         STR(1:1) = 'F'
!
! ------ Write the 1st F-record
!
         WRITE ( UNIT=LUN, FMT='(A)', IOSTAT=IOS ) STR(1:I_LEN(STR))
         IF ( IOS .NE. 0 ) THEN
              CALL CLRCH ( STR )
...
!
     ...
     &                    %REF(AGRA__D_RECORD_TEMPLATE), D_REC )
         ...
         WRITE ( UNIT=D_REC%DATE,      FMT='(A19)'  ) STR_DATE(1:19)
         WRITE ( UNIT=D_REC%DEGREE,    FMT='(I3)'   ) 1
         WRITE ( UNIT=D_REC%ORDER,     FMT='(I3)'   ) 1
         WRITE ( UNIT=D_REC%STOKES_C,  FMT='(1PD12.5)' ) AGRA%STOKES_11(1)
         WRITE ( UNIT=D_REC%STOKES_S,  FMT='(1PD12.5)' ) AGRA%STOKES_11(2)
         CALL LIB$MOVC3 ( LEN(AGRA__D_RECORD_TEMPLATE), D_REC, &
     &                     %REF(STR) )
         STR(1:1) = 'F'
         WRITE ( UNIT=LUN, FMT='(A)', IOSTAT=IOS ) STR(1:I_LEN(STR))
