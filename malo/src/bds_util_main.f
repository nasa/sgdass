...
! *                                                                      *
! *  ### 28-NOV-2024 BDS_UTIL_MAIN v1.1 (d)  L. Petrov  02-JAN-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
      IF ( IARGC() < 2 ) THEN
           WRITE ( 6, * ) 'Usage: bds_util extract|extend|extend_in|info|truncate|checksum|checksum_verb|summary bds_file|bds_dir param'
           CALL  EXIT ( 1 )
         ELSE 
           CALL GETARG ( 1, MODE     )
...
!
         ...
         ELSE IF ( MODE == 'checksum' ) THEN
            IUER = -1
            CALL BDS_CHECKSUM ( FIL_BDS, 0, L_BDS, IUER )
            IF ( IUER .EQ. 0 ) THEN
                 WRITE ( 6, '(A)' ) 'No checksum errors were found '// &
     &                              'in files in '//TRIM(FIL_BDS)
               ELSE
                 WRITE ( 6, '(A)' ) 'Checksum error was found '// &
     &                              'in one of the files in '//TRIM(FIL_BDS)
            END IF
         ELSE IF ( MODE == 'checksum_verb' ) THEN
            IUER = -1
            CALL BDS_CHECKSUM ( FIL_BDS, 1, L_BDS, IUER )
            IF ( IUER .EQ. 0 ) THEN
                 WRITE ( 6, '(A)' ) 'No checksum errors were found '// &
     &                              'in files in '//TRIM(FIL_BDS)
               ELSE
     ...
     &          TRIM(MODE)//' . Supported modes: check, checksum, checksum_verb, '// &
     &         'info, truncate, extract, extend_in, extend, summary' )
           CALL EXIT ( 1 )
      END IF
      IF ( IUER .NE. 0 ) THEN
           CALL EXIT ( 1 )
