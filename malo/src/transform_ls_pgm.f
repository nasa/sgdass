      PROGRAM    TRANSFORM_LS_PGM
! ************************************************************************
! *                                                                      *
! *   Program TRANSFORM_LS_PGM  reads the land/sea mask in PGM format    *
! *   and  writes it into HEB or netCDF format.                          *
! *                                                                      *
...
! *                                                                      *
! * ## 11-OCT-2012  TRANSFORM_LS_PGM  v2.0 (d) L. Petrov  08-MAR-2013 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE    'heb.i'
...
!
      IF ( IARGC() < 3 ) THEN
           WRITE ( 6, '(A)' ) 'Usage: transform_ls_pgm data_type pgm_file filout [plot] [deg]'
           CALL EXIT ( 0 )
         ELSE
           CALL GETARG ( 1, DTYP )
...
!
      ...
      IF ( DTYP == 'bls_mask' ) THEN
        ...
        ELSE IF ( DTYP == 'fls_mask' ) THEN
           ...
           ADD_OFFSET_R4 = 0.0
           SCALE_R4      = 0.01
           IF ( INDEX ( FILOUT, '.nc' ) == ILEN(FILOUT)-3 ) THEN
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 10-OCT-2012    READ_PGM   v1.0 (d)  L. Petrov  10-OCT-2012 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      CHARACTER  FILPGM*(*)
