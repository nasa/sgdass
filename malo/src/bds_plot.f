...
! *                                                                      *
! *  ### 17-APR-2015    BDSP_PLOT  v2.2 (d)  L. Petrov  02-JAN-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
         write ( 6, * ) 'BINDISP__MJD_FMT= ', BINDISP__MJD_FMT ! %%%%
         write ( 6, * ) 'REC2%MJD_FMT    = ', REC2%MJD_FMT     ! %%%%
      KP = 0
      DO 430 J3=IND_BEG,IND_END
         READ ( UNIT=LUN, REC=J3+M__HDR ) RECD
         IF ( REC2%MJD_FMT == BINDISP__MJD_FMT ) THEN
              ...
              END IF
!
              CALL MVBITS ( RECD%EXT_DSP,  0, 4, KX,  0 )
              CALL MVBITS ( RECD%EXT_DSP,  4, 4, KY,  0 )
              CALL MVBITS ( RECD%EXT_DSP,  8, 4, KZ,  0 )
              CALL MVBITS ( RECD%EXT_DSP, 15, 1, BIT_SUM_I2,  0 )
              BIT_SUM_COMP = BIT_CHECKSUM ( RECD%X_DSP, 1, 63 )
              IF ( BIT_SUM_COMP .NE. BIT_SUM_I2 ) THEN
                   IUER = -1
                   CALL CLRCH ( STR )
                   CALL INCH  ( J3, STR )
                   CALL ERR_LOG ( 2830, IUER, 'BDS_PLOT', 'Bit sum '// &
     &                 'check is wrong for record '//TRIM(STR)// &
     &                 ' when processing file '//FILNAM )
                   CALL EXIT ( 1 )
              END IF
            ELSE IF ( REC2%MJD_FMT == BINDISP__MJD_FMT_01 ) THEN
!
