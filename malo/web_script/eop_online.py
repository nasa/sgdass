...
# *                                                                      *
# * ### 06-APR-2016 eop_online.py  v1.7 (c)  L. Petrov  05-MAR-2025 ###  *
# *                                                                      *
# ************************************************************************
...
#
     try:
        date_req  = key_pairs["req_date"][0].lower().rstrip()
        if ( len(date_req) > 27 ):
             date_req = date_req[0:27]
     except:
        if ( pyvers >= "3.12" ):
             date_req = datetime.datetime.now(datetime.UTC).strftime("%Y.%m.%d_%H:%M:%S")[0:27]
        else:
             date_req = datetime.datetime.utcnow().strftime("%Y.%m.%d_%H:%M:%S.%f")[0:27]

     if ( not url_sanitizer ( date_req, "date" ) ):
          if ( content != "html" and ivrb <= 1 ):
               print ( "Content-type: text/html\n\n" )
               print ( "" ) 
               print ( '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2//EN">' )
               print ( '<HTML LANG="en">' )
