...
# ************************************************************************
# *                                                                      *
# *   A wrapper program for VLBI data processing with package PIMA.      *
# *                                                                      *
...
# *                                                                      *
# * ### 20-DEC-2015    pf.py   v 1.28 (d)  L. Petrov   17-AUG-2026  ###  *
# *                                                                      *
# ************************************************************************
"""
...
pf__version = "pf.py 20260817 1.28"
...
#
# --- Read the PIMA control file and extract session code and experiment directory
#
    sess_code = None
    exper_dir = None
    for line in cnt:
        ...
        if ( line.split()[0] == "SESS_CODE:" ): sess_code = line.split()[1]
        if ( line.split()[0] == "EXPER_DIR:" ): exper_dir = line.split()[1]

    if ( sess_code and exper_dir ):
#
# ------ For m the experiment statistic file name
#
         stt_file = exper_dir + "/" + sess_code + ".stt"
         if ( os.path.isfile ( stt_file ) ):
#
# ----------- Read the statistic file name and determine the total number of IFs and 
# ----------- the nuber of phase-cal tones per IF
#
              with open(stt_file) as f:
                   stt = f.readlines()
              f.close()
              ...
              num_frq   = 0
              for line in stt:
                  if ( "Number of phase-cal tones per IF:" in line ):
                       num_tones = int(line.split()[6])
                  if ( "Number of frequencies:" in line ):
                       num_frq   = int(line.split()[3])

              if ( num_tones == 2 ):
#
# --------------- If the number of phase-cal tones is 2, then
# --------------- we run task gean pcal_dzero zero 
#
                  if ( pf.static ):
                       pima_com = pima_exec["static"]  
                  else:
                       pima_com = pima_exec["dynamic"]

                  print ( "pf.py: About to run task gean pcal_dzero zero", file=log )
                  pima_com = pima_com + " " + pf.cnt_file + " " + "gean pcal_dzero zero " + \
                             "BEG_FRQ:            1" + " " + \
                             "END_FRQ:           %d" % num_frq

                  (ret, out ) = exe_noout_log ( pima_com, pf.verb, log )
                  if ( ret != 0  ): 
                       print ( "pf.py: Failure in command %s" % pima_com )
                       print ( "pf.py: Failure in command %s" % pima_com, file=log )
                       log.flush()
                       exit  ( 1 )
                  log.flush()
             
         else:
              print ( "pf.py: Warning: did not find statistics file %s" % stt_file )
    else:
         print ( "pf.py: Warning: did not find defined SESS_CODE " + 
                 "and EXPER_DIR in the pima control file: %s" % pima_cnt )
...
def pf_logs ( pf ):
            ...
            if ( file.find ( pf.exp + "log." ) == 0 ):
                 if ( len(file) == len(pf.exp) + len("log.") + 2 ):
                   ...
                   if ( file[len(pf.exp):len(pf.exp)+3] == "KTN" ):
                        ...
                        kvn_logs.append ( pf.exp_dir + "/" + file )
                        kvn_sta.append  ( "ys" )
                   elif ( file[len(pf.exp):len(pf.exp)+3] == "KPC" ):
                        kvn_logs.append ( pf.exp_dir + "/" + file )
                        kvn_sta.append  ( "kc" )
    ...
    if ( len(ivs_logs) > 0 ):
         for i in range(0,len(ivs_logs)):
             anc_file = pf.exp_dir + "/" + pf.exp + "_" + ivs_sta[i] + ".anc"
             if ( os.path.isfile ( anc_file ) ):
                  if ( not pf.dry_run ):
                       os.unlink ( anc_file )

             pima_com = pima_local.log_to_anc_exec + " -t dat -t met -t tsys -t sefd -t phc -t fmt -t cbl -u" + \
                        " -o " + anc_file + " " + \
                        ivs_logs[i]
...
#
    ...
    for path, dirs, files in os.walk(pf.exp_dir):
        ...
        for file in files:
            ...
            if ( file.find ( pf.exp + "cal.vlba" ) == 0 ):
                 logv.append ( path + "/" + file )
            if ( file.find ( pf.exp ) == 0 and file.find ( ".anc" ) ==  len(pf.exp) + 3 ):
                 logc.append ( path + "/" + file )
            nlogc = nlogc + 1

    if ( len(logv) + len(loga) + len(logc) == 0 ):
    ...
    if ( pf.verb > 1 ):
         ...
         if ( len(logc) > 0 ):
              pima_print_mes ( "pf.py: found %s log-files of anc  type in %s" % \
                               (len(logc), pf.exp_dir), log  )

    if ( len(logv) > 0 ):
#
# ------ Processing telemeetry files in vlba format
#
         ...
         if ( ret != 0 ):
              if ( pf.verb > 0 ):
                   print ( "pf.py: Importing antenna calibration into PIMA " + \
    ...
    if ( len(loga) > 0 ):
#
# ------ Processing telemeetry files in antab format
#
         for file in loga:
    ...
    if ( len(logc) > 0 ):
#
# ------ Processing telemeetry files in anc format
#
         for file in logc:
             com = pima_com + " " + "anc_file " + file
             com = com + " " + \
                   "BANDPASS_FILE:       NO" + " " +  \
                   "BANDPASS_MASK_FILE:  NO" + " " +  \
                   "PCAL_MASK_FILE:      NO" + " " +  \
...
#
    ...
    elif ( subtask == "allfine" ):
         fri_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_all.fri"
         frr_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_all.frr"
         log_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_allfine.log"
    elif ( subtask == "alifine" ):
         fri_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_ali.fri"
         frr_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_ali.frr"
         log_file = pf.exp_dir + "/" + pf.exp + "_" + pf.band + "_alifine.log"
    else:
         print ( "Unknown subtask %s of task fine " % subtask, file=log )
         print ( "Unknown subtask %s of task fine " % subtask           )
         exit  ( 1 )
...
#
              with open(pf.cnt_file) as f:
                   up_cnt = f.readlines()
              f.close()
              up_cnt = pf_resolve_envar ( pf, up_cnt )

#
# ----------- Check there how the 2nd band is defined
#
              for line in up_cnt:
                  if ( len(line.split()) < 2 ): continue
...
#
        ...
        if ( pf.static ):
             pima_com = pima_exec["static"]  + " " + pf.cnt_file + " " + \
                        "gean " + pima_com + " POLARCAL_FILE: NO"
        else:
...
#
    if ( pf.static ):
          pima_com = pima_exec["static"]  + " " + pf.cnt_file + " " + "prga" + " POLARCAL_FILE: NO"
    else:
          pima_com = pima_exec["dynamic"] + " " + pf.cnt_file + " " + "prga" + " POLARCAL_FILE: NO"
    if ( not pf.dry_run ):
         (ret, out) = exe_noout_log ( pima_com, pf.verb, log )
    else:
         print ( pima_com )
        ...
        if ( pf.dry_run ):
             ...
             if ( ret != 0 ):
                  for line in out:
                      print ( "pf.py: ", line.strip("\n") )
                      print ( "pf.py: ", line.strip("\n"), file=log )
                  pima_print_mes ( "Failure in an attempt to run task splt for reference sources", log )
                  exit ( 1 )
...
def pf_gaco ( pf, sources ):
    ...
    for line in cnt:
        ...
        if ( line.split()[0] == "SESS_CODE:" ):
             sess_code = line.split()[1]
        if ( line.split()[0] == "SPLT.GAIN_CORR_FILE:" ):
             gac_file  = line.split()[1]

    if ( gac_file == "NO" or gac_file == "??" ):
        pima_print_mes ( "Trap of internal control: cannot run gaco because " + 
                         "SPLT.GAIN_CORR_FILE is not defined in the control file " + pf.cnt_file, log )
        exit  ( 1 )
...
#
# --- Remiove stale gaco file, if it exists
#
    if ( os.path.isfile(gac_file) ):
         os.unlink ( gac_file )
...
# --- Task: alifine
#
    fine_parser = subparsers.add_parser ( "alifine", \
                                          help="Perform fine fringe fitting for all 4 polarizations" )
    fine_parser.set_defaults ( task="alifine" )
    fine_parser.add_argument ( "-keep", "--keep", \
                               dest="keep", \
                               action="store_true", \
                               default=None, \
                               help="Append results of fringe fitting for 4 polarization combinations and I to the existing output files" )
    fine_parser.add_argument ( "-resume", "--resume", \
                               dest="resume", \
                               action="store_true", \
                               default=False, \
...
#
    ...
    elif ( args.task == "alifine" ):
         if ( args.keep and args.resume ):
              print ( "ERROR pf.py: flags keep and resume are not compatible" )
              exit ( 1 )
         pf_fine ( pf, "alifine", args.keep, args.resume  )
    elif ( args.task == "bpas" ):
