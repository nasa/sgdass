...
! *                                                                      *
! * ### 08-JAN-2006  PIMA_SCAN_SPLIT v1.22 (d) L. Petrov 07-FEB-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
...
!
     &                   ( AP_LEN_MAX > 0.0 .AND. (AP_LEN_MAX - PIM%OBS(IND_OBS)%AP_LEN) > PIM%CONF%AP_TOLERANCE ) ) THEN
!
...
!
                       WRITE ( 6, * ) 'PIMS_SCAN_SPLIT: UV points with different AP lengths were found for obs ', IND_OBS
                       WRITE ( 6, * ) '  J42= ', INT2(J42), ' J43= ', INT2(J43), &
     &                                '  NUM_EPC= ', PIM%OBS(IND_OBS)%NUM_EPC
                       ...
                       WRITE ( 6, * ) '  DIFF1= ', TIM_ARR(J43,J42) - TIM_ARR(1,J42) - IND_AP*PIM%OBS(IND_OBS)%AP_LEN
                       WRITE ( 6, * ) '  DIFF2= ', DABS(TIM_ARR(J43,J42) - TIM_ARR(1,J42) - IND_AP*PIM%OBS(IND_OBS)%AP_LEN) - &
     &                                             IND_AP*(AP_LEN_MAX - PIM%OBS(IND_OBS)%AP_LEN) 
                       IF ( AP_LEN_MAX > 0.0 ) WRITE ( 6, * ) '  DIFF3= ', AP_LEN_MAX - PIM%OBS(IND_OBS)%AP_LEN
                       WRITE ( 6, * ) '  tim= ', MJDSEC_TO_DATE ( PIM%MJD_0, PIM%TAI_0 + TIM_ARR(J42,1), -2 )
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 12-JUN-2020 PIMA_OBS_INIT v1.0 (d)  L. Petrov  12-JUN-2020 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'pima.i'
