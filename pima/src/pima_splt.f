...
! *                                                                      *
! *  ### 30-JAN-2011   PIMA_SPLT  v4.16  (d) L. Petrov  08-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           FL_AC_USE, FL_SPLT_SNR_FRI_ONLY, FL_TSYS_TOUSE
     ...
     &           LU_OBS, NTHR, K_GAI, K_SEFD, U_UVO, NUM_USED_AP(PIM__MCHN,PIM__MPLR), &
     &           IND_TUS_STA, IER
...
!      PARAMETER ( PIMA__PHS_APSO_LIM  = 0.1D0 )
     ...
     &           NOI, SEFD_TOTAL, LOG_SEFD_TOTAL, &
     &           GAIN_TOTAL, LOG_GAIN_TOTAL, &
     &           DECOR_BS, DECOR_TS, &
...
!
 ...
 420  CONTINUE
!
      IF ( PIM%CONF%L_TUS > 0 ) THEN
           DO 440 J4=1,PIM%NSTA
              IND_TUS_STA = LTM_DIF ( 0, PIM%CONF%L_TUS, &
     &                                   PIM%CONF%TSYS_USE_STA, PIM%C_STA(J4) )
              FL_TSYS_TOUSE = .TRUE.
...
!
              ...
              IF ( .NOT. FL_TSYS_TOUSE ) THEN
                   PIM%STA(J4)%TSYS(PIM%CONF%FRQ_GRP)%AVAIL = .FALSE.
              END IF
 440       CONTINUE 
      END IF
!
      DO 460 J6=1,PIM%NSTA
         DO 470 J7=1,PIM%NFRG
            IF ( PIM%STA(J6)%GAIN(J7)%AVAIL ) THEN
                 IF ( .NOT. ASSOCIATED ( PIM%STA(J6)%GAIN(J7)%SENS ) ) THEN
                      CALL CLRCH ( STR ) 
                      CALL INCH  ( J7, STR )
                      CALL ERR_LOG ( 8420, IUER, 'PIMA_SPLT', 'No gain '// &
     &                    'was defined for station '//PIM%C_STA(J7)// &
     &                    ' and frequency group '//STR )
                      RETURN 
                 END IF
            END IF
 470     CONTINUE 
...
!
     ...
     &        PIM%CONF%SPLT_SOU_NAME == 'ALL'      ) THEN
              CONTINUE
            ELSE IF ( PIM%CONF%SPLT_SOU_NAME == PIM%SOU(J8)%NAME ) THEN
              CONTINUE
            ELSE IF ( PIM%CONF%SPLT_SOU_NAME == PIM%SOU(J8)%J2000_NAME ) THEN
              CONTINUE
            ELSE
              GOTO 480
         END IF
...
!
         ...
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 1 ) THEN
              WRITE ( 6, 110 ) PIM%C_SOU(J8), J8, PIM%NSOU
...
!
         CALL ERR_PASS ( IUER, IER )
         CALL PIMA_SPLT_FITSTA ( PIM, VTD, J8, NSCA_USED, NOBS_IN_USED, &
     &                           NOBS_USED, OLD_GR_DEL, OLD_PH_RAT, &
     &                           OLD_GR_RAT, OLD_PH_ACC, IER )
         IF ( NOBS_USED == 0 ) THEN
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 1 ) THEN
                   WRITE ( 6, 170 ) PIM%SOU(J8)%IVS_NAME, PIM%SOU(J8)%J2000_NAME
                   CALL FLUSH ( 6 )
              END IF
              GOTO 480
         END IF
!
     ...
     &            ' based for source '//PIM%C_SOU(J8) )
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 1 ) THEN
                   WRITE ( 6, 120 ) J8, PIM%NSOU, PIM%C_SOU(J8)
...
!
         ...
         DO 490 J9=1,PIM%NSCA
            IF ( PIM%SCA(J9)%SOU_IND == J8 ) THEN
                 AP_LEN_SCA = PIM%OBS(PIM%SCA(J9)%OBS_IND(1))%AP_LEN
!
! -------------- K_SEG is the number of segments in the J9-th scan
!
                 TIM_SCA_BEG = PIM%TIM_R8(PIM%SCA(J9)%TIM_IND)
                 TIM_SCA_END = PIM%TIM_R8(PIM%SCA(J9)%TIM_IND+PIM%SCA(J9)%NUM_EPC-1)
...
!
                    TIM_SEG(1,N_SEG) = PIM%TIM_R8(PIM%SCA(J9)%TIM_IND) + &
     &                                 (J10-1)*AP_LEN_SCA*PIM%CONF%SPLT_TIM_MSEG
...
!
                    IF ( J10 ==     1 ) TIM_SEG(1,N_SEG) =  1.D9
                    IF ( J10 == K_SEG ) TIM_SEG(2,N_SEG) = -1.D9
!
                    IND_SCA_SEG(N_SEG) = J9
                    DO 4110 J11=1,PIM%SCA(J9)%NBAS
                       IND_OBS = PIM%SCA(J9)%OBS_IND(J11)
...
!
                       IF ( .NOT. PIM%STA(PIM%OBS(IND_OBS)%STA_IND(1))%TSYS(PIM%CONF%FRQ_GRP)%AVAIL ) GOTO 8110
                       IF ( .NOT. PIM%STA(PIM%OBS(IND_OBS)%STA_IND(2))%TSYS(PIM%CONF%FRQ_GRP)%AVAIL ) GOTO 8110
                       IF ( .NOT. PIM%USE_OBS(IND_OBS) ) GOTO 8110 ! Bypass the deselected observation
                       IF ( FRG_IND == 0 ) GOTO 8110
!
                       IF ( PIM%CONF%TSYS_CAL_CODE == PIMA__TSYS_MEASURED ) THEN
!
! ------------------------- Case of measured system temperature
!
                            IF ( PIM%OBS(IND_OBS)%TSYS_IND(1,PIM%CONF%FRQ_GRP) .LE. 0 ) GOTO 8110
                            IF ( PIM%OBS(IND_OBS)%TSYS_IND(2,PIM%CONF%FRQ_GRP) .LE. 0 ) GOTO 8110
...
!
     ...
     &                               ' -- please run task tsmo' )
                                 RETURN 
                            END IF
                            IF ( PIM%OBS(IND_OBS)%STMO_IND(1) .LE. 0 ) GOTO 8110
                            IF ( PIM%OBS(IND_OBS)%STMO_IND(2) .LE. 0 ) GOTO 8110
                       END IF
!
                       L_UVO = L_UVO + 1
!
...
!
 ...
 4110               CONTINUE 
                    IF ( PIM%CONF%DEBUG_LEVEL .GE. 7 ) THEN
                         WRITE ( 6, 140 ) IND_OBS, N_SEG, TIM_SEG(1:3,N_SEG), L_UVO 
 140                     FORMAT ( 'PIMA_SPLT Ind_obs: ', I6, ' Seg: ', I5, ' Tim_123: ', &
 ...
 4100            CONTINUE 
     ...
     &                     TIM_SEG(2,N_SEG) < -86400.0D0      ) THEN
                           WRITE ( 6, 150 ) J9, L_UVO, K_SEG, N_SEG
 150                       FORMAT ( 'PIMA_SPLT Sca: ', I4,' L_UVO: ', I5, &
     &                              ' K_SEG: ', I5, ' N_SEG: ', I5, &
     &                              ' Skipped ' )
                        ELSE 
                          WRITE ( 6, 160 ) J9, L_UVO, K_SEG, N_SEG, &
...
!
     ...
     &            'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &            'memory for UVO data for source '//PIM%C_SOU(J8) )
              RETURN
         END IF
         CALL NOUT ( L_UVO*SIZEOF(UVO(1)), UVO )
!
! ------ Check whether we are going to compute weights over an interval
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              ...
              CALL ERR_LOG ( 8430, IUER, 'PIMA_SPLT', 'Failure to '// &
     &            'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     ...
     &             PIM%C_SOU(J8) )
         ...
         END IF
         DO 4120 J12=1,L_UVO
            ALLOCATE ( UVO(J12)%SPE(KFRQ,LPOL), &
     &                 UVO(J12)%WEI(KFRQ,LPOL), &
     &                 UVO(J12)%SNR(KFRQ,LPOL), &
     &                 UVO(J12)%SEFD(KFRQ,LPOL), STAT=IER )
            IF ( IER .NE. 0 ) THEN
                 CALL CLRCH ( STR )
                 CALL IINCH ( 4*KFRQ*LPOL*SIZEOF(UVO(J12)%SPE(1,1)), STR )
...
!
 ...
 4120    CONTINUE 
         IF ( PIM%CONF%SPLT_TOTAL_UV == PIMA__TOTAL_UV_YES ) THEN
              FIL_TOT= TRIM(FITSDIR)//'/'//PIM%SOU(J8)%J2000_NAME//'_'// &
...
!
         K_UVO = 1
         DO 4130 J13=1,PIM%NSCA
            IF ( PIM%SCA(J13)%SOU_IND .NE. J8 ) GOTO 4130
!
! --------- Cycle over all *used* observations of this scans
!
            DO 4140 J14=1,PIM%SCA(J13)%NBAS
               IF ( FL_TIMER ) CALL WALL_TIMER ( %VAL(0) )
               IND_OBS = PIM%SCA(J13)%OBS_IND(J14)
               FRG_IND = PIM%OBS(IND_OBS)%REF_FRG_INDS(PIM%CONF%FRQ_GRP)
               IF ( FRG_IND == 0 ) THEN
                    IF ( PIM%CONF%DEBUG_LEVEL .GE. 4 ) THEN
                         WRITE ( 6, 210 ) IND_OBS
...
!
               IF ( .NOT. PIM%USE_OBS(IND_OBS) ) GOTO 4140 ! Bypass deselected observation
               IF ( PIM%CONF%TSYS_CAL_CODE == PIMA__TSYS_MEASURED ) THEN
 ...
 220                          FORMAT ( 'PIMA_SPLT: observation ', I6, ' is discarded '// &
     &                                 'because there is not Tsys at ', A )
                         END IF
                         GOTO 4140
                    END IF
...
!
               ...
               END IF
!
               IF ( PIM%L_BPS_SUB > 0 .OR. PIM%L_PBS_SUB > 0 ) THEN
!
! ----------------- Perform bandpass substitute if needed
!
                    CALL ERR_PASS        ( IUER, IER )
                    CALL PIMA_BPASS_SUBS ( PIM, IND_OBS, IER )
                    IF ( IER .NE. 0 ) THEN
                         CALL ERR_LOG ( 7534, IUER, 'PIMA_SPLT', 'Error in '// &
     &                       'setting bandpass substitute' )
                         RETURN 
                    END IF
               END IF
!
...
!
                     IF ( FL_BPASS .AND. PIM%CONF%SPLT_BPASS_NRML_METHOD .EQ. PIMA__WEIGHTED ) THEN
                          IF ( PIM%BPASS(1)%PIMA_VERS .GE. PIMA__BPS_AMP_VERS ) THEN
                               IF ( J16 == 1 ) THEN
                                    RENRML_CROS_IF(IFRQ,J16) = &
     &                                   PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%SPLT_NRML_FRQ(J17)* &
     &                                   PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%SPLT_NRML_FRQ(J17)
                                  ELSE
!
...
!
                                     RENRML_CROS_IF(IFRQ,J16) = &
     &                                   PIM%PBP(PIM%OBS(IND_OBS)%STA_IND(1))%SPLT_NRML_FRQ(J17)* &
     &                                   PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%SPLT_NRML_FRQ(J17)
                                END IF
                            ELSE
                               IF ( PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%L_OBS > 0 .AND. &
     &                              PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%L_OBS > 0       ) THEN
                                    RENRML_CROS_IF(IFRQ,J16) = DSQRT ( &
     &                                   PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%SPLT_NRML_FRQ(J17)* &
     &                                   PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%SPLT_NRML_FRQ(J17) &
     &                                                )
                                 ELSE IF ( PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%L_OBS == 0 .AND. &
     &                                     PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%L_OBS >  0       ) THEN
                                    RENRML_CROS_IF(IFRQ,J16) = PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%SPLT_NRML_FRQ(J17) 
                                 ELSE IF ( PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%L_OBS >  0 .AND. &
     &                                    PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(2))%L_OBS == 0       ) THEN
                                    RENRML_CROS_IF(IFRQ,J16) = PIM%BPASS(PIM%OBS(IND_OBS)%STA_IND(1))%SPLT_NRML_FRQ(J17) 
                                 ELSE 
                                    RENRML_CROS_IF(IFRQ,J16) = 1.0
                               END IF
                               IF ( IS_R8_NAN(RENRML_CROS_IF(IFRQ,J16)) ) THEN
                                    RENRML_CROS_IF(IFRQ,J16) = 1.0
                               END IF
                          END IF
                        ELSE 
...
!
                          RENRML_AUTO_IF(IFRQ,1,J16) = PIM%OBS(IND_OBS)%TSRF(IFRQ,1,J16) 
                          RENRML_AUTO_IF(IFRQ,2,J16) = PIM%OBS(IND_OBS)%TSRF(IFRQ,2,J16) 
                          IF ( RENRML_AUTO_IF(IFRQ,2,J16) < PIMA__WEI_MIN ) THEN
!
! ---------------------------- This is a phathological case. Just reset the renormalization
! ---------------------------- factors to 1.
!
                               RENRML_AUTO_IF(IFRQ,1,J16) = 1.0D0
                               RENRML_AUTO_IF(IFRQ,2,J16) = 1.0D0
                          END IF
                        ELSE 
                          RENRML_AUTO_IF(IFRQ,1,J16) = 1.0D0
                          RENRML_AUTO_IF(IFRQ,2,J16) = 1.0D0
                     END IF
!
                     RENRML_CROS_IF_ALL(J16) = RENRML_CROS_IF_ALL(J16)*RENRML_CROS_IF(IFRQ,J16)
                     RENRML_AUTO_IF_ALL(J16) = RENRML_AUTO_IF_ALL(J16)*RENRML_AUTO_IF(IFRQ,1,J16)* &
     &                                                                 RENRML_AUTO_IF(IFRQ,2,J16)
!
                     IF ( PIM%CONF%FRIB_AMPL_EDGE_BEAM_COR == PIMA__BEAM_YES ) THEN
!
! ----------------------- Compute factors for the primary beam attenuation
!
                          NRML_BEAM_ATT(1) = PIMA_BEAM_ATT ( PIM, PIM%FREQ_ARR(1,J17,PIM%CONF%FRQ_GRP),   &
     &                                                            INT(PIM%OBS(IND_OBS)%SOU_IND,KIND=4),   &
     &                                                            INT(PIM%OBS(IND_OBS)%STA_IND(1),KIND=4) )
                          NRML_BEAM_ATT(2) = PIMA_BEAM_ATT ( PIM, PIM%FREQ_ARR(1,J17,PIM%CONF%FRQ_GRP),   &
     &                                                            INT(PIM%OBS(IND_OBS)%SOU_IND,KIND=4),   &
     &                                                            INT(PIM%OBS(IND_OBS)%STA_IND(2),KIND=4) )
                        ELSE 
...
!
                     PIM%OBS(IND_OBS)%UV(1:PIM%NCHN,IFRQ,1:LTIM,J16) = &
     &                       PIM%OBS(IND_OBS)%UV(1:PIM%NCHN,IFRQ,1:LTIM,J16)*RENRML_CROS_IF(IFRQ,J16)/ &
 ...
 4170             CONTINUE
                  IF ( FL_TIMER .AND. PIM%CONF%DEBUG_LEVEL .EQ. 7 ) THEN
                       ...
                       CALL WALL_TIMER ( %VAL(0) )
                  END IF
                  RENRML_CROS_IF_ALL(J16)= RENRML_CROS_IF_ALL(J16)**(1.D0/LFRQ)
                  RENRML_AUTO_IF_ALL(J16)= RENRML_AUTO_IF_ALL(J16)**(1.D0/(2.0D0*LFRQ))
!
                  PH_DEL_APSO = 0.0D0
                  PH_RAT_APSO = 0.0D0
                  PH_ACC_APSO = 0.0D0
...
!
                  ...
                  DURA_ACC = 0.0D0
                  DO 4200 J20=1,LTIM
                     UV_IND  = PIM%OBS(IND_OBS)%UV_IND(J20,FRG_IND)
                     TIM_ARR(J20) = PIM%TIM_R8(PIM%UV_IND(UV_IND)%TIM_IND) - &
     &                              PIM%OBS(IND_OBS)%TIM_BEG
                     IF ( TIM_ARR(J20) < PIM%CONF%SCAN_LEN_SKIP ) THEN
                          PIM%OBS(IND_OBS)%WEI_1D(J20,J16) = 0.0D0
                     END IF 
                     DURA_ACC = DURA_ACC + PIM%OBS(IND_OBS)%WEI_1D(J20,J16)*PIM%OBS(IND_OBS)%AP_LEN
...
!
     ...
     &                    PIM%CONF%BANDPASS_USE == PIMA__BPASS_AMP           ) THEN
                          ...
                          DO 4210 J21=PIM%CONF%BEG_FRQ,PIM%CONF%END_FRQ
                             IFRQ = IFRQ + 1
                             DO 4220 J22=1,PIM%NCHN
                                IF ( PIM%BPASS(1)%PIMA_VERS .GE. PIMA__BPS_AMP_VERS ) THEN
!
! ---------------------------------- Modern, post 2020.03.03 case
!
                                     AMP_BPASS = 1.0/( &
     &                                                 ABS( PIM%BPASS(IND_STA(1))%BPS(J22,J21) ) * &
     &                                                 ABS( PIM%BPASS(IND_STA(2))%BPS(J22,J21) )   &
     &                                                )
                                     IF ( ASSOCIATED ( PIM%PBP ) ) THEN
                                          IF ( J16 == 2 .AND. ASSOCIATED ( PIM%PBP(IND_STA(1))%CMPL ) ) THEN
                                               AMP_BPASS = AMP_BPASS/( &
     &                                                                 ABS( PIM%PBP(IND_STA(1))%CMPL(J22,J21) ) &
     &                                                                )
                                            ELSE IF ( J16 == 3 .AND. ASSOCIATED ( PIM%PBP(IND_STA(2))%CMPL ) ) THEN
                                               AMP_BPASS = AMP_BPASS/( &
     &                                                                 ABS( PIM%PBP(IND_STA(2))%CMPL(J22,J21) ) &
     &                                                     )
                                            ELSE IF ( J16 == 4 .AND.                               &
...
!
                                          AMP_BPASS = 1.0/ABS( PIM%BPASS(IND_STA(2))%BPS(J22,J21) )
...
!
                                          AMP_BPASS = 1.0/ABS( PIM%BPASS(IND_STA(1))%BPS(J22,J21) )
                                        ELSE
!
...
!
                                PIM%OBS(IND_OBS)%UV(J22,IFRQ,J20,J16) = AMP_BPASS * PIM%OBS(IND_OBS)%UV(J22,IFRQ,J20,J16) 
 4220                        CONTINUE 
 4210                     CONTINUE 
                     END IF ! bandpass use
!
                     IF ( PIM%OBS(IND_OBS)%WEI_1D(J20,J16) > PIMA__WEI_MIN ) THEN
!
! ----------------------- Update index of the first and the last used AP
!
                          IF ( IND_AP_USED(1) == 0 ) IND_AP_USED(1) = J20
                          IND_AP_USED(2) = J20
                     END IF
 4200             CONTINUE 
!
...
!
     ...
     &                           PIM%OBS(IND_OBS)%UV(1,1,1,J16), IER )
                       IF ( IER .NE. 0 ) THEN
...
!
     ...
     &                                        PIM%OBS(IND_OBS)%FRT_OFFSET(IND_BND), &
     &                                        TIM_ARR, PIM%OBS(IND_OBS)%WEI_1D(1,J16), &
     ...
     &                         'correction for phase acceleration term in fringe phase' )
                 ...
                 DEALLOCATE ( TIM_ARR )
!
                 IF ( ( PIM%CONF%DEBUG_LEVEL == 31 .AND. J16 == 1 ) .OR. &
     &                ( PIM%CONF%DEBUG_LEVEL == 32 .AND. J16 == 4 )      ) THEN
!
...
!
     ...
     &                                  PIM%OBS(IND_OBS)%UV(1,1,1,J16), &
     &                                  PIM%OBS(IND_OBS)%AC(1,1,1,1,J16), &
     ...
     &                                  SNR, POL_LAB, 3, IER )
                 END IF
!
                 IF ( ( PIM%CONF%DEBUG_LEVEL == 33 .AND. J16 == 1 ) .OR. &
     &                ( PIM%CONF%DEBUG_LEVEL == 34 .AND. J16 == 2 )      ) THEN
!
...
!
              ...
              SRT_WRT_FRT_OFFS = PIM%SUB%TIM_SRT(PIM%SUB%OBS_IND_SUB(IND_OBS),J13) - &
...
!
                 IF ( IND_SCA_SEG(J23) .NE. PIM%OBS(IND_OBS)%SCA_IND ) GOTO 4230
                 IF ( K_UVO > L_UVO ) THEN
                      IER = 0
                      CALL CLRCH ( STR )
                      CALL INCH  ( IND_OBS, STR )
     ...
     &                    ' observation '//TRIM(STR)//' for source '//PIM%C_SOU(J8)// &
...
!
                 UVO(K_UVO)%IND_OBS = IND_OBS
                 UVO(K_UVO)%IND_SEG = J23
                 UVO(K_UVO)%STA_IND = PIM%OBS(IND_OBS)%STA_IND
                 UVO(K_UVO)%IND_SUB = PIM%SUB%OBS_IND_SUB(IND_OBS)
!
                 DO 4240 J24=1,LTIM
                    IND_TIM = PIM%UV_IND(PIM%OBS(IND_OBS)%UV_IND(J24,FRG_IND))%TIM_IND
!
                    IF ( PIM%TIM_R8(IND_TIM) .GE. TIM_SEG(1,J23) - TIM_TOL .AND. &
     &                   PIM%TIM_R8(IND_TIM) .LE. TIM_SEG(2,J23) + TIM_TOL       ) THEN
!
...
!
                         FL_AC_USE = .FALSE.
                         DO 4250 J25=1,LPOL 
                            IF ( PIM%OBS(IND_OBS)%WEI_1D(J24,J25) > PIM%CONF%FRIB_WEIGHTS_THRESHOLD ) THEN
                                 CALL ERR_PASS ( IUER, IER )
                                 CALL PIMA_UVO_UPDATE ( PIM, LFRQ, KFRQ, J25, LTIM, &
     &                                                  IND_OBS, J24, PIM%OBS(IND_OBS)%UV, &
     &                                                  PIM%OBS(IND_OBS)%WEI_1D(J24,J25)*WEI_FRQ_CHN, &
     ...
     &                                    'input FITS-IDI file ' )
                                 ...
                                 END IF
                                 DO 4260 J26=1,KFRQ
                                    IF ( UVO(K_UVO)%SEFD(J26,J25) > PIMA__SEFD_MIN ) THEN
                                         FL_AC_USE = .TRUE.
                                         IF ( WEI_IF(J26,J25) > PIMA__AMP_MIN ) THEN
                                              NUM_USED_AP(J26,J25) = NUM_USED_AP(J26,J25) + 1
                                         END IF
                                    END IF
 4260                            CONTINUE 
                            END IF
...
!
                         IF ( UVO(K_UVO)%TIM_IND(1) == 0 ) THEN
                              UVO(K_UVO)%TIM_IND(1) = PIM%UV_IND(PIM%OBS(IND_OBS)%UV_IND(J24,FRG_IND))%TIM_IND
                         END IF
                         UVO(K_UVO)%TIM_IND(2) = PIM%UV_IND(PIM%OBS(IND_OBS)%UV_IND(J24,FRG_IND))%TIM_IND
                         IND_SEG_LAST = J23
                         IND_AP_LAST  = J24
!
                         IF ( FL_WEI_OBS ) THEN
...
!
                              IF ( FL_AC_USE ) THEN
                                   IAC_SEG = IAC_SEG + 1
                              END IF
                              IF ( IAC_SEG == L_WEI_SEG  .OR. J24 == LTIM ) THEN
!
...
!
     ...
     &                                                                    SUMVIS_WEISEG(1:LPOL,1:KFRQ) 
                                   ...
                                   DO 4270 J27=1,LPOL
                                      DO 4280 J28=1,KFRQ
                                         IF ( UVO(K_UVO)%SEFD(J28,J27) > PIMA__SEFD_MIN ) THEN
                                              SEFD_IF(J28,J27) = UVO(K_UVO)%SEFD(J28,J27)
                                         END IF
                                         IF ( ACC_WEI_SEG(J27,J28) > PIMA__WEI_MIN .AND. &
     &                                        ACC_NAP(J27,J28) > 0                       ) THEN
!
! ------------------------------------------- Compute the weighted amplitude and weighted variance
! ------------------------------------------- of the amplitudes averaged over the segment
!
                                              ACC_AMPL(J27,J28) = ABS(ACCVIS_WEI_SEG(J27,J28))/ACC_WEI_SEG(J27,J28)
                                              ACC_WEI(J27,J28,NUM_SEG_WEI) = ( ACCVIS_WEISQ_SEG(J27,J28)/ACC_WEI_SEG(J27,J28)  - &
     &                                                                         ACC_AMPL(J27,J28)**2 )/ &
     &                                                                         ACC_WEI_SEG(J27,J28)
                                            ELSE 
                                              ACC_AMPL(J27,J28) = 0.0
                                              ACC_WEI(J27,J28,NUM_SEG_WEI) = 0.0
                                         END IF
 4280                                 CONTINUE 
 4270                              CONTINUE 
...
!
                 ...
                 NUMP_ALL = 0
                 DO 4290 J29=1,LPOL
                    SUM_WEI_ALL = SUM_WEI_ALL + SUM_WEI_BAND(J29)
                    NUMP_ALL    = NUMP_ALL    + NUMP_BAND(J29)
 4290            CONTINUE 
!
                 IF ( SUM_WEI_ALL > PIMA__WEI_MIN ) THEN
!
! ------------------- We see that the J23-th segment contributed 
...
!
                         DO 4310 J31=1,KFRQ
                            IF ( UVO(K_UVO)%WEI(J31,J30) > 0.0 ) THEN
                                 UVO(K_UVO)%SPE(J31,J30) = UVO(K_UVO)%SPE(J31,J30)/UVO(K_UVO)%WEI(J31,J30) 
                            END IF
                            IF ( NUMP_SEG(J30,J31) > 0 ) THEN
                                 UVO(K_UVO)%WEI(J31,J30) = UVO(K_UVO)%WEI(J31,J30)/NUMP_SEG(J30,J31)
                            END IF
!
                            IF ( LPOL .GE. 2 .AND. PIMA__POLAR_CODE(POLC_INDS(J30)) == PIMA__POLAR_LL ) THEN
                                 FEED_ANG_DIF = PIM%OBS(IND_OBS)%FEED_ANG(1) - &
     &                                          PIM%OBS(IND_OBS)%FEED_ANG(2) 
                                 UVO(K_UVO)%SPE(J31,J30) = UVO(K_UVO)%SPE(J31,J30)* &
...
!
                                 UVO(K_UVO)%SPE(J31,J30) = CONJG ( UVO(K_UVO)%SPE(J31,J30) )
                            END IF
!
                            IF ( FL_BPASS ) THEN
                                 IF ( PIM%BPASS(1)%PIMA_VERS .GE. PIMA__BPS_AMP_VERS ) THEN
...
!
                                      IF ( RENRML_AUTO_IF(IND_FRQ(4,J31),1,J30) > PIMA__AMP_MIN .AND. &
     &                                     RENRML_AUTO_IF(IND_FRQ(4,J31),2,J30) > PIMA__AMP_MIN       ) THEN
                                           UVO(K_UVO)%SPE(J31,J30) = UVO(K_UVO)%SPE(J31,J30)/ &
     &                                                    DSQRT(RENRML_AUTO_IF(IND_FRQ(4,J31),1,J30)* &
     &                                                          RENRML_AUTO_IF(IND_FRQ(4,J31),2,J30)  )
                                      END IF
...
!
                                       RENRML_AUTO_IF(J31,1,J30) = 1.0D0
!
! ------------------------------------ Renormalization of fringe amplitude
!
                                       UVO(K_UVO)%SPE(J31,J30) = UVO(K_UVO)%SPE(J31,J30)/RENRML_AUTO_IF(J31,1,J30)
                                 END IF
                            END IF
 4310                    CONTINUE
!
                         DO 4320 J32=1,KFRQ
                            IF ( SUM_WEI_SEG(J30,J32) > PIMA__WEI_MIN .AND. NUMP_SEG(J30,J32) > 0 ) THEN
!
! ------------------------------ Now we compute weighted amplitude, phase and square of 
! ------------------------------ amplitude standard deviation for a given segment
!
                                 AMPL = ABS(SUMVIS_WEISEG(J30,J32))/SUM_WEI_SEG(J30,J32)
                                 PHAS = PHAS_CMPL_R4 ( SUMVIS_WEISEG(J30,J32) )
                                 SIG_SQ = SUMVIS_WEISQ_SEG(J30,J32)/SUM_WEI_SEG(J30,J32) - AMPL**2
!
                                 IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
                                      TIM_EFF_SEG  = PIM%OBS(IND_OBS)%AP_LEN * PIM%CONF%SPLT_TIM_MSEG * &
     &                                               SUM_WEI_SEG(J30,J32)/NUMP_SEG(J30,J32)
                                      TIM_EFF_BAND = PIM%OBS(IND_OBS)%EFF_DUR(IND_BND) 
                                      WRITE ( 6, 1110 ) K_UVO, J30, J32, UVO(K_UVO)%IND_OBS, AMPL, PHAS, &
     &                                                  SQRT ( SIG_SQ/NUMP_SEG(J30,J32) ), &
     &                                                  AMPL/( SNR* DSQRT ( TIM_EFF_SEG/TIM_EFF_BAND ) ), &
     &                                                  NUMP_SEG(J30,J32), UVO(K_UVO)%WEI(J32,J30), &
     &                                                  NUM_USED_AP(J32,J30)
 1110                                 FORMAT ( ' PIMA_SPLT uvo ', I6, ' Lpol: ', I1, &
     ...
     &                                         ' Num_used_ap: ', I5 )
                            ...
                            END IF
!
                            IF ( SUM_WEI_SEG(J30,J32) > PIMA__WEI_MIN .AND. &
     &                           NUMP_SEG(J30,J32) > 0                .AND. &
                                 ( ( PIM%CONF%SPLT_WEIGHT_TYPE == PIMA__SPLT_WEI_SEG_RMS .AND.  &
                                     NUMP_SEG(J30,J32)/PIM%NCHN .GE.  NAP__SIG )     .OR.   &
                                   ( PIM%CONF%SPLT_WEIGHT_TYPE == PIMA__SPLT_WEI_AUTO .AND. &
                                     NUMP_SEG(J30,J32)/PIM%NCHN .GE. L_WEI_SEG              ) ) ) THEN
!
...
!
                                 UVO(K_UVO)%SNR(J32,J30) = AMPL/SQRT( SIG_SQ/NUMP_SEG(J30,J32) )
!
! ------------------------------ We scale the weights to the total smearing
! ------------------------------ NB: FITS-IDI convention: weights are reciprocal to the variance
!
                                 UVO(K_UVO)%WEI(J32,J30) = 1.0D0/(SIG_SQ/NUMP_SEG(J30,J32))*(DECOR_BS * DECOR_TS)**2
                               ELSE IF ( SUM_WEI_SEG(J30,J32) > PIMA__WEI_MIN      .AND. &
     &                                   NUMP_SEG(J30,J32) > 0                     .AND. &
                                         PIM%CONF%SPLT_WEIGHT_TYPE == PIMA__SPLT_WEI_ONE ) THEN
!
! ------------------------------ We set weight to unity (except a patological case when there are no data)
!
                                 IF ( NUMP_SEG(J30,J32) > 0 .AND. &
     &                                SUM_WEI_SEG(J30,J32) > PIMA__WEI_MIN ) THEN
                                      UVO(K_UVO)%WEI(J32,J30) = 1.0
                                      UVO(K_UVO)%SNR(J32,J30) = 1.0
                                    ELSE 
                                      UVO(K_UVO)%WEI(J32,J30) = 0.0
                                      UVO(K_UVO)%SNR(J32,J30) = 0.0
                                 END IF
                               ELSE 
!
...
!
                                 IF ( NUMP_SEG(J30,J32) > 0 .AND. &
     &                                SUM_WEI_SEG(J30,J32) > PIMA__WEI_MIN  .AND. &
...
!
     ...
     &                                               SUM_WEI_SEG(J30,J32)/NUMP_SEG(J30,J32)
                                      TIM_EFF_BAND = PIM%OBS(IND_OBS)%EFF_DUR(IND_BND) 
!
                                      UVO(K_UVO)%SNR(J32,J30) = SNR* DSQRT ( TIM_EFF_SEG/TIM_EFF_BAND )
                                      UVO(K_UVO)%WEI(J32,J30) = 1.0D0/(PIM%OBS(IND_OBS)%AMPL(PIMA__DRF,IND_BND)/UVO(K_UVO)%SNR(J32,J30))**2
                                    ELSE 
!
...
! 
                            IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
                                 WRITE ( 6, 1120 ) J30, IND_OBS, K_UVO, J32, &
     &                                   PIM%TAI_0 + UVO(K_UVO)%TIM + PIM%UTC_MTAI, &
     &                                   ABS( UVO(K_UVO)%SPE(J32,J30) ), &
     &                                   PHAS_CMPL_R4 ( UVO(K_UVO)%SPE(J32,J30) ), &
     &                                   UVO(K_UVO)%WEI(J32,J30), 1 
...
!
 ...
 8230         CONTINUE 
!
              LOG_SEFD_TOTAL = 0.0D0
              IFRQ = 0
              K_SEFD  = 0
              K_GAI   = 0
              LOG_GAIN_TOTAL = 0.0D0
              DRF_ALL = 0.0
              WEI_ALL = 0.0
              DO 4330 J33=1,LPOL
                 DO 4340 J34=1,KFRQ
                    IF ( LPOL == 1 .OR.                         &
     &                   LPOL == 2 .OR.                         &
     &                   LPOL == 4 .AND. J33 == 1 .OR. J33 == 4 ) THEN
                         WEI_ALL = WEI_ALL + WEI_IF(J34,J33)
                         WW_ALL  = WW_ALL  + WW_IF(J34,J33)
                         DRF_ALL = DRF_ALL + DRF_IF(J34,J33)* &
     &                                           PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(1))%GAIN_CORR(J34)* &
     &                                           PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(2))%GAIN_CORR(J34)
                    END IF
!
                    IF ( WEI_IF(J34,J33) > PIMA__WEI_MIN  ) THEN
                         IF ( PIM%CONF%SPLT_TOTAL_UV == PIMA__TOTAL_UV_YES ) THEN
!
...
!
     ...
     &                                                          + PIM%OBS(IND_OBS)%FRT_OFFSET(1) &
     &                                                          + PIM%UTC_MTAI, -2 )
                              IF ( IS_R4_NAN ( REAL(DRF_IF(J34,J33)) ) .OR. &
     &                             IS_R4_NAN ( IMAG(DRF_IF(J34,J33)) )      ) THEN
                                   DRF_IF(J34,J33) = 0.0
                              END IF
                              WRITE ( LUN_TOT, 1134 ) PIM%SOU(J8)%J2000_NAME, &
                                                      PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(1)), & 
     &                                                PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2)), &
     &                                                STR(1:23), IND_OBS, J34, J33, &
     &                                                ABS(DRF_IF(J34,J33)/WEI_IF(J34,J33))* &
     &                                                PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(1))%GAIN_CORR(J34)* &
     &                                                PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(2))%GAIN_CORR(J34), &
     &                                                PHAS_CMPL_R4(DRF_IF(J34,J33)), SEFD_IF(J34,J33), &
     &                                                UVO(K_UVO-1)%UVW(1:2)*PIM%FREQ_ARR(1,PIM%CONF%BEG_FRQ,PIM%CONF%FRQ_GRP)
 1134                         FORMAT ( 'Sou: ', A, ' Bas: ', A, 1X, A, &
     &                                 ' UTC_epoch: ' ,A, ' Ind_obs: ', I5, ' I_frq: ', I2, &
     &                                 ' I_pol: ', I1, ' Ampl: ', F14.7, ' Phase: ', F9.6, ' SEFD: ', F9.2, &
     &                                 ' UV: ', 1PD13.6, 1X, 1PD13.6 )
                         END IF
                         IFRQ = IFRQ + 1
                         IF ( SEFD_IF(J34,J33) > PIMA__SEFD_MIN ) THEN
                              LOG_SEFD_TOTAL = LOG_SEFD_TOTAL + DLOG ( MAX(SEFD_IF(J34,J33), 1.0D-6 ) )
                              K_SEFD = K_SEFD + 1
                         END IF
                         IF ( PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(1))%GAIN_CORR(J34)* &
     &                        PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(2))%GAIN_CORR(J34)  > PIMA__WEI_MIN ) THEN
                              LOG_GAIN_TOTAL = LOG_GAIN_TOTAL + DLOG ( MAX ( PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(1))%GAIN_CORR(J34)* &
     &                                                                       PIM%GACO(PIM%OBS(IND_OBS)%STA_IND(2))%GAIN_CORR(J34), 1.0D-12 ) )
                              K_GAI = K_GAI + 1
                         END IF
                    END IF
 4340            CONTINUE 
...
!
                   IF ( K_SEFD > 0 ) THEN
                        SEFD_TOTAL = DEXP ( LOG_SEFD_TOTAL/K_SEFD )
                      ELSE
                        SEFD_TOTAL = 1.0D0
                   END IF
                   IF ( K_GAI > 0 ) THEN
                        GAIN_TOTAL = DEXP ( LOG_GAIN_TOTAL/K_GAI )
                      ELSE
                        GAIN_TOTAL = 1.0D0
                   END IF
     ...
     &                        RENRML_CROS_IF_ALL(1)
                   ...
                   IF ( FL_SPLT_SNR_FRI_ONLY ) THEN
                        SNR_TOT = SNR
                   END IF
!@   write ( 6, * ) 'PIMA_SPLT-1911 ', LOG_SEFD_TOTAL, LOG_GAIN_TOTAL, K_SEFD, K_GAI 
!@   write ( 6, * ) 'PIMA_SPLT-1912 ', ABS(DRF_ALL/WEI_ALL), NOI, SEFD_TOTAL, GAIN_TOTAL ! %%%%%%%%%%%%%%%%%%%%%%
                   IF ( PIM%CONF%SPLT_TOTAL_UV == PIMA__TOTAL_UV_YES ) THEN
                        STR = MJDSEC_TO_DATE ( PIM%MJD_0,   PIM%TAI_0 &
     &                                                    + PIM%OBS(IND_OBS)%TIM_BEG  &
     &                                                    + PIM%OBS(IND_OBS)%FRT_OFFSET(1) &
     &                                                    + PIM%UTC_MTAI, -2 )
                        WRITE ( LUN_TOT, 1140 ) PIM%SOU(J8)%J2000_NAME,                     &
                                                PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(1)),     & 
     &                                          PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2)),     &
     &                                          STR(1:23), IND_OBS,                         &
     &                                          PIM%FREQ_ARR(1,PIM%CONF%BEG_FRQ,PIM%CONF%FRQ_GRP), &
...
!
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 2 ) THEN
                   ...
                   END IF
                   IF ( PIM%OBS(IND_OBS)%RES_GR_RAT(IND_BND) > -0.5D0 ) THEN
                        WRITE ( 6, 180 ) PIM%C_SOU(J8), IND_OBS, &
     &                                   PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(1)), & 
     &                                   PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2)), &
...
!
              DO 4350 J35=K_UVO_LAST,K_UVO-1
                 UVO(J35)%SNR_FRI = SNR
                 UVO(J35)%SNR_TOT = SNR_TOT
 4350         CONTINUE 
!
...
!
                   DO 4360 J36=KACC_UVO_BEG,KACC_UVO_END
                      DO 4370 J37=1,LPOL
                         DO 4380 J38=1,KFRQ
                            IF ( IND_ACC_UVO(J36) .GE.  NUM_SEG_WEI  .AND. &
     &                           ACC_NAP(J37,J38) == 0               .AND. &
     &                           J36 > 1                             .AND. &
     &                           ACC_NAP(J37,J38) == 0 ) THEN
!
! ------------------------------ If case when the last weight segment is empty, take
! ------------------------------ the weights from the previous segment
!
                                 IF ( IND_ACC_UVO(J36-1) > 0 ) THEN
                                      IND_USED_ACC = IND_ACC_UVO(J36-1)
                                    ELSE 
                                      IND_USED_ACC = IND_ACC_UVO(J36)
                                 END IF
                               ELSE 
                                 IND_USED_ACC = IND_ACC_UVO(J36) 
                            ...
                            END IF
!
                            IF ( ACC_WEI(J37,J38,IND_USED_ACC) < PIMA__WEI_MIN**2 ) THEN
                                 UVO(J36)%WEI(J38,J37) = 0.0D0
                               ELSE 
...
!
                                      UVO(J36)%WEI(J38,J37) = 1.0D0/ACC_WEI(J37,J38,IND_USED_ACC)
...
!
                                      UVO(J36)%WEI(J38,J37) = (2.D0/PI__NUM)/ &
     &                                                        (KFRQ*(ACC_AMPL(J37,J38)/SNR))**2
                                 END IF
                            END IF
 4380                    CONTINUE 
 4370                 CONTINUE 
...
!
     ...
     &                                                              RENRML_AUTO_IF(1:LFRQ,2,1))
                   WRITE ( 6, 1150 ) PIM%C_SOU(J8), IND_OBS, &
     &                               PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(1)), & 
     &                               PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2)), &
 ...
 1150              FORMAT ( 'PIMA_SPLT renoromalization ',A, 2X, &
     ...
     &                      ' NRML_autc_frng: ', A )
              ...
              IF ( FL_TIMER ) THEN
                   CALL WALL_TIMER ( STR )
                   WRITE ( 6, '(A,I5,1X,A)' ) 'Processed  observation ', IND_OBS, STR(1:30)
              END IF
 4140       CONTINUE 
 4130    CONTINUE 
!
         DO 4390 J39=1,L_UVO
            DO 4400 J40=1,KFRQ
               IF ( UVO(J39)%STA_IND(1) > 0 .AND. UVO(J39)%STA_IND(2) > 0 ) THEN
                    DO 4410 J41=1,LPOL
                       IF ( PIM%GACO_STATUS == PIMA__LOADED ) THEN
!
                            GAIN_CORR = PIM%GACO(UVO(J39)%STA_IND(1))%GAIN_CORR(J40)* &
     &                                  PIM%GACO(UVO(J39)%STA_IND(2))%GAIN_CORR(J40)
                            UVO(J39)%SPE(J40,J41) = UVO(J39)%SPE(J40,J41) * GAIN_CORR
                            IF ( GAIN_CORR == 0.0D0 ) THEN
                                 UVO(J39)%WEI(J40,J41) = 0.0D0
                               ELSE
                                 UVO(J39)%WEI(J40,J41) = UVO(J39)%WEI(J40,J41) / GAIN_CORR
                            END IF
                       END IF
                       IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
                            WRITE ( 6, 1120 ) J41, IND_OBS, J39, J40, &
     &                              PIM%TAI_0 + UVO(J39)%TIM + PIM%UTC_MTAI, &
     &                              ABS( UVO(J39)%SPE(J40,J41) ), &
     &                              PHAS_CMPL_R4 ( UVO(J39)%SPE(J40,J41) ), &
     &                              UVO(J39)%WEI(J40,J41), 2
                       END IF
 4410               CONTINUE 
               END IF
 4400       CONTINUE 
...
!
                   CALL ERR_PASS ( IUER, IER )
                   CALL PIMA_SPLT_CONSOL ( PIM, K_UVO, UVO, PIM%C_SOU(J8), IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 8446, IUER, 'PIMA_SPLT', 'Failure '// &
    &                       'to consolidate subarrays for source '//PIM%C_SOU(J8) )
                        IF ( PIM%CONF%SPLT_TOTAL_UV == PIMA__TOTAL_UV_YES ) THEN
                             CLOSE ( UNIT=LUN_TOT )
                        END IF
                        RETURN
...
!
         IF ( L_UVO > 0 ) THEN
              DO 4420 J42=1,L_UVO
                 DEALLOCATE ( UVO(J42)%SPE  )
                 DEALLOCATE ( UVO(J42)%WEI  )
                 DEALLOCATE ( UVO(J42)%SNR  )
                 DEALLOCATE ( UVO(J42)%SEFD )
 4420         CONTINUE 
         END IF
...
!
         ...
         END IF 
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 1 ) THEN
              WRITE ( 6, 1160 ) PIM%SOU(J8)%IVS_NAME, PIM%SOU(J8)%J2000_NAME, &
     &                          NOBS_USED, NSCA_USED
...
!
              DO 4430 J43=1,LU_OBS
                 IND_OBS = INDS_OBS_USED(J43)
