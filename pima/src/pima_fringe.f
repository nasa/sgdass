...
! *                                                                      *
! * ### 11-JAN-2006    PIMA_FRINGE  v2.12 (d) L. Petrov 22-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           COV_PR_PH, COV_GR_MD, NOI_AVR, FEED_HORN_DIF, TEC, TEC_RATE, &
     &           TEC_ERR, TEC_RATE_ERR, FEED_ANG_DIF, PA_USED, DECOR_TIM, &
     ...
     &           POL_MODE, POL_LAB_IND(2), IND_OBS_PREV, IPTM, IPOL, IER
      CHARACTER  DIRNAM*128, FINAM_2D_PLOT*128, FINAM_1D_PLOT*128, &
     &           FINAM_FRI*128, POLAR_SAVE*8, POL_BAS_LABEL*2, PIMAVAR_VTD_DEBUG*8
      INTEGER*2  MASK
      DATA       MASK / O'0755' /
...
!
      ...
      END IF
!
      CALL GETENVAR ( 'PIMAVAR_VTD_DEBUG', PIMAVAR_VTD_DEBUG )
...
!
                ...
                IF ( IS .NE. 0 ) THEN
                     CALL GERROR  ( STR )
                     CALL ERR_LOG ( 7511, IUER, 'PIMA_FRINGE', 'Failure '// &
...
!
      IF ( PIM%CONF%BEG_FRQ < 1 ) THEN
           ...
           CALL ERR_LOG ( 7512, IUER, 'PIMA_FRINGE', 'Wrong parameter '// &
     ...
     &         ' a positive number' )
      ...
      IF ( PIM%CONF%END_FRQ > PIM%NFRQ ) THEN
           ...
           CALL ERR_LOG ( 7513, IUER, 'PIMA_FRINGE', 'Wrong parameter '// &
     ...
     &         'the total number of frequencies: '//STR1 )
      ...
      IF ( PIM%CONF%END_FRQ < PIM%CONF%BEG_FRQ ) THEN
           ...
           CALL ERR_LOG ( 7514, IUER, 'PIMA_FRINGE', 'Wrong parameter '// &
     &         'END_FRQ: '//STR(1:I_LEN(STR))//' -- it exceeds '// &
     &         'BEG_FRQ: '//STR1 )
           RETURN 
      END IF
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7515, IUER, 'PIMA_FRINGE', 'Failure to write '// &
...
!
     ...
     &             PIM%CONF%FRIB_NOBS        == -1                  ) THEN
           ...
           IF ( IER .NE. 0 ) THEN
                ...
                CALL ERR_LOG ( 7518, IUER, 'PIMA_FRINGE', &
 ...
 410       CONTINUE
     ...
     &       PIM%CONF%FRIB_SEARCH_TYPE == PIMA__3FFT      )  .AND. &
           ...
           IF ( STREAM < 0 ) THEN
                CALL CLRCH  ( STR )
                CALL GERROR ( STR )
                CALL ERR_LOG ( 7519, IUER, 'PIMA_FRINGE', 'Error during '// &
     &              'opening FFTW wisdom file '//PIM%CONF%FFT_CONFIG_FILE )
                RETURN
           END IF
           IS = FFTWF_IMPORT_WISDOM_FROM_FILE ( %VAL(STREAM) )
           IF ( IS < 0 ) THEN
                CALL ERR_LOG ( 7520, IUER, 'PIMA_FRINGE', 'Error during '// &
...
!
      IF ( PIM%CONF%FRT_USE == PIMA__FRT_FILE ) THEN
           ...
           IF ( IER .NE. 0 ) THEN
                ...
                CALL ERR_LOG ( 7521, IUER, 'PIMA_FRINGE', 'Failure to '// &
...
!
     ...
     &                                  TIME_FRT_OFFSET_ARR, IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7522, IUER, 'PIMA_FRINGE', 'Failure in '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7523, IUER, 'PIMA_FRINGE', 'Error in an attempt to '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7524, IUER, 'PIMA_FRINGE', 'Error in an attempt '// &
     &         'to read configuration file '//PIM%CONF%VTD_CONFIG_FILE )
           RETURN
      END IF
      IF ( ILEN(PIMAVAR_VTD_DEBUG) > 0 ) THEN
           CALL CHIN ( PIMAVAR_VTD_DEBUG, VTD%CONF%IVRB ) 
      END IF
!
...
!
     ...
     &                PIM%TAI_0 + PIM%TIM_R8(PIM%NEPC) + PIM__MSCL, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7525, IUER, 'PIMA_FRINGE', 'Error in an '// &
...
!
     ...
     &                         PIM%NZO%TIM_CODE, PIM%NZO%COO_CODE, IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7526, IUER, 'PIMA_FRINGE', 'Error in an '// &
     &              'attempt to read NZO data into VTD data structure' )
           ...
           IF ( PIM%NZO%CENTER_NAME == 'EARTH BARYCENTER' ) THEN
                NZO_REF = VTD__EME
              ELSE 
                CALL ERR_LOG ( 7527, IUER, 'PIMA_FRINGE', 'Unsupported '// &
...
!
     ...
     &                             L_NOD, L_DEG, IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7528, IUER, 'PIMA_FRINGE', 'Error in an '// &
     &              'attempt to read NZO data into VTD data structure' )
                RETURN 
           END IF
      END IF
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7529, IUER, 'PIMA_FRINGE', 'Error in '// &
...
!
     ...
     &                       ' REF_FRG_INDS= ', INT2(PIM%OBS(IND_OBS)%REF_FRG_INDS), &
     &                       ' GLO_FRQ_INSD= ', INT2(PIM%OBS(IND_OBS)%GLO_FRG_INDS)
              WRITE ( 6, 265 ) IND_OBS, PIM%CONF%FRQ_GRP, PIM%NFRG
 265          FORMAT ( 'Observation ', I6, &
     &                 ' does not have accumulation periods for frequency group ', I1, &
     &                 ' among ', I1, ' groups' )
              IF ( PIM%OBS(IND_OBS)%GLO_FRG_INDS(1) > 0 ) THEN
                   WRITE  ( 6, 267 ) PIM%OBS(IND_OBS)%GLO_FRG_INDS(1)
 267               FORMAT ( 'Hint: there are uv data for frequency group ', I1 )
              END IF
              GOTO 420
         END IF
         IF (  PIM%OBS(IND_OBS)%NUM_EPC(FRG_IND) < 2 ) THEN
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   ...
                   CALL ERR_LOG ( 7530, IUER, 'PIMA_FRINGE', 'Error '// &
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( IND_OBS, STR )
              CALL ERR_LOG ( 7531, IUER, 'PIMA_FRINGE', 'Cannot set polarization '// &
     &            'mode for observation '//STR )
              RETURN
         END IF
         IF ( PIM%CONF%POLAR == PIMA__POLAR_ORIG .OR. &
     &        PIM%CONF%POLAR == PIMA__POLAR_ALL  .OR. &
     &        PIM%CONF%POLAR == PIMA__POLAR_ALI  .OR. &
     &        PIM%CONF%POLAR == PIMA__POLAR_1ST  .OR. &
     &        PIM%CONF%POLAR == PIMA__POLAR_2ND       ) THEN
              IF ( PIM%CONF%POLAR == PIMA__POLAR_ALI ) THEN
                   LPOL = 5 
                ELSE
                   LPOL = 4
              END IF
              IF ( PIM%NSTK .NE. 4 ) THEN
                   CALL ERR_LOG ( 7532, IUER, 'PIMA_FRINGE', 'Wrong value '// &
     &                 'of POLAR: keyword. ORIG or ALL or ALI can be specified '// &
     &                 'only if all 4 Stokes parameters have been correlated' )
                   RETURN 
              END IF
            ELSE IF ( PIM%CONF%POLAR == PIMA__POLAR_PAR ) THEN
              LPOL = 2
              IF ( PIM%NSTK .NE. 2 .AND. PIM%NSTK .NE. 4 ) THEN
                   CALL ERR_LOG ( 7533, IUER, 'PIMA_FRINGE', 'Wrong value '// &
     &                 'of POLAR: keyword. PAR can be specified only if '// &
     &                 '2 or 4 Stokes parameters have been correlated' )
                   RETURN 
              END IF
            ELSE 
              LPOL = 1
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7534, IUER, 'PIMA_FRINGE', 'Error in '// &
...
!
     ...
     &                       -1.0D0, -1.0D0, -1.0D0, '??', FRI_STS, IER )
              ...
              CALL INCH  ( IND_OBS, STR )
              CALL ERR_PASS ( IUER, IER )
              CALL ERR_LOG  ( 7535, IER, 'PIMA_FRINGE', 'Failure to get '// &
...
!
     ...
     &                       -1.0D0, -1.0D0, -1.0D0, '??', FRI_STS, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7536, IUER, 'PIMA_FRINGE', &
     ...
     &           PIM%CONF%POLAR == PIMA__POLAR_2ND       ) THEN
                 IF ( J7 == 1 ) THEN
                    ...
                    ELSE IF ( J7 == 4 ) THEN
                      ...
                      IPOL = J7
                    ELSE IF ( J7 == 5 ) THEN
                      PIM%OBS(IND_OBS)%POLAR_USED = PIMA__POLAR_I
                      CALL ERR_PASS ( IUER, IER )
                      CALL PIMA_GET_POL_MODE ( PIM, PIMA__POLAR_I, IND_OBS, POL_MODE, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL CLRCH ( STR )
                           CALL INCH  ( IND_OBS, STR )
                           CALL ERR_LOG ( 7537, IUER, 'PIMA_FRINGE', 'Cannot set polarization '// &
     &                                   'mode for observation '//STR )
                           RETURN
                      END IF
!
     ...
     &                                    FL_AC, .FALSE., .FALSE., FRI_STS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL CLRCH ( STR )
                           CALL INCH  ( IND_OBS, STR )
                           CALL ERR_LOG  ( 7538, IER, 'PIMA_FRINGE', 'Failure to get '// &
     &                         'UV data for the '//STR(1:I_LEN(STR))//'-th observation '// &
     &                         'from input FITS-IDI file ' )
                           RETURN
                      END IF
                 ...
                 IF ( PIM%CONF%POLAR == PIMA__POLAR_ALL ) THEN
                      CONTINUE 
                    ELSE IF ( PIM%CONF%POLAR == PIMA__POLAR_ALI ) THEN
                      CONTINUE 
...
!
     ...
     &                            FREQ_REF, PIM%OBS(IND_OBS)%UV(1,1,1,IPOL), &
...
!
            IF ( IER .NE. 0 ) THEN
                 ...
                 CALL INCH  ( IND_OBS, STR )
                 CALL ERR_PASS ( IUER, IER )
                 CALL ERR_LOG ( 7539, IER, 'PIMA_FRINGE', 'Error '// &
     ...
     &                          -1.0D0, -1.0D0, -1.0D0, '??', FRI_STS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7540, IUER, 'PIMA_FRINGE', &
     &                         'Failure to write output file with results '// &
     &                         'of fringing '//PIM%CONF%FRINGE_FILE )
                           RETURN
                      END IF
...
!
     ...
     &                                      DECOR_BS, DECOR_TS, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7541, IUER, 'PIMA_FRINGE', 'Error in '// &
...
!
     ...
     &                            AMPL(PIMA__DRF), SNR_FRA, SB_DEL, SB_DEL_ERR, IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7542, IUER, 'PIMA_FRINGE', 'Failure in '// &
...
!
     ...
     &                                  GR_RAT, PHAS(IND_FRA), AMPL(IND_FRA), IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7543, IUER, 'PIMA_FRINGE', 'Failure in '// &
...
!
                       ...
                       END IF
                       DO 4110 J11=1,LTIM
                          PIM%OBS(IND_OBS)%UV(J10,IFRQ,J11,IPOL) = PIM%OBS(IND_OBS)%UV(J10,IFRQ,J11,IPOL)*BPASS_C8
 4110                  CONTINUE
 4100               CONTINUE
 490             CONTINUE
            END IF
...
!
            CALL CHIN ( PIM%CONF%PHAS_CAL_CODE, NUM_PCAL_TONES )
            IF ( PIM%CONF%PHAS_CAL_CODE == PIMA__PCAL_USE     .OR. &
     &           PIM%CONF%PHAS_CAL_CODE == PIMA__PCAL_USE_ONE .OR. &
     &           PIM%CONF%PHAS_CAL_CODE == PIMA__PCAL_USE_ALL .OR. &
     &           ( NUM_PCAL_TONES .GE. 1 .AND. NUM_PCAL_TONES .LE. PIM__MTON ) ) THEN
                 DO 4120 J12=1,2
                    FL_PCAL_TOUSE = .TRUE.
...
!
     ...
     &                              FRI_STS, IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7544, IUER, 'PIMA_FRINGE', 'Failure to write '// &
...
!
     ...
     &                                FRI_STS, IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7545, IUER, 'PIMA_FRINGE', 'Failure to write '// &
...
!
 ...
 130             FORMAT ( A, 1X, I6,')', 1X, A, 1X, A, '/', A, 1X, A, &
     &                    ' SNR=', F7.1 )
            ...
            END IF
!
! --------- Determine the polarization label
!
...
!
     ...
     &                                     PIM%OBS(IND_OBS)%UV(1,1,1,IPOL), &
     &                                     PIM%OBS(IND_OBS)%AC(1,1,1,1,IPOL), &
     ...
     &                                     SB_DEL, SNR_FRA, POL_BAS_LABEL, IDEV, IER )
                 ...
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_PASS ( IUER, IER )
                      CALL ERR_LOG  ( 7546, IER, 'PIMA_FRINGE', 'Failure in '// &
...
!
     ...
     &                                     SNR_FRA, POL_BAS_LABEL, IDEV, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_PASS ( IUER, IER )
                      CALL ERR_LOG  ( 7547, IER, 'PIMA_FRINGE', 'Failure in '// &
...
!
     ...
     &                                     SNR_FRA, IDEV, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_PASS ( IUER, IER )
                      CALL ERR_LOG  ( 7548, IER, 'PIMA_FRINGE', 'Failure in '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7549, IER, 'PIMA_FRINGE', 'Failure in '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7550, IER, 'PIMA_FRINGE', 'Failure in '// &
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 16-APR-2020 PIMA_UV_PRINT v1.0 (d)  L. Petrov  16-APR-2020 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'pima.i'
