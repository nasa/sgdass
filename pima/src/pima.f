...
! *   Administration. All Rights Reserved.                               *
! *                                                                      *
! *  ### 06-JAN-2006      PIMA    v1.37 (d)  L. Petrov  04-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &          KEYWORD(1)(1:3) == '--v'     ) THEN
                ...
                CALL EXIT ( 0 )
              ELSE IF ( KEYWORD(1)(1:12) == '-dumpversion' .OR. &
...
!
                ...
                CALL EXIT ( 0 )
             ELSE IF ( KEYWORD(1)(1:7) == '--share' ) THEN
                WRITE ( 6, '(A)' ) PIMA__ROOT//'/share/pima'
                CALL EXIT ( 0 )
             ELSE IF ( KEYWORD(1)(1:3) == '--r' ) THEN
...
!
      ...
      DO 420 J2=1,LIND
         IF ( ACT_CODE(IND(1,J2):IND(2,J2)) == 'load' ) THEN
            ...
            ELSE IF ( ACT_CODE(IND(1,J2):IND(2,J2)) == 'chpo' ) THEN
              PIM(1)%CONF%ACT_CODE = PIMA__CHPO_CODE
     ...
     &             'opag opal onof mppl gaco frtr tsmo gena gepm clpc chpo' )
              CALL EXIT ( 1 )
         ENDIF
 420  CONTINUE
...
!
     ...
     &              PIM(1)%CONF%EXPER_DIR )
                IF ( PIM(1)%CONF%CHECK_SEVERITY > 0 ) THEN
                     CALL EXIT ( 1 )
                   ELSE
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__OPAG_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7026, IUER, 'PIMA', 'Keyword spd_url was '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7029, IUER, 'PIMA', 'Failure to '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7030, IUER, 'PIMA', 'Error in an attempt '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7033, IUER, 'PIMA', 'Failure to load '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7034, IUER, 'PIMA', 'Error in attempt to '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__BMGE_CODE ) THEN
           IF ( KEYWORD(1) .NE. 'mask_gen' ) THEN
                CALL ERR_LOG ( 7035, -2, 'PIMA', 'Upsupported option '// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for operation bmge: '// &
     &              ' mask_gen was expected' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                CALL ERR_LOG ( 7036, -2, 'PIMA', 'No value is provided '// &
...
!
     ...
     &                              PIM(1)%CONF%BANDPASS_MASK_FILE, IUER )
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7038, -2, 'PIMA', 'Error '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__PMGE_CODE ) THEN
           IF ( KEYWORD(1) .NE. 'mask_gen' ) THEN
                CALL ERR_LOG ( 7039, -2, 'PIMA', 'Upsupported option '// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for operation pmge: '// &
     &              ' mask_gen was expected' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                CALL ERR_LOG ( 7040, -2, 'PIMA', 'No value is provided '// &
     &              'after option '//KEYWORD(1)(1:I_LEN(KEYWORD(1)))// &
     &              ' . Name of the the mask generator file was exepcted' )
                CALL EXIT ( 1 )
           END IF
...
!
     ...
     &                              PIM(1)%CONF%PCAL_MASK_FILE, IUER )
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7042, -2, 'PIMA', 'Error '// &
     ...
     &          PIM(1)%CONF%INTMOD_TYPE == PIMA__MOD_VERA2000      )  THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7043, -2, 'PIMA', 'Failure to '// &
     ...
     &                    PIM(1)%CONF%INTMOD_FILE(1) )
             ...
             ELSE IF ( PIM(1)%CONF%INTMOD_TYPE == PIMA__MOD_NO ) THEN
               CALL ERR_LOG ( 7046, -2, 'PIMA', 'Interferometric model type '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7048, IUER, 'PIMA', 'Error in attempt to '// &
          &         'write observation file into output directory '// &
     &              PIM(1)%CONF%EXPER_DIR )
                CALL EXIT ( 1 )
           END IF
     ...
     &              ' while sta1 was expected' )
           ...
           IF ( KEYWORD(2) .EQ. 'sta2' ) THEN
              ...
              ELSE IF ( ILEN(KEYWORD(2)) .NE. 0 ) THEN
                CALL ERR_LOG ( 7051, -2, 'PIMA', 'Unsupported 2nd keyword: '// &
...
!
           IF ( PIM(1)%CONF%ACT_CODE == PIMA__BPLT_CODE ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7053, IUER, 'PIMA', 'Error in attempt to '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__TSMO_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                CALL ERR_LOG ( 7057, -2, 'PIMA', 'Mandatory option mode of '// &
     &              'task tsmo was not provided' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( KEYWORD(1) .NE. 'mode' ) THEN
                CALL ERR_LOG ( 7058, -2, 'PIMA', 'Upsupported option '// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for operation tsmo: '// &
     &              ' mode was expected' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                CALL ERR_LOG ( 7059, -2, 'PIMA', 'No value is provided '// &
     &              'after option '//KEYWORD(1)(1:I_LEN(KEYWORD(1)))// &
     ...
     &          VALUE(1) == 'elev,if'      ) THEN
                CONTINUE 
              ELSE
                CALL ERR_LOG ( 7060, -2, 'PIMA', 'Unsupported value of mode '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7061, IUER, 'PIMA', 'Error in modeling Tsys' )
                CALL EXIT ( 1 )
           END IF
!
! -------- Write updated object PIM(1) into the file with extension .pim
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7062, IUER, 'PIMA', 'Error in attempt to '// &
          &         'write observation file for experiment '// &
     &               PIM(1)%CONF%SESS_CODE(1:I_LEN(PIM(1)%CONF%SESS_CODE))// &
     &              'into the output directory '// &
     &               PIM(1)%CONF%EXPER_DIR )
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__GEPM_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                CALL ERR_LOG ( 7063, -2, 'PIMA', 'Mandatory option sta of '// &
     ...
     &              'task gepm was required, but got '//KEYWORD(1) )
           ...
           END IF
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                CALL ERR_LOG ( 7065, -2, 'PIMA', 'No value is provided '// &
...
!
     ...
     &          TRIM(VALUE(1)) .NE. 'all' .AND. TRIM(VALUE(1)) .NE. 'ALL' ) THEN
                CALL ERR_LOG ( 7066, -2, 'PIMA', 'Station '//TRIM(VALUE(1))// &
...
!
           IF ( ILEN(KEYWORD(2)) > 0 ) THEN
                IF ( KEYWORD(2) .NE. 'tim_mseg' ) THEN
                     CALL ERR_LOG ( 7067, -2, 'PIMA', 'Unsupported 2nd keyword '// &
     &                    TRIM(KEYWORD(2))//' while tim_mseg was expected' )
                     CALL EXIT ( 1 )
                END IF 
                IF ( ILEN(VALUE(2)) == 0 ) THEN
                     CALL ERR_LOG ( 7068, -2, 'PIMA', 'No value is provided '// &
     &                   'after option '//KEYWORD(2) )
                     CALL EXIT ( 1 )
                END IF
                READ ( UNIT=VALUE(2), FMT=*, IOSTAT=IUER ) TIM_MSEG
                IF ( IuER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7069, -2, 'PIMA', 'Error in parsing '// &
     ...
     &                   ' -- a real number was expected' )
           ...
           IF ( ILEN(KEYWORD(3)) > 0 ) THEN
                IF ( KEYWORD(3)(1:4) .NE. 'over' ) THEN
                     CALL ERR_LOG ( 7070, -2, 'PIMA', 'Unsupported 3rd keyword '// &
     &                    TRIM(KEYWORD(3))//' while overwrite was expected' )
                     CALL EXIT ( 1 )
                END IF 
                IF ( ILEN(VALUE(3)) == 0 ) THEN
                     CALL ERR_LOG ( 7071, -2, 'PIMA', 'No value is provided '// &
     &                   'after option '//KEYWORD(3) )
                ...
                IF ( VALUE(3)(1:1) == 'Y' .OR. VALUE(3)(1:1) == 'y' ) THEN
                   ...
                   ELSE IF ( VALUE(3)(1:1) == 'N' .OR. VALUE(3)(1:1) == 'n' ) THEN
                     GEPM_OVR = .FALSE.
                   ELSE
                     CALL ERR_LOG ( 7072, -2, 'PIMA', 'Unsupported value is provided '// &
...
!
           IF ( ILEN(KEYWORD(4)) > 0 ) THEN
                IF ( KEYWORD(4)(1:10) .NE. 'tim_thresh' ) THEN
                     CALL ERR_LOG ( 7073, -2, 'PIMA', 'Unsupported 4th keyword '// &
     &                    TRIM(KEYWORD(4))//' while tim_thresh was expected' )
                     CALL EXIT ( 1 )
                END IF 
                IF ( ILEN(VALUE(4)) == 0 ) THEN
                     CALL ERR_LOG ( 7074, -2, 'PIMA', 'No value is provided '// &
     &                   'after option '//KEYWORD(4) )
                     CALL EXIT ( 1 )
                END IF
                READ ( UNIT=VALUE(4), FMT=*, IOSTAT=IUER ) TIME_THRESH
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7075, -2, 'PIMA', 'Error in parsing '// &
...
!
           IF ( ILEN(KEYWORD(5)) > 0 ) THEN
                IF ( KEYWORD(5)(1:11) .NE. 'diff_thresh' ) THEN
                     CALL ERR_LOG ( 7076, -2, 'PIMA', 'Unsupported 5th keyword '// &
     &                    TRIM(KEYWORD(5))//' while diff_thresh was expected' )
                     CALL EXIT ( 1 )
                END IF 
                IF ( ILEN(VALUE(5)) == 0 ) THEN
                     CALL ERR_LOG ( 7077, -2, 'PIMA', 'No value is provided '// &
     &                   'after option '//KEYWORD(5) )
                     CALL EXIT ( 1 )
                END IF
                READ ( UNIT=VALUE(5), FMT=*, IOSTAT=IUER ) DIFF_THRESH
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7078, -2, 'PIMA', 'Error in parsing '// &
...
!
           IF ( ILEN(KEYWORD(6)) > 0 ) THEN
                IF ( KEYWORD(6)(1:9) .NE. 'max_count' ) THEN
                     CALL ERR_LOG ( 7079, -2, 'PIMA', 'Unsupported 6th keyword '// &
     &                    TRIM(KEYWORD(6))//' while max_count was expected' )
                     CALL EXIT ( 1 )
                END IF 
                IF ( ILEN(VALUE(6)) == 0 ) THEN
                     CALL ERR_LOG ( 7080, -2, 'PIMA', 'No value is provided '// &
     &                   'after option '//KEYWORD(6) )
                     CALL EXIT ( 1 )
                END IF
                READ ( UNIT=VALUE(6), FMT=*, IOSTAT=IUER ) MAX_COUNT
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7081, -2, 'PIMA', 'Error in parsing '// &
...
!     
           ...
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7082, -2, 'PIMA', 'Error in an attempt to '// &
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
           IUER = -1
           CALL ERR_LOG ( 7083, IUER, 'PIMA', 'Error in attempt to '// &
...
!
      IF ( PIM(1)%CONF%FFT_METHOD == FFT_MKL ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7084, IUER, 'PIMA', 'Error in initializing'// &
          &         ' FFT using the MKL' )
                CALL EXIT ( 1 )
           END IF
         ELSE
     ...
     &                      PIM(1)%CONF%NUM_THREADS, IUER )
      ...
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__ACTA_CODE ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                CALL ERR_LOG ( 7086, IUER, 'PIMA', 'Failure to '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7087, IUER, 'PIMA', 'Error in attempt to '// &
...
!
      IF ( ILEN(PIM(1)%CONF%TIME_FLAG_FILE) > 0 ) THEN
           ...
           IF ( LEX ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7093, IUER, 'PIMA', 'Error in an '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__ONOF_CODE ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7094, IUER, 'PIMA', 'Error in an attempt to '// &
...
!
     ...
     &              'perform baseline fringe search' )
           ...
           END IF
      END IF
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__CHPO_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7096, IUER, 'PIMA', 'No qualifier of '// &
     &              'task chpo was supplied. Expected qualifier is metric ' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( KEYWORD(1) .NE. 'metric' ) THEN
                IUER = -1
                CALL ERR_LOG ( 7097, IUER, 'PIMA', 'Wrong qualifier of '// &
     &              'task chpo '//TRIM(KEYWORD(1))//' Supported '// &
     &              'qualifier is metric' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7098, IUER, 'PIMA', 'No value qualifier of '// &
     &              'the qualifer metric of task chpo was supplied. Expected '// &
     &              ' values are par_ampl, cross_ampl, par_phas, i_loss' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(KEYWORD(2)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7099, IUER, 'PIMA', 'No 2nd qualifier of '// &
     &              'task chpo was supplied. Expected qualifier is level' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( KEYWORD(2) .NE. 'level' ) THEN
                IUER = -1
                CALL ERR_LOG ( 7100, IUER, 'PIMA', 'Wrong 2nd qualifier of '// &
     &              'task chpo '//TRIM(KEYWORD(2))//' Supported '// &
     &              'qualifier is level' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( VALUE(2)(1:3) .EQ. 'sum' .OR. VALUE(2)(1:3) .EQ. 'obs' ) THEN
                CONTINUE 
              ELSE
                IUER = -1
                CALL ERR_LOG ( 7101, IUER, 'PIMA', 'Upsuported value of '// &
     &              'the 2nd qualifier level '//TRIM(VALUE(2))//' in task chpo. '// &
     &              'Supported values are summary and observations ' )
                CALL EXIT ( 1 )
           END IF
!
           IUER = -1
           CALL PIMA_CHPO ( PIM(1), VALUE(1), VALUE(2), IUER )
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7102, IUER, 'PIMA', 'Error in an attempt to '// &
     &              'perform checking polarization bandpass caliration' )
                CALL EXIT ( 1 )
           END IF
      END IF
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__FRIP_CODE ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7103, IUER, 'PIMA', 'Error in an attempt to '// &
      ...
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__TSPL_CODE ) THEN
           IF ( KEYWORD(1) == 'data' ) THEN
                IF ( VALUE(1) == 'measured' ) THEN
                   ...
                   ELSE
                     IUER = -1
                     CALL ERR_LOG ( 7104, IUER, 'PIMA', 'Unsupported '// &
...
!
           ...
           DO 430 J3=1,128
              IF ( KEYWORD(J3) .EQ. 'pcal_type' ) THEN
                   IF ( ILEN(VALUE(J3)) == 0 ) THEN
                        CALL ERR_LOG ( 7105, -2, 'PIMA', 'No value of the '// &
     ...
     &                         VALUE(J3) == 'avr'           ) THEN
                        PCAL_TYPE = PIMA__AVR
                     ELSE
                        CALL ERR_LOG ( 7106, -2, 'PIMA', 'Unsupported value of '// &
...
!      
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__PDPL_CODE ) THEN
           IF ( PIM(1)%NPOL < 2 ) THEN
                CALL ERR_LOG ( 7107, -2, 'PIMA', 'Task pdpl assumes '// &
     &              'the number of polarizations is more than one' )
      ...
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__GEAN_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                CALL ERR_LOG ( 7108, -2, 'PIMA', 'keyword antab_type '// &
     &              'or vlba_log_file with a value log file name should '// &
     &              'follow gean operation code' )
                CALL EXIT ( 1 )
           END IF
           IF ( KEYWORD(1) .NE. 'anc_file'        .AND. &
     &          KEYWORD(1) .NE. 'evn_antab_file'  .AND. &
     &          KEYWORD(1) .NE. 'evn_gain'        .AND. &
     &          KEYWORD(1) .NE. 'lvim_dir'        .AND. &
     &          KEYWORD(1) .NE. 'pcal_dzero'      .AND. &
     ...
     &          KEYWORD(1) .NE. 'vla_antab_file'  .AND. &
     &          KEYWORD(1) .NE. 'vlba_gain'       .AND. &
     &          KEYWORD(1) .NE. 'vlba_log_file'   .AND. &
     &          KEYWORD(1) .NE. 'wvr'                   ) THEN
                CALL ERR_LOG ( 7109, -2, 'PIMA', 'Upsupported option '// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for operation gean'// &
     &              ' anc_file, evn_antab_file, evn_gain, lvim_dir, '// &
     &              ' pcal_dzero, pcal_off, pcal_on, pima_antab_file, '// &
     &              'vlba_log_file, vlba_gain, or wvr '// &
     &              'were expected' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                CALL ERR_LOG ( 7110, -2, 'PIMA', 'No value is provided '// &
     &              'after option '//KEYWORD(1)(1:I_LEN(KEYWORD(1)))// &
     &              ' . Name of the antab_file was exepcted' )
                CALL EXIT ( 1 )
           END IF
!
           IF ( KEYWORD(1) == 'pcal_dzero' .OR. &
     &          KEYWORD(1) == 'pcal_on'    .OR. &
...
!
                ...
                IF ( .NOT. LEX ) THEN
                     CALL ERR_LOG ( 7111, -2, 'PIMA', 'Calibration file '// &
...
!
     ...
     &          KEYWORD(1) .EQ. 'vla_antab_file'  .OR. &
     &          KEYWORD(1) .EQ. 'pima_antab_file'      ) THEN
!
                IUER = -1
                CALL PIMA_GET_ANTAB ( PIM(1), VTD, KEYWORD(1), VALUE(1), IUER )
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7112, -2, 'PIMA', 'Error in '// &
     &                   'an attempt to parse antenna calibration '// &
     &                   ' file '//VALUE(1)(1:I_LEN(VALUE(1)))//' and '// &
     &                   'insert it into pima object ' )
                     CALL EXIT ( 1 )
                END IF
              ELSE IF ( KEYWORD(1) .EQ. 'anc_file' ) THEN
                IUER = -1
                CALL PIMA_LOAD_ANC ( PIM(1), VTD, VALUE(1), IUER )
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7113, -2, 'PIMA', 'Error in '// &
     &                   'an attempt to parse antenna calibration '// &
     &                   ' file '//VALUE(1)(1:I_LEN(VALUE(1)))//' in anc '// &
     &                   'format and insert it into pima object' )
                     CALL EXIT ( 1 )
                END IF
     ...
     &                                   VALUE(1), IUER )
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7114, -2, 'PIMA', 'Error in '// &
     ...
     &                   'insert it into pima object ' )
              ...
              ELSE IF ( KEYWORD(1) .EQ. 'vlba_gain' ) THEN
                IF ( KEYWORD(2) .NE. 'gain_band' ) THEN
                     CALL ERR_LOG ( 7115, -2, 'PIMA', 'The second keyword '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7116, -2, 'PIMA', 'Failure in an '// &
     ...
     &                   'file: '//KEYWORD(1) )
              ...
              ELSE IF ( KEYWORD(1) .EQ. 'evn_gain' ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7117, -2, 'PIMA', 'Failure in an '// &
     &                   'attempt to get the gain from external VLBA key '// &
     &                   'file: '//KEYWORD(1) )
                     CALL EXIT ( 1 )
                END IF
              ELSE IF ( KEYWORD(1) .EQ. 'pcal_dzero' ) THEN
                IUER = -1
                CALL PIMA_PCAL_DZERO ( PIM, VALUE(1), IUER )
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7118, -2, 'PIMA', 'Failure to '// &
     &                   'comput pcal substitues for the values that are zero' )
                     CALL EXIT ( 1 )
                END IF
     ...
     &                  KEYWORD(1) .EQ. 'pcal_off'      ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7119, -2, 'PIMA', 'Failure to '// &
     ...
     &                  KEYWORD(1) .EQ. 'tsys_off'      ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7120, -2, 'PIMA', 'Failure to '// &
     ...
     &                    VALUE(1) )
              ...
              ELSE IF ( KEYWORD(1) .EQ. 'wvr' ) THEN
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7121, -2, 'PIMA', 'Failure to load WVR '// &
     &                   'data into PIMA' )
                     CALL EXIT ( 1 )
                END IF
              ELSE
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7122, -2, 'PIMA', 'Unrecognized keyword '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7123, IUER, 'PIMA', 'Error in attempt to '// &
          &         'write observation file into output directory '// &
     &              PIM(1)%CONF%EXPER_DIR )
                CALL EXIT ( 1 )
           END IF
...
!
           IF ( KEYWORD(1)(1:1) == " " ) THEN
                CALL ERR_LOG ( 7124, -2, 'PIMA', 'Task GENA requires 1st '// &
     &              'argument sta' )
                CALL EXIT ( 1 )
           END IF
           IF ( KEYWORD(1) .NE. 'sta' ) THEN
                CALL ERR_LOG ( 7125, -2, 'PIMA', 'Upsupported first keyword '// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for task GENA. '// &
     &              ' keyword sta was expected' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(KEYWORD(2)) == 0 ) THEN
                CALL ERR_LOG ( 7126, -2, 'PIMA', 'Task GENA requires 2st '// &
     &              'argument type' )
                CALL EXIT ( 1 )
           END IF
           IF ( KEYWORD(2) .NE. 'type' ) THEN
                CALL ERR_LOG ( 7127, -2, 'PIMA', 'Upsupported second keyword'// &
     &               KEYWORD(1)(1:I_LEN(KEYWORD(1)))//' for task GENA. '// &
...
!e
           IF ( VALUE(2) == 'monica' ) THEN
                IF ( KEYWORD(3) == 'file' )  THEN
                   ...
                   ELSE IF ( ILEN(KEYWORD(3)) == 0 )  THEN
                     CALL ERR_LOG ( 7128, -2, 'PIMA', 'Task GENA with type '// &
     &                   'monica requires the third keyword: file' )
                     CALL EXIT ( 1 )
                   ELSE
                     CALL ERR_LOG ( 7129, -2, 'PIMA', 'Upsupported keyword '// &
     ...
     &                  ' keyword file was expected' )
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7130, -2, 'PIMA', 'Error in an attempt '// &
     ...
     &                   'file for station '//TRIM(VALUE(2)) )
              ...
              ELSE IF ( VALUE(2) == 'const' ) THEN
                IF ( KEYWORD(3) == 'trec' )  THEN
                     CONTINUE 
                   ELSE IF ( ILEN(KEYWORD(3)) == 0 )  THEN
                     CALL ERR_LOG ( 7131, -2, 'PIMA', 'Task GENA with type '// &
     &                   'const requires the third keyword: trec' )
                     CALL EXIT ( 1 )
                   ELSE
                     CALL ERR_LOG ( 7132, -2, 'PIMA', 'Upsupported keyword '// &
     &                  KEYWORD(3)(1:I_LEN(KEYWORD(3)))//' for task GENA. '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7133, -2, 'PIMA', 'Error in decording '// &
     ...
     &                   'real number was expected' )
                ...
                IF ( KEYWORD(4) == 'tatm' )  THEN
                   ...
                   ELSE IF ( ILEN(KEYWORD(4)) == 0 )  THEN
                     CALL ERR_LOG ( 7134, -2, 'PIMA', 'Task GENA with type '// &
     &                   'const requires the fourth keyword: tatm' )
                     CALL EXIT ( 1 )
                   ELSE
                     CALL ERR_LOG ( 7135, -2, 'PIMA', 'Upsupported keyword '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7136, -2, 'PIMA', 'Error in decording '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7137, -2, 'PIMA', 'Error in an attempt '// &
     &                   'to generate output antab file with constant Trec '// &
     &                   'for station '//TRIM(VALUE(1)) )
                     CALL EXIT ( 1 )
                END IF
              ELSE
                CALL ERR_LOG ( 7138, -2, 'PIMA', 'Unsupported value of '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__MKDB_CODE ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7139, IUER, 'PIMA', 'Error in attempt to '// &
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__DIPC_CODE ) THEN
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7140, IUER, 'PIMA', 'Error in attempt to '// &
...
!
           ...
           IF ( IUER .NE. 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7141, IUER, 'PIMA', 'Error in attempt to '// &
          &         'write observation file into output directory '// &
     &              PIM(1)%CONF%EXPER_DIR )
                CALL EXIT ( 1 )
           END IF
...
!
      IF ( PIM(1)%CONF%ACT_CODE == PIMA__GACO_CODE ) THEN
           IF ( ILEN(KEYWORD(1)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7142, IUER, 'PIMA', 'Missing value of the '// &
     &             '1st qualifier. init or sou were expected' )
                CALL EXIT ( 1 )
           END IF
           IF ( ILEN(VALUE(1)) == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 7143, IUER, 'PIMA', 'Missing value of the '// &
     &             '1st qualifier '//KEYWORD(1) )
           ...
           IF ( KEYWORD(1)(1:4) .EQ. 'init' ) THEN
                ...
                IF ( IUER .NE. 0 .OR. GAIN_VAL < 0.0D0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7144, IUER, 'PIMA_GACO_COMP', 'Failure '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7145, IUER, 'PIMA_GACO_COMP', 'Failure '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7146, IUER, 'PIMA_GACO_COMP', 'Failure '// &
     ...
     &                          PIM(1)%CONF%SESS_CODE(1:I_LEN(PIM(1)%CONF%SESS_CODE))//'_uvs'
                   ELSE
                     IF ( KEYWORD(2)(1:3) .NE. 'dir' ) THEN
                          IUER = -1
                          CALL ERR_LOG ( 7147, IUER, 'PIMA', 'Unsupported name of '// &
     ...
     &                        ' while directory was expected' )
                     ...
                     IF ( ILEN(VALUE(2)) == 0 ) THEN
                          IUER = -1
                          CALL ERR_LOG ( 7148, IUER, 'PIMA', 'Missing value of the '// &
...
!          
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 7149, IUER, 'PIMA', 'Error during '// &
     ...
     &                  PIM(1)%CONF%SPLT_GAIN_CORR_FILE(1:I_LEN(PIM(1)%CONF%SPLT_GAIN_CORR_FILE))
              ELSE
                IUER = -1
                CALL ERR_LOG ( 7150, IUER, 'PIMA', 'Unsupported name of '// &
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
           IUER = -1
           CALL ERR_LOG ( 7151, IUER, 'PIMA', 'Trap of internal control: '// &
