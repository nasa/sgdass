...
! *                                                                      *
! *  ### 18-SEP-2012   UVA_MERGE   v2.3 (d)  L. Petrov  09-AUG-2026  ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      ...
      PARAMETER  ( UVA_MERGE__LABEL = 'UVA_MERGE  v 2.3  2026.08.09' )
!
      INTEGER*4  NFIL, IUER
     ...
     &           IND_SUB(PIM__MSUB), GCC_LAST, NAXIS2(PIM__MSUB), &
     &           NAXIS3(PIM__MSUB), IND, ISTA_N(2), IER
...
!
     ...
     &                       ' from file '//PIM%FILE(J3)%NAME )
               ...
               END IF
               IF ( PIM%FILE(J3)%KEY(J5,J4)(1:9) == 'NAXIS3  =' ) THEN
                    READ ( UNIT=PIM%FILE(J3)%KEY(J5,J4)(25:30), FMT='(I6)' ) NAXIS3(J3) 
               END IF
...
!
     ...
     &               'an element GRP_ARR in the data structure TABL' )
                 RETURN 
            END IF
            ALLOCATE ( TABL(GCC)%UV_DATA(3,KFRQ,NAXIS3(J8)), STAT=IER )
            IF ( IER .NE. 0 ) THEN
...
!
     ...
     &                    %VAL(INT8(3*KFRQ*NAXIS3(1))), %VAL(0.0), TABL(GCC)%UV_DATA, &
     &                    ANY_VAL, FT_STATUS )
            IF ( FT_STATUS .NE. 0 ) THEN
                 CALL ERR_PASS ( IUER, IER )
...
!
     ...
     &                 %VAL(INT8(3*KFRQ*NAXIS3(1))), TABL(J25)%UV_DATA, FT_STATUS )
         IF ( FT_STATUS .NE. 0 ) THEN
              CALL ERR_PASS ( IUER, IER )
