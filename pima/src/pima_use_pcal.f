...
! *                                                                      *
! * ### 12-JAN-2006  PIMA_USE_PCAL   v6.2 (d) L. Petrov  24-APR-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'pima.i'
     ...
     &           NUM_USED_TONES, IAMB, IND_PUS_STA, PCI, IER
      INTEGER*4  IND_STA, IND_PCAL(4,2)
      LOGICAL*1  FL_GRD, FL_PCAL_TOUSE, FL_PCAL_USED, FL_BPASS, FL_NOSUBS
      CHARACTER  STR*128, STR1*128, STR_PCAL_NOAVR*8, STR_PCAL_NOSUBS
...
!
      FL_NOSUBS = .FALSE.
      CALL CLRCH    ( STR_PCAL_NOAVR )
      CALL GETENVAR ( 'PIMAVAR_PCAL_NOSUBS', STR_PCAL_NOSUBS )
      IF ( ILEN(STR_PCAL_NOSUBS) > 0 ) THEN
           CALL TRAN ( 11, STR_PCAL_NOSUBS, STR_PCAL_NOSUBS )
           IF ( STR_PCAL_NOSUBS(1:1) == 'Y' ) THEN
                FL_NOSUBS = .TRUE.
           END IF
      END IF
!
...
!
         IF ( .NOT. ( IND_POL == 1 .OR. IND_POL == 2 ) ) THEN
              ...
              CALL ERR_LOG ( 7575, IUER, 'PIMA_USE_PCAL', 'Trap of internal '// &
...
!
                 ...
                 ELSE
                   CALL CLRCH ( STR )
                   CALL INCH  ( PIM%CONF%PUS_TYPE, STR )
                   CALL ERR_LOG ( 7576, IUER, 'PIMA_USE_PCAL', 'Trap of '// &
     &                 'internal control: variable PIM%CONF%PUS_TYPE has a '// &
     &                 'wrong value of '//STR )
                   RETURN 
              END IF
         END IF
...
!
     ...
     &                IND_PCAL(4,J2) .LE. PIM%STA(IND_STA)%PCAL(U_FRG)%NPOI ) THEN
                    ...
                    ELSE
                      PCI = IND_PCAL(1,J2)
                 END IF
                 PHAS_PCAL = SGN*PIM%STA(IND_STA)%PCAL(U_FRG)%PHAS_SCA(IND_TONE,U_FRQ,PCI,IND_POL) 
                 IF ( .NOT. FL_NOSUBS .AND. PIM%STA(IND_STA)%PCAL(U_FRG)%LSCA_SUBS > 0 ) THEN
                      IF ( PIM%STA(IND_STA)%PCAL(U_FRG)%IND_SCA_SUBS(IND_TONE,J5,PCI,IND_POL) > 0 ) THEN
                           PHAS_PCAL = SGN*PIM%STA(IND_STA)%PCAL(U_FRG)%PHAS_SCA_SUBS(PIM%STA(IND_STA)%PCAL(U_FRG)%IND_SCA_SUBS(IND_TONE,J5,PCI,IND_POL))
                      END IF
                 END IF
!
                 IF ( I_FRQ == 1 .AND. PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL IINCH ( 8*L_PCT, STR )
                   CALL ERR_LOG ( 7579, IUER, 'PIMA_USE_PCAL', 'Failure to '// &
     &                 'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &                 'memory' )
                   RETURN 
              END IF
!
              ...
              IF ( IER .NE. 0 ) THEN
                   ...
                   CALL ERR_LOG ( 7580, IUER, 'PIMA_USE_PCAL', 'Failure to '// &
     &                 'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &                 'memory' )
                   RETURN 
              END IF
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL IINCH ( 8*LA_CHN, STR )
                   CALL ERR_LOG ( 7581, IUER, 'PIMA_USE_PCAL', 'Failure to '// &
     &                 'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &                 'memory' )
                   RETURN 
              END IF
...
!
                    IF ( .NOT. FL_NOSUBS .AND. PIM%STA(IND_STA)%PCAL(U_FRG)%LSCA_SUBS > 0 ) THEN
                         IF ( PIM%STA(IND_STA)%PCAL(U_FRG)%IND_SCA_SUBS(J8,J7,PCI,IND_POL) > 0 ) THEN
                              PCAL_PHAS(IP) = PIM%STA(IND_STA)%PCAL(U_FRG)%PHAS_SCA_SUBS(PIM%STA(IND_STA)%PCAL(U_FRG)%IND_SCA_SUBS(J8,J7,PCI,IND_POL))
                              PCAL_AMPL(IP) = PIM%STA(IND_STA)%PCAL(U_FRG)%AMPL_SCA_SUBS(PIM%STA(IND_STA)%PCAL(U_FRG)%IND_SCA_SUBS(J8,J7,PCI,IND_POL))
                              PCAL_AMPL(IP) = 0.001 ! temporary!
                         END IF
                    END IF
!
...
!
                   ...
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 7582, IUER, 'PIMA_USE_PCAL', 'Error '// &
...
!
                 IF ( NUM_USED_TONES == 0 ) THEN
                      ...
                      CALL CLRCH ( STR )
                      CALL INCH  ( J9,  STR1 )
!@                      CALL ERR_LOG ( 7583, IUER, 'PIMA_USE_PCAL', 'Trap of '// &
...
!
                      CALL ERR_LOG ( 7584, IUER, 'PIMA_USE_PCAL', 'Trap of '// &
     &                    'internal control in computing parameters phase '// &
     &                    'calibration model for station '//PIM%C_STA(IND_STA)// &
     &                    ' observation '//STR )
                      RETURN
...
!
           ...
           ELSE IF ( FL_PCAL_TOUSE ) THEN
              ...
              CALL ERR_LOG ( 7585, IUER, 'PIMA_USE_PCAL', 'This number '// &
