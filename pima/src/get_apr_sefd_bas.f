...
! *                                                                      *
! * ### 01-FEB-2009  GET_APR_SEFD_BAS v1.0 (d) L. Petrov 01-FEB-2009 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 01-FEB-2009 GET_APR_SEFD_BAS_FRQ v6.0 (d) L. Petrov 21-FEB-2026 # *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      ...
      REAL*8     TSYS_VAL(2,2), AGAIN_VAL(2,2), SEFD_VAL(2,2), OPA_BAS, &
     &           OPA_VAL(2), POLY_VAL
...
!
     ...
     &                PIM%STA(PIM%OBS(IND_OBS)%STA_IND(J2))%TSYS(U_FRG)%AVAIL      .AND. &
     &                PIM%STA(PIM%OBS(IND_OBS)%STA_IND(J2))%GAIN(U_FRG)%AVAIL            ) THEN
...
!
                      ...
                      END IF
                      AGAIN_VAL(J2,J1)= PIM%STA(PIM%OBS(IND_OBS)%STA_IND(J2))%GAIN(U_FRG)%SENS(U_FRQ,IND_POL(J1))
                      IF ( PIM%STA(PIM%OBS(IND_OBS)%STA_IND(J2))%STMO(U_FRG)%OPA_AVAIL ) THEN
                           OPA_VAL(J2) = PIM%STA(PIM%OBS(IND_OBS)%STA_IND(J2))%STMO(U_FRG)%OPA(PIM%OBS(IND_OBS)%STMO_IND(J2),IND_FRQ)
                         ELSE
                           OPA_VAL(J2) = 0.0
                      END IF
                    ELSE
...
!
 ...
 210                  FORMAT ( 'Wrong Tsys/Again Ind_obs: ', I5, &
     ...
     &                         ' Again= ', 1PD12.5, ' Opa= ', F7.3 )
                 END IF
 1               GET_APR_SEFD_BAS_FRQ = 0.0
                 RETURN
            END IF
!
...
!
     ...
     &                                   DSQRT ( SEFD_VAL(1,2)*SEFD_VAL(2,2) )   )/2.0D0
           END IF
      END IF
      OPA_BAS = DSQRT ( OPA_VAL(1)*OPA_VAL(2) )
      GET_APR_SEFD_BAS_FRQ = GET_APR_SEFD_BAS_FRQ * DEXP(OPA_BAS)
!
      IF ( PIM%CONF%DEBUG_LEVEL == 14 ) THEN
           WRITE ( 6, 220 ) IND_OBS, U_FRQ, PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(1)), &
     &                      PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2)), IND_POL(1), &
     &                      AGAIN_VAL, TSYS_VAL, OPA_VAL, GET_APR_SEFD_BAS_FRQ, &
