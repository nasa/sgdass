...
! *                                                                      *
! *  ### 02-JUL-2009   PIMA_MKDB   v1.16 (d) L. Petrov  13-JAN-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           STR_BAS*16, PIMA_FRI_USED_VERS*24, PIMAVAR_REF_FREQ_ADD*24
...
!
      CALL CLRCH    (  PIMAVAR_REF_FREQ_ADD )
      REF_FREQ_ADD  = 0.0D0
      CALL GETENVAR (    'PIMAVAR_REF_FREQ_ADD', PIMAVAR_REF_FREQ_ADD )
      CALL TRAN     ( 11, PIMAVAR_REF_FREQ_ADD,  PIMAVAR_REF_FREQ_ADD )
!
      IF ( ILEN(PIMAVAR_REF_FREQ_ADD) > 0 ) THEN
           IF ( INDEX ( PIMAVAR_REF_FREQ_ADD, '.' ) == 0 ) THEN
                PIMAVAR_REF_FREQ_ADD = TRIM(PIMAVAR_REF_FREQ_ADD)//'.0'
           END IF
           READ ( UNIT=PIMAVAR_REF_FREQ_ADD, FMT='(F22.12)', IOSTAT=IER, &
     &            IOMSG=STR ) REF_FREQ_ADD
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7711, IUER, 'PIMA_MKDB', 'Error in an attempt to '// &
     &              'parse variable PIMAVAR_REF_FREQ_ADD '//PIMAVAR_REF_FREQ_ADD )
                RETURN
           END IF
           WRITE ( 6, * ) 'PIMA_MKDB: modified REF_FREQ by '//PIMAVAR_REF_FREQ_ADD
      END IF
!
...
!
            ...
            END IF
!
! --------- Update the reference frequency if the user has requested it
!
            IF ( ILEN(PIMAVAR_REF_FREQ_ADD) > 0 ) THEN
                 PIM%OBS(IND_OBS)%REF_FREQ(1) = PIM%OBS(IND_OBS)%REF_FREQ(1) + REF_FREQ_ADD
                 IF ( PIM%OBS(IND_OBS)%REF_FREQ(2) > PIMA__MIN_FRQ ) THEN
                      PIM%OBS(IND_OBS)%REF_FREQ(2) = PIM%OBS(IND_OBS)%REF_FREQ(2) + REF_FREQ_ADD
                 END IF
            END IF
!
            IF ( PIM%CONF%MKDB_OUTPUT_TYPE == PIMA__MKDB_TEXT ) THEN
...
!
     ...
     &                           PIM%C_STA(PIM%OBS(IND_OBS)%STA_IND(2))
            ...
            END IF
!
! --------- Restore the reference frequency if the user has requested its modification
!
            IF ( ILEN(PIMAVAR_REF_FREQ_ADD) > 0 ) THEN
                 PIM%OBS(IND_OBS)%REF_FREQ(1) = PIM%OBS(IND_OBS)%REF_FREQ(1) - REF_FREQ_ADD
                 IF ( PIM%OBS(IND_OBS)%REF_FREQ(2) > PIMA__MIN_FRQ ) THEN
...
!
                 IF ( ILEN(PIMAVAR_REF_FREQ_ADD) > 0 ) THEN
                      ...
                      END IF
                 END IF
                 PIM%OBS(IND_OBS)%REF_FREQ(1) = NINT(PIM%OBS(IND_OBS)%REF_FREQ(1)*1.D3,KIND=8)/1.0D3
!
! -------------- Adjust rounding to one milliHerz
!
                 IF ( PIM%OBS(IND_OBS)%REF_FREQ(2) > PIMA__MIN_FRQ ) THEN
                      PIM%OBS(IND_OBS)%REF_FREQ(2) = NINT(PIM%OBS(IND_OBS)%REF_FREQ(2)*1.D3,KIND=8)/1.0D3
                 END IF
                 CALL ERR_PASS ( IUER, IER )
