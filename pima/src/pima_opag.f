...
! *                                                                      *
! *  ### 16-SEP-2014   PIMA_OPAG   v1.1 (d)  L. Petrov  19-FEB-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'pima.i'
     ...
     &           STA*8, SPD_MOD*128,  URL*128
      ...
      REAL*8     TAI_BEG, TAI_EPC, TAI_END, SPD_STP
...
!
     ...
     &               STR )
      ...
      END IF
!
      SPD_STP = PIMA__SPD_STP
      IF ( INDEX ( SPD_URL, 'merra2' ) > 0 ) THEN
           SPD_STP = 2*PIMA__SPD_STP
      END IF
      MJD_BEG = PIM%MJD_0
      TAI_BEG = PIM%TAI_0 - (ME__EPC-1)*SPD_STP - EPS
      ...
      END IF 
      TAI_BEG = SPD_STP*IDINT(TAI_BEG/SPD_STP)
!
      MJD_END = PIM%MJD_0
      TAI_END = PIM%TAI_0 + PIM%TIM_R8(PIM%NEPC) + ME__EPC*SPD_STP + EPS
      IDAY = TAI_END/86400.0D0
      MJD_END = MJD_END + IDAY
      TAI_END = TAI_END - IDAY*86400.0D0
      TAI_END = SPD_STP*(IDINT(TAI_END/SPD_STP)+1)
      IF ( TAI_END > 86400.0D0 ) THEN
           MJD_END = MJD_END + 1
           TAI_END = TAI_END - 86400.0D0
      END IF
!
      N_SOB = IDNINT ( ((MJD_END - MJD_BEG)*86400.0D0 + (TAI_END - TAI_BEG))/SPD_STP ) + 1
!
      DO 410 J1=1,N_SOB
         MJD_EPC = MJD_BEG
         TAI_EPC = TAI_BEG + (J1-1)*SPD_STP 
