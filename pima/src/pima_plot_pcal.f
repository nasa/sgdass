...
! *                                                                      *
! * ### 07-AUG-2006  PIMA_PLOT_PCAL  v4.0 (d)  L. Petrov 14-MAY-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           MES_SELSTA*80, STA_STR*8, STR_PCAL_NOSUBS*8, STR_PCAL_NOAVR*8
     ...
     &           IND_SCA, IER 
      ...
      LOGICAL*4  FL_STA, FL_PCAL_AMBIG, FL_NOSUBS
...
!       
                    ...
                    DO 430 J3=1,PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%NPOI
                       ...
                       IF ( MODE == 1 ) THEN
                            ...
                            IF ( PCAL_TYPE == PIMA__AVR .AND. PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PCAL_SCA ) THEN
                                 ...
                                 IF ( IND_SCA > 0 .AND. PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%LSCA_SUBS > 0 .AND. .NOT. FL_NOSUBS ) THEN
                                      IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SCA_SUBS(J2,J1,IND_SCA,IND_POL) > 0 ) THEN
                                           X8(NP,I_TONE,IFRQ) = PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS_SCA_SUBS( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SCA_SUBS(J2,J1,IND_SCA,IND_POL) )
                                      END IF
                                 END IF
                               ELSE
                                 X8(NP,I_TONE,IFRQ) = PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS(J2,J1,J3,IND_POL)
                                 IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%LPOI_SUBS > 0 .AND. .NOT. FL_NOSUBS ) THEN
                                      IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL) > 0 ) THEN
                                           X8(NP,I_TONE,IFRQ) = PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS_SUBS(PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL))
                                      END IF
                                 END IF
                            END IF
     ...
     &                                                     PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS(J2,J1,J3,2)
                                      IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%LPOI_SUBS > 0 .AND. .NOT. FL_NOSUBS ) THEN
                                           IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL) > 0 ) THEN
                                                X8(NP,I_TONE,IFRQ) = X8(NP,I_TONE,IFRQ) + PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS(J2,J1,J3,2) - &
     &                                                                  PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PHAS_SUBS(PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL))
                                           END IF
                                      END IF
                                 END IF
                            END IF
                         ...
                         ELSE IF ( MODE == 2  ) THEN
                            ...
                            IF ( PCAL_TYPE == PIMA__AVR .AND. PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%PCAL_SCA ) THEN
                               ...
                               ELSE
                                 ...
                                 IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%LPOI_SUBS > 0 .AND. .NOT. FL_NOSUBS ) THEN
                                      IF ( PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL) > 0 ) THEN
                                           X8(NP,I_TONE,IFRQ) = PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%AMPL_SUBS(PIM%STA(ISTA)%PCAL(PIM%CONF%FRQ_GRP)%IND_SUBS(J2,J1,J3,IND_POL))
                                      END IF
                                 END IF
                            END IF
                            IF ( PIM%CONF%ACT_CODE == PIMA__PDPL_CODE ) THEN
