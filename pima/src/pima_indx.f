...
! *                                                                      *
! *  ### 06-JAN-2006   PIMA_INDX  v2.47 (d)  L. Petrov  09-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &           ISOU_REN, FRG_ORIG_IND(PIM__MFRG), IER
     ...
     &           POLAR_ANG_SORTED(PIM__MFRQ,PIM__MSTA)
      ...
      INTEGER*8  TIM_I8, FRQ_I8, &
     &           FREQ_FIRST_ARR_I8(PIM__MFRQ,PIM__MFRG), &
     &           FREQ_LAST_ARR_I8(PIM__MFRQ,PIM__MFRG), &
     &           FREQ_FIRST_SORTED_ARR_I8(PIM__MFRQ,PIM__MFRG), &
     &           FREQ_LAST_SORTED_ARR_I8(PIM__MFRQ,PIM__MFRG), &
     &           FREQ_GRP_ARR_MIN_I8(PIM__MFRG), &
     &           FREQ_GRP_ARR_MAX_I8(PIM__MFRG), &
     &           NPOL_BAD
     ...
     &           STR_KEEP_FRG*8, STR_STA_NOSORT*8, STR_SOU_RENAME*1024, &
     &           SOU_REN_OLD(PIM__MSOU)*16, SOU_REN_NEW(PIM__MSOU)*16, &
     &           STR_FL_PRINT_UV_INFO*3
!
...
!
      ...
      CALL GETENVAR ( 'PIMAVAR_STA_NOSORT',          STR_STA_NOSORT )
...
!
                 ...
                 IF ( STR(1:1) == ' ') THEN
                      STR(1:1) = 'R' 
                      IF ( PIM%CONF%DEBUG_LEVEL > 0 ) THEN
                           WRITE ( 6, '(A)' ) 'WARNING: polarization for station '// &
     &                            ANT(J4)%NAM(J17)//' is not defined. Field typeA '// &
     &                            ' is empty. Set R. You may alter it with keyword '// &
     &                            'PIMAVAR_POL_CORRECT'
                      END IF
                 END IF
                 ...
                 IF ( POLAR_TYPA_UNSORTED(ANT(J4)%REF(J17)) < 1 ) THEN
                      CALL CLRCH ( STR1 )
                      CALL INCH  ( J17, STR1 )
                      IF ( ILEN(STR_POL_CORRECT) > 0 .AND. PIM%CONF%CHECK_SEVERITY < 2 ) THEN
                           IER = -1
                           CALL ERR_LOG ( 7189, IER, 'PIMA_INDX', 'Trap of '// &
     &                         'internal control: wrong polarization typeA '// &
     &                         STR(1:1)//' for the '//TRIM(STR1)//'st antenna '// &
     &                         ANT(J4)%NAM(J17)//' was found in the FITS-IDI file '// &
     &                         TRIM(PIM%FILE(J4)%NAME)//' Nevertheless, continue. '// &
     &                         'Temporarily set polarization R' )
                           POLAR_TYPA_UNSORTED(ANT(J4)%REF(J17)) = INDEX ( PIMA__POL_STR, 'R' )
                         ELSE
...
!
                 ...
                 IF ( POLAR_TYPB_UNSORTED(ANT(J4)%REF(J17)) < 1 ) THEN
                      ...
                      IF ( ILEN(STR_POL_CORRECT) > 0 .AND. PIM%CONF%CHECK_SEVERITY < 2 ) THEN
                           IER = -1
                           CALL ERR_LOG ( 7191, IER, 'PIMA_INDX', 'Trap of '// &
     ...
     &                         ' Nevertheless, continue. Set temporarily polarzation L' )
                           POLAR_TYPB_UNSORTED(ANT(J4)%REF(J17)) = INDEX ( PIMA__POL_STR, 'L' )
                        ELSE
...
!
                 ...
                 IF ( I_SOU .LE. 0 ) THEN
                      I_SOU = LTM_DIF ( 0, L_CSO, C_SOU_DB, PIM%FILE(J4)%SOU_NAME_ORIG(J18)(1:10) )
                      IF ( I_SOU .LE. 0 ) THEN
...
!
      ...
      IF ( STR_STA_NOSORT(1:1) .NE. 'Y' ) THEN
...
!
! ------ Fill the frequency group cross-reference table from the new to the original group
!
         FRG_ORIG_IND(J22) = IFIND_PL8 ( INT8(PIM%NFRG), FREQ_GRP_ARR_MIN_I8, PIM%FRQ(1,j22)%FREQ_I8 )
!
...
!
 ...
 4240       CONTINUE
            FREQ_FIRST_SORTED_ARR_I8(J23,J22) = NINT ( PIM%FREQ_ARR(1,J23,J22),        KIND=8 )
            FREQ_LAST_SORTED_ARR_I8(J23,J22)  = NINT ( PIM%FREQ_ARR(PIM%NCHN,J23,J22) + PIM%FRQ(J23,J22)%CHAN_WIDTH, KIND=8 )
            IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
                 WRITE  ( 6, 127 ) J23, J22, FRG_ORIG_IND(J22), &
     &                             PIM%FREQ_ARR(1,J23,J22), &
     &                             FREQ_FIRST_SORTED_ARR_I8(J23,J22), FREQ_LAST_SORTED_ARR_I8(J23,J22), &
     &                             FREQ_FIRST_ARR_I8(J23,J22), FREQ_LAST_ARR_I8(J23,J22)
 127             FORMAT ( 'FREQ_arr: ', I4, ' Freq_grp: ', I2, ' Frg_orig_ind: ', I1, ' Freq: ', 1PD16.8, &
     &                    ' Freq_first_i8: ', I15, ' Freq_last_i8: ', I15, &
     &                    ' Orig_Freq_first_i8: ', I15, ' Orig_Freq_last_i8: ', I15 )
            END IF
 4230    CONTINUE
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
              WRITE  ( 6, '(A)' ) ' '
...
!
      ...
      DO 4260 J26=1,PIM%NSTA
         ...
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
              WRITE ( 6, * ) 'PIMA_INDX-2745 J26= ', INT2(J26), ' C_STA= ', C_STA(J26), &
     &                       ' C_STA_ORIG= ', C_STA_ORIG(J26), ' PIM%STA(J26)%IVS_NAME= ', PIM%STA(J26)%IVS_NAME
         ...
         IF ( ILEN(STR_POL_CORRECT) > 0 ) THEN
              ...
              IF ( IS > 0 ) THEN
                   ...
                   IF ( ID1 == IS ) THEN
                        CALL ERR_LOG ( 7204, IUER, 'PIMA_INDX', 'Error in parsing '// &
     ...
     &                       TRIM(C_STA(J26))//' -- a colon should follow the station name ' )
                   ...
                   IF ( ID2 == ID1 ) THEN
                        CALL ERR_LOG ( 7205, IUER, 'PIMA_INDX', 'Error in parsing '// &
     &                      'PIMAVAR_POL_CORRECT environment variable for station '// &
     ...
     &                                   LTM_DIF ( 0, PIMA__POL_MAX, PIMA__POL, STR_POL_CORRECT(ID1+1:ID2-1) ), &
     &                                   LTM_DIF ( 0, PIMA__POL_MAX, PIMA__POL, STR_POL_CORRECT(ID2+1:ID3-1) )
...
!
         DO 4350 J35=1,ANT(J34)%N_ARR
            ...
            IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN 
                 WRITE ( 6, * ) 'PIMA_INDX-3039  Ind_fil ', INT2(J34), ' J35= ', INT2(J35), ' Ant_nam: ', ANT(J34)%NAM(J35), &
...
!
 ...
 8390          CONTINUE
               IF ( IND_FRQ < 1 .OR. IND_FRG < 1 ) THEN
                    ...
                    IF ( PIM%NFRG .GE. 1 ) WRITE ( 6, * ) ' FRG-1 first" ', FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,1), ' last: ', FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,1)
                    IF ( PIM%NFRG .GE. 2 ) WRITE ( 6, * ) ' FRG-2 first" ', FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,2), ' last: ', FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,2)
                    IF ( PIM%NFRG .GE. 3 ) WRITE ( 6, * ) ' FRG-3 first" ', FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,3), ' last: ', FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,3)
                    IF ( PIM%NFRG .GE. 4 ) WRITE ( 6, * ) ' FRG-4 first" ', FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,4), ' last: ', FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,4)
!
                    CALL CLRCH ( STR1 ) 
                    CALL CLRCH ( STR2 ) 
...
!
               IF ( IND_FRQ == 0 ) THEN
                    ...
                    WRITE ( 6, * ) ' IND_FRQ= ', IND_FRQ, ' IND_FRG= ', IND_FRG
                    WRITE ( 6, * ) ' KFRG=1: FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,1) = ', FREQ_FIRST_SORTED_ARR_I8(1:PIM%NFRQ,1)
                    WRITE ( 6, * ) ' KFRG=1: FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,1)  = ', FREQ_LAST_SORTED_ARR_I8(1:PIM%NFRQ,1)
                    CALL CLRCH ( STR )
...
! ----------------- the second time the same frequency. If yes, then put it
!
                    IF ( FREQ_FIRST_SORTED_ARR_I8(1,IND_FRG) > 0 ) THEN
                         IND_FRQ_2ND = IFIND_PL8 ( INT8(PIM%NFRQ), &
     &                                   FREQ_FIRST_SORTED_ARR_I8(IND_FRQ+1,IND_FRG), &
     &                                   FRQ_ORIG(J38,J37,J34)%FREQ_I8 ) + IND_FRQ
                       ELSE
                         FIRST_NZERO = PIM%NFRQ
                         DO 5400 J40=1,PIM%NFRQ
                            IF ( FREQ_FIRST_SORTED_ARR_I8(J40,IND_FRG) > 0 ) THEN
                                 FIRST_NZERO = J40
                                 GOTO 8400
                            END IF 
 5400                    CONTINUE 
 8400                    CONTINUE 
     ...
     &                                  ' f8 = ', FREQ_FIRST_SORTED_ARR_I8(IND_FRQ+1,IND_FRG), &
...
!
                      ...
                      IF ( PIM%CONF%DEBUG_LEVEL .GE. 7 ) THEN
                           WRITE ( 6, * ) 'PIMA_INDX-4861 Fil: ', INT2(J60), ' Rec: ', INT2(J61), &
