...
! *                                                                      *
! *  ### 29-AUG-2010  PIMA_PBP_FINE  v5.0 (d) L. Petrov  17-NOV-2020 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
      FL_DEBUG = .FALSE. ! Should be false for a normal run
      LCHN = PIM%NCHN
...
!
      DO 410 J1=1,PIM%NSTA
         DO 420 J2=1,2
            IF ( BPS%NUM_OBS_FINE(J1) .LE. 0 ) GOTO 420
            DO 430 J3=1,BPS%NUM_OBS_FINE(J1)
                IF ( DABS(BPS%SNR(J3,J1,J2)) < PIM%CONF%BPS_SNR_MIN_FINE ) GOTO 430
                IND_STA(1) = PIM%OBS(BPS%IND_OBS_SEL(J3,J1))%STA_IND(1)
                IND_STA(2) = PIM%OBS(BPS%IND_OBS_SEL(J3,J1))%STA_IND(2)
...
!
      DO 450 J5=1,PIM%NSTA
         DO 460 J6=1,2
            IF ( BPS%NUM_OBS_FINE(J5) .LE. 0 ) GOTO 460
            FL_STA = .FALSE.
            DO 470 J7=1,BPS%NUM_OBS_FINE(J5)
...
!
      DO 480 J8=1,PIM%NSTA
         DO 490 J9=1,2
            IF ( BPS%NUM_OBS_FINE(J8) .LE. 0 ) GOTO 490
            DO 4100 J10=1,BPS%NUM_OBS_FINE(J8)
               ...
               IOBS = IOBS + 1
               IND_OBS(IOBS) = BPS%IND_OBS_SEL(J10,J8)
...
!
                IND_POL = 1
                CALL PIMA_GET_RESID ( PIM, VTD, BPS%IND_OBS_SEL(J10,J8), IND_POL, &
     &                     LCHN, LFRQ, LTIM, &
     ...
     &                     RES, RES_AVR, SNR_ARR(IOBS), IER )
                IF ( IER .NE. 0 ) THEN
                     WRITE ( 6, * ) ' IER = ', IER
                     CALL CLRCH ( STR )
                     CALL INCH  ( BPS%IND_OBS_SEL(J10,J8), STR )
...
!
      DO 4230 J23=1,L_EQU_ORIG
         ...
         DO 4240 J24=1,PIM%NSTA
            DO 4250 J25=1,2
               IF ( BPS%NUM_OBS_FINE(J24) .LE. 0 ) GOTO 4250
               DO 4260 J26=1,BPS%NUM_OBS_FINE(J24)
                  ...
                  IOBS = IOBS + 1
                  IND_OBS(IOBS) = BPS%IND_OBS_SEL(J26,J24)
                  IEQU = IND_EQU(IOBS) - 1
...
!
      DO 4320 J32=1,PIM%NSTA
         DO 4330 J33=1,2
            IF ( BPS%NUM_OBS_FINE(J32) .LE. 0 ) GOTO 4330
            IFRQ = 0
