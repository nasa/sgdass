...
! *                                                                      *
! *  ### 08-JUL-2009  PIMA_APR_DELAY  v3.8 (d) L. Petrov 19-DEC-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
           ...
           END IF
         ELSE
           CALL ERR_LOG ( 7781, IUER, 'PIMA_APR_DELAY', 'Unknown time mode '// &
...
!
                 ...
                 ELSE
                   ...
                   CALL INCH    ( IND_OBS, STR )
                   WRITE ( 6, * ) 'TIM_ARG= ', TIM_ARG
                   CALL ERR_LOG ( 7783, IUER, 'PIMA_APR_DELAY', 'Cannot find apriori '// &
...
!
     ...
     &                DER_DEL, DER_RAT, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL CLRCH ( STR )
                      CALL INCH  ( IND_OBS, STR )
                      CALL ERR_LOG ( 7784, IUER, 'PIMA_APR_DELAY', 'Error in '// &
...
!
         IF ( FL_CONT(J1) ) THEN
              IF ( IND_MOD(J1) == 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( IND_OBS, STR )
                   CALL ERR_LOG ( 7786, IUER, 'PIMA_APR_DELAY', 'Trap of internal '// &
     &                 'control: IND_MOD=0 for station '//PIM%STA(IND_STA(J1))%IVS_NAME// &
     &                 ' observation '//TRIM(STR)//' of source '// &
     &                  PIM%C_SOU(PIM%OBS(IND_OBS)%SOU_IND)//' of scan '// &
...
!
 ...
 210              FORMAT ( 'PIMA_APR_DELAY ',A, 1X, ' Sou: ', A, &
     ...
     &                     ' Ind_mod: ', I6, ' Mod_sou: ', A, ' Scan: ', A  )
            ...
            ELSE 
!
! ----------- Compute path delay using VTD and subtract
