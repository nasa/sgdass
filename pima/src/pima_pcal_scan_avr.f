...
! *                                                                      *
! * ## 13-DEC-2022 PIMA_PCAL_SCAN_AVR v2.0 (d) L. Petrov 12-MAY-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'pima.i'
     ...
     &           DR_SIG, SH_SIG, TMIN_BEG(PIM__MSCA), TMIN_END(PIM__MSCA)
      ...
      REAL*4,    ALLOCATABLE :: PHAS_SUBS(:), AMPL_SUBS(:)
      REAL*4     AMP_1, AMP_2, PHS_1, PHS_2
     ...
     &           IMIN_BEG(PIM__MSCA), IMIN_END(PIM__MSCA), &
     &           LSCA_SUBS, IVRB, IER
      LOGICAL*1  FL_MISSED, FL_SUB
...
!
      IND_POI_SCA = 0
      LSCA_SUBS = 0
      IF ( PCAL%LPOI_SUBS > 0 ) THEN
           ALLOCATE ( PCAL%IND_SCA_SUBS(PCAL%NO_TONES,PIM%NFRQ,PCAL%NSCA,PCAL%NPOL), &
     &                PHAS_SUBS(PCAL%NO_TONES*PIM%NFRQ*PCAL%NSCA*PCAL%NPOL), &
     &                AMPL_SUBS(PCAL%NO_TONES*PIM%NFRQ*PCAL%NSCA*PCAL%NPOL), STAT=IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 7681, IUER, 'PIMA_PCAL_SCAN_AVR', 'Error in '// &
     &              'an attempt to allocate memory for %IND_SCA_SUBS pcal object' )
                RETURN 
           END IF
           PCAL%IND_SCA_SUBS = 0
      END IF
                        ...
                        DO 460 J6=1,PCAL%NPOL
                           DO 470 J7=1,PIM%NFRQ
                              DO 480 J8=1,PCAL%NO_TONES
                                 ...
                                 IF ( PCAL%LPOI_SUBS > 0 ) THEN
                                      IF ( PCAL%IND_SUBS(J8,J7,IMIN_BEG(J5),J6) > 0 ) THEN
                                           PCAL%PHAS_SCA(J8,J7,J5,J6) = PCAL%PHAS_SUBS(PCAL%IND_SUBS(J8,J7,IMIN_BEG(J5),J6))
                                           PCAL%AMPL_SCA(J8,J7,J5,J6) = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J8,J7,IMIN_BEG(J5),J6))
!
                                           LSCA_SUBS = LSCA_SUBS + 1
                                           PCAL%IND_SCA_SUBS(J8,J7,J5,J6) = LSCA_SUBS
                                           PHAS_SUBS(LSCA_SUBS) = PCAL%PHAS_SCA(J8,J7,J5,J6)
                                           AMPL_SUBS(LSCA_SUBS) = PCAL%AMPL_SCA(J8,J7,J5,J6)
                                      END IF
                                 END IF
 480                          CONTINUE 
 470                       CONTINUE 
                        ...
                        DO 490 J9=1,PCAL%NPOL
                           DO 4100 J10=1,PIM%NFRQ
                              DO 4110 J11=1,PCAL%NO_TONES
                                 ...
                                 IF ( PCAL%LPOI_SUBS > 0 ) THEN
                                      IF ( PCAL%IND_SUBS(J11,J10,IMIN_END(J5),J9) > 0 ) THEN
                                           PCAL%PHAS_SCA(J11,J10,J5,J9) = PCAL%PHAS_SUBS(PCAL%IND_SUBS(J11,J10,IMIN_END(J5),J9))
                                           PCAL%AMPL_SCA(J11,J10,J5,J9) = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J11,J10,IMIN_END(J5),J9))
                                           LSCA_SUBS = LSCA_SUBS + 1
                                           PCAL%IND_SCA_SUBS(J11,J10,J5,J6) = LSCA_SUBS
                                           PHAS_SUBS(LSCA_SUBS) = PCAL%PHAS_SCA(J11,J10,J5,J6)
                                           AMPL_SUBS(LSCA_SUBS) = PCAL%AMPL_SCA(J11,J10,J5,J6)
                                      END IF
                                 END IF
 4110                         CONTINUE 
 4100                      CONTINUE 
...
!
     ...
     &                               SNGL(TIM_SCA(1,J5)), SNGL(TIM_SCA(2,J5))
                       ...
                       DO 4150 J15=IND_POI_SCA(1,J5),IND_POI_SCA(2,J5)
                          ...
                          IF ( PCAL%LPOI_SUBS > 0 ) THEN
                               IF ( PCAL%IND_SUBS(J14,J13,J15,J12) > 0 ) THEN
                                    PCAL_REA(NPC) = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J14,J13,J15,J12))*COS(PCAL%PHAS_SUBS(PCAL%IND_SUBS(J14,J13,J15,J12)))
                                    PCAL_IMA(NPC) = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J14,J13,J15,J12))*SIN(PCAL%PHAS_SUBS(PCAL%IND_SUBS(J14,J13,J15,J12)))
                                    FL_SUB = .TRUE.
                               END IF
                          END IF
 4150                  CONTINUE 
...
!
 ...
 4160                  CONTINUE 
                       ...
                       IF ( FL_SUB ) THEN
                            LSCA_SUBS = LSCA_SUBS + 1
                            PCAL%IND_SCA_SUBS(J14,J13,J5,J12) = LSCA_SUBS
                            PHAS_SUBS(LSCA_SUBS) = PCAL%PHAS_SCA(J14,J13,J5,J12)
                            AMPL_SUBS(LSCA_SUBS) = PCAL%AMPL_SCA(J14,J13,J5,J12)
                       END IF
     ...
     &                                  PCAL%AMPL(J14,J13,IND_POI_SCA(2,J5),J12)*SIN(PCAL%PHAS(J14,J13,IND_POI_SCA(2,J5),J12))   )/2.0
                       IF ( PCAL%LPOI_SUBS > 0 ) THEN
                            IF ( PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12) > 0 ) THEN
                                 AMP_1 = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12))
                                 PHS_1 = PCAL%PHAS_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12))
                                 FL_SUB = .TRUE.
                               ELSE
                                 AMP_1 = PCAL%AMPL(J14,J13,IND_POI_SCA(1,J5),J12)
                                 PHS_1 = PCAL%PHAS(J14,J13,IND_POI_SCA(1,J5),J12)
                            END IF
                            IF ( PCAL%IND_SUBS(J14,J13,IND_POI_SCA(2,J5),J12) > 0 ) THEN
                                 AMP_2 = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(2,J5),J12))
                                 PHS_2 = PCAL%PHAS_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(2,J5),J12))
                                 FL_SUB = .TRUE.
                               ELSE
                                 AMP_2 = PCAL%AMPL(J14,J13,IND_POI_SCA(2,J5),J12)
                                 PHS_2 = PCAL%PHAS(J14,J13,IND_POI_SCA(2,J5),J12)
                            END IF
!
                            PCAL_REA_SCA = ( AMP_1*COS(PHS_1) + AMP_2*COS(PHS_2) )/2.0
                            PCAL_IMA_SCA = ( AMP_1*SIN(PHS_1) + AMP_2*SIN(PHS_2) )/2.0
                       END IF
...
!                       
     ...
     &                                                                (PCAL%TIME_MID_R8(IND_POI_SCA(2,J5)) - PCAL%TIME_SCA_R8(J5))
                     ...
                     ELSE IF ( IND_POI_SCA(1,J5) > 0 .AND. IND_POI_SCA(2,J5) == IND_POI_SCA(1,J5) ) THEN
                       ...
                       IF ( PCAL%LPOI_SUBS > 0 ) THEN
                            IF ( PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12) > 0 ) THEN
                                 PCAL%PHAS_SCA(J14,J13,J5,J12) = PCAL%PHAS_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12))
                                 PCAL%AMPL_SCA(J14,J13,J5,J12) = PCAL%AMPL_SUBS(PCAL%IND_SUBS(J14,J13,IND_POI_SCA(1,J5),J12))
                                 FL_SUB = .TRUE.
                            END IF
                       END IF
                       PCAL%PHAS_RGR(J14,J13,IND_POI_SCA(1,J5),J12) = PCAL%PHAS_SCA(J14,J13,J5,J12) 
...
!
 ...
 450  CONTINUE 
      IF ( PCAL%LPOI_SUBS > 0 ) THEN
           IF ( LSCA_SUBS > 0 ) THEN
                PCAL%LSCA_SUBS = LSCA_SUBS
                ALLOCATE ( PCAL%PHAS_SCA_SUBS(PCAL%LSCA_SUBS), &
     &                     PCAL%AMPL_SCA_SUBS(PCAL%LSCA_SUBS), STAT=IER  )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 7681, IUER, 'PIMA_PCAL_SCAN_AVR', 'Error in '// &
     &                   'an attempt to allocate memory for %PHAS_SCA_SUBS and '// &
     &                   '%AMPL_SCA_SUBS of the pcal object' )
                     RETURN 
                END IF
                PCAL%PHAS_SCA_SUBS(1:PCAL%LSCA_SUBS) = PHAS_SUBS(1:PCAL%LSCA_SUBS)
                PCAL%AMPL_SCA_SUBS(1:PCAL%LSCA_SUBS) = AMPL_SUBS(1:PCAL%LSCA_SUBS)
           END IF
           DEALLOCATE ( PHAS_SUBS, AMPL_SUBS )
      END IF
!
      PCAL%PCAL_SCA = .TRUE.
      CALL ERR_LOG ( 0, IUER )
