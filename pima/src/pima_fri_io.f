...
! *                                                                      *
! * ## 06-JUL-2009  PIMA_FRI_WRI_HEAD  v1.1 (d) L. Petrov 08-FEB-2014 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 06-JUL-2009 PIMA_FRI_WRI_TAIL v1.0 (d) L. Petrov 06-JUL-2009 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 06-JUL-2009 PIMA_FRI_WRI_OBS v3.3 (d) L. Petrov  15-DEC-2022 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
     ...
     &           DECOR_TIM
      ...
      LOGICAL*1  FL_PIMAVAR_FEED_HORN_WRITE
      CHARACTER, EXTERNAL :: MJDSEC_TO_DATE*30, GET_CDATE*19
      INTEGER*4, EXTERNAL :: GET_UNIT
      INTEGER*4, EXTERNAL :: ILEN, I_LEN
!
      CALL GETENVAR ( 'PIMAVAR_FEED_HORN_WRITE', STR )
      IF ( STR == 'YES' .OR. STR == 'yes' ) THEN
           FL_PIMAVAR_FEED_HORN_WRITE = .TRUE.
         ELSE
           FL_PIMAVAR_FEED_HORN_WRITE = .FALSE.
      END IF
!
...
!
     ...
     &           COV_PR_PH, COV_GR_MD, TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &           POLAR_USED, FEED_HORN, PA_USED, DECOR_TIM, PCAL_GDEL, &
     &           FRI_STS, IUER )
! ************************************************************************
! *                                                                      *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 06-JUL-2009  PIMA_FRI_REA_OBS v2.3 (d) L. Petrov 15-DEC-2022 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
     ...
     &           TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, FEED_HORN(2), &
     &           PA_USED, DECOR_TIM
      REAL*4     PCAL_GDEL(2,2)
...
!
     ...
     &                  TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &                  FEED_HORN, DECOR_TIM, PCAL_GDEL, PA_USED, POLAR_USED, FRI_STS
 110       FORMAT ( I6, 1X, I4, 1X, A, 1X, A, 1X, A, 1X, A, &
     &              6X, F8.2, 7X, F9.7, &
     &              3X, A, 2X, A, 13X, F9.7, &
     &              8X, I6, 13X, F16.9,      &
     ...
     &                  EFF_FRQ_RAT, COV_PR_PH, COV_GR_MD, &
     &                  TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &                  FEED_HORN, DECOR_TIM, PA_USED, POLAR_USED, FRI_STS
                PCAL_GDEL = 0.0
 ...
 120       FORMAT ( I6, 1X, I4, 1X, A, 1X, A, 1X, A, 1X, A, &
     ...
     &                  EFF_FRQ_RAT, COV_PR_PH, COV_GR_MD, &
     &                  TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &                  FEED_HORN, PA_USED, POLAR_USED, FRI_STS
 130       FORMAT ( I6, 1X, I4, 1X, A, 1X, A, 1X, A, 1X, A, &
     &              6X, F8.2, 7X, F9.7, &
     &              3X, A, 2X, A, 13X, F9.7, &
     &              8X, I6, 13X, F16.9,      &
     ...
     &                  EFF_FRQ_RAT, COV_PR_PH, COV_GR_MD, &
     &                  TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &                  POLAR_USED, FEED_HORN, PA_USED, FRI_STS
                PCAL_GDEL = 0.0
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 06-JUL-2009 PIMA_FRI_READ v4.2 (d)  L. Petrov  16-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
      ...
      INTEGER*4  MB
      PARAMETER  ( MB = 8192 )
      CHARACTER  STR*128, STR1*128, FRI_FILE*128, CONF_BUF(MB)*128, &
     &           POLAR_USED*2, POLAR_FRI*8
      LOGICAL*4  LEX
      REAL*8     SNR, AMPL(PIM__MFRA), AMPL_INTG, TIME_FRT, &
...
!
      POLAR_FRI = '????????'
      DO 440 J4=1,NBUF
         IF ( BUF(J4)(1:13) == '# FRIB.POLAR:' ) POLAR_FRI = BUF(J4)(31:) 
...
!
     ...
     &                           EFF_FRQ_RAT, COV_PR_PH, COV_GR_MD, &
     &                           TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, &
     &                           POLAR_USED, FEED_HORN, PA_USED, DECOR_TIM, &
     ...
     &            'fringe file '//FRI_FILE )
              ...
              RETURN
         END IF
         IF ( POLAR_FRI == PIMA__POLAR_ALI .AND. POLAR_USED .NE. PIMA__POLAR_I ) GOTO 440
         IF ( IND_BND == 2 ) THEN
...
!
     ...
     &                           TEC_ERR, TEC_RATE_ERR, POLAR_USED, FEED_HORN, PA_USED, &
...
!
     ...
     &                           TEC_RATE_ERR, POLAR_USED, FEED_HORN, PA_USED, &
     &                           DECOR_TIM, PCAL_GDEL, FRI_STS, IER )
                   END IF
                 ELSE
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 22-AUG-2009 PIMA_RESID_WRI_HEAD v1.0 (d) L. Petrov 22-AUG-2009 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 06-JUL-2009 PIMA_RESID_WRI_OBS v1.2 (d) L. Petrov  03-MAY-2020 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 06-JUL-2009 PIMA_RESID_WRI_TAIL v1.0 (d) L. Petrov 06-JUL-2009 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 06-JUL-2009 PIMA_RESID_REA_OBS v1.0 (d) L. Petrov  06-JUL-2009 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 30-SEP-2009   PIMA_RESID_REA  v1.0 (d) L. Petrov  30-SEP-2009 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
