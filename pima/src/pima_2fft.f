...
! *                                                                      *
! * ###  01-APR-2006    PIMA_2FFT   v2.15 (d)  L. Petrov 21-MAY-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &         'have APs with frequency index '//STR1 )
      ...
      END IF
!
      DEL_WIN =PIM%CONF%FRIB_DELAY_WINDOW_WIDTH 
      IF ( DEL_WIN .LE. 0.0D0 ) THEN
!
! -------- Set the natural search window for group delay
!
           IF ( PIM%FRQ(1,FRG_IND)%CHAN_WIDTH > PIMA__MIN_FRQ ) THEN
                DEL_WIN = 1.D0/PIM%FRQ(1,FRG_IND)%CHAN_WIDTH
              ELSE 
                DEL_WIN = 1.D0/PIMA__MIN_FRQ 
           END IF
      END IF
!
      RAT_WIN = PIM%CONF%FRIB_RATE_WINDOW_WIDTH
      IF ( RAT_WIN .LE. 0.0D0 ) THEN
!
...
!
     ...
     &          PIM%OBS(IND_OBS)%AP_LEN > PIMA__MIN_AP_LEN       ) THEN
                RAT_WIN = 1.D0/(PIM%OBS(IND_OBS)%AP_LEN*PIM%FRQ(PIM%CONF%BEG_FRQ,FRG_IND)%FREQ)
              ELSE 
                RAT_WIN = 1.D0/(PIMA__MIN_AP_LEN*PIMA__MIN_FRQ)
           END IF
      END IF
!
...
!
      IF ( KTIM > L_RT) THEN
           ...
           write ( 6, * ) ' AP_LEN = ', AP_LEN, ' FRG_GRP= ', PIM%CONF%FRQ_GRP
...
!
     ...
     &                         ' tim_beg: ', PIM%OBS(IND_OBS)%TIM_BEG, &
     &                         ' tim_end: ', PIM%OBS(IND_OBS)%TIM_END, &
     &                         ' FRG_GRP= ', PIM%CONF%FRQ_GRP 
           END IF
!
           SNR  = 0.0D0
           AMPL = 0.0D0
...
!
         ...
         DO 4130 J13=1,L_MD
            ...
            END IF
            IF ( GR_DEL_GRID < DEL_CEN - DEL_WIN .OR. &
     &           GR_DEL_GRID > DEL_CEN + DEL_WIN      ) THEN
 ...
 4130    CONTINUE 
         IF ( N_GR_DEL < 2 ) THEN
              CALL CLRCH ( STR )
              WRITE ( UNIT=STR(1:11), FMT='(1PD11.4)' ) DEL_WIN
