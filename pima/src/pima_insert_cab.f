...
! *                                                                      *
! * ### 04-AUG-2009  PIMA_INSERT_CAB v3.3 (d) L. Petrov  24-APR-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           TIM_CAB(PIM__MEPC), CAB_DEL(PIM__MEPC)
      ...
      PARAMETER  ( EPS = 1.D-6 )
      LOGICAL*1  FL_USE(PIM__MEPC), FL_NOINTRP, FL_NOCABLE
...
!
      FL_NOCABLE = .FALSE.
      CALL GETENVAR ( 'PIMAVAR_NO_CABLE', STR )
      IF ( STR(1:3) == 'YES' .OR. STR(1:3) == 'yes' ) THEN
           FL_NOCABLE = .TRUE.
      ENDIF
!
...
!
      ...
      DO 410 J1=1,PIM%NSTA
         IF ( FL_NOCABLE ) GOTO 410
         IF ( PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI < 3 ) GOTO 410
         NPOI = PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI
         DO 420 J2=1,PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI
            IF ( IS_R8_NAN ( PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J2) ) ) PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J2) = 0.0D0
            IF ( PIM%CONF%DEBUG_LEVEL .GE. 4 ) THEN
                 WRITE ( 6, 120 ) PIM%STA(J1)%IVS_NAME, J2, &
     &                            PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI,   &
     &                            PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(J2), &
     &                            PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J2)
 120             FORMAT ( 'PIMA_INSERT_CAB Sta: ', A, ' Poi: ', I5, ' Npoi: ', I5, &
     &                    ' Tim= ', F8.2, ' Cab= ', 1PD12.5 )
            END IF          
!
            IF ( J2 > 1 .AND. NPOI == PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI ) THEN
                 IF ( ( PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(J2) - PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(J2-1) ) < EPS ) THEN
                      NPOI = J2-1
                      IF ( PIM%CONF%DEBUG_LEVEL .GE. 1 ) THEN
                            WRITE ( 6, 130 ) PIM%STA(J1)%IVS_NAME, NPOI, PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI, &
     &                                                                   PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(NPOI)
 130                        FORMAT ( 'Truncated cable calibratrion for station ', A, &
     &                               ' to ', I5, ' points of ', I5, ' at epoch ', F8.2 )
            ...
            END IF
 420     CONTINUE 
         PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI = NPOI 
!
         FL_USE = .TRUE.
         KSP = 0
...
!
                    IF ( DABS(PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J4+J3) - PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J4-1)) < CAB_THR_SMOOTH ) THEN
!
! ---------------------- Cycle over the points in the spike test interval 
!
                         DO 450 J5=J4,J4+J3-1
                            IF ( DABS(PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J5) - PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J4-1)) > CAB_THR_SPIKE .AND. &
     &                           FL_USE(J5)  ) THEN
!
                                 ...
                                 DO 460 J6=J4,J4+J3-1
                                    IF ( FL_USE(J6) ) THEN
                                         ...
                                         IF ( PIM%CONF%DEBUG_LEVEL .GE. 8 ) THEN
                                              WRITE ( 6, 140 ) PIM%STA(J1)%IVS_NAME, J6, PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J6)
...
!
         ...
         DO 470 J7=1,NPOI
            IF ( FL_USE(J7) ) THEN
                 KPOI = KPOI + 1
                 TIM_CAB(KPOI) = PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(J7)
                 CAB_DEL(KPOI) = PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J7)
                 IF ( IS_R8_NAN(TIM_CAB(KPOI)) ) THEN
                      KPOI = KPOI - 1
                      GOTO 470
                 END IF
                 IF ( IS_R8_NAN(CAB_DEL(KPOI)) ) THEN
                      KPOI = KPOI - 1
                      GOTO 470
                 END IF
                 IF ( TIM_CAB(KPOI) < -200.0 .OR. TIM_CAB(KPOI) > 1.0D6 ) THEN
                      KPOI = KPOI - 1
                      GOTO 470
                 END IF
            END IF
 470     CONTINUE 
         NPOI = KPOI
!
         IF ( .NOT. FL_NOINTRP .AND. NPOI > 3 ) THEN
              CALL ERR_PASS ( IUER, IER )
              CALL MAKE_SPLINE ( 3, NPOI, TIM_CAB, CAB_DEL, &
     &                           0.0D0, 0.0D0, SPL_COE, TMP_ARR, IER )
              IF ( IER .NE. 0 ) THEN
                   WRITE (6, * ) 'N_cab: ', PIM%STA(J1)%CABLE(PIM%CONF%FRQ_GRP)%NPOI
