...
! *                                                                      *
! * ### 30-JAN-2006  PIMA_PLOT_TSYS  v6.1 (d)  L. Petrov 20-JAN-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
      ...
      IF ( ILEN(PIMAVAR_TSPL_FUDGE) == 0 ) THEN
         ...
         ELSE
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 5711, IUER, 'PIMA_PLOT_TSYS', 'Wrong value of '// &
...
!
     ...
     &             PIM%NPOL == 1 ) THEN
        ...
        ELSE IF ( PIM%CONF%POLAR == PIMA__POLAR_LR ) THEN
           IND_POL = 2
           POLAR_STR = '??'
        ELSE IF ( PIM%CONF%POLAR == PIMA__POLAR_HR ) THEN
           IND_POL = 1
           POLAR_STR = '??'
        ELSE IF ( PIM%CONF%POLAR == PIMA__POLAR_VR ) THEN
           ...
           POLAR_STR = '??'
        ELSE
           CALL ERR_LOG ( 5713, IUER, 'PIMA_PLOT_TSYS', 'Unsupported '// &
     &         'polarization code '//PIM%CONF%POLAR )
           RETURN 
      END IF
           ...
           IF ( TSPL_DATA == PIMA__TABMOD ) THEN
              ...
              ELSE 
                IF ( TSPL_DATA == PIMA__TABL .AND. PIM%CONF%TSYS_CAL_CODE == PIMA__TSYS_MEASURED ) THEN
                   ...
                   ELSE
                     CALL ERR_LOG ( 5714, IUER, 'PIMA_PLOT_TSYS', 'Tsys mode '// &
     ...
     &                   'data type '//TSPL_DATA )
           ...
           IF ( IER .NE. 0 ) THEN
                ...
                CALL ERR_LOG ( 5715, IUER, 'PIMA_PLOT_TSYS', 'Error in allocating '// &
     ...
     &                                 PIM%CONF%TSYS_CAL_CODE//' is specified. '
                   ...
                   GOTO 410
                ELSE
                   CALL ERR_LOG ( 5716, IUER, 'PIMA_PLOT_TSYS', 'Trap of internal '// &
...
!
         ...
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 3 ) THEN
              WRITE ( 6, 230 ) PIM%C_STA(J1), PIM%STA(J1)%TSYS(PIM%CONF%FRQ_GRP)%NPOI, IND_POL
 230          FORMAT ( 'PIMS_PLOT_TSYS Sta: ', A, ' Num_tsys: ', I5, ' IND_POL= ', I1 )
         END IF
...
!
 ...
 4110         CONTINUE 
     ...
     &                          TSYS_MIN, TSYS_MAX, TSYS_0, IVRB, IER )
              IF ( IER .NE. 0 ) THEN
                   WRITE ( 6, * ) 'Station ',J1,' Name: ', PIM%STA(J1)%IVS_NAME
                   CALL ERR_LOG ( 5718, IUER, 'PIMA_PLOT_TSYS', 'Error in '// &
     &                 'decomposition of system temerature on time dependence '// &
     &                 'of Tsys in zenith direction and elevation dependence' )
                   RETURN
              END IF
...
!
         CALL CLRCH ( STR )
         PIM%FREQ_ARR(1,IND_FRQ,PIM%CONF%FRQ_GRP) = PIM%FRQ(IND_FRQ,PIM%CONF%FRQ_GRP)%FREQ
...
!
      IF ( TSPL_DATA == PIMA__TABL .OR. TSPL_DATA == PIMA__TABMOD ) THEN
           ...
           DEALLOCATE ( OUT )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 5719, IUER, 'PIMA_PLOT_TSYS', 'Error in an attempt '// &
...
!
     ...
     &                   ICODE, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 5721, IUER, 'PIMA_PLOT_TSYS', 'Failure to make '// &
