...
! *                                                                      *
! *  ### 06-JAN-2006   PIMA_CONF  v1.50 (d)  L. Petrov  04-MAY-2026  ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'fftw3.f'
     ...
     &           KEYWORD(PIM__MOPT)*(*), VALUE(PIM__MOPT)*(*)
      ...
      PARAMETER  ( MIND =   32 )
      PARAMETER  ( MBUF = 1024 )
      CHARACTER  STR*128, STR1*128, STR2*4096, FIL_STT*128, &
     &           PCAL_USE_STR*8, TSYS_USE_STR*8, &
     ...
     &           J11, J12, J13, J14, J15, LIND, IND(2,MIND), NBUF, LKEY, &
     &           LFRIB_KEY, LFRIP_KEY, LMKDB_KEY, LBPS_KEY, LSPLT_KEY, &
     &           LONOF_KEY, IP, IH, IL, IVAL, IVALS, IVAL1, IVAL2, NB, NS, ID, &
...
!
                   IF ( KEYWORD(J1) == 'anc_file'        ) GOTO 410
                   ...
                   IF ( KEYWORD(J1) == 'pcal_dzero'      ) GOTO 410
                   ...
                   IF ( KEYWORD(J1) == 'vla_antab_file'  ) GOTO 410
...
!
              ...
              END IF
              IF ( PIM%CONF%ACT_CODE == PIMA__CHPO_CODE ) THEN
                   IF ( KEYWORD(J1)(1:6) == 'metric' ) GOTO 410
                   IF ( KEYWORD(J1)(1:5) == 'level'  ) GOTO 410
              END IF
!
              ...
              IF ( BUF(J3)(IND(1,2):IND(2,2)) == 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( PIM%CONF%L_INM > PIM__MFIL ) THEN
                        CALL CLRCH (            STR )
                        CALL INCH  ( PIM__MFIL, STR )
                        CALL ERR_LOG ( 8130, IUER, 'PIMA_CONF', 'Too many model '// &
     &                      'input files specified in the control file '// &
     &                       CONF_FILE(1:I_LEN(CONF_FILE))//' more than the '// &
     &                      'limit PIM__MFIL '//STR )
                        RETURN
...
!
     ...
     &                    BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__TSYS_INTRP         ) THEN
                ...
                ELSE IF ( BUF(J3)(IND(1,2):IND(1,2)+6) == PIMA__TSYS_CLEANED ) THEN
                   PIM%CONF%TSYS_CAL_CODE = PIMA__TSYS_CLEANED
                ELSE IF ( BUF(J3)(IND(1,2):IND(1,2)+6) == PIMA__TSYS_MODELED ) THEN
...
!
              ...
              IF ( IP < 1 ) THEN
                   TSYS_USE_STR = BUF(J3)(IND(1,2):IND(2,2))
                 ELSE
!
! ---------------- Yes, it has at least one station in the not-to-use list
!
                   TSYS_USE_STR = BUF(J3)(IND(1,2):IND(1,2)+IP-2)
                   STR2 = BUF(J3)(IND(1,2)+IP:)
!
! ---------------- Parse the station list and out its contents into PIM%CONF%TSYS_USE_STA(J3)
!
                   DO 450 J5=1,PIM__MSTA+1
                      IP = INDEX ( STR2, ':' )
                      ...
                      IF ( J5 == 1 ) THEN
                           ...
                           IF ( TRIM(STR) == 'TO_USE' ) THEN
                                PIM%CONF%TUS_TYPE = PIMA__USE
                              ELSE IF ( TRIM(STR) == 'NOT_TO_USE' ) THEN
                                PIM%CONF%TUS_TYPE = PIMA__NOT_USE
                              ELSE
                                CALL CLRCH (     STR1 )
                                CALL INCH  ( J5, STR1 )
     ...
     &                              'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                              ' -- keyword TSYS: -- qualifer TO_USE or NOT_TO_USE '// &
     &                              'was expected after column, but got '//STR )
                           ...
                           END IF
                      END IF
                      PIM%CONF%L_TUS = PIM%CONF%L_TUS + 1
                      PIM%CONF%TSYS_USE_STA(PIM%CONF%L_TUS) = STR
                      IF ( PIM%NSTA > 0 ) THEN
                           IF ( LTM_DIF ( 0, PIM%NSTA, PIM%C_STA, PIM%CONF%TSYS_USE_STA(PIM%CONF%L_TUS) ) < 1 ) THEN
                                CALL ERR_LOG ( 8134, IUER, 'PIMA_CONF', 'Error during '// &
     &                              'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                              'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                              ' -- keyword TSYS: -- a use station '// &
     &                              PIM%CONF%TSYS_USE_STA(PIM%CONF%L_TUS)// &
     &                             ' was not observed in VLBI experiment '// &
     &                              PIM%CONF%SESS_CODE )
                                RETURN
                           END IF
                      END IF
...
!
                   ...
                   STR2 = BUF(J3)(IND(1,2)+IP:)
!
! ---------------- Parse the station list and out its contents into PIM%CONF%PCAL_USE_STA
!
                   DO 460 J6=1,PIM__MSTA+1
                      IP = INDEX ( STR2, ':' )
                      IF ( IP < 1 ) IP = INDEX ( STR2, ',' )
                      IF ( IP > 0 ) THEN
                           STR = STR2(1:IP-1)
     ...
     &                              'was expected after column, but got '//STR )
                      ...
                      IF ( PIM%NSTA > 0 ) THEN
                           IF ( LTM_DIF ( 0, PIM%NSTA, PIM%C_STA, PIM%CONF%PCAL_USE_STA(PIM%CONF%L_PUS) ) < 1 ) THEN
                                CALL ERR_LOG ( 8136, IUER, 'PIMA_CONF', 'Error during '// &
     &                              'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                              'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                              ' -- keyword PCAL: -- a use station '// &
     &                              PIM%CONF%PCAL_USE_STA(PIM%CONF%L_PUS)// &
     &                             ' was not observed in VLBI experiment '// &
     &                              PIM%CONF%SESS_CODE )
                                RETURN
                           END IF
  ...
  860              CONTINUE 
              ...
              IF ( PCAL_USE_STR == PIMA__PCAL_NO(1:2) ) THEN
                ...
                ELSE
                   CALL ERR_LOG ( 8137, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- unrecognized value '//BUF(J3)(IND(1,2):IND(2,2))// &
     &                 ' of keyword PCAL PCAL_USE_STR: '//PCAL_USE_STR )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'GAIN:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__GAIN_NO ) THEN
                ...
                ELSE
                   CALL ERR_LOG ( 8138, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- unrecognized value '//BUF(J3)(IND(1,2):IND(2,2))// &
     &                 ' of keyword GAIN: USE or NO were expected' )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'SAMPLER_CAL:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__SMPL_NO ) THEN
                ...
                ELSE
                   CALL ERR_LOG ( 8139, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- unrecognized value '//BUF(J3)(IND(1,2):IND(2,2))// &
     &                 ' of keyword SAMPLER_CAL' )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'EXPER_DIR:' ) THEN
              ...
              IF ( DIR_DESC .EQ. 0 ) THEN
                   CALL GERROR ( STR )
                   CALL ERR_LOG ( 8140, IUER, 'PIMA_CONF', 'Error during '// &
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'FFT_CONFIG_FILE:' ) THEN
              ...
              IF ( PIM%CONF%FFT_CONFIG_FILE .EQ. 'NO' ) THEN
                ...
                ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8142, IUER, 'PIMA_CONF', 'Cannot find '// &
     ...
     &                        BUF(J3)(IND(1,2):IND(2,2)) )
              IF ( BUF(J3)(IND(1,2):IND(2,2)) == 'ESTIMATE' ) THEN
                 ...
                 ELSE
                   CALL ERR_LOG ( 8143, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &               PIM%CONF%NUM_THREADS
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8144, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &                'UV_EXCLUDE_FILE:' ) THEN
              ...
              IF ( PIM%CONF%EXCLUDE_UV_FINAM == PIMA__EXC_NO ) THEN
                 ...
                 ELSE IF ( PIM%CONF%EXCLUDE_UV_FINAM(1:2) .NE. 'NO' ) THEN
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8145, IUER, 'PIMA_CONF', 'Cannot find '// &
     ...
     &                'BANDPASS_USE:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__BPASS_NO ) THEN
                 ...
                 ELSE
                   CALL ERR_LOG ( 8146, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- unrecognized value '//BUF(J3)(IND(1,2):IND(2,2))// &
     ...
     &                'INCLUDE_OBS_FILE:' ) THEN
              ...
              IF ( PIM%CONF%INCLUDE_OBS_FILE .EQ. 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8147, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &                'BANDPASS_FILE:' ) THEN
              ...
              IF ( PIM%CONF%BANDPASS_FILE .EQ. 'NO' ) THEN
                   PIM%CONF%BANDPASS_USE = PIMA__BPASS_NO
                   IF ( PIM%CONF%ACT_CODE == PIMA__BPLT_CODE ) THEN
                        CALL ERR_LOG ( 8149, IUER, 'PIMA_CONF', 'Bandpass '// &
     ...
     &                'BANDPASS_MASK_FILE:' ) THEN
              ...
              IF ( PIM%CONF%BANDPASS_MASK_FILE .EQ. 'NO' ) THEN
                   ...
                   IF ( PIM%CONF%ACT_CODE .EQ. PIMA__BMGE_CODE       ) THEN
                        CALL ERR_LOG ( 8151, IUER, 'PIMA_CONF', 'Bandpass mask '// &
     &                      'file name is not specified. It should be specified '// &
     ...
     &                  PIM%CONF%ACT_CODE .NE. PIMA__ACPL_CODE       ) THEN
                        ...
                        IF ( .NOT. LEX ) THEN
                             CALL ERR_LOG ( 8152, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &                  PIM%CONF%ACT_CODE .EQ. PIMA__GEPM_CODE      ) THEN
                        CALL ERR_LOG ( 8153, IUER, 'PIMA_CONF', 'Phase-cal mask '// &
     &                      'file name is not specified. It should be specified '// &
     ...
     &                  PIM%CONF%ACT_CODE .NE. PIMA__ACPL_CODE       ) THEN
                        ...
                        IF ( .NOT. LEX ) THEN
                             CALL ERR_LOG ( 8154, IUER, 'PIMA_CONF', 'Cannot '// &
...
!
                             INQUIRE ( FILE=PIM%CONF%PCAL_MASK_FILE, EXIST=LEX )
                             IF ( LEX ) THEN
                                  CALL ERR_LOG ( 8155, IUER, 'PIMA_CONF', 'Found '// &
...
!
                   CALL ERR_LOG ( 8156, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &                'TIME_FLAG_FILE:' ) THEN
              ...
              IF ( PIM%CONF%TIME_FLAG_FILE .EQ. 'NO' ) THEN
                   ...
                   IF ( PIM%CONF%ACT_CODE == PIMA__ONOF_CODE ) THEN
                        CALL ERR_LOG ( 8157, IUER, 'PIMA_CONF', 'File '// &
     ...
     &                   PIM%CONF%ACT_CODE .NE. PIMA__MOIM_CODE .AND. &
     &                   PIM%CONF%ACT_CODE .NE. PIMA__CLPC_CODE .AND. &
     &                   PIM%CONF%ACT_CODE .NE. PIMA__CHPO_CODE .AND. &
     ...
     &                'TEC_FILE:' ) THEN
              ...
              IF ( PIM%CONF%TEC_FILE .EQ. 'NO' ) THEN
                   ...
                   IF ( PIM%CONF%ACT_CODE == PIMA__TECG_CODE ) THEN
                        CALL ERR_LOG ( 8159, IUER, 'PIMA_CONF', 'File '// &
     ...
     &                      'could write results' )
                 ...
                 ELSE
                   IF ( PIM%CONF%ACT_CODE .NE. PIMA__TECG_CODE ) THEN
                        ...
                        IF ( .NOT. LEX ) THEN
                             CALL ERR_LOG ( 8160, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &                'WVR_FILE:' ) THEN
              ...
              IF ( PIM%CONF%L_WVR > PIM__MWVR ) THEN
                   ...
                   CALL ERR_LOG ( 8161, IUER, 'PIMA_CONF', 'Error during '// &
...
!
              ...
              IF ( PIM%CONF%WVR_FILE(PIM%CONF%L_WVR) .EQ. 'NO' ) THEN
                   ...
                   IF ( PIM%CONF%L_WVR > 1 ) THEN
                        CALL ERR_LOG ( 8162, IUER, 'PIMA_CONF', 'Failure in '// &
     ...
     &                      'value NO is not acceptable' )
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8163, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &               PIM%CONF%WVR_SMOOTHING_INTERVAL
              IF ( IER .NE. 0 .OR. PIM%CONF%WVR_SMOOTHING_INTERVAL < 0.0D0 ) THEN
                   CALL ERR_LOG ( 8165, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &               PIM%CONF%WVR_SMOOTHING_SIGMA
              IF ( IER .NE. 0 .OR. PIM%CONF%WVR_SMOOTHING_INTERVAL < 0.0D0 ) THEN
                   CALL ERR_LOG ( 8166, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &               PIM%CONF%BEG_FRQ
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8167, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- failure to decode value '// &
     &                  BUF(J3)(IND(1,2):IND(2,2))// &
     ...
     &                 ' of keyword END_FRQ' )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'POLAR:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__POLAR_RR ) THEN
                 ...
                 ELSE IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__POLAR_ALI ) THEN
                   PIM%CONF%POLAR = PIMA__POLAR_ALI
                 ELSE IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__POLAR_PAR ) THEN
                   PIM%CONF%POLAR = PIMA__POLAR_PAR
     ...
     &                 'HH, HV, VV, VH, XX, XY, YX, YY, ALL, PAR, ORIG, ALL_1ST, ALL_2ND' )
                   RETURN
              END IF
              LKEY = LKEY + 1
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == &
     ...
     &               'EPHEMERIDES_FILE:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(2,2)) == 'NO' ) THEN
                 ...
                 ELSE 
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8171, IUER, 'PIMA_CONF', 'Cannot '// &
                 ...
                 ELSE 
                   ...
                   IF ( PIM%CONF%EPHEMERIDES_USE == 'NO' ) THEN
                      ...
                      ELSE IF ( PIM%CONF%EPHEMERIDES_USE == 'RA_GB140' ) THEN
                        CONTINUE 
                      ELSE 
                        CALL ERR_LOG ( 8172, IUER, 'PIMA_CONF', 'Unsupported value '// &
     ...
     &                'FRIB.FINE_SEARCH:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(2,2)) == 'NO' ) THEN
                 ...
                 ELSE
                   CALL ERR_LOG ( 8189, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &                'FRIB.FRQ_TRANSFER_BAND:' ) THEN
              ...
              IF ( STR(1:2) == 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8194, IUER, 'PIMA_CONF', 'Cannot '// &
...
!@                   PIM%CONF%FRIB_1D_RESFRQ_PLOT  =  PIMA__PLOT_SAV
     ...
     &                 ' of keyword FRIB.1D_TIM_MSEG' )
              ...
              IF ( PIM%CONF%FRIB_1D_TIM_MSEG < 1 ) THEN
                   CALL ERR_LOG ( 8209, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- wrong value '// &
     &                 BUF(J3)(IND(1,2):IND(2,2))// &
     ...
     &               PIM%CONF%FRIB_1D_FRQ_MSEG
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8210, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- error in recoding value '// &
     &                 BUF(J3)(IND(1,2):IND(2,2))// &
     &                 ' of keyword FRIB.1D_FRQ_MSEG' )
              ...
              IF ( PIM%CONF%FRIB_1D_FRQ_MSEG < 1 ) THEN
                   CALL ERR_LOG ( 8211, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- wrong value '// &
     &                 BUF(J3)(IND(1,2):IND(2,2))// &
     ...
     &               FMT='(F20.0)' ) PIM%CONF%FRIB_1D_DRF_SPAN
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8212, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- error in recoding value '// &
     &                 BUF(J3)(IND(1,2):IND(2,2))// &
     &                 ' of keyword FRIB.1D_DRF_SPAN' )
              ...
              IF ( PIM%CONF%FRIB_1D_DRF_SPAN < 0.001 ) THEN
                   CALL ERR_LOG ( 8213, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- wrong value '// &
     &                 BUF(J3)(IND(1,2):IND(2,2))// &
     ...
     &                BUF(J3)(IND(1,1):IND(2,1)) == 'FRIB.OBS:' ) THEN
              IF ( BUF(J3)(IND(1,2):IND(1,2)+7) == PIMA__OBS_NO ) THEN
                 ...
                 ELSE
                   ...
                   IF ( IP == 1 ) THEN
                        CALL ERR_LOG ( 8214, IUER, 'PIMA_CONF', &
     ...
     &                       'a colon sign' )
                      ...
                      ELSE IF ( IP == ILEN(BUF(J3)(IND(1,2):IND(2,2))) ) THEN
                        CALL ERR_LOG ( 8215, IUER, 'PIMA_CONF', &
     &                      'Wrong value of the keyword OBS: '// &
     &                       PIM%CONF%FRIB_OBS_COMMAND(1:I_LEN(PIM%CONF%FRIB_OBS_COMMAND))// &
...
!
     ...
     &                                ' -- 0 or -1 can be used after the experiment is loaded' )
                             ...
                             IF ( IER .NE. 0 ) THEN
                                  CALL ERR_LOG ( 8219, IUER, 'PIMA_CONF', 'Failure in reading '// &
     ...
     &                                'the experiment' )
                             ...
                             PIM%NOBS = 0
                             DO 470 J7=1,NS
                                IF ( BUFS(J7)(1:23) == 'Number of observations:' ) THEN
                                     STR = BUFS(J7)(24:)
                                     CALL CHASHL ( STR )
 ...
 470                         CONTINUE 
                             IF ( PIM%NOBS == 0 ) THEN
                                  CALL ERR_LOG ( 8220, IUER, 'PIMA_CONF', 'Trap of internal '// &
...
!
     ...
     &                             STAT=IER )
                        IF ( IER .NE. 0 ) THEN
                             CALL CLRCH ( STR )
                             CALL IINCH ( 4*PIM%CONF%FRIB_NOBS, STR )
                             CALL ERR_LOG ( 8224, IUER, 'PIMA_CONF', &
     &                           'Failure to allocate '//STR(1:I_LEN(STR))// &
     &                           'bytes of dynamic memory for the list '// &
     &                           'of observations' )
                             RETURN
...
!
                        IP = 0
                        DO 480 J8=IVAL1, IVAL2
                           IP = IP + 1
                           PIM%CONF%FRIB_OBS(IP) = J8
 480                    CONTINUE
                      ELSE IF ( IVAL == -1 ) THEN
!
...
!
                        ...
                        IF ( IER .NE. 0 ) THEN
                             ...
                             CALL ERR_LOG ( 8225, IUER, 'PIMA_CONF', &
     &                           'Failure to allocate '//STR(1:I_LEN(STR))// &
...
!
     ...
     &                                 BUF_OBS, NB, IER )
                        IF ( IER .NE. 0 ) THEN
                             CALL ERR_LOG ( 8226, IUER, 'PIMA_CONF', &
...
!
                        ...
                        DO 490 J9=1,NB
                           IF ( BUF_OBS(J9)(1:1)  == '#' ) GOTO 490
                           IF ( ILEN(BUF_OBS(J9)) ==  0  ) GOTO 490
...
!
                        IP = 0
                        DO 4100 J10=1,NB
                           IF ( BUF_OBS(J10)(1:1)  == '#' ) GOTO 4100
                           IF ( ILEN(BUF_OBS(J10)) ==  0  ) GOTO 4100
                           IP = IP + 1
                           CALL CHASHL ( BUF_OBS(J10) )
                           IL = INDEX ( BUF_OBS(J10), ' ' )
                           IF ( IL .LE. 0 ) IL = ILEN(BUF_OBS(J10)) + 1
                           READ ( UNIT=BUF_OBS(J10)(1:IL-1), FMT='(I8)', &
     &                            IOSTAT=IER ) IVAL
                           IF ( IER .NE. 0 ) THEN
                                CALL CLRCH ( STR )
                                CALL INCH  ( J10, STR )
     ...
     &                               PIM%CONF%FRIB_OBS_COMMAND(1:I_LEN(PIM%CONF%FRIB_OBS_COMMAND))// &
     &                              ' -- '// &
     &                               BUF_OBS(J10)(1:I_LEN(BUF_OBS(J10))) )
                                DEALLOCATE ( BUF_OBS )
                                RETURN
                           END IF
 ...
 4100                   CONTINUE
     ...
     &               'FRIP.STA_INC_FILE:' ) THEN
              ...
              IF ( PIM%CONF%FRIP_STA_INC_FILE .EQ. 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8229, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &               'FRIP.STA_EXC_FILE:' ) THEN
              ...
              IF ( PIM%CONF%FRIP_STA_EXC_FILE .EQ. 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8230, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &               'FRIP.STA_REFS:' ) THEN
              ...
              DO 4110 J11=2,LIND
                 PIM%CONF%FRIP_STA_REFS(J11-1) = BUF(J3)(IND(1,J11):IND(2,J11))
                 IF ( PIM%CONF%FRIP_STA_REFS(J11-1) == 'SAME' ) THEN
                      PIM%CONF%FRIP_STA_REFS(J11-1) = PIM%CONF%STA_REF
                 END IF
 ...
 4110         CONTINUE
     ...
     &               'FRIP.RESOLUTION:' ) THEN
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8231, IUER, 'PIMA_CONF', 'Wrong value '// &
     ...
     &                       'a colon sign' )
                        RETURN
                      ELSE IF ( IP == ILEN(BUF(J3)(IND(1,2):IND(2,2))) ) THEN
                        CALL ERR_LOG ( 8233, IUER, 'PIMA_CONF', &
     &                      'Wrong value of the keyword FRIP.SCA: '// &
     &                       SCA_COMMAND(1:I_LEN(SCA_COMMAND))// &
     &                       ' -- an integer value should follow a colon sign' )
                        RETURN
     ...
     &               'FRIP.SCAN_FILE:' ) THEN
              ...
              IF ( PIM%CONF%FRIP_SCAN_FILE .EQ. 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8238, IUER, 'PIMA_CONF', 'Cannot '// &
...
!
                   ...
                   IF ( IER .NE. 0 ) THEN
                        ...
                        CALL ERR_LOG ( 8239, IUER, 'PIMA_CONF', &
     &                      'Failure to allocate '//STR(1:I_LEN(STR))// &
     &                      'bytes of dynamic memory for the list '// &
...
!
     ...
     &                            BUF_SCA, NB, IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 8240, IUER, 'PIMA_CONF', &
     &                      'Failure to read the file with '// &
     &                      'observations list '// &
...
!
                   ...
                   DO 4120 J12=1,NB
                      IF ( BUF_SCA(J12)(1:1)  == '#' ) GOTO 4120
                      IF ( ILEN(BUF_SCA(J12)) ==  0  ) GOTO 4120
...
!
     ...
     &                        STAT=IER )
                   IF ( IER .NE. 0 ) THEN
                        ...
                        CALL ERR_LOG ( 8241, IUER, 'PIMA_CONF', &
     &                      'Failure to allocate '//STR(1:I_LEN(STR))// &
     &                      'bytes of dynamic memory for the list '// &
     &                      'of observations' )
                        DEALLOCATE ( BUF_SCA )
...
!
                   ...
                   PIM%CONF%FRIP_LAST_SCA   = 0
                   DO 4130 J13=1,NB
                      IF ( BUF_SCA(J13)(1:1)  == '#' ) GOTO 4130
                      IF ( ILEN(BUF_SCA(J13)) ==  0  ) GOTO 4130
                      IP = IP + 1
                      CALL EXWORD ( BUF_SCA(J13), MIND, LIND_SCA, &
     &                              IND_SCA, REG, IER )
!!                      DO 4140 J14=1,MIN(LIND_SCA,PIM__MPCS)
                      DO 4140 J14=1,2
                         READ ( UNIT=BUF_SCA(J13)(IND_SCA(1,J14):IND_SCA(2,J14)), &
     &                          FMT='(I8)', IOSTAT=IER ) IVAL
                         IF ( IER .NE. 0 ) THEN
                              CALL CLRCH ( STR )
                              CALL INCH  ( J13, STR )
                              CALL ERR_LOG ( 8242, IUER, 'PIMA_CONF', &
     &                            'Failure to decode the '// &
     &                             STR(1:I_LEN(STR))//'th line of the '// &
     &                            'observation file '// &
     &                             PIM%CONF%FRIP_SCAN_FILE(1:I_LEN(PIM%CONF%FRIP_SCAN_FILE))// &
     &                            ' -- '// &
     &                             BUF_SCA(J13)(1:I_LEN(BUF_SCA(J13))) )
                              DEALLOCATE ( BUF_SCA )
                              RETURN
                          END IF
                          PIM%CONF%FRIP_SCA(J14-1,IP) = IVAL
 4140                 CONTINUE
...
!
                             PIM%CONF%FRIP_SCA_STATUS = PIMA__SCA_ALL
                           ELSE
                             CALL ERR_LOG ( 8243, IUER, 'PIMA_CONF', &
     &                           'Wrong value of the keyword FRIP.SCA: '// &
     ...
     &               'FRIP.DEC_RANGE:' ) THEN
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'MKDB.OUTPUT_TYPE:' ) THEN
              ...
              IF  ( BUF(J3)(IND(1,2):IND(2,2)) == 'TEXT' ) THEN
                 ...
                 ELSE
                    CALL ERR_LOG ( 8244, IUER, 'PIMA_CONF', 'Wrong '// &
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'MKDB.SRT:' ) THEN
              ...
              IF ( STR(1:8) == PIMA__MKDB_MID_SCAN ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8245, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &                'MKDB.FRINGE_ALGORITHM:' ) THEN
              ...
              IF ( BUF(J3)(IND(1,2):IND(2,2)) == PIMA__FRA_DRF ) THEN
                 ...
                 ELSE
                   CALL ERR_LOG ( 8247, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &                 ' of keyword MKDB.FRINGE_ALGORITHM' )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'MKDB.FILTER:' ) THEN
              ...
              IF ( STR == 'NO' ) THEN
                 ...
                 ELSE
                   CALL ERR_LOG ( 8248, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &               FMT='(F20.0)' ) PIM%CONF%MKDB_GD_MAX_ADD_ERROR
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8249, IUER, 'PIMA_CONF', 'Failure to '// &
     ...
     &               FMT='(F20.0)' ) PIM%CONF%MKDB_GD_MAX_SCL_ERROR
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8250, IUER, 'PIMA_CONF', 'Failure to '// &
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'MKDB.VCAT_CONFIG:' ) THEN
              ...
              IF ( STR == 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8251, IUER, 'PIMA_CONF', 'Cannot '// &
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'MKDB.DESC_FILE:' ) THEN
              ...
              IF ( STR == 'NO' ) THEN
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX ) THEN
                        CALL ERR_LOG ( 8252, IUER, 'PIMA_CONF', 'Cannot '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8279, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     &                 ' -- Wrong value of the keyword SPLT.BPASS_NRML_RANGE: '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8280, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &                 ' -- an real number > 0 was expected' )
            ...
            ELSE IF ( BUF(J3)(IND(1,1):IND(2,1)) == 'SPLT.GAIN_CORR_FILE:'  ) THEN
              ...
              IF ( STR == 'NO' ) THEN
                   ...
                   IF ( PIM%CONF%ACT_CODE == PIMA__GACO_CODE ) THEN
                        CALL ERR_LOG ( 8284, IUER, 'PIMA_CONF', 'Gain correction '// &
     ...
     &                      'SPLT.GAIN_CORR_FILE in order to run task gaco' )
                 ...
                 ELSE
                   ...
                   IF ( .NOT. LEX .AND. PIM%CONF%ACT_CODE == PIMA__SPLT_CODE ) THEN
                        CALL ERR_LOG ( 8285, IUER, 'PIMA_CONF', 'Cannot '// &
     ...
     &               PIM%CONF%ONOF_NSIG_THRESHOLD 
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8289, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &               PIM%CONF%ONOF_COHERENT_INTERVAL
              IF ( IER .NE. 0 .OR. PIM%CONF%ONOF_COHERENT_INTERVAL .LE. 0.0 ) THEN
                   CALL ERR_LOG ( 8290, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &               PIM%CONF%ONOF_MIN_LOW_AP
              IF ( IER .NE. 0 .OR. PIM%CONF%ONOF_MIN_LOW_AP .LE. 0 ) THEN
                   CALL ERR_LOG ( 8291, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &               PIM%CONF%ONOF_KERNEL_START_SHARE
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8292, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
     ...
     &               PIM%CONF%ONOF_KERNEL_END_SHARE
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8293, IUER, 'PIMA_CONF', 'Error during '// &
     &                 'parsing the '//STR(1:I_LEN(STR))//'-th line of the '// &
     &                 'control file '//CONF_FILE(1:I_LEN(CONF_FILE))// &
...
!
     ...
     &                       %VAL(0) )
               IF ( IS .NE. 0 ) THEN
                   CALL ERR_LOG ( 8294, IUER, 'PIMA_CONF', 'Error in '// &
     ...
     &                  BUF(J3)(IND(1,2):IND(2,LIND)) )
               ...
               END IF
            ELSE
              CALL ERR_LOG ( 8295, IUER, 'PIMA_CONF', 'Error during parsing '// &
     &            'the '//STR(1:I_LEN(STR))//'-th line of the control file '// &
     &             CONF_FILE(1:I_LEN(CONF_FILE))//' -- "'// &
...
!
      IF ( LKEY < PIMA__MKEY ) THEN
           ...
           CALL ERR_LOG ( 8296, IUER, 'PIMA_CONF', 'Not all keywords were '// &
...
!
      IF ( LFRIP_KEY < PIMA__FRIP_KEY ) THEN
           ...
           CALL ERR_LOG ( 8298, IUER, 'PIMA_CONF', 'Not all keywords '// &
...
!
      IF ( LMKDB_KEY < PIMA__MKDB_KEY ) THEN
           ...
           CALL ERR_LOG ( 8299, IUER, 'PIMA_CONF', 'Not all keywords '// &
...
!
      IF ( LBPS_KEY < PIMA__BPS_KEY ) THEN
           ...
           CALL ERR_LOG ( 8300, IUER, 'PIMA_CONF', 'Not all keywords '// &
...
!
      IF ( LSPLT_KEY < PIMA__SPLT_KEY ) THEN
           ...
           CALL ERR_LOG ( 8301, IUER, 'PIMA_CONF', 'Not all keywords '// &
...
!
      IF ( LONOF_KEY < PIMA__ONOF_KEY ) THEN
           ...
           CALL ERR_LOG ( 8302, IUER, 'PIMA_CONF', 'Not all keywords '// &
...
!
             CALL ERR_LOG ( 8303, IUER, 'PIMA_CONF', 'Keywords '// &
...
!
      IF ( PIM%CONF%BPS_INTRP_METHOD == PIMA__SPLINE .AND. PIM%CONF%BPS_DEG_AMP < 3 ) THEN
           CALL ERR_LOG ( 8304, IUER, 'PIMA_CONF', 'Wrong value of '// &
...
!
      IF ( PIM%CONF%BPS_INTRP_METHOD == PIMA__SPLINE .AND. PIM%CONF%BPS_DEG_PHS < 3 ) THEN
           CALL ERR_LOG ( 8305, IUER, 'PIMA_CONF', 'Wrong value of '// &
...
!
     ...
     &       PIM%CONF%PHASE_ACCEL_MAX .NE. 0.0D0      ) ) THEN
            CALL ERR_LOG ( 8306, IUER, 'PIMA_CONF', 'Unsupported '// &
...
!
      IF ( PIM%CONF%PHASE_ACCEL_MAX < PIM%CONF%PHASE_ACCEL_MIN ) THEN
           CALL ERR_LOG ( 8307, IUER, 'PIMA_CONF', 'Unsupported combination '// &
...
!
           IF ( PIM%CONF%POLARCAL_FILE .NE. PIMA__POLARCAL_NO ) THEN
                ...
                IF ( .NOT. LEX ) THEN
                     CALL ERR_LOG ( 8308, IUER, 'PIMA_CONF', 'Cannot '// &
...
!
     ...
     &     PIM%CONF%SPLT_TIM_MSEG < PIM__SPLT_SNR_MINSEG             ) THEN
           ...
           CALL ERR_LOG ( 8309, IUER, 'PIMA_CONF', 'Conflict of parameters '// &
...
!
      IF ( PIM%CONF%ONOF_KERNEL_START_SHARE < 0.0 .OR. PIM%CONF%ONOF_KERNEL_START_SHARE > 1.0 ) THEN
           ...
           CALL ERR_LOG ( 8310, IUER, 'PIMA_CONF', 'Wrong value of parameter '// &
     ...
     &         'in a range [0.0, 1.0] was expected' )
      ...
      IF ( PIM%CONF%ONOF_KERNEL_END_SHARE < 0.0 .OR. PIM%CONF%ONOF_KERNEL_END_SHARE > 1.0 ) THEN
           ...
           CALL ERR_LOG ( 8311, IUER, 'PIMA_CONF', 'Wrong value of parameter '// &
     ...
     &         'in a range [0.0, 1.0] was expected' )
      ...
      IF ( PIM%CONF%ONOF_KERNEL_END_SHARE .LE. PIM%CONF%ONOF_KERNEL_START_SHARE ) THEN
           ...
           CALL ERR_LOG ( 8312, IUER, 'PIMA_CONF', 'Wrong value of parameter '// &
...
!
     ...
     &          PIM%CONF%ONOF_GEN_FLAGS_MODE == PIMA__ONOF_UPDATE      ) THEN
                CONTINUE 
              ELSE
                CALL ERR_LOG ( 8313, IUER, 'PIMA_CONF', 'Wrong keyword '// &
...
!
      IF ( PIM%CONF%FRQ_GRP < 1 ) THEN
           ...
           CALL ERR_LOG ( 8314, IUER, 'PIMA_CONF', 'Wrong keyword '// &
...
!
      IF ( PIM%CONF%BEG_FRQ < 1 ) THEN
           ...
           CALL ERR_LOG ( 8315, IUER, 'PIMA_CONF', 'Wrong keyword '// &
...
!
      IF ( PIM%CONF%END_FRQ < PIM%CONF%BEG_FRQ ) THEN
           ...
           CALL ERR_LOG ( 8316, IUER, 'PIMA_CONF', 'Wrong keyword '// &
...
!
     ...
     &          PIM%CONF%POLAR .NE. PIMA__POLAR_I                 ) THEN
                CALL ERR_LOG ( 8317, IUER, 'PIMA_CONF', 'Wrong combination '// &
     ...
     &              'LL or HH or VV, whichever is suitable for this experiment and '// &
     &              'specify POLARCAL_FILE: NO' )
                RETURN 
           END IF
      END IF
!
     ...
     &       PIM%CONF%POLAR .NE. PIMA__POLAR_I                   ) THEN
             CALL ERR_LOG ( 8318, IUER, 'PIMA_CONF', 'Wrong combination '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 8319, IUER, 'PIMA_CONF', 'Failure to allocate '// &
...
!
      DO 4150 J15=1,PIM%L_FIL
         CALL NOUT ( SIZEOF(PIM%FILE(J15)), PIM%FILE(J15) )
         PIM%FILE(J15)%NAME      = PIM%CONF%UVFILE_NAME(J15)
         PIM%FILE(J15)%ORIG_NAME = PIM%CONF%UVFILE_NAME(J15)
 4150 CONTINUE
!
