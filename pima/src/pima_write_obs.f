...
! *                                                                      *
! * ### 10-JAN-2006  PIMA_WRITE_OBS v 1.41 (d) L. Petrov 10-MAY-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &                                     IER )
              IF ( IER .NE. 0 ) THEN
                   ...
                   IF ( PIM%CONF%CHECK_SEVERITY > 0 ) THEN
...
!
     ...
     &                    'PIMA data structure into the output file '//FINAM )
                 ...
                 END IF
!
                 IF ( PIM%STA(J18)%PCAL(J19)%LPOI_SUBS > 0 ) THEN
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'I4', &
     &                                   PIM%STA(J18)%PCAL(J19)%NO_TONES*PIM%NFRQ*PIM%STA(J18)%PCAL(J19)%NPOI*PIM%STA(J18)%PCAL(J19)%NPOL, &
     &                                   PIM%STA(J18)%PCAL(J19)%IND_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7656, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%IND_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'R4', PIM%STA(J18)%PCAL(J19)%LPOI_SUBS, &
     &                                   PIM%STA(J18)%PCAL(J19)%PHAS_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7657, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%PHAS_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'R4', PIM%STA(J18)%PCAL(J19)%LPOI_SUBS, &
     &                                   PIM%STA(J18)%PCAL(J19)%AMPL_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7658, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%AMPL_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
                 END IF
!
                 IF ( PIM%STA(J18)%PCAL(J19)%LSCA_SUBS > 0 ) THEN
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'I4', &
     &                                   PIM%STA(J18)%PCAL(J19)%NO_TONES*PIM%NFRQ*PIM%STA(J18)%PCAL(J19)%NSCA*PIM%STA(J18)%PCAL(J19)%NPOL, &
     &                                   PIM%STA(J18)%PCAL(J19)%IND_SCA_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7659, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%IND_SCA_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'R4', PIM%STA(J18)%PCAL(J19)%LSCA_SUBS, &
     &                                   PIM%STA(J18)%PCAL(J19)%PHAS_SCA_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7660, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%PHAS_SCA_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS ( IUER, IER )
                      CALL WRBIN_ARRAY ( LUN, 'R4', PIM%STA(J18)%PCAL(J19)%LSCA_SUBS, &
     &                                   PIM%STA(J18)%PCAL(J19)%AMPL_SCA_SUBS, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7661, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                         'an attempt to write a PCAL(J19)%AMPL_SCA_SUBS part of '// &
     &                         'PIMA data structure into the output file '//FINAM )
                           RETURN
                      END IF
                 END IF
 ...
 4190    CONTINUE
         DO 4200 J20=1,PIM%NFRG
            SIZE_BYTES = LOC(PIM%STA(J18)%TSYS(J20)%NTYP) - LOC(PIM%STA(J18)%TSYS(J20)) + &
     &                   SIZEOF(PIM%STA(J18)%TSYS(J20)%NTYP)
            CALL ERR_PASS ( IUER, IER )
            CALL WRBIN_ARRAY ( LUN, 'B1', SIZE_BYTES, PIM%STA(J18)%TSYS(J20), IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7662, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                              PIM%STA(J18)%TSYS(J20)%TSYS, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7663, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                   SIZEOF(PIM%STA(J18)%STMO(J20)%STATUS)
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7669, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                              PIM%STA(J18)%STMO(J20)%TREC, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7676, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                   SIZEOF(PIM%STA(J18)%GAIN(J20)%NTAB)
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7682, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                              PIM%STA(J18)%GAIN(J20)%X_TYP, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7685, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                               PIM%STA(J18)%GAIN(J20)%Y_VAL, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7688, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                       'structure into the output file '//FINAM )
            ...
            END IF
!
            SIZE_BYTES = LOC(PIM%STA(J18)%CABLE(J20)%CABLE_SIGN) - LOC(PIM%STA(J18)%CABLE(J20)) + &
     &                       SIZEOF(PIM%STA(J18)%CABLE(J20)%CABLE_SIGN)
            CALL ERR_PASS ( IUER, IER )
            CALL WRBIN_ARRAY ( LUN, 'B1', SIZE_BYTES, PIM%STA(J18)%CABLE(J20), IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7691, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
     &               'attempt to write a CABLE part of PIMA data '// &
     &               'structure into the output file '//FINAM )
                 RETURN
            END IF
!
            IF ( PIM%STA(J18)%CABLE(J20)%CAB_AVAIL ) THEN
!
! -------------- Write down information related to cable calibration
!
                 CALL ERR_PASS ( IUER, IER )
                 CALL WRBIN_ARRAY ( LUN, 'R8', PIM%STA(J18)%CABLE(J20)%NPOI, &
     &                              PIM%STA(J18)%CABLE(J20)%TIM_CAB, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7692, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                    'an attempt to write a PIM%STA(J18)%CABLE(J20)%TIM_CAB '// &
...
!
                 CALL ERR_PASS ( IUER, IER )
                 CALL WRBIN_ARRAY ( LUN, 'R8', PIM%STA(J18)%CABLE(J20)%NPOI, &
     &                              PIM%STA(J18)%CABLE(J20)%CAB_DEL, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7693, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                    'an attempt to write a PIM%STA(J18)%CABLE(J20)%CAB_DEL '// &
     &                    'part of PIMA data structure into the output '// &
     &                    'file '//FINAM )
                      RETURN
                 END IF
...
!
     ...
     &                SIZEOF(PIM%STA(J18)%WEATHER%NPOI)
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7694, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                           PIM%STA(J18)%WVR%TIM_ARR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7710, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J18)%WVR%DEL_ARR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7711, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J18)%WVR%DEL_ERR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7712, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
     &                 'an attempt to write a WVR%DEL_ARR part of PIMA '// &
     &                 'data structure into the output file '//FINAM )
                   RETURN
              END IF
!
     ...
     &                           PIM%STA(J18)%WVR%EL_ARR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7713, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J18)%WVR%AZ_ARR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7714, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J18)%WVR%HEI_WVR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7715, IUER, 'PIMA_WRITE_OBS', 'Error in '// &
...
!
     ...
     &                      PIM%OBS(J25)%CORR_FLAG, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7725, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                SIZEOF(PIM%AUT(J26)%REF_FRG_INDS(PIM__MUVS))
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7726, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                      PIM%AUT(J26)%UV_IND, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7727, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
     ...
     &                   IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7728, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7729, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL INCH  ( IER, STR )
                CALL ERR_LOG ( 7730, IUER, 'PIMA_WRITE_OBS', 'Error in an '// &
     &              'attempt to open output file '//FINAM(1:I_LEN(FINAM))// &
     &              ' '//STR )
                RETURN
           END IF
           ...
           WRITE ( UNIT=LUN, FMT='(A,F11.6,A)' ) 'Spectral channel width:     ', 1.D-6*PIM%CHAN_BW, ' MHz'
           WRITE ( UNIT=LUN, FMT='(A,F11.6,A)' ) 'IF width:                   ', 1.D-6*PIM%NCHN*PIM%CHAN_BW, ' MHz'
