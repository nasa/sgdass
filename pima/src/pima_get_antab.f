...
! *                                                                      *
! * ### 26-DEC-2008  PIMA_GET_ANTAB  v3.5 (d) L. Petrov 27-DEC-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           TIM_MET_ARR(M_TIM), TSYS_VALS(M_FRQ)
      ...
      CHARACTER  STR*128, STR1*128, STR2*128, STA_NAM*8, SOU_NAM*10, REG*5, &
...
!
     ...
     &         'an attempt to read antab file '//ANTAB_FILE )
      ...
      END IF
!
      IF ( ANTAB_MODE == 'evn_antab_file' .OR. &
     &     ANTAB_MODE == 'vla_antab_file'      ) THEN
!
...
!
 ...
 820       CONTINUE
           IF ( IND_STA == 0 ) THEN
                CALL ERR_LOG ( 8239, IUER, 'PIMA_GET_ANTAB', 'Failure '// &
     &              'in an attempt to parse antab file '// &
     &               ANTAB_FILE(1:I_LEN(ANTAB_FILE))//' -- station '// &
     &              STA_NAM //' did not observe in experiment '// &
     &               PIM%CONF%SESS_CODE )
...
!
      IF ( VTD%STATUS .NE. VTD__INIT ) THEN
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 8240, IUER, 'PIMA_GET_ANTAB', 'Error in an '// &
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 8241, IUER, 'PIMA_GET_ANTAB', 'Error in '// &
...
!
     ...
     &                    PIM%TAI_0 + PIM%TIM_R8(PIM%NEPC), IER )
          IF ( IER .NE. 0 ) THEN
               CALL ERR_LOG ( 8242, IUER, 'PIMA_GET_ANTAB', 'Error in an '// &
...
!
     ...
     &                  'attempt to read NZO data into VTD data structure' )
               ...
               IF ( PIM%NZO%CENTER_NAME == 'EARTH BARYCENTER' ) THEN
                    NZO_REF = VTD__EME
                  ELSE 
                    CALL ERR_LOG ( 8244, IUER, 'PIMA_GET_ANTAB', 'Unsupported '// &
...
!
     ...
     &                                  L_NOD, L_DEG, IER )
               IF ( IER .NE. 0 ) THEN
                    CALL ERR_LOG ( 8245, IUER, 'PIMA_GET_ANTAB', 'Error in an '// &
     &                  'attempt to read NZO data into VTD data structure' )
                    RETURN 
               END IF
          END IF
...
!
         IF ( LIND .GE. 5 ) THEN
            ...
            ELSE
              CALL CLRCH ( WORD5 )
         END IF
         IF ( ANTAB_MODE == 'vla_antab_file' .AND. LIND == 4 .AND. &
     &        BUF(J4)(IND(1,1):IND(2,1)) == '!' ) THEN
              IF ( INDEX ( BUF(J4)(IND(1,3):IND(2,3)), ':' ) > 0 .AND. &
     &             INDEX ( BUF(J4)(IND(1,4):IND(2,4)), ':' ) > 0       ) THEN
                   SOU_NAM = BUF(J4)(IND(1,2):IND(2,2)) 
              END IF
         END IF
!
         CALL CLRCH ( STR )
...
!
                   ...
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 8250, IUER, 'PIMA_GET_ANTAB', 'Error '// &
     &                      'in reading DPFU from line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '//ANTAB_FILE )
                        GOTO 810
                   END IF
!
     ...
     &                      ' of the antab file '//ANTAB_FILE )
                ...
                ELSE IF ( BUF(J4)(IND(1,6):IND(1,6)+4) == 'DPFU=' ) THEN
                   READ ( UNIT=BUF(J4)(IND(1,7):IND(2,7)), FMT=*, IOSTAT=IER ) DPFU(1)
                   IF ( IER .NE. 0 ) THEN
...
!
                   DPFU(2) = DPFU(1)
                 ELSE
                   WRITE ( 6, * ) 'Offending line: '//TRIM(BUF(J4))
                   WRITE ( 6, * ) 'word5: '//BUF(J4)(IND(1,5):IND(2,5))
                   CALL ERR_LOG ( 8253, IUER, 'PIMA_GET_ANTAB', 'Cannot '// &
...
!
 ...
 470          CONTINUE
              IF ( IND_SOU == 0 ) THEN
                   CALL ERR_LOG ( 8258, IUER, 'PIMA_GET_ANTAB', 'Error '// &
     ...
     &                 PIM%CONF%SESS_CODE  )
            ...
            ELSE IF ( BUF(J4)(IND(1,1):IND(2,1)) == 'NUMB_FRQ:' ) THEN
              ...
              IF ( NUMB_FRQ > M_FRQ )   THEN
                   ...
                   CALL ERR_LOG ( 8259, IUER, 'PIMA_GET_ANTAB', 'Antenna '// &
...
!!              DO 480 J8=1,PIM%NFRG
 ...
 480          CONTINUE
            ELSE IF ( BUF(J4)(IND(1,1):IND(2,1)) == 'TSYS:' ) THEN
              IF ( ANTAB_MODE == 'vla_antab_file' ) THEN
                   GOTO 440
              END IF
              CALL ERR_PASS ( IUER, IER )
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8261, IUER, 'PIMA_GET_ANTAB', 'Error '// &
 ...
 4100         CONTINUE
              IF ( IND_SOU == 0 ) THEN
                   ...
                   CALL INCH  ( J4, STR )
                   CALL ERR_PASS ( IUER, IER )
                   CALL ERR_LOG ( 8262, IER, 'PIMA_GET_ANTAB', 'Unknown source '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8266, IUER, 'PIMA_GET_ANTAB', 'Error '// &
     &                 'in parsing minutes field in line '//STR(1:I_LEN(STR))// &
     &                 ' of the antab file '//ANTAB_FILE )
                   GOTO 810
              END IF
              IF ( MIN_R8 < 0.0D0 .OR. MIN_R8 .GE. 60.0D0 ) THEN
                   CALL ERR_LOG ( 8267, IUER, 'PIMA_GET_ANTAB', 'Wrong value '// &
...
!
              ...
              CALL VTD_UTC_TO_TAI ( VTD, MJD, UTC, TAI, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8268, IUER, 'PIMA_GET_ANTAB', 'Error '// &
     &                 'in converting UTC to TAI' )
                   GOTO 810
              END IF
              TIM_TSYS = (MJD - PIM%MJD_0)*86400.0D0 + (TAI - PIM%TAI_0)
              ...
              IF ( ANTAB_MODE == 'vla_antab_file' ) THEN
                   IND_SOU = 0
                   DO 5140 J14=1,PIM%NSOU
                      IF ( SOU_NAM == PIM%SOU(J14)%NAME ) THEN
                           IND_SOU = J14
                         ELSE IF ( SOU_NAM == PIM%SOU(J14)%IVS_NAME ) THEN
                           IND_SOU = J14
                         ELSE IF ( SOU_NAM == PIM%SOU(J14)%J2000_NAME ) THEN
                           IND_SOU = J14
                         ELSE IF ( SOU_NAM == PIM%SOU(J14)%DB_NAME    ) THEN
                           IND_SOU = J14
                      END IF
 5140              CONTINUE 
              END IF
                   ...
                   IF ( ANTAB_MODE == 'evn_antab_file' ) THEN
                        ...
                        DO 4160 J16=3,LIND
                           ...
                           IF ( IER .NE. 0 ) THEN
                                CALL CLRCH ( STR1 )
                                CALL INCH  ( J16, STR1 )
                                CALL ERR_LOG ( 8269, IUER, 'PIMA_GET_ANTAB', &
...
!
 ...
 4170                      CONTINUE
 4160                   CONTINUE
                     ELSE IF ( ANTAB_MODE == 'vla_antab_file' ) THEN
                        IND_FRG = PIM%CONF%FRQ_GRP
                        IF ( PIM%NPOL > 1 .AND. LIND .GE. 7 ) THEN
                             READ ( UNIT=BUF(J4)(IND(1,7):IND(2,7)), FMT=*, IOSTAT=IER ) TSYS_VALS(1)
                             IF ( IER .NE. 0 ) THEN
                                  CALL CLRCH ( STR1 )
                                  CALL INCH  ( J16, STR1 )
                                  CALL ERR_LOG ( 8270, IUER, 'PIMA_GET_ANTAB', &
     &                                'Error in parsing field '//STR1(1:I_LEN(STR1))// &
     &                                ' of line '//STR(1:I_LEN(STR))//' of '// &
     &                                'antab file '//ANTAB_FILE )
                                  GOTO 810
                             END IF
                        END IF
                        IF ( PIM%NPOL .EQ. 1 .AND. LIND .GE. 10 ) THEN
                             READ ( UNIT=BUF(J4)(IND(1,10):IND(2,10)), FMT=*, IOSTAT=IER ) TSYS_VALS(1)
                             IF ( IER .NE. 0 ) THEN
                                  CALL CLRCH ( STR1 )
                                  CALL INCH  ( J16, STR1 )
     ...
     &                                'antab file '//ANTAB_FILE )
                             ...
                             END IF
                        END IF
                        IF ( PIM%NPOL > 1 .AND. LIND .GE. 8 ) THEN
                             READ ( UNIT=BUF(J4)(IND(1,8):IND(2,8)), FMT=*, IOSTAT=IER ) TSYS_VALS(2)
                             IF ( IER .NE. 0 ) THEN
                                  CALL CLRCH ( STR1 )
                                  CALL INCH  ( J16, STR1 )
     ...
     &                                'antab file '//ANTAB_FILE )
                        ...
                        END IF
                        FL_TSYS = .TRUE.
                        DO 5170 J17=1,PIM%NFRQ
                           NUM_TSYS(J17,IND_SCA,IND_FRG) = NUM_TSYS(J17,IND_SCA,IND_FRG) + 1
                           TIM_TSYS_ARR(J17,IND_SCA,IND_FRG)  = ( TIM_TSYS_ARR(J17,IND_SCA,IND_FRG)* &
     &                                (NUM_TSYS(J17,IND_SCA,IND_FRG)-1) + TIM_TSYS)/NUM_TSYS(J17,IND_SCA,IND_FRG)
                           TSYS_ARR(J17,IND_SCA,IND_FRG,1) = ( TSYS_ARR(J17,IND_SCA,IND_FRG,1)* &
     &                                (NUM_TSYS(J17,IND_SCA,IND_FRG)-1) + TSYS_VALS(1))/NUM_TSYS(J17,IND_SCA,IND_FRG)
                           IF ( PIM%NPOL > 1 ) THEN
                                TSYS_ARR(J17,IND_SCA,IND_FRG,2) = ( TSYS_ARR(J17,IND_SCA,IND_FRG,2)* &
     &                                  (NUM_TSYS(J17,IND_SCA,IND_FRG)-1) + TSYS_VALS(2))/NUM_TSYS(J17,IND_SCA,IND_FRG)
                           END IF
 5170                   CONTINUE 
                   END IF
                   IND_SOU_ARR(IND_SCA) = IND_SOU
                ELSE
                   IF ( ANTAB_MODE == 'evn_antab_file' ) THEN
                        CALL CLRCH ( STR  )
                        CALL INCH  ( J4, STR  )
                        CALL ERR_LOG ( 8273, IUER, 'PIMA_GET_ANTAB', 'Trap of internal '// &
...
!
     ...
     &                STAT=IER )
           IF ( IER .NE. 0 ) THEN
                ...
                CALL ERR_LOG ( 8274, IUER, 'PIMA_GET_ANTAB', 'Failure to '// &
...
!
     ...
     &                STAT=IER )
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL IINCH ( 8*PIM%STA(IND_STA)%WEATHER%NPOI, STR )
                CALL ERR_LOG ( 8277, IUER, 'PIMA_GET_ANTAB', 'Failure to '// &
     &               'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
     ...
     &                STAT=IER )
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL IINCH ( 8*PIM%STA(IND_STA)%WEATHER%NPOI, STR )
                CALL ERR_LOG ( 8278, IUER, 'PIMA_GET_ANTAB', 'Failure to '// &
                     'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
     ...
     &                    IOSTAT=IER ) PIM%STA(IND_STA)%WEATHER%TEMP(K_MET)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J25, STR )
                        CALL ERR_LOG ( 8280, IUER, 'PIMA_GET_ANTAB', &
     &                      'Error in reading line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '// &
     &                      ANTAB_FILE(1:I_LEN(ANTAB_FILE))// &
...
!
     ...
     &                    IOSTAT=IER ) PIM%STA(IND_STA)%WEATHER%PRES(K_MET)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J25, STR )
                        CALL ERR_LOG ( 8281, IUER, 'PIMA_GET_ANTAB', &
     &                      'Error in reading line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '// &
     &                      ANTAB_FILE(1:I_LEN(ANTAB_FILE))// &
...
!
     ...
     &                    IOSTAT=IER ) PIM%STA(IND_STA)%WEATHER%HUMID(K_MET)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J25, STR )
                        CALL ERR_LOG ( 8282, IUER, 'PIMA_GET_ANTAB', &
     &                      'Error in reading line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '// &
     &                      ANTAB_FILE(1:I_LEN(ANTAB_FILE))// &
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                ...
                CALL ERR_LOG ( 8283, IUER, 'PIMA_GET_ANTAB', 'Failure to '// &
     &               'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL IINCH ( 8*N_CAB, STR )
                CALL ERR_LOG ( 8284, IUER, 'PIMA_GET_ANTAB', 'Failure to '// &
     &               'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
     ...
     &                                 UTC_TAG, IER  )
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( J26, STR )
                        CALL ERR_LOG ( 8285, IUER, 'PIMA_GET_ANTAB', &
     &                      'Error in reading line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '// &
     &                      ANTAB_FILE(1:I_LEN(ANTAB_FILE))// &
     &                      ' -- cannot parse the date '// &
     &                      BUF(J26)(IND(1,2):IND(2,2)) )
                   ...
                   END IF
!
                   PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%TIM_CAB(K_CAB) = &
     &                     ( MJD_TAG - PIM%MJD_0 )*86400.0D0 + &
     &                     ( UTC_TAG - PIM%UTC_MTAI - PIM%TAI_0 )
!
                   READ ( UNIT=BUF(J26)(IND(1,3):IND(2,3)), FMT='(D12.5)', &
     &                    IOSTAT=IER ) PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(K_CAB)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( J26, STR )
                        CALL ERR_LOG ( 8286, IUER, 'PIMA_GET_ANTAB', &
     &                      'Error in reading line '//STR(1:I_LEN(STR))// &
     &                      ' of the antab file '// &
     &                      ANTAB_FILE(1:I_LEN(ANTAB_FILE))// &
...
! ---------------- Apply cable sign
!
                   PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(K_CAB) = SIGN_CAB* &
     &                              PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(K_CAB)
              END IF
!
              IV(J26) = 1
 4260      CONTINUE
...
!
              ...
              DO 4280 J28=1,K_CAB
                 IF ( IV(J28) == 1 ) THEN
                      AVR_CAB = AVR_CAB + PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J28)
                      NP = NP + 1
                 END IF
 4280         CONTINUE
              AVR_CAB = AVR_CAB/NP
...
!
              DO 4290 J29=1,K_CAB
                 IF ( DABS ( PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J29) - AVR_CAB ) > &
     &                MAX_DEV  .AND.  IV(J29) == 1 ) THEN
!
                      MAX_DEV = DABS( PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J29) - AVR_CAB )
                      IND_MAX_DEV = J29
                 END IF
 4290         CONTINUE
!
...
!
           ...
           PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CABLE_SIGN = SIGN_CAB
!
! -------- Remove the mean cable calibration from cable lenghts
!
           DO 4300 J30=1,K_CAB
              PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J30) = PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_DEL(J30) &
     &                                              - AVR_CAB
 4300      CONTINUE
        ELSE 
           PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%CAB_AVAIL = .FALSE.
           PIM%STA(IND_STA)%CABLE(PIM%CONF%FRQ_GRP)%NPOI = 0
      END IF
!
      IF ( FL_GAIN ) THEN
!
