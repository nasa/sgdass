...
! *                                                                      *
! *  ###  17-SEP-2014   PIMA_OPAL  v1.4 (d)  L. Petrov  21-FEB-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      ...
      CHARACTER  SOB_DIR*128, STR*128, STR1*128, FINAM*128, DATE_BEG_STR*21, &
     &           DATE_END_STR*21
     ...
     &           INDS(4), DIMS(4), IND_POI, KPOL, J1, J2, J3, J4, J5, J6, J7, &
     &           J8, J9, J10, J11, J12, J13, J14, J15, J16, J17, J18, J19, &
     &           J20, J21, J22, IER
      REAL*8,    EXTERNAL :: DEL_ISA 
      REAL*4,    EXTERNAL :: VAL_4D_BSPL4
...
!
      ...
      IF ( DIR_DESC(1) == 0 ) THEN
           CALL ERR_LOG ( 7711, IUER, 'PIMA_OPAL', 'Directory with atmosphere '// &
...
! --- in ascii representation, decode them into the binary form, and put into
...
!
 ...
 440     CONTINUE 
     ...
     &            PIM%C_STA(J3) )
         ...
         END IF
!
         IF ( ISTA > SAT(1)%NS ) THEN
              WRITE ( 6, * ) 'ISTA   = ', ISTA
              WRITE ( 6, * ) 'SAT(1)%NS = ', SAT(1)%NS
              CALL ERR_LOG ( 7726, IUER, 'PIMA_OPAL', 'Trap of internal '// &
     &            'control: ISTA > SAT%NS for station '//PIM%C_STA(J3) )
              RETURN
         END IF
!
...
!@         IF ( SPD_RES(J3)%N_FRQ > PIM%NFRQ ) THEN
!@              CALL CLRCH   ( STR )
!@              CALL CLRCH   ( STR1 )
!@              CALL INCH    ( SPD_RES(J3)%N_FRQ, STR )
!@              CALL INCH    ( PIM%NFRQ, STR1 )
!@              CALL ERR_LOG ( 7726, IUER, 'PIMA_OPAL', 'Trap of internal '// &
!@     &            'control: the number of frequencies in the atmosheic '// &
!@     &            'opacity file is '//TRIM(STR)//' which is greater than '// &
!@     &            'PIM%NFRQ= '//TRIM(STR1)//'. Please check whether you have '// &
!@     &            'stale opacity files' )
!@              RETURN
!@         END IF
!
         ...
         DIMS(4) = SPD_RES(J3)%N_FRQ
!
! ------ Alocate dynamic memory in SPD_RES data structures for various fields.
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7731, IUER, 'PIMA_OPAL', 'Failure in an '// &
...
!
         DO 480 J8=1,SPD_RES(J3)%N_FRQ
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7735, IUER, 'PIMA_OPAL', 'Trap of internal '// &
...
!
     ...
     &                              1-SPD__MDEG:SPD_RES(J3)%N_FRQ), STAT=IER   )
         IF ( IER .NE. 0 ) THEN
              CALL CLRCH ( STR ) 
...
!
     ...
     &                       (SPD_RES(J3)%N_TIM + SPD__MDEG)* &
     &                        SPD_RES(J3)%N_FRQ, STR ) 
              CALL ERR_LOG ( 7737, IUER, 'PIMA_OPAL', 'Failure in an '// &
     &            'attempt to allocate '//STR(1:I_LEN(STR))//' bytes '// &
...
!
     ...
     &                      SPD_RES(J3)%TAT(J12,J11,J10,J9)
                     IF ( IER .NE. 0 ) THEN
                          CALL ERR_LOG ( 7739, IUER, 'PIMA_OPAL', 'Trap of internal '// &
...
!
         CALL ERR_PASS ( IUER, IER )
         CALL BSPL4_4D_CMP ( SPD__MDEG, 10, DIMS, SPD_RES(J3)%ELV%MAP, SPD_RES(J3)%AZM%AZIM, &
     &                       SPD_RES(J3)%TIM_ARR, SPD_RES(J3)%FRQ_ARR, &
     &                       SPD_RES(J3)%OPA, IER )
         IF ( IER .NE. 0 ) THEN
     ...
     &            'attempt to expand opacity in 3D B-spline basis' )
         ...
         END IF
!
!%        write ( 6, * ) 'PIMA_OPAL-478 J9=  ', INT2(J9), ' J3= ', INT2(J3), ' Sta: ', PIM%C_STA(J3), ' IUER= ', IUER              ; call flush ( 6 ) ! %%%%%%%%%%
!%        write ( 6, * ) 'PIMA_OPAL-479 DIMS= ', DIMS                            ; call flush ( 6 ) ! %%%%%%%%%%
!%        write ( 6, * ) 'PIMA_OPAL-480 MAP:  ', SPD_RES(J3)%ELV%MAP(1:DIMS(1))  ; call flush ( 6 ) ! %%%%%%%%%%
!%        write ( 6, * ) 'PIMA_OPAL-481 AZIM: ', SPD_RES(J3)%AZM%AZIM(1:DIMS(2)) ; call flush ( 6 ) ! %%%%%%%%%%
!%        write ( 6, * ) 'PIMA_OPAL-482 TIM:  ', SPD_RES(J3)%TIM_ARR(1:DIMS(3))  ; call flush ( 6 ) ! %%%%%%%%%%
!
         CALL ERR_PASS ( IUER, IER )
         CALL BSPL4_4D_CMP ( SPD__MDEG, 10, DIMS, SPD_RES(J3)%ELV%MAP, SPD_RES(J3)%AZM%AZIM, &
     &                       SPD_RES(J3)%TIM_ARR, SPD_RES(J3)%FRQ_ARR, &
...
!
     ...
     &                 STAT=IER )
            ...
            IF ( IER .NE. 0 ) THEN
                  CALL ERR_LOG ( 7747, IUER, 'PIMA_OPAL', 'Failure in an '// &
     &                'attempt to allocate dynamic memory for array '// &
     &                '%STMO(J13)%TAT' )
                  RETURN 
            END IF
...
!
! ------------ Compute azimuth elevation for J17-station that observes
...
!
     ...
     &                             VTD, AZ, EL, IER )
               IF ( IER .NE. 0 ) THEN
                    CALL CLRCH ( STR )
                    CALL INCH  ( J1, STR )
                    CALL ERR_LOG ( 7749, IUER, 'PIMA_OPAL', 'Error in an '// &
...
!
                  DO 4190 J19=1,PIM%NFRQ
                     ARGS(4) = PIM%FRQ(J19,J18)%FREQ + PIM%FRQ(J19,J18)%BAND_WIDTH/2.0D0
                     INDS(4) = IXMN4 ( DIMS(4), SPD_RES(J17)%FRQ_ARR, ARGS(4) )
                     IF ( INDS(4) < 1 ) THEN
                          ARGS(4) =  SPD_RES(J17)%FRQ_ARR(1) + PIM%FRQ(J19,J18)%BAND_WIDTH/2.0D0
                          INDS(4) = 1
                     END IF
!
...
!
                     IF ( INDS(1) > 0 ) THEN
                          OPA_VAL = VAL_4D_BSPL4 ( ARGS, SPD__MDEG, DIMS, INDS, &
     &                                  SPD_RES(J17)%ELV%MAP, &
     &                                  SPD_RES(J17)%AZM%AZIM, &
     &                                  SPD_RES(J17)%TIM_ARR, &
     &                                  SPD_RES(J17)%FRQ_ARR, &
     &                                  SPD_RES(J17)%OPA )
                          TAT_VAL = VAL_4D_BSPL4 ( ARGS, SPD__MDEG, DIMS, INDS, &
     &                                  SPD_RES(J17)%ELV%MAP, &
     &                                  SPD_RES(J17)%AZM%AZIM, &
     &                                  SPD_RES(J17)%TIM_ARR, &
     &                                  SPD_RES(J17)%FRQ_ARR, &
