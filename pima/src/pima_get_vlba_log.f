...
! *                                                                      *
! * ### 17-SEP-2009 PIMA_GET_VLBA_LOG v3.18 (d) L. Petrov 18-FEB-2026 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &           IVAL, IDOY, I_SOU, K_BAD_OBS, IP, IL, L_FRG, I_FRG, LIS_FRG(PIM__MFRG), &
     &           L_NOD, L_DEG, NZO_REF, &
     ...
     &           IND_BEG, NX_TONES, IND_NX(PIM__MTON), IND_IN_NX
      CHARACTER  X_TONES(PIM__MTON)*4
     ...
     &           TAI_BEG, TAI_END
      ...
      INTEGER*4, EXTERNAL ::  ADD_CLIST, ADD_LIS, IFIND_PL, ILEN, I_LEN, LTM_DIF
!
      LPT = 0
...
!
      NX_TONES = 0
      CALL GETENVAR ( 'PIMAVAR_GET_NO_PCAL_TONES', STR )
      IF ( ILEN(STR) > 0 ) THEN
           CALL MULTI_INDICES ( PIM__MTON, NX_TONES, IND_NX, STR, ':' )
           IND_BEG = 1
           IF ( NX_TONES > 0 ) THEN
                DO 510 J1=1,NX_TONES
                   X_TONES(J1) = STR(IND_BEG:IND_NX(J1)-1)
                   IND_BEG = IND_NX(J1)+1
 510           CONTINUE 
           END IF
           NX_TONES = NX_TONES + 1
           X_TONES(NX_TONES) = STR(IND_BEG:ILEN(STR))
!
           WRITE ( 6, * ) 'PIMA_GET_VLBA_LOG-145: ', NX_TONES, ' phase cal tones will be excluded'
      END IF
!
...
!
      DO 410 J1=2,NBUF
         ...
         IF ( BUF(J1)(75:77) == ' F ' .OR. &
     &        BUF(J1)(75:77) == ' R '      ) GOTO 410
...
!
 ...
 420               CONTINUE
         ...
         IF ( FL_FR ) THEN
              IF ( BUF(J1)(1:1) == '!' ) GOTO 410
              IF ( NX_TONES > 0 ) THEN
                   IND_IN_NX = LTM_DIF ( 0, NX_TONES, X_TONES, BUF(J1)(IND(1,1):IND(2,1)) )
                   IF ( IND_IN_NX > 0 ) GOTO 410
              END IF
              NPT = NPT + 1
...
!
              ...
              DO 530 J3=PIM_MIN_FRG,PIM_MAX_FRG
                 DO 540 J4=1,PIM%NFRQ
                    ...
                    IF ( BUF(J1)(IND(1,3):IND(2,3)) == "L" ) THEN
     ...
     &                        FRQ_SKY(NPT) .LE. FRQ_IF + PIM%FRQ(J4,J3)%BAND_WIDTH + FRQ_TOL ) THEN
                              IF ( IND_FRQ == 0  .AND.  IND_FRG == 0 ) THEN
                                   ...
                                   I_FRG = ADD_LIS ( PIM__MFRG, L_FRG, LIS_FRG, IND_FRG, IER )
                                 ELSE
!
...
!
     ...
     &                                                (PIM%FRQ(J4,J3)%FREQ + PIM%FRQ(J4,J3)%BAND_WIDTH/2.0D0 ) )
                              ...
                              END IF
                         END IF
                      ELSE IF ( BUF(J1)(IND(1,3):IND(2,3)) == "U" ) THEN
                         IF ( FRQ_SKY(NPT) .GE. FRQ_IF - FRQ_TOL .AND. &
     &                        FRQ_SKY(NPT) .LE. FRQ_IF + PIM%FRQ(J4,J3)%BAND_WIDTH + FRQ_TOL ) THEN
                              IF ( IND_FRQ == 0  .AND.  IND_FRG == 0 ) THEN
                                   IND_FRQ = J4
...
!
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 8 ) THEN
                   WRITE ( 6, * ) 'PIMA_GET_VLBA_LOG-686 line= ', J1, ' IND_FRQ= ', INT2(IND_FRQ), &
...
!
     ...
     &                           ' not observed in experiment '//PIM%CONF%SESS_CODE )
                             GOTO 810
                        END IF
                   END IF
              END IF
         ...
         IF ( PIM%CONF%DEBUG_LEVEL .GE. 9 ) THEN
              WRITE ( 6, * ) 'PIMA_GET_VLBA_LOG-771 line= ', J1, ' FL_PC= ', FL_PC, ' FL_PU= ', FL_PU, &
     &                       ' FL_FR= ', FL_FR, ' IND_FRG= ', INT2(IND_FRG), &
     &                       ' IND_FRQ= ', INT2(IND_FRQ), ' NPT= ', INT2(NPT), ' IND_PT= ', INT2(IND_PTC), &
     &                       ' TONE_NAME='//BUF(J1)(IND(1,3):IND(2,3))
              CALL FLUSH ( 6 )
         END IF
!
         IF ( FL_PU .AND. BUF(J1)(1:1) .NE. '!' ) THEN
              IF ( BUF(J1)(IND(1,3):IND(2,3)) == 'CC' ) THEN
                   NPC_STA(IND_STA,LIS_FRG(1)) = NPC_STA(IND_STA,LIS_FRG(1)) + 1
                   IF ( IND_FRG == 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
...
!
     ...
     &                    IOSTAT=IER ) CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
     ...
     &                     ' -- faiulre in decoding the 4th word' )
                   ...
                   END IF
!
! ---------------- Check for sanity of cable calibration
!
                   IF ( IS_R4_INF ( CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA) ) .OR. &
     &                  IS_R4_NAN ( CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA) )      ) THEN
                        NPC_STA(IND_STA,LIS_FRG(1)) = NPC_STA(IND_STA,LIS_FRG(1)) - 1
                        GOTO 410
                   END IF
                   CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA) = &
     &                       1.D-12*CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA)
                   DO 550 J5=1,L_FRG
                      CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(J5)),LIS_FRG(J5),IND_STA) = CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(1)),LIS_FRG(1),IND_STA) 
                      CABLE_MEAN(IND_STA,LIS_FRG(J5)) = CABLE_MEAN(IND_STA,LIS_FRG(J5)) + &
     &                                                  CABLE_CAL(NPC_STA(IND_STA,LIS_FRG(J5)),LIS_FRG(J5),IND_STA)
 550               CONTINUE 
                 ELSE
                   IF ( NX_TONES > 0 ) THEN
                        IND_IN_NX = LTM_DIF ( 0, NX_TONES, X_TONES, BUF(J1)(IND(1,3):IND(2,3)) )
                        IF ( IND_IN_NX > 0 ) GOTO 410
                   END IF
                   IND_PTC = LTM_DIF ( 0, NPT, PCAL_TAB, &
     &                                 BUF(J1)(IND(1,3):IND(2,3)) )
                   IF ( IND_PTC .LE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
     ...
     &                     ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
     &                     'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
     &                     ' -- unknown tone code '//BUF(J1)(IND(1,3):IND(2,3)) )
                        GOTO 810
                   END IF
...
!
     ...
     &                                 ' -- IND_FRG=0' )
                                  RETURN 
                             END IF
!@                             IF ( NPC_STA(IND_STA,IND_FRG) == 0 ) THEN
!@                                  CALL CLRCH ( STR )
!@                                  CALL INCH  ( J1, STR )
!@                                  CALL CLRCH ( STR1 )
!@                                  CALL INCH  ( IND_FRG, STR1 )
!@                                  WRITE ( 6, * ) ' IND_PTC = ', IND_PTC, ' IND_PTC_PREV= ', IND_PTC_PREV
!@                                  CALL ERR_LOG ( 8349, IUER, 'PIMA_GET_VLBA_LOG', &
!@     &                                 'Error in parsing the '//STR(1:I_LEN(STR))// &
!@     &                                 ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
!@     &                                 'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
!@     &                                 ' IND_FRG='//TRIM(STR1)// &
!@     &                                 ' -- NPC_STA(IND_STA,IND_FRG)=0' )
!@                                  RETURN 
!@                             END IF
                        END IF
                   END IF
...
!
     ...
     &                    IOSTAT=IER ) PCAL_AMP(IND_TONE,IND_FREQ,IND_POL_USE,NPC_STA(IND_STA,IND_FRG),IND_FRG,IND_STA)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
                        CALL ERR_LOG ( 8350, IUER, 'PIMA_GET_VLBA_LOG', &
     &                     'Error in parsing the '//STR(1:I_LEN(STR))// &
     &                     ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
     &                     'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
     &                     ' -- faiulre in decoding the 4th word' )
...
!
     ...
     &                    IOSTAT=IER ) PCAL_PHS(IND_TONE,IND_FREQ,IND_POL_USE,NPC_STA(IND_STA,IND_FRG),IND_FRG,IND_STA)
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
                        CALL ERR_LOG ( 8351, IUER, 'PIMA_GET_VLBA_LOG', &
     &                     'Error in parsing the '//STR(1:I_LEN(STR))// &
     &                     ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
     &                     'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
     &                     ' -- faiulre in decoding the 5th word' )
...
!
              ...
              IF ( IDOY < 1 .OR. IDOY > 366 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 8352, IUER, 'PIMA_GET_VLBA_LOG', 'Error '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 8353, IUER, 'PIMA_GET_VLBA_LOG', 'Error '// &
...
!
                                  IF ( BUF(J1)(50:57) == ' reason=' ) THEN
                                       BUF(J1)(50:) = BUF(J1)(51:)
                                     ELSE IF ( BUF(J1)(48:55) == ' reason=' ) THEN
                                       BUF(J1)(49:) = BUF(J1)(48:)
                                  END IF
                                  IF ( BUF(J1)(58:81) == 'source change in progres' ) THEN
                                    ...
                                    ELSE IF ( BUF(J1)(58:80) == 'Antenna elevation error' ) THEN
                                       PIM%OBS(J4)%CORR_FLAG(J5,J6) = 2
                                    ELSE IF ( BUF(J1)(58:80) == 'channel 4 bbc synthesize' ) THEN
                                       PIM%OBS(J4)%CORR_FLAG(J5,J6) = 2
                                    ELSE
                                       CALL CLRCH ( STR )
                                       CALL INCH  ( J1, STR )
...
!
              IF ( NX_TONES > 0 ) THEN
                   STR1 = 'T'//BUF(J1)(IND(2,2)-1:IND(2,2))
                   CALL BLANK_TO_ZERO ( STR1(1:3) )
                   IND_IN_NX = LTM_DIF ( 0, NX_TONES, X_TONES, STR1(1:3) )
                   IF ( IND_IN_NX > 0 ) GOTO 410
              END IF
...
!!!                   FRQ_VAL= DABS(FRQ_VAL)
     ...
     &                      ' -- faiulre in decoding the 10th word' )
                   ...
                   IF ( PIMAVAR_SUB_BAND == 'L' ) BUF(J1)(IND(1,7):IND(2,7)) = "L" ! I am still in a loss here. 2026.02.17_13:38:12
 ...
 570               CONTINUE
                   IF ( IND_FRG == 0 ) THEN
                        IF ( PIM%CONF%CHECK_SEVERITY .GE. 2 ) THEN
                             CALL CLRCH ( STR )
                             CALL INCH  ( J1, STR )
                             CALL ERR_LOG ( 8369, IUER, 'PIMA_GET_VLBA_LOG', &
     &                           'Error in parsing the '//STR(1:I_LEN(STR))// &
     &                           ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
     &                           'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
...
!
     ...
     &                          PIM%NPOL > 1 ) THEN
                      ...
                      ELSE
                        CALL CLRCH ( STR )
                        CALL INCH  ( J1, STR )
                        CALL ERR_LOG ( 8370, IUER, 'PIMA_GET_VLBA_LOG', &
     &                      'Error in parsing the '//STR(1:I_LEN(STR))// &
     &                      ' line '//BUF(J1)(1:I_LEN(BUF(J1)))//' of the '// &
     &                      'log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
     &                      ' -- unsupported polarization code '// &
...
!
 ...
 860                    CONTINUE
                        IF ( IND_SOU .LE. 0 ) THEN
                             ...
                             CALL CLRCH ( STR )
                             CALL INCH  ( J1, STR )
                             CALL ERR_LOG ( 8371, IUER, 'PIMA_GET_VLBA_LOG', 'Error'// &
     &                           ' in parsing the '//STR(1:I_LEN(STR))//'-th line'// &
     &                           ' of the log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
     &                           ' source '//BUF(J1)(IND(1,4):IND(2,4))//' was'// &
     &                           ' not observed in experiment '//PIM%CONF%SESS_CODE )
...
!
              IF ( IND_FRG .LE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 8372, IUER, 'PIMA_GET_VLBA_LOG', 'Error'// &
     &                 ' in parsing the '//STR(1:I_LEN(STR))//'-th line'// &
     &                 ' of the log file '//LOG_FILE(1:I_LEN(LOG_FILE))// &
...
!
     ...
     &                                                  PIM%STA(J18)%PCAL(J19)%NPOI)   )
                      PIM%STA(J18)%PCAL(J19)%FREQ = 0.0D0
...
!
!
! ------------------- Reallocate arrays AMPL_RGR, PHAS_RGR, PHAS_SCA, AMPL_SCA, PRAT_SCA
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%AMPL_RGR ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%AMPL_RGR )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%AMPL_RGR(PIM%STA(J18)%PCAL(J19)%NO_TONES,PIM%NFRQ, &
          &                              PIM%STA(J18)%PCAL(J19)%NPOI,PIM%STA(J18)%PCAL(J19)%NPOL) )
                      PIM%STA(J18)%PCAL(J19)%AMPL_RGR = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%PHAS_RGR ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%PHAS_RGR )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%PHAS_RGR(PIM%STA(J18)%PCAL(J19)%NO_TONES,PIM%NFRQ, &
          &                              PIM%STA(J18)%PCAL(J19)%NPOI,PIM%STA(J18)%PCAL(J19)%NPOL) )
                      PIM%STA(J18)%PCAL(J19)%PHAS_RGR = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%PHAS_SCA ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%PHAS_SCA )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%PHAS_SCA(PIM%STA(J18)%PCAL(J19)%NO_TONES,PIM%NFRQ, &
          &                              PIM%STA(J18)%PCAL(J19)%NPOI,PIM%STA(J18)%PCAL(J19)%NPOL) )
                      PIM%STA(J18)%PCAL(J19)%PHAS_SCA = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%AMPL_SCA ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%AMPL_SCA )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%AMPL_SCA(PIM%STA(J18)%PCAL(J19)%NO_TONES,PIM%NFRQ, &
          &                              PIM%STA(J18)%PCAL(J19)%NPOI,PIM%STA(J18)%PCAL(J19)%NPOL) )
                      PIM%STA(J18)%PCAL(J19)%AMPL_SCA = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%PRAT_SCA ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%PRAT_SCA )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%PRAT_SCA(PIM%STA(J18)%PCAL(J19)%NO_TONES,PIM%NFRQ, &
          &                              PIM%STA(J18)%PCAL(J19)%NPOI,PIM%STA(J18)%PCAL(J19)%NPOL) )
                      PIM%STA(J18)%PCAL(J19)%PRAT_SCA = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%IPOI_SCA ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%IPOI_SCA )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%IPOI_SCA(PIM%STA(J18)%PCAL(J19)%NPOI) )
                      PIM%STA(J18)%PCAL(J19)%IPOI_SCA = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%ISCA_POI ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%ISCA_POI )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%ISCA_POI(PIM%STA(J18)%PCAL(J19)%NPOI) )
                      PIM%STA(J18)%PCAL(J19)%ISCA_POI = 0
!
                      IF ( ASSOCIATED ( PIM%STA(J18)%PCAL(J19)%TIME_SCA_R8 ) ) THEN
                           DEALLOCATE ( PIM%STA(J18)%PCAL(J19)%TIME_SCA_R8 )
                      END IF
                      ALLOCATE ( PIM%STA(J18)%PCAL(J19)%TIME_SCA_R8(PIM%STA(J18)%PCAL(J19)%NPOI) )
                      PIM%STA(J18)%PCAL(J19)%TIME_SCA_R8 = 0.0D0
!
                      IF ( PIM%STA(J18)%CABLE(J19)%CAB_AVAIL ) THEN
                           DEALLOCATE ( PIM%STA(J18)%CABLE(J19)%TIM_CAB )
                           DEALLOCATE ( PIM%STA(J18)%CABLE(J19)%CAB_DEL )
                      END IF
                      PIM%STA(J18)%CABLE(J19)%CAB_AVAIL  = .TRUE.
                      PIM%STA(J18)%CABLE(J19)%CABLE_SIGN = 1
                      PIM%STA(J18)%CABLE(J19)%MEAN_CABLE = CABLE_MEAN(J18,J19)/NPC_STA(J18,J19)
                      PIM%STA(J18)%CABLE(J19)%NPOI = PIM%STA(J18)%PCAL(J19)%NPOI
                      ALLOCATE ( PIM%STA(J18)%CABLE(J19)%TIM_CAB(PIM%STA(J18)%CABLE(J19)%NPOI)  )
                      ALLOCATE ( PIM%STA(J18)%CABLE(J19)%CAB_DEL(PIM%STA(J18)%CABLE(J19)%NPOI)  )
                      PIM%STA(J18)%CABLE(J19)%TIM_CAB = 0.0D0
                      PIM%STA(J18)%CABLE(J19)%CAB_DEL = 0.0D0
!
...
!
 ...
 4210                    CONTINUE
     ...
     &                                         STR(31:51), STR(61:68)
 210                          FORMAT ( 'PIMA_GET_VLBA_LOG-1651 Sta: ', A, ' I_cal: ', I4, ' [ ', A, ' , ', A, &
     &                              ' ]  Sou: ',A )
                         END IF
                         PIM%STA(J18)%CABLE(J19)%TIM_CAB(J20) = PIM%STA(J18)%PCAL(J19)%TIME_MID_R8(J20)
                         PIM%STA(J18)%CABLE(J19)%CAB_DEL(J20) = CABLE_CAL(J20,J19,J18) - &
     &                                                          PIM%STA(J18)%CABLE(J19)%MEAN_CABLE
 4200                 CONTINUE
                 END IF
 4190         CONTINUE
...
!
                    OBS_BEG = PIM%TIM_R8(PIM%UV_IND(PIM%OBS(J26)%UV_IND(1,IND_OBS_FRG))%TIM_IND)
                    OBS_END = PIM%TIM_R8(PIM%UV_IND(PIM%OBS(J26)%UV_IND(PIM%OBS(J26)%NUM_EPC(IND_OBS_FRG),IND_OBS_FRG))%TIM_IND)
                    IND_TSYS = 0
                    IND_TSYS_SHADOW = 0
 ...
 4390 CONTINUE
      IF ( K_BAD_OBS > 0 ) THEN
           ...
           IF ( PIM%CONF%CHECK_SEVERITY .GE. 3 ) THEN
                CALL ERR_LOG ( 8379, IUER, 'PIMA_GET_VLBA_LOG', &
     ...
     &                                   STR(1:I_LEN(STR))//' observations'
      ...
      IF ( NTSYS_MIS > 0 ) THEN
           ...
           IF ( PIM%CONF%CHECK_SEVERITY .GE. 3 ) THEN
                CALL ERR_LOG ( 8380, IUER, 'PIMA_GET_VLBA_LOG', &
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 06-JAN-2010 CORR_FLAG_INIT  v1.0 (d)  L. Petrov  06-JAN-2010 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
