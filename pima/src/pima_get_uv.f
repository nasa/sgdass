...
! *                                                                      *
! *  ### 12-JAN-2006  PIMA_GET_UV  v10.6 (d)  L. Petrov  23-FEB-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
     ...
     &           AUT_IND, IND_POLC, IND_POLA(2,2), LTIM, &
     ...
     &           N_UV_BYTES, E_FRG, K_FRQ, TIM_IND, IND_POL, IER
      INTEGER*2  STA_IND(2)
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           RETURN
      END IF
!%    write ( 6, * ) 'PIMA_GET_UV-241 pol_conf= ', pol_conf, ' pol_mode= ', pol_mode, ' l_pol= ', l_pol ; call flush(6) ! %%%%%%%%%%%%%%%%%%
!
...
!
                 ...
                 END IF
!
!%   write ( 6, * ) 'PIMA_GET_UV-476 j1= ', int2(j1), ' weis= ', WEI_ARR(1:4,FIL_FRQ_IND,E_FRG) ! %%%%
...
!
     ...
     &                                              CMPLX ( COS(BPAS_PHS(2,2)-BPAS_PHS(1,2)), &
     &                                                      SIN(BPAS_PHS(2,2)-BPAS_PHS(1,2))  )
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1331 ', UV(J17,I_FRQ,J1,1), UV(J17,I_FRQ,J1,2)  ! %%%%%%%%%%%%%%
                        ELSE IF ( POL_CONF == PIMA__PC_LC .AND. POL_MODE == PIMA__PAR ) THEN
!
! ------------------------ Cir-cir polarization,  RR-pol and LL-pol, parallactic angle rotation is applied
!
                           UV(J17,I_FRQ,J1,1) = CAL_OBS(1,1) * &
     ...
     &                                              CMPLX ( COS(BPAS_PHS(2,2)-BPAS_PHS(1,2)), &
     &                                                      SIN(BPAS_PHS(2,2)-BPAS_PHS(1,2))  )
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1342 ', UV(J17,I_FRQ,J1,1), UV(J17,I_FRQ,J1,2)  ! %%%%%%%%%%%%%%
                        ELSE IF ( POL_CONF == PIMA__PC_CC .AND. POL_MODE == PIMA__PALL_CIR .AND. PIM%NSTK == 4 ) THEN
!
...
!
     ...
     &                                              CMPLX ( COS(BPAS_PHS(2,2)-BPAS_PHS(1,2)), &
     &                                                      SIN(BPAS_PHS(2,2)-BPAS_PHS(1,2))  )
                        ELSE IF ( POL_CONF == PIMA__PC_CC .AND. POL_MODE == PIMA__PALL_CIR .AND. PIM%NSTK == 2 ) THEN
!
! ------------------------ Cir-cir polarization, all polarizations, parallactic angle rotation is applied
!
                           UV(J17,I_FRQ,J1,1) = CAL_OBS(1,1) * &
...
!
     ...
     &                                                CMPLX ( COS(-BPAS_PHS(2,2)),  &
     &                                                        SIN(-BPAS_PHS(2,2))  )
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1481 ', UV(J17,I_FRQ,J1,1), UV(J17,I_FRQ,J1,2)  ! %%%%%%%%%%%%%%
...
!
     ...
     &                                                        SIN(-BPAS_PHS(2,2)) ) &
     &                                          )
                        ELSE IF ( POL_CONF == PIMA__PC_LC .AND. POL_MODE == PIMA__IPLL ) THEN
                           UV(J17,I_FRQ,J1,1) = ( CAL_OBS(1,1) * &
     &                                                CMPLX ( COS(-(P1-P2)), SIN(-(P1-P2)) ) +   &
     &                                            CAL_OBS(2,2) * &
     &                                                CMPLX ( COS(  P1-P2 ), SIN(  P1-P2 ) )/    &
     ...
     &                                                        SIN(BPAS_PHS(2,2)-BPAS_PHS(1,2)) ) &
     &                                          )/2.0
!% if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1521 ', UV(J17,I_FRQ,J1,1)  ! %%%%%%%%%%%%%%
...
!
                        ...
                        ELSE IF ( POL_CONF == PIMA__PC_LC .AND. POL_MODE == PIMA__HRLC ) THEN
                           UV(J17,I_FRQ,J1,1) = CAL_OBS(1,1)
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1560 ', UV(J17,I_FRQ,J1,1)  ! %%%%%%%%%%%%%%
                        ELSE IF ( POL_CONF == PIMA__PC_LC .AND. POL_MODE == PIMA__VRLC ) THEN
                           UV(J17,I_FRQ,J1,1) = CAL_OBS(2,1)
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1576 ', UV(J17,I_FRQ,J1,1)  ! %%%%%%%%%%%%%%
...
!@     &                                          ) / 2.0
     ...
     &                                                        SIN(BPAS_PHS(2,2)-BPAS_PHS(1,2)) ) &
     &                                          )/2.0
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1647 ', UV(J17,I_FRQ,J1,1),  ' cal_obs= ', cal_obs, ' dp= ', p1-p2, ' bp_12= ', BPAS_PHS(1,2), ' bp_22= ', BPAS_PHS(2,2) ! %%%%%%%%%%%%%%
!
...
!
     ...
     &                                               CMPLX ( COS(-BPAS_PHS(2,2)), SIN(-BPAS_PHS(2,2)) )  &
     &                                          )/ SQRT(2.0)
!%  if ( j17 == 222 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1743 ', UV(J17,I_FRQ,J1,1),  ' cal_obs= ', cal_obs, ' dp= ', p1-p2, ' bp_12= ', BPAS_PHS(1,2), ' bp_22= ', BPAS_PHS(2,2) ! %%%%%%%%%%%%%%
                        ELSE IF ( POL_CONF == PIMA__PC_CL .AND. POL_MODE == PIMA__IPCL ) THEN
                           CONTINUE 
                    END IF
!%  if ( j17 == 2 .and. i_frq == 2 .and. j1 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1735 ', UV(J17,I_FRQ,J1,1)  ! %%%%%%%%%%%%%%
 4170            CONTINUE
            END IF
 4150    CONTINUE
!$OMP    END PARALLEL DO
         ...
         END IF
 410  CONTINUE
!%  write ( 6, * ) 'PIMA_GET_UV-1747 ', UV(2,2,2,1)  ! %%%%%%%%%%%%%%
      IF ( FL_PLD ) THEN
           DEALLOCATE ( PCAL_C8 )
      END IF
!
...
!
      DO 4180 J18=1,L_POL
         IF ( PIM%NSTK == 1 .OR. PIM%NSTK == 4 ) THEN
              IND_POL = J18
              IF ( PIM%NSTK == 4 .AND. POL_MODE == PIMA__PAR ) THEN
                   IND_POL = 4
              END IF
            ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__RRCC .AND. L_POL == 1 ) THEN
              IND_POL = 1
            ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__LLCC .AND. L_POL == 1 ) THEN
              IND_POL = 2
            ELSE
              IND_POL = 1
         END IF
...
! ------------ Compute frequencies
!
               IF ( J18 == 1 .AND. I_FRQ == 1 .AND.  J20 == 1 ) THEN
                    FREQ_REF = PIM%FRQ(PIM%CONF%BEG_FRQ,PIM%CONF%FRQ_GRP)%FREQ
               END IF
               DO 4210 J21=1,LTIM
!!  if ( j20 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1807 i_frq=', int2(i_frq), ' j21= ', int2(J21), ' FLAG= ', PIM%OBS(IND_OBS)%CORR_FLAG(J21,FRG_IND), ' UV= ', UV(J20,I_FRQ,J21,J18), ' we= ', WEI_1D(J21,1:PIM%NSTK)  ! %%%%%%%%%%%%%%
!!  write ( 6, * ) 'PIMA_GET_UV-1808 i_frq=', int2(i_frq), ' j20= ', j20, ' j21= ', int2(J21), ' frg_ind = ', int2(frg_ind), ' nuvs= ', PIM%OBS(IND_OBS)%NUVS, ' sha_CORR_FLAG= ', shape ( PIM%OBS(IND_OBS)%CORR_FLAG ) ; call flush ( 6 ) ! %%%%%%%%%
!
! --------------- 1(7) Correct insane data
!
                  IF ( ABS( REAL ( UV(J20,I_FRQ,J21,J18) ) ) >  VIS_MAX .OR. &
     &                 ABS( IMAG ( UV(J20,I_FRQ,J21,J18) ) ) >  VIS_MAX      ) THEN
                       UV(J20,I_FRQ,J20,J18) = CMPLX ( 0.0, 0.0 )
                  END IF
!
                  IF ( J18 == 1 .AND. I_FRQ == 1 .AND.  J20 == 1 ) THEN
...
!
                       ...
                       END IF
                  END IF
!%  if ( j20 == 222 .and. ifrq == 2 .and. j21 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1815 ', UV(J20,I_FRQ,J21,J18)  ! %%%%%%%%%%%%%%
                  IF ( PIM%BANDPASS_MASK_STYLE .NE. PIMA__NO ) THEN
!
! -------------------- 4(7) Apply fine bandpass mask if available
!
                       UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)* &
...
!
                       IF ( PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(1)) == 4 .AND. &
     &                      PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(2)) == 4      ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/PIMA__DIG_22
                          ELSE IF ( PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(1)) == 4 .AND. &
     &                              PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(2)) == 2       ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/PIMA__DIG_12
                          ELSE IF ( PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(1)) == 2 .AND. &
     &                              PIM%NLEV(PIM%OBS(IND_OBS)%STA_IND(2)) == 4       ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/PIMA__DIG_12
                          ELSE
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/PIMA__DIG_11
                       END IF
                  END IF
!%  if ( j20 == 222 .and. ifrq == 2 .and. j21 == 2 ) write ( 6, * ) 'PIMA_GET_UV-1842 ', UV(J20,I_FRQ,J21,J18)  ! %%%%%%%%%%%%%%
!
...
!
                       IF ( PIM%NPOL == 1 ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/(1.0D0 + WEI_1D(J21,IND_POL)/8.0D0)
                          ELSE IF ( PIM%NPOL .GE. 2 ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/(1.0D0 + WEI_1D(J21,IND_POL)/4.0D0)
                       END IF
...
!
                       IF ( PIM%NPOL == 1 ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/(1.0D0 + 1.0D0/8.0D0)
                          ELSE IF ( PIM%NPOL .GE. 2 ) THEN
                            UV(J20,I_FRQ,J21,J18) = UV(J20,I_FRQ,J21,J18)/(1.0D0 + 1.0D0/4.0D0)
                       END IF
     ...
     &                 L_POL    == 2     .AND.  &
     &                 J18      == L_POL .AND.  &
     &                 POL_MODE == PIMA__PAR    ) THEN
!
                       WEI_1D(J21,2) = WEI_1D(J21,IND_POL)
                  END IF
!
 ...
 260                        FORMAT ( 'PIMA_GET_UV  Ind_obs: ', I6, ' AP_ind: ', I4, &
     ...
     &                                   J18, IND_POL, WEI_1D(J21,IND_POL), UV(J20,I_FRQ,J21,J18)
  270                  FORMAT ( 'PIMA_GET_UV Obs: ', I5, ' I_FRQ= ', I2, ' ICHN= ', I4, &
     &                          ' Ind_tim= ', I4, ' Date: ', A24, ' Poli= ', I1, ' POLI= ', I1, &
     &                          ' Wei: ', F8.5, ' UV= ( ', 1PE11.4, ' , ', 1PE11.4 , ')' )
                  END IF
 4210          CONTINUE
 4200       CONTINUE
 4190    CONTINUE
