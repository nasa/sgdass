...
! *                                                                      *
! * ### 18-NOV-2014   PIMA_GET_OBS  v2.6 (d)  L. Petrov  23-FEB-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           J14, J15, J16, J17, J18, IND_POL, FRG_IND, IND_BND, &
...
!
      ...
      IF ( FRG_IND < 1 ) THEN
           WRITE ( 6, * ) 'PIMA_GET_OBS-145: PIM%OBS(IND_OBS)%REF_FRG_INDS: ', &
     &                     PIM%OBS(IND_OBS)%REF_FRG_INDS
           WRITE ( 6, * ) 'PIMA_GET_OBS-147: PIM%OBS(IND_OBS)%GLO_FRG_INDS: ', &
     &                     PIM%OBS(IND_OBS)%GLO_FRG_INDS
           CALL CLRCH ( STR  )
           CALL CLRCH ( STR1 )
           CALL INCH  ( IND_OBS, STR )
           CALL INCH  ( PIM%CONF%FRQ_GRP, STR1 )
           CALL ERR_LOG ( 9112, IUER, 'PIMA_GET_OBS', 'Error in an '// &
     &         'attempt to read observation '//TRIM(STR)// &
     &         ' of frequency group '//TRIM(STR1)//' -- there are no data '// &
     &         ' for this frequency group' )
           RETURN
      END IF
!
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 9113, IUER, 'PIMA_GET_OBS', 'Error in an '// &
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 9114, IUER, 'PIMA_GET_OBS', 'Error in an attempt '// &
...
!
     ...
     &              'memory for array AC_AVR_TIM' )
      ...
      IF ( FL_AC ) THEN
!
! -------- Last dimension runs over Stokes
!
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL IINCH ( 4, STR )
                CALL ERR_LOG ( 9124, IUER, 'PIMA_GET_OBS', 'Failure to '// &
     &              'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &              'memory for array AC_AVR_TIM' )
                RETURN
           END IF
...
!
           ...
           IF ( IER .NE. 0 ) THEN
                CALL CLRCH ( STR )
                CALL INCH  ( IND_OBS, STR )
                CALL ERR_LOG ( 9125, IUER, 'PIMA_GET_OBS', 'Error '// &
...
!     
     ...
     &                           WEI_1D(1,1,J2), PIM%OBS(IND_OBS)%AP_LEN, IER  )
              IF ( IER .NE. 0 ) THEN
                   ...
                   CALL INCH  ( J2, STR )
                   CALL ERR_PASS ( IUER, IER )
                   CALL ERR_LOG  ( 9128, IUER, 'PIMA_GET_OBS', 'Failure to '// &
...
!     
     ...
     &                                  PIM%OBS(IND_OBS)%AC_MEAN(1,J2,J3), .FALSE., IER  )
                 IF ( IER .NE. 0 ) THEN
                      CALL CLRCH ( STR )
                      CALL INCH  ( IND_OBS, STR )
                      CALL ERR_LOG ( 9129, IUER, 'PIMA_GET_OBS', 'Failure in an '// &
...
!
     ...
     &                                         WEI_1D(1,J3,J2), AC_THR, IER )
                         IF ( IER .NE. 0 ) THEN
                              CALL CLRCH   ( STR ) 
                              CALL INCH    ( IND_OBS, STR )
                              CALL ERR_LOG ( 9130, IUER, 'PIMA_GET_OBS', 'Failure in '// &
...
!
 ...
 290       FORMAT ( 'PIMA_GET_OBS Obs: ', I6, &
     &              ' C_STA: ', A, ' / ',A, &
     &              ' AC_MEAN_1: ', (F10.6,1X), &
     &              ' AC_MEAN_2: ', (F10.6,1X), &
...
!
           CALL ERR_LOG ( 9131, IUER, 'PIMA_GET_OBS', 'Failure to get '// &
...
!
      ...
      PH_ACC_APSO = 0.0D0
!
! --- Allocate and fill array with time epochs
!
      ALLOCATE ( TIM_ARR(LTIM), STAT=IER )
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( 8*LTIM, STR )
           CALL ERR_LOG ( 9132, IUER, 'PIMA_GET_OBS', 'Error in '// &
...
!
     ...
     &                                PIM%OBS(IND_OBS)%UV(1,1,1,J7), IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 9134, IUER, 'PIMA_GET_OBS', 'Failre to perform '// &
...
!$OMP&           DEFAULT  ( NONE ), &
!$OMP&           PRIVATE  ( J8, J9, J10, J11, J12, DRF_IF, WPOI_IF, PHS_MOD, IND_POL ),  &
!$OMP&           SHARED   ( PIM, J7, LFRQ, IND_OBS, WEI_1D, TIM_ARR, LPOL, FL_FRI, FL_AC, &
!$OMP&                      POL_CROSS_IND, POL_MODE ), &
!$OMP&           SCHEDULE ( STATIC )
              DO 480 J8=1,LFRQ
                 DRF_IF = (0.0, 0.0)
                 WPOI_IF =  0.0
     ...
     &                                            (TIM_ARR(J7) - PIM%OBS(IND_OBS)%FRT_OFFSET(1)) 
                    ...
                    DO 4100 J10=1,LPOL
                       IF ( PIM%NSTK == 1 .OR. PIM%NSTK == 4 ) THEN
                            IND_POL = J10
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__RRCC .AND. LPOL == 1 ) THEN
                            IND_POL = 1
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__LLCC .AND. LPOL == 1 ) THEN
                            IND_POL = 2
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__RRCC .AND. LPOL .GE. 2 ) THEN
                            IND_POL = J10
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__LLCC .AND. LPOL .GE. 2 ) THEN
                            IND_POL = J10
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__IPCC .AND. LPOL .GE. 2 ) THEN
                            IND_POL = J10
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__PALL_CIR .AND. LPOL .GE. 2 ) THEN
                            IND_POL = J10
                          ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__PAR  .AND. LPOL .GE. 2 ) THEN
                            IND_POL = J10
                       END IF
!
                       IF ( WEI_1D(J7,IND_POL,0) < PIM%CONF%FRIB_WEIGHTS_THRESHOLD ) THEN
                            WEI_1D(J7,IND_POL,0) = 0.0
                            PIM%OBS(IND_OBS)%UV(J9,J8,J7,POL_CROSS_IND(IND_POL)) = CMPLX ( 0.0, 0.0 )
                       END IF
!
                       DRF_IF(J10)  = DRF_IF(J10)  + &
     &                                   CMPLX(WEI_1D(J7,IND_POL,0), 0.0)*PIM%OBS(IND_OBS)%UV(J9,J8,J7,J10)* &
     &                                   CMPLX ( COS(PHS_MOD), SIN(PHS_MOD) )
                       WPOI_IF(J10) = WPOI_IF(J10) + WEI_1D(J7,IND_POL,0)
!
                       DRF_BAND(J10)  = DRF_BAND(J10) + &
     &                                 CMPLX(WEI_1D(J7,IND_POL,0), 0.0)*PIM%OBS(IND_OBS)%UV(J9,J8,J7,J10)* &
     &                                 CMPLX ( COS(PHS_MOD), SIN(PHS_MOD) )
                       WPOI_BAND(J10) = WPOI_BAND(J10) + WEI_1D(J7,IND_POL,0)
 4100               CONTINUE 
!
...
!$OMP         END PARALLEL DO
 ...
 4130         CONTINUE 
         END IF
         IF ( LPOL == 1 ) THEN
              IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__RRCC ) THEN
                   IND_POL = 1
                ELSE IF ( PIM%NSTK == 2 .AND. POL_MODE == PIMA__LLCC ) THEN
                   IND_POL = 2
                ELSE
                   IND_POL = 1
              END IF
...
!
              PIM%OBS(IND_OBS)%WEI_1D(J7,1)  = WEI_1D(J7,IND_POL,0)
            ELSE
!
