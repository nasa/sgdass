...
! *                                                                      *
! * ### 18-AUG-2010  PIMA_PBP_INIT  v4.0 (d)  L. Petrov  09-DEC-2020 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      ...
      INTEGER*4  IUER
      CHARACTER  POL_ARR(PIMA__POL_MAX)*1, STR*128, STR1*128
      LOGICAL*1  FL_BMASK, FL_PIMAVAR_LINPOL_ROTSWAP
      COMPLEX*8  RES_AVR(4)
      COMPLEX*8, ALLOCATABLE :: RES(:,:,:)
      REAL*4,    ALLOCATABLE :: AC_AVR(:,:,:)
      CHARACTER  POLAR_SAVE*8, POLAR_STYLE*8, SAVE_BPS_NORML*8, POL_CONF*7, &
     &           FINAM_2D_PLOT*128, PIMAVAR_SHORT_BAS*32
     ...
     &           FL_LIN, FL_CIR, FL_PROC(2,PIM__MSTA), &
     &           FL_PIMAVAR_SHORT_BASELINE
...
!
      ...
      END IF
!
      CALL GETENVAR ( 'PIMAVAR_SHORT_BASELINE', STR )
      IF ( ILEN(STR) > 0 ) THEN
           CALL TRAN ( 11, STR, STR )
           IF ( STR(1:1) == 'Y' ) THEN
                FL_PIMAVAR_SHORT_BASELINE = .TRUE.
             ELSE
                FL_PIMAVAR_SHORT_BASELINE = .FALSE.
           END IF
         ELSE
           FL_PIMAVAR_SHORT_BASELINE = .FALSE.
      END IF
...
!
      FL_LIN = .FALSE.
      FL_CIR = .FALSE.
      IF ( LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'H' ) > 0 .OR. &
     &     LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'V' ) > 0 .OR. &
     &     LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'X' ) > 0 .OR. &
     &     LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'Y' ) > 0      ) THEN
           FL_LIN = .TRUE.
      END IF
      IF ( LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'R' ) > 0 .OR. &
           LTM_DIF ( 0, PIMA__POL_MAX, POL_ARR, 'L' ) > 0      ) THEN
           FL_CIR = .TRUE.
      END IF
!
      IF ( FL_LIN ) THEN
           CALL ERR_PASS ( IUER, IER )
           CALL PIMA_PBP_LIN_MOD ( PIM, VTD, POL_ARR, IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 6382, IUER, 'PIMA_PBP_INIT', 'Error in an '// &
     &              'attempt to compute the linear model of the polarization '// &
     &              'bandpass' )
                RETURN 
           END IF
           call exit ( 187 )! %%%%%%%%55
...
!
         ...
         L_PBP = L_PBP + 1
         SNR_NEW = 0.0
         IF ( FL_PIMAVAR_SHORT_BASELINE ) THEN
              BPS%NUM_OBS_ACCUM(J1)  = BPS%NUM_OBS_ACCUM(J1)
         END IF
         DO 420 J2=1,2  !! PIM__MPLR  ! Par Crs Ref Rem
            IF ( FL_LIN                      .AND. &
                 J2 > 2                      .AND. &
     &           BPS%NUM_OBS_ACCUM(J1) == 0        ) THEN
                 GOTO 420
            END IF
!
            IF ( BPS%NUM_OBS_ACCUM(J1) == 0 ) THEN
     ...
     &                                          PIM%C_STA(BPS%IND_STA_REF)//' / '//PIM%C_STA(J1)
            ...
            END IF
!
            DO 430 J3=1,PIM%BPS%NUM_OBS_ACCUM(J1)
!@               IF ( FL_PIMAVAR_SHORT_BASELINE ) THEN
!@                    BPS%IND_OBS_SEL(J3,J1)  = BPS%IND_OBS_SEL(J3,J1) 
!@                    BPS%IND_OBS_POL(J3,J1,J2) = 2
!@               END IF
               IND_OBS = BPS%IND_OBS_SEL(J3,J1)
               IND_POL = J2
...
!
                    ...
                    IF ( IER .NE. 0 ) THEN
                         ...
                         CALL ERR_LOG ( 6383, IUER, 'PIMA_PBP_INIT', 'Error in an '// &
     &                       'attempt to allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
                    ...
                    IF ( IER .NE. 0 ) THEN
                         CALL CLRCH ( STR  )
                         CALL IINCH ( 8*PIM%NCHN*PIM%NFRQ, STR )
                         CALL ERR_LOG ( 6384, IUER, 'PIMA_PBP_INIT', 'Error in an '// &
     &                       'attempt to allocate '//STR(1:I_LEN(STR))//' bytes of '// &
     &                       'dynamic memory for the field CMPL in PIM%PBP' )
                         RETURN
                    END IF
...
!
               ...
               IF ( POLAR_SAVE .NE. PIMA__POLAR_I ) THEN
                    CALL ERR_LOG ( 6385, IUER, 'PIMA_PBP_INIT', 'Trap '// &
     ...
     &                  ' I is supported' )
               ...
               END IF
!
               IF ( POL_CONF == PIMA__PC_CC .OR. ( POL_CONF == PIMA__PC_LC .AND. FL_PIMAVAR_SHORT_BASELINE ) ) THEN
!
! ----------------- Get UV data and put them in PIM%OBS(IND_OBS)%UV, PIM%OBS(IND_OBS)%UV_IF, PIM%OBS(IND_OBS)%UV_BAND
!
                    LPOL = 2
                    CALL ERR_PASS ( IUER, IER )
                    IF ( .NOT. FL_PIMAVAR_SHORT_BASELINE ) THEN
                         CALL PIMA_GET_OBS ( PIM, VTD, IND_OBS, PIMA__LLCC, LPOL, &
     &                                       .TRUE., .TRUE., .FALSE., FRI_STS, IER )
                       ELSE IF ( FL_PIMAVAR_SHORT_BASELINE ) THEN
                         CALL PIMA_GET_OBS ( PIM, VTD, IND_OBS, PIMA__PAR, LPOL, &
     &                                       .TRUE., .TRUE., .FALSE., FRI_STS, IER )
                    END IF
                    IF ( IER .NE. 0 ) THEN
                         CALL ERR_LOG ( 6386, IUER, 'PIMA_PBP_INIT', 'Error in getting '// &
     &                       'visibility data' )
                         RETURN 
                    END IF
...
!
     ...
     &                               TEC, TEC_RATE, TEC_ERR, TEC_RATE_ERR, DECOR_TIM, IER )
                    IF ( IER .NE. 0 ) THEN
                         ...
                         CALL ERR_LOG ( 6387, IUER, 'PIMA_PBP_INIT', 'Failure in '// &
...
!
               IF ( POL_CONF == PIMA__PC_CC ) THEN
                  ...
                  ELSE
                    ...
                    IF ( POL_CONF == PIMA__PC_LC .AND. FL_PIMAVAR_SHORT_BASELINE ) THEN
                         POL_MODE = PIMA__PAR
                    END IF
               END IF
!
     ...
     &                    RES, RES_AVR, SNR_NEW(J2), IER )
               ...
               IF ( IER .NE. 0 ) THEN
                     CALL CLRCH ( STR )
                     CALL INCH  ( IND_OBS, STR )
                     CALL ERR_LOG ( 6388, IUER, 'PIMA_PBP_INIT', 'Error '// &
...
!
     ...
     &                                     WEI_SEG(1,IFRQ), IER )
                  IF ( IER .NE. 0 ) THEN
                       ...
                       CALL CLRCH ( STR1 )
                       CALL INCH  ( J8, STR1 )
                       CALL ERR_LOG ( 6389, IUER, 'PIMA_PBP_INIT', 'Error '// &
...
!
     ...
     &                                  FREQ_SEG, WEI_SEG, IER )
                    IF ( IER .NE. 0 ) THEN
                         CALL CLRCH ( STR )
                         CALL INCH  ( IND_OBS, STR )
                         CALL ERR_LOG ( 6390, IUER, 'PIMA_PBP_INIT', 'Error '// &
...
!
                    ...
                    IF ( SNR_MIN_REF(1) == 0.0 ) THEN
                       ...
                       ELSE
                         IF ( IS_R8_NAN(SNR_NEW(1)) .OR. IS_R8_NAN(SNR_NEW(2)) ) THEN
                              ...
                              CALL ERR_LOG ( 6391, IUER, 'PIMA_PBP_INIT', 'Trap of internal '// &
     &                            'control: SNR is not a number' )
               ...
               IF ( J1 == BPS%IND_STA_REF ) THEN
                    IND_OBS_REF(J2) = BPS%IND_OBS_SEL(J3,J1)
                    TIM_FRT_REF(J2) = BPS%TIME_FRT(J3,J1,J2)
               END IF
               IF ( FL_PIMAVAR_SHORT_BASELINE ) THEN
                    IND_OBS_REF(1) = BPS%IND_OBS_SEL(J3,J1)
                    IND_OBS_REF(2) = BPS%IND_OBS_SEL(J3,J1)
                    TIM_FRT_REF(J2) = BPS%TIME_FRT(J3,J1,J2)
               END IF
!
...
!
      ...
      IF ( POL_CONF .NE. PIMA__PC_CC  ) THEN
           IF ( IND_OBS_REF(1) > 0 .AND. IND_OBS_REF(2) > 0 ) THEN
...
!
 ...
 220                               FORMAT ( 'PIMA_PBP_INIT STA= ', A, ' Ifrq= ', I2, ' Ichn= ', I4, ' Sgn_rem= ', I2, &
     &                                      ' Phs: ', F9.5, ' Cphs: ', F9.5 )
                              END IF
...
!
     ...
     &                                   RES, RES_AVR, SNR_NEW(J10), IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR )
                        CALL INCH  ( IND_OBS, STR )
                        CALL ERR_LOG ( 6392, IUER, 'PIMA_PBP_INIT', 'Error '// &
     &                      'in an attempt to compute residuals for '// &
     &                      'observation '//STR )
                        RETURN
                   END IF
...
!
 ...
 230                     FORMAT ( 'PIMA_PBP_INIT STA= ', A, ' Ifrq= ', I2, ' Ichn= ', I4, ' Sgr= ', I2, &
...
!
      IF ( NUM_PROC_OBS == 0 ) THEN
           CALL ERR_LOG ( 6393, IUER, 'PIMA_PBP_INIT', 'No observations suitable for '// &
...
!
      IF ( .NOT. ASSOCIATED ( PIM%PBP(BPS%IND_STA_REF)%CMPL ) ) THEN
           CALL CLRCH ( STR )
           CALL ERR_LOG ( 6394, IUER, 'PIMA_PBP_INIT', 'Trap of internal control: '// &
