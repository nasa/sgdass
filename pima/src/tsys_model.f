...
! *                                                                      *
! *  ###  18-JAN-1999  TSYS_MODEL  v2.8  (d)  L. Petrov 16-FEB-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INTEGER*4  MAX_SCA
      PARAMETER  ( MAX_SCA = 32*1024 )
      INCLUDE    'lspline.i'
     ...
     &           IV_T_SAVE, MITER, NM_MIN, N1, N2, NUT, NUS, NITER_DEF, &
     &           NITER, IER
...
!
     ...
     &              TRIM(STR)//' -- a real number was expected' )
      ...
      END IF
!
      CALL GETENVAR ( 'PIMAVAR_TSYS_NITER', STR )
      IF ( ILEN(STR) > 0 ) THEN
           READ ( UNIT=STR, FMT='(I3)', IOSTAT=IER ) NITER
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 5132, IUER, 'TSYS_MODEL', 'Error in attempt '// &
     &              'to parse environment variable PIMA_TSYS_NUITER'// &
     &              TRIM(STR)//' -- an  integer number was expected' )
                RETURN
           END IF
         ELSE
           NITER = NITER_DEF
...
!
      ...
      DO 410 J1=1,N
         ...
         IF ( ELEV(J1) > EL_MIN ) THEN
              SIGA(J1) = 1.D0/DSIN(ELEV(J1))
            ELSE 
              SIGA(J1) = 1.D0/DSIN(EL_MIN)
         END IF
         SIGB(J1) = 1.0D0
...
!
      IF ( NM > 0 ) THEN
           ...
           TSYS_AV = MAX ( TSYS__LIM_MIN, TSYS_T0(MAX(1,NM/2)) )
        ELSE 
!
! -------- Handline a pathological case of bad Tsys
!
           TSYS_AV = TSYS__LIM_MIN
      END IF
      CALL NOUT_R8 ( N, TSYS_T0  )
!
      DO 430 J3=1,NITER
...
!
         ...
         DO 470 J7=1,N
            ...
            IF ( TIME(J7) > 1.D-6 ) THEN
                 NUT = NUT + 1
                 LAST_TIME = TIME(J7)
            END IF
            IF ( J7 > 1 .AND. TSYS(J7) < 1.D-6 ) IV_T(J7) = 0
            IF ( IV_T(J7) == 1 ) NUS = NUS + 1
 470     CONTINUE
         IF ( NUS < MINABS_OBS ) THEN
              NM = 0
              NT = 0
              NE = 0
              WRITE ( 6, '(A)' ) 'TSYS_MODEL Cannot continue computation since too few points remained' 
              CALL ERR_LOG ( 0, IUER )
              RETURN 
         END IF
         IF ( IVRB .GE. 4 ) THEN
              WRITE ( 6, * ) 'TSYS_MODEL: NUS = ', NUS 
         END IF
!
...
!
         ...
         IV_T(N) = 1
!
!   write ( 6, * ) 'TTT-3 n= ', n, nut, ' time-nut = ', time(nut)
!   do 570 j7=n-2,n
!      write ( 6, * ) ' j7= ', int2(j7), ' tim = ', sngl(time(j7)), ' iv_t= ', int2(iv_t(j7))
! 570  CONTINUE 
!
         CALL ERR_PASS ( IUER, IER )
...
!
               ...
               IF ( IER .NE. 0 ) THEN
                    WRITE ( 6, * ) ' j3=',j3, ' j9= ', j9, ' tim = ', time(j9), ' l_lsp= ', lspline_t%l_lsp
                    WRITE ( 6, * ) ' epoch= ', lspline_t%epoch(1:lspline_t%l_lsp)
