...
! *                                                                      *
! * ### 11-JAN-2006  PIMA_READ_OBS  v1.28  (d) L. Petrov 22-MAY-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
     ...
     &           J14, J15, I_BPS, I_PBS, IP, IND_REF_STA, &
     &           LUN, LN, SIZE_BYTES, NBND_SAVE, DEB_LEV_SAVE, MAX_NUM_OBS, LEN_MDC, &
     &           NFRG, KPOL, LIND, IND(2,MIND), IND_OBS_BEG, IND_OBS_END, IER
      CHARACTER  STR*128, FILNAM_SAVE(PIM__MFIL)*128, STAGING_DIR_SAVE*128
      CHARACTER  PIMAVAR_SUB_BPS_STR*1024, PIMAVAR_SUB_PBS_STR*1024, DEF_STR*128, &
     &           DEF_RANGE_STR*128
      LOGICAL*4  LEX
      CHARACTER, EXTERNAL :: MJDSEC_TO_DATE*30
      INTEGER*4, EXTERNAL :: ADD_CLIST, ILEN, I_LEN, GET_UNIT, LTM_DIF, MAX_I4
...
!
      ...
      IF ( .NOT. LEX ) THEN
           CALL ERR_LOG ( 7411, IUER, 'PIMA_READ_OBS', 'Input file '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( IER, STR )
           CALL ERR_LOG ( 7412, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7413, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      IF ( STR(1:MAX(1,LN)) .NE. PIMA__FORMAT_LABEL ) THEN
           ...
           CALL ERR_LOG ( 7414, IUER, 'PIMA_READ_OBS', 'The first line '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7415, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &             SIZEOF(PIM%CONF%LAST_FIELD) 
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7416, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7417, IUER, 'PIMA_READ_OBS', 'Checks showed '// &
...
!
      DO 410 J1=1,PIM%NSOU
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7418, IUER, 'PIMA_READ_OBS', 'Error in an '// &
     &         'attempt to read a part CONF of PIMA data '// &
     &         'structure from the input file '//FINAM )
              RETURN
         END IF
...
!
     ...
     &                SIZEOF(PIM%STA(J2)%STATUS)
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7419, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &                   SIZEOF(PIM%STA(J2)%PCAL(J3)%NPOL)
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7420, IUER, 'PIMA_READ_OBS', 'Error in an '// &
     ...
     &               'structure from the input file '//FINAM )
            ...
            IF ( PIMA__VERSION < '20260504' ) THEN
                 PIM%STA(J2)%PCAL(J3)%LPOI_SUBS = 0
            END IF
...
!
     ...
     &                   PIM%STA(J2)%PCAL(J3)%AMPL_SCA, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7426, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                              PIM%STA(J2)%PCAL(J3)%TIME_MID_R8, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7433, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                    'PIMA data structure from the input file '//FINAM )
                 ...
                 END IF
!
                 IF ( PIM%STA(J2)%PCAL(J3)%LPOI_SUBS > 0 ) THEN
!
! ------------------- Allocate dynamic memory for phase calibration substitutes
!
                      ALLOCATE ( PIM%STA(J2)%PCAL(J3)%IND_SUBS(PIM%STA(J2)%PCAL(J3)%NO_TONES,PIM%NFRQ,PIM%STA(J2)%PCAL(J3)%NPOI,PIM%STA(J2)%PCAL(J3)%NPOL), &
     &                           PIM%STA(J2)%PCAL(J3)%PHAS_SUBS(PIM%STA(J2)%PCAL(J3)%LPOI_SUBS), &
     &                           PIM%STA(J2)%PCAL(J3)%AMPL_SUBS(PIM%STA(J2)%PCAL(J3)%LPOI_SUBS) )
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'I4', &
     &                     PIM%STA(J2)%PCAL(J3)%NO_TONES*PIM%NFRQ*PIM%STA(J2)%PCAL(J3)%NPOI*PIM%STA(J2)%PCAL(J3)%NPOL, &
     &                     PIM%STA(J2)%PCAL(J3)%IND_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7436, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%IND_SUBS part of PIMA data '// &
     &                         'structure from the input file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'R4', PIM%STA(J2)%PCAL(J3)%LPOI_SUBS, &
     &                                   PIM%STA(J2)%PCAL(J3)%PHAS_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7437, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%PHAS_SUBS part of PIMA data '// &
     &                         'structure from the input file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'R4', PIM%STA(J2)%PCAL(J3)%LPOI_SUBS, &
     &                                   PIM%STA(J2)%PCAL(J3)%AMPL_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7438, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%AMPL_SUBS part of PIMA data '// &
     &                         'structure from the input file '//FINAM )
                           RETURN
                      END IF
                 END IF
!
                 IF ( PIM%STA(J2)%PCAL(J3)%LSCA_SUBS > 0 ) THEN
!
! ------------------- Allocate dynamic memory for phase calibration substitutes
!
                      ALLOCATE ( PIM%STA(J2)%PCAL(J3)%IND_SCA_SUBS(PIM%STA(J2)%PCAL(J3)%NO_TONES,PIM%NFRQ,PIM%STA(J2)%PCAL(J3)%NSCA,PIM%STA(J2)%PCAL(J3)%NPOL), &
     &                           PIM%STA(J2)%PCAL(J3)%PHAS_SCA_SUBS(PIM%STA(J2)%PCAL(J3)%LSCA_SUBS), &
     &                           PIM%STA(J2)%PCAL(J3)%AMPL_SCA_SUBS(PIM%STA(J2)%PCAL(J3)%LSCA_SUBS) )
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'I4', &
     &                     PIM%STA(J2)%PCAL(J3)%NO_TONES*PIM%NFRQ*PIM%STA(J2)%PCAL(J3)%NSCA*PIM%STA(J2)%PCAL(J3)%NPOL, &
     &                     PIM%STA(J2)%PCAL(J3)%IND_SCA_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7439, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%IND_SCA_SUBS part of '// &
     &                         'PIMA data structure from the input file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'R4', PIM%STA(J2)%PCAL(J3)%LSCA_SUBS, &
     &                                   PIM%STA(J2)%PCAL(J3)%PHAS_SCA_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7440, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%PHAS_SCA_SUBS part of '// &
     &                         'PIMA data structure from the input file '//FINAM )
                           RETURN
                      END IF
!
                      CALL ERR_PASS    ( IUER, IER )
                      CALL RDBIN_ARRAY ( LUN, 'R4', PIM%STA(J2)%PCAL(J3)%LSCA_SUBS, &
     &                                   PIM%STA(J2)%PCAL(J3)%AMPL_SCA_SUBS, LN, IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 7441, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                         'an attempt to read a PCAL(J3)%AMPL_SCA_SUBS part of '// &
     &                         'PIMA data structure from the input file '//FINAM )
                           RETURN
                      END IF
                 END IF
               ELSE
                 ...
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%IND_SUBS )
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%PHAS_SUBS )
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%AMPL_SUBS )
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%IND_SCA_SUBS )
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%PHAS_SCA_SUBS )
                 NULLIFY ( PIM%STA(J2)%PCAL(J3)%AMPL_SCA_SUBS )
            END IF
  430    CONTINUE 
!
         DO 440 J4=1,PIM%NFRG
            SIZE_BYTES = LOC(PIM%STA(J2)%TSYS(J4)%NTYP) - LOC(PIM%STA(J2)%TSYS(J4)) + &
     &                SIZEOF(PIM%STA(J2)%TSYS(J4)%NTYP)
            CALL ERR_PASS ( IUER, IER )
            CALL RDBIN_ARRAY ( LUN, 'B1', SIZE_BYTES, PIM%STA(J2)%TSYS(J4), LN, IER )
            IF ( IER .NE. 0 ) THEN
                 CALL CLRCH ( STR )
                 CALL INCH  ( J4, STR )
                 CALL ERR_LOG ( 7442, IUER, 'PIMA_READ_OBS', 'Error in an '// &
     &               'attempt to read a TSYS part of PIMA data '// &
     &               'structure from the input file '//TRIM(FINAM)//' J4= '//STR )
                 RETURN
            END IF
...
!
     ...
     &                              PIM%STA(J2)%TSYS(J4)%AZ_R4, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7446, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                   SIZEOF(PIM%STA(J2)%STMO(J4)%STATUS)
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7449, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &                              PIM%STA(J2)%STMO(J4)%TAT, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7455, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                              PIM%STA(J2)%STMO(J4)%TSYS_CLN, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7458, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                              PIM%STA(J2)%STMO(J4)%TSYS_MOD, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7459, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
            IF ( PIM%STA(J2)%STMO(J4)%TSRAT_AVAIL ) THEN
                 ...
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7460, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                              PIM%STA(J2)%STMO(J4)%TTOA, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7461, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                   SIZEOF(PIM%STA(J2)%GAIN(J4)%NTAB)
            ...
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7462, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &                              PIM%STA(J2)%GAIN(J4)%TYP, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7463, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                               PIM%STA(J2)%GAIN(J4)%GAIN, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7469, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                    'structure from the input file '//FINAM )
            ...
            END IF
!
            SIZE_BYTES = LOC(PIM%STA(J2)%CABLE(J4)%CABLE_SIGN) - LOC(PIM%STA(J2)%CABLE(J4)) + &
     &                       SIZEOF(PIM%STA(J2)%CABLE(J4)%CABLE_SIGN)
            CALL ERR_PASS ( IUER, IER )
            CALL RDBIN_ARRAY ( LUN, 'B1', SIZE_BYTES, PIM%STA(J2)%CABLE(J4), LN, IER )
            IF ( IER .NE. 0 ) THEN
                 CALL ERR_LOG ( 7471, IUER, 'PIMA_READ_OBS', 'Error in an '// &
     &               'attempt to read a CABLE part of PIMA data '// &
     &               'structure from the input file '//FINAM )
                 RETURN
            END IF
            PIM%STA(J2)%CABLE(J4)%CAB_AVAIL  = SANITIZE_LOGVAR_L4 ( PIM%STA(J2)%CABLE(J4)%CAB_AVAIL )
            IF ( PIM%STA(J2)%CABLE(J4)%CAB_AVAIL ) THEN
!
! -------------- Read information related to cable calibration
!
                 ALLOCATE ( PIM%STA(J2)%CABLE(J4)%TIM_CAB(PIM%STA(J2)%CABLE(J4)%NPOI) )
                 ALLOCATE ( PIM%STA(J2)%CABLE(J4)%CAB_DEL(PIM%STA(J2)%CABLE(J4)%NPOI) )
!
                 CALL ERR_PASS    ( IUER, IER )
                 CALL RDBIN_ARRAY ( LUN, 'R8', PIM%STA(J2)%CABLE(J4)%NPOI, &
     &                                 PIM%STA(J2)%CABLE(J4)%TIM_CAB, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7472, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                    'an attempt to read a PIM%STA(J2)%CABLE(J4)%TIM_CAB '// &
...
!
                 CALL ERR_PASS    ( IUER, IER )
                 CALL RDBIN_ARRAY ( LUN, 'R8', PIM%STA(J2)%CABLE(J4)%NPOI, &
     &                                 PIM%STA(J2)%CABLE(J4)%CAB_DEL, LN, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 7473, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                    'an attempt to read a PIM%STA(J2)%CABLE(J4)%CAB_DEL '// &
     &                    'part of PIMA data structure from the input '// &
     &                    'file '//FINAM )
                      RETURN
                 END IF
               ELSE
                 NULLIFY ( PIM%STA(J2)%CABLE(J4)%TIM_CAB )
                 NULLIFY ( PIM%STA(J2)%CABLE(J4)%CAB_DEL )
            END IF
 440     CONTINUE 
!
     ...
     &                SIZEOF(PIM%STA(J2)%WEATHER%NPOI)
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7474, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   ...
                   CALL ERR_LOG ( 7485, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_MDC, STR )
                   CALL ERR_LOG ( 7493, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_MDC, STR )
                   CALL ERR_LOG ( 7494, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_MDC, STR )
                   CALL ERR_LOG ( 7495, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
     ...
     &                           PIM%STA(J2)%MDC%CLOCK_RATE, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7499, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%MDC%ATMO_DELAY, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7500, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%MDC%ATMO_RATE, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7501, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%MDC%GDELAY, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7502, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%MDC%GRATE, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7503, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_WVR, STR )
                   CALL ERR_LOG ( 7505, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_WVR, STR )
                   CALL ERR_LOG ( 7506, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL IINCH ( 8*PIM%STA(J2)%L_WVR, STR )
                   CALL ERR_LOG ( 7508, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &                  'allocate '//STR(1:I_LEN(STR))//' bytes of '// &
...
!
     ...
     &                           PIM%STA(J2)%WVR%TIM_ARR, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7509, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%WVR%DEL_ARR, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7510, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%WVR%DEL_ERR, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7511, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'an attempt to read a WVR%DEL_ARR part of PIMA '// &
     &                 'data structure from the input file '//FINAM )
                   RETURN
              END IF
!
     ...
     &                           PIM%STA(J2)%WVR%EL_ARR, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7512, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                           PIM%STA(J2)%WVR%AZ_ARR, LN, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7513, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
              ...
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7514, IUER, 'PIMA_READ_OBS', 'Error in '// &
...
!
     ...
     &                SIZEOF(PIM%SCA(J7)%SCAN_NAME)
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7516, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
         IF ( ALLOCATED ( PIM%SCA(J7)%AUT_IND ) ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( J7, STR )
              CALL ERR_LOG ( 7517, IUER, 'PIMA_READ_OBS', 'Attempt to '// &
     ...
     &            'already been allocated ' )
         ...
         IF ( ALLOCATED ( PIM%SCA(J7)%OBS_IND ) ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( J7, STR )
              CALL ERR_LOG ( 7518, IUER, 'PIMA_READ_OBS', 'Attempt to '// &
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7519, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
     ...
     &                      LN, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7522, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 7523, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &         'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
...
!
     ...
     &                SIZEOF(PIM%FILE(J8)%STATUS)
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7524, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &                     PIM%FILE(J8)%L_HDR, STR )
              CALL ERR_LOG ( 7525, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &            'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
     &            'memory for keys' )
              RETURN
         END IF
!
     ...
     &                LEN(PIM%FILE(J8)%KEY(1,1))
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7526, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 7527, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &         'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 7529, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 7530, IUER, 'PIMA_READ_OBS', 'Failure to '// &
     &         'allocate '//STR(1:I_LEN(STR))//' bytes of dynamic memory' )
           RETURN
      END IF
!
     ...
     &                SIZEOF(PIM%OBS(J9)%REF_FRG_INDS(PIM__MUVS))
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7531, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &             STAT=IER )
         IF ( IER .NE. 0 ) THEN
              ...
              CALL ERR_LOG ( 7532, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
     ...
     &                      PIM%OBS(J9)%UV_IND, LN, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7533, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              CALL CLRCH ( STR )
              CALL IINCH ( PIM%NOBS*SIZEOF(PIM%OBS(1)), STR )
              CALL ERR_LOG ( 7534, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
     ...
     &                      PIM%OBS(J9)%CORR_FLAG, LN, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7535, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
         ...
         IF ( IER .NE. 0 ) THEN
              ...
              CALL ERR_LOG ( 7536, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
     ...
     &                SIZEOF(PIM%AUT(J10)%REF_FRG_INDS(PIM__MUVS))
         ...
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7537, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
     ...
     &              STAT=IER )
         IF ( IER .NE. 0 ) THEN
              CALL CLRCH ( STR )
              CALL IINCH ( PIM%NOBS*SIZEOF(PIM%OBS(1)), STR )
              CALL ERR_LOG ( 7538, IUER, 'PIMA_READ_OBS', 'Failure to '// &
...
!
     ...
     &                      PIM%AUT(J10)%UV_IND, LN, IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7539, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           ...
           CALL ERR_LOG ( 7540, IUER, 'PIMA_READ_OBS', 'Failure to allocate '// &
...
!
     ...
     &                   PIM%FREQ_ARR, LN, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7541, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IND_REF_STA .LE. 0 ) THEN
           CALL ERR_LOG ( 7542, IUER, 'PIMA_READ_OBS', 'Cannot find '// &
...
!
     ...
     &                     IER )
         IF ( IER .NE. 0 ) THEN
              CALL ERR_LOG ( 7543, IUER, 'PIMA_READ_OBS', 'Error in an '// &
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 7544, IUER, 'PIMA_READ_OBS', 'Error in an '// &
     &         'attempt to close input file '//FINAM )
           RETURN
      END IF
!
! --- Process baandpass and polarization bandpasses substitutions
!
      CALL GETENVAR ( 'PIMAVAR_SUB_BPS', PIMAVAR_SUB_BPS_STR )
      CALL GETENVAR ( 'PIMAVAR_SUB_PBS', PIMAVAR_SUB_PBS_STR )
      PIM%L_BPS_SUB = 0
      PIM%L_PBS_SUB = 0
      PIM%IND_BPS_SUB = 0
      PIM%IND_PBS_SUB = 0
!
      IF ( ILEN(PIMAVAR_SUB_BPS_STR) > 0 ) THEN
!
! -------- Split the definition line into words separated by semicolon
!
           CALL EXWORD ( PIMAVAR_SUB_BPS_STR, MIND, LIND, IND, ';', IER )
!
! -------- Cycle over bandpass substitution expressions
!
           DO 4120 J12=1,LIND
!
! ----------- Extract the bandpass substitution expressions for the J12-th substitute
!
              DEF_STR = PIMAVAR_SUB_BPS_STR(IND(1,J12):IND(2,J12))
!
! ----------- Split the expression into the index range and the bandpass file name
!
              IP = INDEX ( DEF_STR, ':' )
              IF ( IP .LE. 1  .OR. IP .GE. LEN(DEF_STR) ) THEN
                   CALL ERR_LOG ( 7545, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing value of the environment variable '// &
     &                 'PIMAVAR_SUB_BPS: no colon delimiter was found '// &
     &                 'in procerssing definition '//DEF_STR )
                   RETURN
              END IF
!
! ----------- Check whether the bandpass file exists
!
              INQUIRE ( FILE=DEF_STR(IP+1:), EXIST=LEX )
              IF ( .NOT. LEX ) THEN
                   CALL ERR_LOG ( 7546, IUER, 'PIMA_READ_OBS', 'Bandpass '// &
     &                 'substitute file '//TRIM(DEF_STR)//' defined in '// &
     &                 'keyword PIMAVAR_SUB_BPS '//' does not exist' )
                   RETURN
              END IF
!
! ----------- Add the bandpass file to the list
!
              CALL ERR_PASS ( IUER, IER ) 
              I_BPS = ADD_CLIST ( PIM__MBPS, PIM%L_BPS_SUB, PIM%BPS_SUB_NAME, &
     &                            DEF_STR(IP+1:), IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7547, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing PIMAVAR_SUB_BPS: variable -- cannot append '// &
     &                 ' the bandpass file to the list. Too many substitutes.' )
                   RETURN
              END IF
!
! ----------- Extrate obsevatin index range
!
              DEF_RANGE_STR = DEF_STR(1:IP-1)
              IP = INDEX ( DEF_STR, '-' )
              IF ( IP .LE. 1  .OR. IP .GE. LEN(DEF_RANGE_STR) ) THEN
                   CALL ERR_LOG ( 7548, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing value of the environment variable '// &
     &                 'PIMAVAR_SUB_BPS: no hyphen delimiter was found '// &
     &                 'in procerssing definition '//DEF_STR )
                   RETURN
              END IF
!
! ----------- Get the start index of range
!
              CALL CHIN ( DEF_RANGE_STR(1:IP-1), IND_OBS_BEG )
              IF ( IND_OBS_BEG < 1 ) THEN
                   CALL ERR_LOG ( 7549, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the first observation index of the '// &
     &                 'environment variable PIMAVAR_SUB_BPS: in the range '// &
     &                  DEF_RANGE_STR(1:IP-1)//' -- should be an interger > 0' )
                   RETURN
              END IF
!
! ----------- Get the stop index of range
!
              CALL CHIN ( DEF_RANGE_STR(IP+1:),  IND_OBS_END )
!
! ----------- Special meaning of -1: the last observation index
!
              IF ( IND_OBS_END == -1 ) IND_OBS_END = PIM%NOBS
              IF ( IND_OBS_END <  -1 ) THEN
                   CALL ERR_LOG ( 7550, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the last observation index in the range '// &
     &                  TRIM(DEF_RANGE_STR(IP+1:))//' -- should be an '// &
     &                  'interger > 0, but less than the total number '// &
     &                  'of observations' )
                   RETURN
              END IF
!
...
!
              IF ( IND_OBS_END > PIM%NOBS ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( PIM%NOBS, STR )
                   CALL ERR_LOG ( 7551, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the last observation index in the range '// &
     &                  TRIM(DEF_RANGE_STR(IP+1:))//' -- should be an '// &
     &                  ' but less than the total number of observations '// &
     &                  STR )
                   RETURN
              END IF
              IF ( IND_OBS_BEG > IND_OBS_END ) THEN
                   CALL ERR_LOG ( 7552, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the observation indices of the '// &
     &                 'environment variable PIMAVAR_SUB_BPS '// &
     &                  TRIM(DEF_RANGE_STR)//' -- the last index should be '// &
     &                  'equal to the first index' ) 
                   RETURN
              END IF
! 
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 5 ) THEN
                   WRITE ( 6, * ) 'PIMA_READ_OBS I_BPS= ', INT2(I_BPS), &
     &                            ' File_name: '//TRIM(PIM%BPS_SUB_NAME(I_BPS)), &
     &                            ' INDS= ', IND_OBS_BEG, IND_OBS_END
              END IF
!
! ----------- Set the bandpass subsitutes for the requested
! ----------- observation index range
!
              DO 4130 J13=IND_OBS_BEG,IND_OBS_END
                 PIM%IND_BPS_SUB(J13) = I_BPS
 4130         CONTINUE 
 4120      CONTINUE 
      END IF
!
      IF ( ILEN(PIMAVAR_SUB_PBS_STR) > 0 ) THEN
!
! -------- Split the definition line into words separated by semicolon
!
           CALL EXWORD ( PIMAVAR_SUB_PBS_STR, MIND, LIND, IND, ';', IER )
!
! -------- Cycle over bandpass substitution expressions
!
           DO 4140 J14=1,LIND
!
! ----------- Extract the polarization bandpass substitution expressions 
! ----------- for the J14-th substitute
!
              DEF_STR = PIMAVAR_SUB_PBS_STR(IND(1,J14):IND(2,J14))
!
! ----------- Split the expression into the index range and the bandpass file name
!
              IP = INDEX ( DEF_STR, ':' )
              IF ( IP .LE. 1  .OR. IP .GE. LEN(DEF_STR) ) THEN
                   CALL ERR_LOG ( 7553, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing value of the environment variable '// &
     &                 'PIMAVAR_SUB_PBS: no colon delimiter was found '// &
     &                 'in procerssing definition '//DEF_STR )
                   RETURN
              END IF
!
! ----------- Check whether the polarizaition bandpass file exists
!
              INQUIRE ( FILE=DEF_STR(IP+1:), EXIST=LEX )
              IF ( .NOT. LEX ) THEN
                   CALL ERR_LOG ( 7554, IUER, 'PIMA_READ_OBS', 'Bandpass '// &
     &                 'substitute file '//TRIM(DEF_STR)//' defined in '// &
     &                 'keyword PIMAVAR_SUB_PBS '//' does not exist' )
                   RETURN
              END IF
!
! ----------- Add the bandpass file to the list
!
              CALL ERR_PASS ( IUER, IER ) 
              I_PBS = ADD_CLIST ( PIM__MBPS, PIM%L_PBS_SUB, PIM%PBS_SUB_NAME, &
     &                            DEF_STR(IP+1:), IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 7555, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing PIMAVAR_SUB_PBS: variable -- cannot append '// &
     &                 ' the bandpass file to the list. Too many substitutes.' )
                   RETURN
              END IF
!
...
!
              ...
              IP = INDEX ( DEF_STR, '-' )
              IF ( IP .LE. 1  .OR. IP .GE. LEN(DEF_RANGE_STR) ) THEN
                   CALL ERR_LOG ( 7556, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing value of the environment variable '// &
     &                 'PIMAVAR_SUB_PBS: no hyphen delimiter was found '// &
     &                 'in procerssing definition '//DEF_STR )
                   RETURN
              END IF
!
...
!
              CALL CHIN ( DEF_RANGE_STR(1:IP-1), IND_OBS_BEG )
              IF ( IND_OBS_BEG < 1 ) THEN
                   CALL ERR_LOG ( 7557, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the first observation index of the '// &
     &                 'environment variable PIMAVAR_SUB_PBS: in the range '// &
     &                  DEF_RANGE_STR(1:IP-1)//' -- should be an interger > 0' )
                   RETURN
              END IF
!
...
!
              IF ( IND_OBS_END == -1 ) IND_OBS_END = PIM%NOBS
              IF ( IND_OBS_END <  -1 ) THEN
                   CALL ERR_LOG ( 7558, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the last observation index in the range '// &
     &                  TRIM(DEF_RANGE_STR(IP+1:))//' -- should be an '// &
     &                  'interger > 0, but less than the total number '// &
     &                  'of observations' )
...
!
              IF ( IND_OBS_END > PIM%NOBS ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( PIM%NOBS, STR )
                   CALL ERR_LOG ( 7559, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the last observation index in the range '// &
     &                  TRIM(DEF_RANGE_STR(IP+1:))//' -- should be an '// &
     &                  ' but less than the total number of observations '// &
     &                  STR )
              ...
              END IF
              IF ( IND_OBS_BEG > IND_OBS_END ) THEN
                   CALL ERR_LOG ( 7560, IUER, 'PIMA_READ_OBS', 'Error in '// &
     &                 'processing the observation indices of the '// &
     &                 'environment variable PIMAVAR_SUB_PBS '// &
     &                  TRIM(DEF_RANGE_STR)//' -- the last index should be '// &
     &                 'equal to the first index' ) 
                   RETURN
              END IF
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 5 ) THEN
                   WRITE ( 6, * ) 'PIMA_READ_OBS I_PBS= ', INT2(I_PBS), &
     &                            ' File_name: '//TRIM(PIM%PBS_SUB_NAME(I_PBS)), &
     &                            ' INDS= ', IND_OBS_BEG, IND_OBS_END
              END IF
!
! ----------- Set the bandpass subsitutes for the requested
! ----------- observation index range
!
              DO 4150 J15=IND_OBS_BEG,IND_OBS_END
                 PIM%IND_PBS_SUB(J15) = I_PBS
 4150         CONTINUE 
 4140      CONTINUE 
      END IF
!
