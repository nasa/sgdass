...
! *                         IDEV == 4 -- the plot will be in png format. *
...
! *                                                                      *
! *  ### 26-FEB-2004  GEN_MAPPLOT   v4.0 (d) L. Petrov  18-MAR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE    'diagi.i'
...
!!      PARAMETER  ( MC = 64 ) 
      ...
      REAL*4     XP, YP, X_MAR, Y_MAR, X_BL, X_TR, Y_BL, Y_TR, TR(6), TICK_STEP, &
     &           X2(2), Y2(2), XMIN, XMAX, YMIN, YMAX, XL, XR, YB, YT, VAL_R4, &
     &           IMA_WIN_EPS
      REAL*8     XC_BEAM, YC_BEAM, ARG, AX_FACTOR, BD_FACTOR, &
     &           BD_FACTOR_DEF, BEAM_SIZE, DX
      PARAMETER  ( BD_FACTOR_DEF = 5.0 )
      CHARACTER  CH*1, STR*256, COM_STR*256, BAND*1
      INTEGER*4  ID_XW, ISG, J1, J2, J3, J4, J5, J6, J7, J8, J9, J10, J11, &
     &           J12, J13, IP, IL, NC, GREY_COL, NLAB, COL_SHIFT, &
     &           ICLR_BAND, QUAL_CODE, NCOL, INIT_COL, IPAL, LUN, &
     &           IPX_C, IPY_C, IP_XMIN, IP_XMAX, IP_YMIN, IP_YMAX, IPB, IER
      CHARACTER  OUT*4096, MAP_UNIT_STR*6, MAP_LOG_STR*4
      REAL*4,    ALLOCATABLE :: IMAGE(:,:)
      INTEGER*4, ALLOCATABLE :: IMAGE_I4(:,:)
      REAL*4      PI, PI2, P2I, RAD_TO_MAS, RAD_TO_DEG
      PARAMETER ( PI=3.1415926, PI2=2.0*PI, P2I=PI/2.0 ) ! Pi number
      PARAMETER ( RAD_TO_MAS = 180.0*3600.0*1000.0/PI )
      PARAMETER ( RAD_TO_DEG =       180.0/PI )
...
!
      ...
      END IF
      CALL GETENVAR ( 'FITS_MAP_UNITS', MAP_UNIT_STR )
      IF ( ILEN(MAP_UNIT_STR) == 0 ) MAP_UNIT_STR = 'mas'
      MAP_LOG_STR = 'NONE'
      CALL GETENVAR ( 'FITS_MAP_LOG',   MAP_LOG_STR )
      CALL TRAN ( 11, MAP_LOG_STR, MAP_LOG_STR )
      IPAL = 7
      CALL GETENVAR ( 'FITS_MAP_PAL', STR  )
      IF ( ILEN(STR) > 0 ) CALL CHIN ( STR, IPAL ) 
      CALL GETENVAR ( 'FITS_MAP_FREQ', STR  )
      IF ( ILEN(STR) > 0 ) THEN
           READ ( UNIT=STR, FMT='(D12.5)' ) MAP%FREQ
      END IF
      IF ( ICLR == 41 ) IPAL = 12
      IF ( ICLR == 42 ) IPAL = 13
      IF ( ICLR == 43 ) IPAL = 14
      ...
      FT = 0.0
      FM = -100.0
      CALL GETENVAR ( 'FITS_MAP_CENTER', STR  )
      IF ( ILEN(STR) > 0 ) THEN
           IP = INDEX ( STR, ',' )
           IF ( IP < 1 ) THEN
                CALL ERR_LOG ( 6269, IUER, 'GEN_MAPPLOT', 'Wrong '// &
     &              'format of FITS_MAP_CENTER environment variable: '// &
     &              'it does not contain comma' )
                RETURN
           END IF
           CALL CHIN ( STR(1:IP-1), IPX_C )
           CALL CHIN ( STR(IP+1:),  IPY_C )
        ELSE
           IPX_C = MAP%DIM1/2
           IPY_C = MAP%DIM2/2
      END IF
...
!
 ...
 410       CONTINUE 
         ...
         ELSE 
           AX_LEN = ABS(RAD_TO_MAS*MAP%STEP_RA*MAP%DIM2/2)
      END IF
      TR(1) = -RAD_TO_MAS*MAP%STEP_RA*IPX_C
      TR(2) =  RAD_TO_MAS*MAP%STEP_RA
      TR(3) =  0.0
      TR(4) = -RAD_TO_MAS*MAP%STEP_DL*IPY_C
      TR(5) =  0.0
      TR(6) =  RAD_TO_MAS*MAP%STEP_DL
!
      IF ( MAP_UNIT_STR == 'arcsec' ) THEN
           AX_LEN = AX_LEN/1000.0
      END IF
!
! --- Deterimine the window world coordinates
!
      X_BL =  AX_LEN
      X_TR = -AX_LEN
      IF ( FL_RA_POS ) THEN
...
!
      ...
      END IF
      IF ( ISIZE == 0 ) THEN
           CALL PGSVP  (  0.30, 0.90,  0.40, 0.90 )
         ELSE IF ( ISIZE == 1 ) THEN
           CALL PGSVP  (  0.25, 0.90,  0.25, 0.90 )
         ELSE IF ( ISIZE == 2 ) THEN
           CALL PGSVP  (  0.20, 0.90,  0.20, 0.90 )
         ELSE IF ( ISIZE == 3 ) THEN
           IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
                CALL PGSVP  (  0.14, 0.88,  0.14, 0.94 )
              ELSE
                CALL PGSVP  (  0.14, 0.94,  0.14, 0.94 )
           END IF
         ELSE IF ( ISIZE == 4 ) THEN
           CALL PGSVP  (  0.12, 0.80,  0.24, 0.92 )
         ELSE IF ( ISIZE == 5 ) THEN
           CALL PGSVP  (  0.18, 0.88,  0.18, 0.88 )
      END IF
      IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
           IF ( ISIZE == 3 ) THEN
                XMIN = 0.12 ; XMAX = 0.70; YMIN = 0.24 ; YMAX = 0.92
              ELSE
                XMIN = 0.12 ; XMAX = 0.80; YMIN = 0.24 ; YMAX = 0.92
           END IF
      END IF
!
...
!
      CALL GETENVAR ( 'FITS_MAP_NAME', STR ) 
      IF ( ILEN(STR) == 0 ) THEN
...
!
      IF ( MAP%FREQ < 1.0D9  ) THEN
           WRITE ( UNIT=STR, FMT='("Freq: ",F4.2," GHz")' ) 1.D-9*MAP%FREQ
        ELSE IF ( MAP%FREQ < 1.0D11  ) THEN
           WRITE ( UNIT=STR, FMT='("Freq: ",F4.1," GHz")' ) 1.D-9*MAP%FREQ
        ELSE
           WRITE ( UNIT=STR, FMT='("Freq: ",F4.0," GHz")' ) 1.D-9*MAP%FREQ
      END IF
...
!
      ...
      IER = 0
      IF ( MAP_UNIT_STR == 'arcsec' ) THEN
           DX = (IPX_C - MAP%DIM1/2)*MAP%STEP_RA
           CALL RG_TAT ( MAP%DELTA*DCOS(DX) + (IPY_C - MAP%DIM2/2 -2)*MAP%STEP_DL*DCOS(DX), 3, STR, IER )
!
           DX = (-MAP%DIM1/2)*MAP%STEP_RA
           CALL RG_TAT ( MAP%DELTA*DCOS(DX), 0, STR, IER )
        ELSE
      ...
      IF ( IER == 0 ) THEN
           ...
           IF ( ISIZE == 0 .OR. ISIZE == 1 ) THEN
              ...
              ELSE 
                X_MAR = X_BL*1.18
                IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
                     STR = 'Decl. relative to '//STR
                     X_MAR = X_BL*1.22
                   ELSE
                     STR = 'Declination relative to '//STR
                END IF
                IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
                     CALL PGSLW   ( 2 )
                     CALL PGSCH   ( 1.4 )
           ...
           END IF
         ELSE 
           STR = 'Relative declination'//' ('//TRIM(MAP_UNIT_STR)//')'
           X_MAR = X_BL*1.18
      END IF
      CALL PGPTXT  ( X_MAR, (Y_TR+Y_BL)/2.0, 90.0, 0.61, STR(1:I_LEN(STR)) )
      CALL PGPTXT  ( X_MAR, Y_TR, 90.0, 1.00, '('//TRIM(MAP_UNIT_STR)//')' )
!
      IER = 0
      IF ( MAP_UNIT_STR == 'arcsec' ) THEN
           CALL RH_TAT ( MAP%ALPHA - (IPX_C - MAP%DIM1/2)*MAP%STEP_DL/DCOS(MAP%DELTA), 0, STR, IER )
         ELSE
      ...
      IF ( IER == 0 ) THEN
           ...
           IF ( ISIZE == 0 .OR. ISIZE == 1 ) THEN
              ...
              ELSE
                IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
                     STR = 'Right asc. relative to '//TRIM(STR)
                   ELSE
                     STR = 'Right ascension relative to '//TRIM(STR)
                END IF
                IF ( ICLR .GE. 40 .AND. ICLR .LE. 49 ) THEN
                     CALL PGSLW   ( 2 )
                  ELSE IF ( ISIZE == 5 ) THEN
      ...
      IF ( ISIZE == 0 ) THEN
         ...
         ELSE IF ( ISIZE == 5 ) THEN
           ...
           Y_LEVEL_FACTOR = 1.32
      END IF
      CALL PGPTXT  ( (X_TR+X_BL)/2.0, Y_RA_FACTOR*Y_BL, 0.0, 0.58, &
     &               STR(1:I_LEN(STR)) )
      CALL PGPTXT  ( X_TR, Y_RA_FACTOR*Y_BL, 0.0, 0.5, '('//TRIM(MAP_UNIT_STR)//')' )
!
! --- Search for minimum and maximum among the viewable part of the image
!
      RVAL_MIN =  1.0E12
      RVAL_MAX = -1.0E12
      DO 430 J3=1,MAP%DIM1
         DO 440 J4=1,MAP%DIM2 
            IF ( IS_R4_NAN ( MAP%IMAGE(J3,J4) ) ) THEN
                 MAP%IMAGE(J3,J4) = 0.0
            END IF
            IF ( ( ABS(J3 - IPX_C) < -IMA_WIN/(RAD_TO_MAS*MAP%STEP_RA) .AND. &
     &             ABS(J4 - IPY_C) <  IMA_WIN/(RAD_TO_MAS*MAP%STEP_DL)       ) .OR. &
     &           IMA_WIN < IMA_WIN_EPS                                              ) THEN
!
                 IF ( MAP%IMAGE(J3,J4) > RVAL_MAX ) THEN
                      RVAL_MAX = MAP%IMAGE(J3,J4) 
                 END IF
                 IF ( MAP%IMAGE(J3,J4) < RVAL_MIN ) THEN
                      RVAL_MIN = MAP%IMAGE(J3,J4) 
                 END IF
            END IF
 440     CONTINUE 
 430  CONTINUE 
      MAP%FLUX_MAX = RVAL_MAX
!
      ...
      IF ( ISIZE == 0 ) THEN
         ...
         ELSE 
           IF ( ISIZE == 4 .OR. ISIZE == 5 ) THEN
                CALL PGSLW   ( 3 )
           END IF
           IF ( ICLR .GE. 40 .AND. ICLR .LE. 44 ) THEN
                CALL CHASHL ( STR(12:) )
                STR = 'rms= '//STR(12:)
                IF ( ISIZE .LE. 3 ) THEN
                     CALL PGPTXT  ( X_TR, Y_BEAM_FACTOR*Y_BL, 0.0, 0.70, STR(1:I_LEN(STR)) )
                   ELSE IF ( ISIZE .EQ. 4 ) THEN
                     CALL PGPTXT  ( X_TR, Y_BEAM_FACTOR*Y_BL, 0.0, 0.55, STR(1:I_LEN(STR)) )
                   ELSE IF ( ISIZE .EQ. 5 ) THEN
                     CALL PGPTXT  ( X_TR, Y_BEAM_FACTOR*Y_BL, 0.0, 0.69, STR(1:I_LEN(STR)) )
                END IF 
              ELSE
...
!
      DO 450 J5=2,MC
         NC = NC + 1
         LEV_CONT(J5) = 2.0D0*LEV_CONT(J5-1)
         IF ( LEV_CONT(J5) > MAP%FLUX_MAX ) THEN
!
...
!
         IL = ILEN(STR)
         IF ( ISIZE == 0 .AND. J5 > 5 ) GOTO 450
!         
         IF ( LEV_CONT(J5) < 0.001 ) THEN
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", F3.1)' ) 1.E3*LEV_CONT(J5)
            ELSE IF ( LEV_CONT(J5) < 0.01 ) THEN
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", I1)' ) NINT(1.E3*LEV_CONT(J5))
            ELSE IF ( LEV_CONT(J5) < 0.1 ) THEN
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", I2)' ) NINT(1.E3*LEV_CONT(J5))
            ELSE IF ( LEV_CONT(J5) < 1.0 ) THEN
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", I3)' ) NINT(1.E3*LEV_CONT(J5))
            ELSE IF ( LEV_CONT(J5) < 10.0 ) THEN
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", I4)' ) NINT(1.E3*LEV_CONT(J5))
            ELSE 
              WRITE ( UNIT=STR(IL+1:), FMT='(", ", I5)' ) NINT(1.E3*LEV_CONT(J5))
         END IF
 450  CONTINUE 
 850  CONTINUE 
      IL = ILEN(STR)
...
!
      ALLOCATE ( IMAGE(MAP%DIM1,MAP%DIM2), IMAGE_I4(MAP%DIM1,MAP%DIM2), STAT=IER )
      IF ( IBEAM > 0 ) THEN
!
...
!
           DO 460 J6=1,MAP%DIM1
              XC = MAP%STEP_RA*(J6 - MAP%DIM1/2)
              DO 470 J7=1,MAP%DIM2 
                 YC = MAP%STEP_DL*(J7 - MAP%DIM2/2)
                 DO 480 J8=1,MAP%NUM_CC
!
...
! ---------------------- initial image
!
                         ARG = -( (XC - MAP%COOR_CC(1,J8))*COS(-MAP%BEAM_POS_ANG) &
     &                         -(  YC - MAP%COOR_CC(2,J8))*SIN(-MAP%BEAM_POS_ANG) )**2/ &
     &                             (MAP%BEAM_MIN/2.0D0)**2 &
     &                         -( (XC - MAP%COOR_CC(1,J8))*SIN(-MAP%BEAM_POS_ANG) &
     &                           +(YC - MAP%COOR_CC(2,J8))*COS(-MAP%BEAM_POS_ANG) )**2/ &
     &                             (MAP%BEAM_MIN/2.0D0)**2
                      ELSE IF ( IBEAM == 21 ) THEN
                         ARG = -( (XC - MAP%COOR_CC(1,J8)) &
     &                         -(  YC - MAP%COOR_CC(2,J8)) )**2/ &
     &                             (BEAM_SIZE/2.0D0)**2 &
     &                         -( (XC - MAP%COOR_CC(1,J8)) &
     &                           +(YC - MAP%COOR_CC(2,J8)) )**2/ &
     &                             (BEAM_SIZE/2.0D0)**2
...
!
     ...
     &                             (MAP%BEAM_MAJ/2.0D0)**2
                      ELSE IF ( IBEAM == 6 ) THEN
                         IF ( ABS(XC - MAP%COOR_CC(1,J8)) < ABS(MAP%STEP_RA/2.0) .AND. & 
     &                        ABS(YC - MAP%COOR_CC(2,J8)) < ABS(MAP%STEP_DL/2.0)       ) THEN
                              ARG = 0.0D0
                            ELSE
                              ARG = -100.0D0
                         END IF
...
!
                    IF ( IBEAM == 4 ) THEN
                         IMAGE(J6,J7) = IMAGE(J6,J7) + MAP%FLUX_CC(J8)
                       ELSE 
                         IF ( ARG > -15.0D0 ) THEN
                              IMAGE(J6,J7) = IMAGE(J6,J7) + MAP%FLUX_CC(J8)*EXP( ARG )
                         END IF
                    END IF
                    IF ( J6 == 1 .AND. J7 == 1 ) THEN
                         CX = CX + MAP%FLUX_CC(J8)*MAP%COOR_CC(1,J8)
                         CY = CY + MAP%FLUX_CC(J8)*MAP%COOR_CC(2,J8)
                         FT = FT + MAP%FLUX_CC(J8)
                         IF ( MAP%FLUX_CC(J8) > FM ) THEN
                              FM = MAP%FLUX_CC(J8) 
                              MX = MAP%COOR_CC(1,J8)
                              MY = MAP%COOR_CC(2,J8)
                         END IF
     ...
     &                   1, MAP%DIM2, LEV_CONT, NC, TR )
         ELSE
           IF ( ICLR .GE. 40 .AND. ICLR .LE. 44 ) THEN
                INIT_COL = 20
                COL_SHIFT = 0
!!                        INIT_COL  = 40 ; COL_SHIFT = 36
                INIT_COL = INIT_COL + COL_SHIFT/2
                NCOL = 200
                RVAL_MIN = SIGLEV*MAP%NOISE
                IF ( MAP_LOG_STR == 'YES' ) THEN
                     RVAL_MIN = LOG10 ( RVAL_MIN )
                     RVAL_MAX = LOG10 ( RVAL_MAX )
                END IF
!
                DO 490 J9=1,MAP%DIM1
                   DO 4100 J10=1,MAP%DIM2 
                      IF ( MAP_LOG_STR == 'YES' ) THEN
                           MAP%IMAGE(J9,J10) = LOG10 ( MAX( SIGLEV*MAP%NOISE/10, MAP%IMAGE(J9,J10) ) )
                      END IF
                      IMAGE_I4(J9,J10) = (MAP%IMAGE(J9,J10) - RVAL_MIN)/(RVAL_MAX - RVAL_MIN)* &
     &                                   (NCOL-1-COL_SHIFT) + INIT_COL
                      IMAGE_I4(J9,J10) = MAX(IMAGE_I4(J9,J10),INIT_COL)
                      IF ( IMAGE_I4(J9,J10) < + INIT_COL + 50  ) IMAGE_I4(J9,J10) = IMAGE_I4(J9,J10)
 4100               CONTINUE 
 490            CONTINUE 
                IF ( MAP_LOG_STR == 'YES' ) THEN
                     DO 4110 J11=1,NC
                        LEV_CONT(J11) = LOG10 ( LEV_CONT(J11) )
 4110                CONTINUE 
                END IF
                CALL PGSLW  ( ITHICK ) 
!
...
!
                INIT_COL = INIT_COL - COL_SHIFT
                CALL DEFINE_PALETTE ( IPAL, IDEV, NCOL, INIT_COL )
                IF ( MAP_UNIT_STR == 'arcsec' ) THEN
                     IPB = 1000*X_BL/(MAP%STEP_DL*RAD_TO_MAS)
                     TR  = TR/1000.0
                   ELSE
                     IPB = X_BL/(MAP%STEP_DL*RAD_TO_MAS)
                END IF
!
! ------------- Draw pixels
!
                CALL PGPIXL ( IMAGE_I4, MAP%DIM1, MAP%DIM2, MAX(1,IPX_C-IPB), IPX_C+IPB, MAX(1,IPY_C-IPB), IPY_C+IPB, &
     &                        X_BL, X_TR, Y_BL, Y_TR )
                IF ( IBEAM == 0  .OR. IBEAM == 3 ) THEN
!
! ------------------ First, fill the area of the beam with grey color
...
! -------- Draw contours
!
           CALL PGCONT ( MAP%IMAGE, MAP%DIM1, MAP%DIM2, MAX(1,IPX_C-IPB), IPX_C+IPB, &
     &                   MAX(1,IPY_C-IPB), IPY_C+IPB, LEV_CONT, -NC, TR )
!
           IF ( ISIZE == 0 ) THEN
                CALL PGSCH   ( 1.6 )
                CALL PGSLW   ( 1 )
...
!
                DO 4120 J12=1,NCOL
                   ...
                   XR = XMAX
                   YT = J12-1
                   CALL PGSCI  ( J12-1+INIT_COL + COL_SHIFT )
                   CALL PGSCI  ( MIN(NCOL,J12-1+INIT_COL + COL_SHIFT) )
                   CALL PGSFS  ( 1 )
                   CALL PGRECT ( XL, XR, YB, YT )
 4120           CONTINUE 
                CALL PGSCI  ( 1 )
                CALL PGSFS ( 2 )
                CALL PGRECT ( XMIN+0.0001, XMAX-0.0001, 0.0, 1.0*NCOL )
                NLAB = 7
!
                DO 4130 J13=1,NLAB
                   IF ( MAP_LOG_STR == 'YES' ) THEN
                        VAL_R4 = 10**(RVAL_MIN + (RVAL_MAX - RVAL_MIN)*(J13-1)/FLOAT(NLAB-1))
                      ELSE 
                        VAL_R4 = RVAL_MIN + (RVAL_MAX - RVAL_MIN)*(J13-1)/FLOAT(NLAB-1)
                   END IF
                   IF ( VAL_R4 .LE.  0.0001 ) THEN
                        WRITE ( UNIT=STR(1:7), FMT='(F7.2)' ) 1000*VAL_R4
                      ELSE IF ( VAL_R4 .LE.  0.1 ) THEN
                        WRITE ( UNIT=STR(1:7), FMT='(F7.1)' ) 1000*VAL_R4
                      ELSE IF ( VAL_R4 .GT.  0.1 ) THEN
                        WRITE ( UNIT=STR(1:7), FMT='(F7.0)' ) 1000*VAL_R4
                   END IF 
                   CALL CHASHL ( STR(1:7) )
                   XL = XMIN - 0.01
                   YB = NCOL*(J13-1)/FLOAT(NLAB)
                   IF ( J13 == 1 ) THEN
                        CALL PGPTXT ( XL, YB*1.1, 90.0, 0.8, STR(1:6) )
                      ELSE IF ( J13 == NLAB ) THEN
                        CALL PGPTXT ( XL, YB*1.1, 90.0, 0.2, STR(1:6) )
                      ELSE
                        CALL PGPTXT ( XL, YB*1.1, 90.0, 0.5, STR(1:6) )
                   END IF
 4130           CONTINUE 
!
                CALL PGPTXT ( 0.98, NCOL/2.0, 90.0, 0.5, 'Flux density mJy/beam' )
           END IF
      END IF
      DEALLOCATE ( IMAGE, IMAGE_I4 )
!
      FL_ANG_UPDATE = .FALSE.
      XC = 0.0
      YC = 0.0
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ###  26-JAN-2007   DRAW_BEAM v1.1  (d)  L. Petrov  26-JAN-2007 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
