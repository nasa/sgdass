...
! *                                                                      *
! *  ### 29-AUG-2010  PIMA_PBP_ACCUM  v4.3 (d) L. Petrov 16-DEC-2023 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &                  LCHN, LFRQ, LTIM, &
     &                  GR_DEL, PH_RAT, BPS%GR_RAT(J3,J1,1), PHS, &
     &                  BPS%SNR(J3,J1,1), BPS%TIME_FRT(J3,J1,1), &
     &                  SEARCH_MODE, PIMA__BPASS_PHS, POL_MODE, &
     &                  RES, RES_AVR, SNR, IER )
             IF ( IER .NE. 0 ) THEN
                  CALL CLRCH ( STR )
                  CALL INCH  ( BPS%IND_OBS_SEL(J3,J1), STR )
...
!
     ...
     &                             PIM%CONF%BPS_AMP_MIN*BPS%AMPL(J3,J1,1), &
...
!
                 ...
                 DO 4120 J12=1,KCHN
                    DO 4130 J13=1,PIM%CONF%BPS_MSEG_ACCUM ! cycle over the spectral channales of the segment
                       KP = KP + 1
                       IF ( AMPL_RES(J12,1) > PIM%CONF%BPS_AMP_MIN*BPS%AMPL(J3,J1,1) .AND. WEI_CHN(J12) > 0 ) THEN
                            IF ( SGN_REM == -1 ) THEN
!
...
!
                 DO 4140 J14=1,KCHN
                    IF ( AMPL_RES(J14,2) > PIM%CONF%BPS_AMP_MIN*BPS%AMPL(J3,J1,1) .AND. WEI_CHN(J14) > 0 ) THEN
...
!
     ...
     &                       ABS(PIM%PBP(BPS%IND_STA_REF)%CMPL(J17,IFRQ)) > PIMA__AMP_MIN       ) THEN
!
...
!
              IF ( PIM%CONF%DEBUG_LEVEL .GE. 2 ) THEN
                   WRITE ( 6, 210 ) PIM%C_STA(J1), AMPL_RMS, PHAS_RMS, &
     &                              STR(1:8)
 210               FORMAT ( 'PBP   Sta: ',A, ' Ampl_rms: ', F6.3, &
     &                 ' Phas_rms: ', F8.4, 2X, A )
              END IF
 430      CONTINUE
!
...
!
                      CALL ERR_PASS ( IUER, IER )
                      CALL BPASS_MOD_POLY ( 1, PIM%CONF%BPS_AMP_MIN,  0, 1, &
     &                          PIM%NCHN, FREQ_ARR_R4(1,J18), &
     &                                    ACC_REM_PHAS(1,IFRQ,J1), &
     &                                    ACC_REM_AMPL(1,IFRQ,J1), &
     &                                    PHAS_MOD(1,J18,1), AMPL_MOD(1,J18,1), &
