...
! *                                                                      *
! *  ### 30-JAN-2006   PIMA_THEO   v1.9 (d)  L. Petrov  03-MAY-2020 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
              WRITE ( 6, * ) 'J1= ', J1, ' PIM%NOBS= ', PIM%NOBS, &
...
!
         IF ( PIM%NFRG == 1 ) THEN
            ...
            ELSE
              IF ( PIM%OBS(J1)%UV_IND(1,FRG_IND) < 1 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   CALL CLRCH ( STR1 )
                   CALL INCH  ( FRG_IND, STR1 )
                   CALL ERR_LOG ( 3719, IUER, 'PIMA_THEO', 'Trap of internal '// &
     &                 'control: PIM%OBS(J1)%UV_IND(1,FRG_IND) is zero for '// &
     &                 'observation '//TRIM(STR)//' and frg_ind= '//TRIM(STR1) )
                   RETURN 
              END IF
