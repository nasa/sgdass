...
! *                                                                      *
! * ### 14-NOV-2009  PIMA_BPASS_FINE v3.4 (d) L. Petrov  21-JAN-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           GR_DEL(PIM__MFRA), PH_RAT(PIM__MFRA), PHS(PIM__MFRA), SEQ_ARG
      REAL*8     T1(8192), X1(8192), X2(8192), E1(8192), E2(8192)
...
!
      DO 410 J1=1,PIM%NSTA
         PCI = PIM%BPS%PCI(J1)
         IF ( PCI < 1 ) GOTO 410
         IF ( BPS%NUM_OBS_FINE(J1) .LE. 0 ) GOTO 410
         FL_STA = .FALSE.
         DO 420 J2=1,BPS%NUM_OBS_FINE(J1)
...
!
      ...
      DO 430 J3=1,PIM%NSTA
         PCI = PIM%BPS%PCI(J3)
         IF ( PCI == 0 ) GOTO 430
         IF ( BPS%NUM_OBS_FINE(J3) .LE. 0 ) GOTO 430
         NUSED_OBS_STA(J3) = 0
         DO 440 J4=1,BPS%NUM_OBS_FINE(J3)
            ...
            IOBS = IOBS + 1
            IND_OBS(IOBS) = BPS%IND_OBS_SEL(J4,J3)
...
!
     ...
     &                 POL_MODE, RES, RES_AVR, SNR_ARR(IOBS), IER )
            IF ( IER .NE. 0 ) THEN
                 WRITE ( 6, * ) ' IER = ', IER
                 CALL CLRCH ( STR )
                 CALL INCH  ( BPS%IND_OBS_SEL(J4,J3), STR )
...
!
                  IF ( PIM%CONF%DEBUG_LEVEL == 21 ) THEN
                       IPLT = IPLT + 1
                       T1(IPLT) = FREQ_CHN(J9) 
                       X1(IPLT) = PHAS_RES(J9)
                       E1(IPLT) = 0.0001/WEI_VEC(IEQU)
                  END IF
!
...
!
 ...
 480        CONTINUE
            IF ( PIM%CONF%DEBUG_LEVEL == 21 ) THEN
                 CALL CLRCH ( STR )
                 CALL INCH  ( IND_OBS(IOBS), STR )
                 CALL DIAGI_SETDEF ( IER, 'DIAGI_CTIT', 'BPASS_FINE Residual phase at '// &
     &                               PIM%C_STA(PIM%OBS(IND_OBS(IOBS))%STA_IND(IND_REM))// &
     &                               ' obs '//STR )
                 CALL DIAGI_1E ( IPLT, T1, X1, E1, IER )
            END IF
 440     CONTINUE
...
!
      DO 4170 J17=1,L_EQU_ORIG
         ...
         DO 4180 J18=1,PIM%NSTA
            PCI = PIM%BPS%PCI(J18)
            IF ( PCI == 0 ) GOTO 4180
            IF ( BPS%NUM_OBS_FINE(J18) .LE. 0 ) GOTO 4180
            DO 4190 J19=1,BPS%NUM_OBS_FINE(J18)
               ...
               IOBS = IOBS + 1
               IND_OBS(IOBS) = BPS%IND_OBS_SEL(J19,J18)
               IF ( PIM%OBS(IND_OBS(IOBS))%REF_FRG_INDS(PIM%CONF%FRQ_GRP) == 0 ) THEN
                    GOTO 4190
               END IF
               IEQU = IND_EQU(IOBS) - 1
...
!
      DO 4250 J25=1,PIM%NSTA
         PCI = PIM%BPS%PCI(J25)
         IF ( PCI == 0 ) GOTO 4250
         IF ( BPS%NUM_OBS_FINE(J25) .LE. 0 ) GOTO 4250
         IFRQ = 0
         IPAR = IPAR_STA(J25)
