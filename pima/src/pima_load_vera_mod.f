...
! *                                                                      *
! * ## 26-FEB-2012 PIMA_LOAD_VERA_MOD v2.2 (d) L. Petrov 19-DEC-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      ...
      CHARACTER  MOD_FILE, MDU_FILE*128, MDC_FILE*128, ERRSTR*128
      INTEGER*4  LEV, J1, J2, J3, J4, J5, J6, J7, J8, J9, J10, &
     &           J11, J12, J13, J14, J15, J16, J17, J18, J19, J20, &
     &           L_NOD, L_DEG, NZO_REF, IND_ANT_NAME, &
     &           IND_SOU_VTD, IL, IS, L_FIL, K_MOD(PIM__MSTA), &
     &           IND_FRA, LUN_MOD, LUN_MDU, LUN_MDC, IP, ID, IND, IER
     ...
     &           MEAN_T, DR_VAL, SH_VAL, DR_SIG, SH_SIG
      ...
      LOGICAL*1  FL_FOU(2), FL_DIR, FL_ANT_IND_NAME
...
! --- Set up for VTD
!
      CALL GETENVAR ( 'PIMAVAR_VERA_ANT_IND_NAME', STR )
      IF ( STR(1:1) == 'Y' .OR. STR(1:1) == 'y' ) THEN
           FL_ANT_IND_NAME = .TRUE.
         ELSE      
           FL_ANT_IND_NAME = .FALSE.
      END IF
!
...
!
      IF ( PIM%CONF%DEBUG_LEVEL .GE. 2 ) THEN
           WRITE ( 6, '(A,I3,A)' ) 'PIMA_LOAD_VERA_MOD: ', PIM%CONF%L_INM, &
     &                             ' IM model files will be processed' 
      END IF
!
      DO 420 J2=1,2
         L_FIL = 0
         DO 430 J3=1,PIM%CONF%L_INM
            LEV = 0
            FL_DIR = IS_DIR_EXIST ( PIM%CONF%INTMOD_FILE(J3), ERRSTR )
...
!
     ...
     &                                  FINAM(1:I_LEN(FINAM))
               ...
               IF ( INDEX ( FINAM, 'ANT.' ) > 0 ) THEN
                    IF ( FL_ANT_IND_NAME ) THEN
                         IND = INDEX ( FINAM, 'ANT.' )   
                         CALL CHIN ( FINAM(IND+4:), IND_ANT_NAME )
                       ELSE
                         IND_ANT_NAME = 0
                    END IF
!
...
! ------------------------------------------------------------------------
!
      SUBROUTINE GET_CODA_IFMOD_HEADER ( FINAM, INTMOD_TYPE, &
     &                                   IND_ANT_NAME, CIH, IUER )
! ************************************************************************
! *                                                                      *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * # 27-FEB-2012 GET_CODA_IFMOD_HEADER v3.0 (d) L. Petrov 19-DEC-2025 # *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
...
!
     ...
     &              'length' )
           ...
           IF ( IND_ANT_NAME > 0 ) THEN
                CIH%IND_ANT = IND_ANT_NAME
           END IF
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!  write ( 6, * ) 'HEADER: ', cih%exp_name(1:i_len(cih%exp_name)), cih%npoly, cih%vers, cih%nss, cih%ndat, cih%ind_ant
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 27-FEB-2012 GET_CODA_IFMOD_DATA v3.0 (d) L. Petrov 19-DEC-2025 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
 ...
 real*8  vec(8) ! %%%%%%%%%
      CHARACTER  FINAM*(*), INTMOD_TYPE*(*)
      INTEGER*4  K_MOD(PIM%NSTA), IND_ANT_NAME, IUER
      CHARACTER  STR*128
