...
! *                                                                      *
! * ### 09-JAN-2006   PIMA_GET_PCAL  v4.13 (d) L. Petrov 25-JUL-2022 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'pima.i'
...
! 
              ...
              PIM%STA(J7)%CABLE(J6)%NPOI       = PIM%STA(J7)%PCAL(J6)%NPOI
              PIM%STA(J7)%CABLE(J6)%CAB_AVAIL  = .TRUE.
              PIM%STA(J7)%CABLE(J6)%MEAN_CABLE = 0.0D0
...
!
              PIM%STA(J7)%CABLE(J6)%CABLE_SIGN = -1 ! Do not know why. Tradition?
!
...
!
     ...
     &                   PIM%STA(J7)%PCAL(J6)%NPOI,PIM%STA(J7)%PCAL(J6)%NPOL) )
              ALLOCATE ( PIM%STA(J7)%CABLE(J6)%TIM_CAB(PIM%STA(J7)%CABLE(J6)%NPOI)  )
              ALLOCATE ( PIM%STA(J7)%CABLE(J6)%CAB_DEL(PIM%STA(J7)%CABLE(J6)%NPOI)  )
...
!
              ...
              PIM%STA(J7)%CABLE(J6)%TIM_CAB  = 0.0D0
              PIM%STA(J7)%CABLE(J6)%CAB_DEL  = 0.0D0
!
            LP = 0
...
!
     ...
     &                       PIM%TAI_0
                     PIM%STA(J7)%CABLE(J6)%TIM_CAB(LP) = PIM%STA(J7)%PCAL(J6)%TIME_MID_R8(LP)
!
...
!
     ...
     &                          J9, PIM%FILE(J8)%KEY(IND_CAB(J8),IND_TAB(J8)), &
      &                         1, PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP), IER )
                     IF ( IER .NE. 0 ) THEN
                          CALL CLRCH ( STR )
                          CALL INCH  ( J9, STR )
     ...
     &                        'FITS-IDI file '//PIM%FILE(J8)%NAME )
                         RETURN
                     END IF
                     IF ( IS_R8_INF ( PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP) ) ) PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP) = 0.0D0
                     IF ( IS_R8_NAN ( PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP) ) ) PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP) = 0.0D0
!
! ------------------ Get pcal frequency
!
                     CALL ERR_PASS ( IUER, IER )
...
!
            CAB_TMP%CAB_AVAIL  = PIM%STA(J7)%CABLE(J6)%CAB_AVAIL
            CAB_TMP%NPOI = PIM%STA(J7)%CABLE(J6)%NPOI
...
!
     ...
     &                     PIM%STA(J7)%CABLE(J6)%TIM_CAB, CAB_TMP%TIM_CAB )
            CALL COPY_R8 ( CAB_TMP%NPOI, &
     &                     PIM%STA(J7)%CABLE(J6)%CAB_DEL, CAB_TMP%CAB_DEL )
!
            LP = 0 ! points counter
!
...
!
               PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP) = CAB_TMP%CAB_DEL(IND_PCAL)
               PIM%STA(J7)%CABLE(J6)%TIM_CAB(LP) = CAB_TMP%TIM_CAB(IND_PCAL)
               IF ( J6 == 1 ) THEN
                    PIM%STA(J7)%CABLE(J6)%MEAN_CABLE  = PIM%STA(J7)%CABLE(J6)%MEAN_CABLE &
     &                                            + PIM%STA(J7)%CABLE(J6)%CAB_DEL(LP)
               END IF
...
!
 ...
 4160       CONTINUE
            ...
            IF ( J6 == 1 ) THEN
                 PIM%STA(J7)%CABLE(J6)%MEAN_CABLE = PIM%STA(J7)%CABLE(J6)%MEAN_CABLE/LP
            END IF
!
            DO 4270 J27=1,PIM%STA(J7)%PCAL(J6)%NPOI 
               PIM%STA(J7)%CABLE(J6)%CAB_DEL(J27) = PIM%STA(J7)%CABLE(J6)%CAB_DEL(J27) &
     &                                        - PIM%STA(J7)%CABLE(J6)%MEAN_CABLE
 4270       CONTINUE 
!
...
!
 ...
 4380      CONTINUE 
     ...
     &                             PIM%STA(J39)%CABLE(J40)%CAB_AVAIL, &
     &                             PIM%STA(J39)%CABLE(J40)%CABLE_SIGN
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 03-AUG-2011  ARR_R8_IS_ZERO  v1.0 (d) L. Petrov  03-AUG-2011 ### *
! *                                                                      *
! ************************************************************************
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 03-AUG-2011  ARR_R4_HAS_NAN  v1.0 (d) L. Petrov  03-AUG-2011 ### *
! *                                                                      *
! ************************************************************************
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 03-AUG-2011  ARR_R8_HAS_NAN  v1.0 (d) L. Petrov  28-SEP-2019 ### *
! *                                                                      *
! ************************************************************************
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 12-FEB-2020  PCAL_FRQ_FIX   v1.2 (d) L. Petrov  13-MAR-2020 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
