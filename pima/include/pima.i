!
! >>>>> pima.i   2006.01.06   v 2.50  --  2026.05.22_08:43:00
!
	INTEGER*4    PIM__MACT, PIM__MOPT, PIM__MFIL, PIM__MHDR, PIM__MKWD, &
     ...
     &               PIM__MPLR, PIM__MPIF, PIM__MWVR, PIM__MBPS
	REAL*8       PIM__MSCL
...
!
	...
	PARAMETER  ( PIM__MSCA = 128*1024  ) ! Maximal number of scans
...
!
	...
	PARAMETER  ( PIM__MBPS =   8       )  ! Max number of polarization subsitutes
  	INTEGER*4  PIM__FILL, PIM__CNFL
	PARAMETER  ( PIM__CNFL =   282888 )  ! Size in bytes of the configuration section
...
!
	...
	PARAMETER  ( PIMA__LABEL= 'PIMA 20260710  version 2.50b' )
	PARAMETER  ( PIMA__FORMAT_LABEL     = 'PIMA DUMP  Format v 2.49  2026.05.12' )
...
!
	      ...
	      INTEGER*4  L_TUS
 	      INTEGER*4  TUS_TYPE
	      CHARACTER  TSYS_USE_STA(PIM__MSTA)*8
...
!
	      INTEGER*4,  POINTER :: IND_SUBS(:,:,:,:)      => NULL()
	      REAL*4,     POINTER :: PHAS_SUBS(:)           => NULL()
	      REAL*4,     POINTER :: AMPL_SUBS(:)           => NULL()
!
	      INTEGER*4,  POINTER :: IND_SCA_SUBS(:,:,:,:)  => NULL()
	      REAL*4,     POINTER :: PHAS_SCA_SUBS(:)       => NULL()
	      REAL*4,     POINTER :: AMPL_SCA_SUBS(:)       => NULL()
...
!
	TYPE  PIM_STA__TYPE
	      ...
	      TYPE     ( PIM_CABLE__TYPE ) :: CABLE(PIM__MFRG)
...
!
! ---------- PCAL_IND(1,...) corresponds to the first point of pcal that fits this observation
! ---------- PCAL_IND(2,...) corresponds to the last  point of pcal that fits this observation
! ---------- PCAL_IND(3,...) corresponds to the pcal index that is th closest to the mid-epoch of the observation
! ---------- PCAL_IND(4,...) the same as 3 ??
!
	     INTEGER*2  PCAL_IND(4,2,PIM__MFRG) ! 2nd dimension runs over stations, 3rd dimension runs over frequency groups
...
!
        TYPE      PIM_BPS_STA__TYPE
	     ...
	     REAL*8     SNR_ALL(PIM__MSCA,PIM__MSTA)
	     REAL*8     SNR_PAR(PIM__MSCA,PIM__MSTA)
	     REAL*8     SNR_CRS(PIM__MSCA,PIM__MSTA)
	     ...
	     INTEGER*4  NUM_OBS_ACCUM(PIM__MSTA)
	     INTEGER*4  NUM_OBS_FINE(PIM__MSTA)
	     INTEGER*4  IND_OBS_SEL(PIM__MSCA,PIM__MSTA)
	     INTEGER*4  IND_OBS_POL(PIM__MSCA,PIM__MSTA)
...
!
        TYPE      PIM_GACO__TYPE
	     INTEGER*4  NFRQ
	     REAL*4     GAIN_CORR(PIM__MCHN)
	     REAL*4     GACO_ERR(PIM__MCHN)
	     REAL*8     GACO_FRQ(PIM__MCHN)
	     INTEGER*4  IND_FREQ(PIM__MCHN)   ! Index in PIM%FRQ table
...
!
	      INTEGER*4  L_BPS_SUB
	      INTEGER*4  L_PBS_SUB
	      CHARACTER  BPS_SUB_NAME(PIM__MBPS)*128
	      CHARACTER  PBS_SUB_NAME(PIM__MBPS)*128
	      INTEGER*4  IND_BPS_SUB(PIM__MOBS)
	      INTEGER*4  IND_PBS_SUB(PIM__MOBS)
!
	      REAL*8,    POINTER            :: TSYS(:,:,:)             => NULL()
...
!
       ...
       &             PIMA__CHPO_CODE*8,        PIMA__TSMO_CODE*8, &
...
!
	CHARACTER    PIMA__PCAL_NO*8,          PIMA__PCAL_USE*8,         &
       ...
       &             PIMA__POLAR_ALI*8,        PIMA__POLAR_ORIG*8,       &
       &             PIMA__POLAR_1ST*8,        PIMA__POLAR_2ND*8,        &
       &             PIMA__POLARCAL_NO*8,      PIMA__FILTER_NO*8,        &
       &             PIMA__ONLY_DET*8,         PIMA__FRT_AUTO*8,         &
       &             PIMA__FRT_FILE*8,         PIMA__EDGE_AMP_USE*8,     &
       &             PIMA__EDGE_AMP_NO*8,      PIMA__FRIB_SUBS*8,        &
       &             PIMA__NORML_NO*8,         PIMA__NORML_BAND*8,       &
       &             PIMA__NORML_IF*8,         PIMA__NORML_IFACC*8,      &
       &             PIMA__FRIP_NO*8,          PIMA__MOD_NO*8,           &
       &             PIMA__MOD_VERA1000*8,     PIMA__MOD_VERA2000*8,     &
       &             PIMA__MOD_SHANGHAI*8,     PIMA__MOD_SFXC*8,         &
       &             PIMA__MOD_KJCC*8,         PIMA__INTERPLA*8,         &
       &             PIMA__EARTH_OB*8,         PIMA__STA_ORB*8,          &
       &             PIMA__RA_PUSCH*8,         PIMA__RA_GBT*8,           &
       &             PIMA__RA_GB140*8,         PIMA__MOIM*8,             &
       &             PIMA__PLOT_TXT*8,         PIMA__STA_BASED_NO*8,     &
       &             PIMA__STA_BASED_YES*8,    PIMA__STA_BASED_ALL*8,    &
       &             PIMA__BEAM_NO*8,          PIMA__BEAM_YES*8,         &
       &             PIMA__LEGENDRE*8,         PIMA__CHEBYSH*8,          &
       &             PIMA__SPLINE*8,           PIMA__LINEAR*8,           &
       &             PIMA__AVERAGED*8,         PIMA__WEIGHTED*8,         &
       &             PIMA__SPLT_WEI_ONE*8,     PIMA__SPLT_WEI_OBS_SNR*8, &
       &             PIMA__SPLT_WEI_AUTO*8,    PIMA__SPLT_WEI_OBS_RMS*8, &
       &             PIMA__SPLT_WEI_SEG_RMS*8, PIMA__PLOT_SAV*8,         &
       &             PIMA__MEASURED*8,         PIMA__NWM*8,              &
       &             PIMA__OPACITY*8,          PIMA__TREC*8,             &
       &             PIMA__MODELED*8,          PIMA__ONOF_CREATE*8,      &
       &             PIMA__ONOF_UPDATE*8,      PIMA__ONOF_NO*8,          &
       &             PIMA__MPPL_OBS*8,         PIMA__MPPL_STA*8,         &
       &             PIMA__WVR_USE*8,          PIMA__WVR_NO*8,           &
       &             PIMA__WVR_SPLINE_3RD*8,   PIMA__WVR_SPLINE_LIN*8,   &
       &             PIMA__WVR_SPLINE_AVR*8,   PIMA__SA_NO*8,            &
       &             PIMA__SA_MIN*8,           PIMA__SA_MAX*8,           &
       &             PIMA__SA_ALL*8,           PIMA__TOTAL_UV_YES*8,     &
       &             PIMA__TOTAL_UV_NO*8,      PIMA__ASIS*8,             &
       &             PIMA__FRQTRA_NO*8,        PIMA__TATM*8,             &
       &             PIMA__TSMO*8,             PIMA__TABL*8,             &
       &             PIMA__TABMOD*8
!
        ...
        PARAMETER  ( PIMA__CHPO_CODE        = 'CHPO    ' )
