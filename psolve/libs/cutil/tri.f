...
! *                                                                      *
! *  ###  06-NOV-1991  TRI_GRP    v 4.0  (d)  L. Petrov 13-JUL-2021 ###  *
! *                                                                      *
! ************************************************************************
        IMPLICIT   NONE 
        INTEGER*4  L_STA, L_BAS, M_TRI, IUER
        INTEGER*4  LIS_STA(L_STA), LIS_BAS(L_BAS), LIS_TRI(3,M_TRI), IBTR(3)
        INTEGER*4  K1, K2, K3, J1, J2, J3, J4, J5, J6, J7, &
     &             L_TRI, ISTA_1, ISTA_2, ISTA_3, &
     &             IBMN, IBMX, IBMD, ISG(3), LAST_STA, LAST_BAS
        LOGICAL*4  LM(3), FL_BAS_LIST_SORTED
        REAL*8     TW
        CHARACTER  STR*20
        INTEGER*4, EXTERNAL :: NSTBA, IFIND_PL, IFIND_SORT_PL, I_LEN
...
!%  write ( 6, * ) 'l_tri = ', l_tri
!%  do 510 j1=1,l_sta
!%      write ( 6, * ) 'j1= ', int2(j1), ' lis_sta= ', lis_sta(j1)
!%  510 continue 
!%  do 520 j2=1,l_bas
!%      write ( 6, * ) 'j2= ', int2(j2), ' lis_bas= ', lis_bas(j2)
!% 520  continue 
!%  call flush ( 6 ) 
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
...
!
     ...
     &                       'the list ' )
                   ...
                   END IF
 330            CONTINUE
                IF ( LIS_BAS(K2) < LIS_BAS(K2-1) ) THEN
                     FL_BAS_LIST_SORTED = .FALSE.
                END IF
           END IF
           LAST_BAS = LIS_BAS(K2)
 320    CONTINUE
...
!
!%  write ( 6, * ) 'tri-171 l_sta= ', l_sta, ' l_bas= ', l_bas, ' so= ', fl_bas_list_sorted ; call flush( 6 ) ! %%%%%%%%%%%%%%
        DO 410 J1=1,L_STA-2
!%           write ( 6, * ) 'tri-173 j1= ',j1  ; call flush( 6 ) ! %%%%%%%%%%%%%%
!%  tw =  wall_timer ( %val(0) )  ! %%%%%%%%%%%%
           DO 420 J2=J1+1,L_STA-1
              DO 430 J3=J1+2,L_STA
!
                  L_TRI=L_TRI+1
!
...
!
                  IF ( FL_BAS_LIST_SORTED ) THEN
                       LIS_TRI(1,L_TRI) = IFIND_SORT_PL ( L_BAS, LIS_BAS, IBMN )
                     ELSE
...
!
                 IF ( FL_BAS_LIST_SORTED ) THEN
                      LIS_TRI(2,L_TRI) = IFIND_SORT_PL ( L_BAS, LIS_BAS, IBMD )
                    ELSE 
...
!
                 IF ( FL_BAS_LIST_SORTED ) THEN
                      LIS_TRI(3,L_TRI)=IFIND_SORT_PL ( L_BAS, LIS_BAS, IBMX )
                    ELSE 
...
!
                      ...
                      IF ( FL_BAS_LIST_SORTED ) THEN
                           DO 440 J4=1,3
                              DO 450 J5=1,L_TRI-1
                                 IF ( .NOT. LM(J4) ) THEN
                                       IF ( LIS_TRI(1,J5) .EQ.  LIS_TRI(J4,L_TRI) ) LM(J4) = .TRUE.
                                       IF ( LIS_TRI(2,J5) .EQ.  LIS_TRI(J4,L_TRI) ) LM(J4) = .TRUE.
                                       IF ( LIS_TRI(3,J5) .EQ.  LIS_TRI(J4,L_TRI) ) LM(J4) = .TRUE.
                                 END IF
 450                          CONTINUE 
 440                       CONTINUE 
                        ELSE
                           DO 460 J6=1,3
                              DO 470 J7=1,L_TRI-1
                                 IF ( .NOT. LM(J6) ) THEN
                                      IF ( (LIS_TRI(1,J7) .EQ.  LIS_TRI(J6,L_TRI)) .OR. &
           &                               (LIS_TRI(1,J7) .EQ. -LIS_TRI(J6,L_TRI)) .OR. &
           &                               (LIS_TRI(2,J7) .EQ.  LIS_TRI(J6,L_TRI)) .OR. &
           &                               (LIS_TRI(2,J7) .EQ. -LIS_TRI(J6,L_TRI)) .OR. &
           &                               (LIS_TRI(3,J7) .EQ.  LIS_TRI(J6,L_TRI)) .OR. &
           &                               (LIS_TRI(3,J7) .EQ. -LIS_TRI(J6,L_TRI))      ) THEN
                                           LM(J6)=.TRUE.
                                      END IF
                                 END IF
 470                          CONTINUE 
...
!
                      IF ( LM(1)  .AND.  LM(2)  .AND.  LM(3) ) THEN
                           L_TRI=L_TRI-1
                           GOTO 430
                      END IF
                 END IF
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ###  29-DEC-94    SIGN_TRI   V2.1  (d)   L. Petrov  05-AUG-97  ###  *
! *                                                                      *
! ************************************************************************
        IMPLICIT   NONE
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ###  22-MAY-1998  SIGN_TRICLS   v1.1 (d) L. Petrov  03-AUG-1998  ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
