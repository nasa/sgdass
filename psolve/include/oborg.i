...
!      pet 2025.12.12  Added FREQ_SKY_S
!      pet 2026.04.01  Added MOD_ATM_PRES and MOD_WPR_PRES
!
!CCCCCC
!
!     BEGIN 'OBSFIL' SPECIFICATION  2026.04.01_16:31:19
!
      INTEGER*2    OBORG_IFILL_LEN
      PARAMETER  ( OBORG_IFILL_LEN = 103 ) ! Length of secondary space in 2-byte
!                                          ! words
      REAL*8 &
     &         FJD, &
     &         FRACT, &
...
!                           ! to theoretical delay rate (dimensionless)!!
     ...
     &         TROP_WZD(2),     &    ! Troposphere non-hydrostatic zenith delay
     ...
     &         IONO_APR_DEL,    &    ! Error in the differential total electron contents  (in TEC units)
     &         FREQ_SKY_S,      &    ! Sky frequency at the low band
     &         MOD_ATM_PRES(2), &    ! Atmospheric pressure from the output of numerical weather model (Pa)
     &         MOD_WPR_PRES(2)       ! Partial pressure of the water wapor from the output of numerical weather model (Pa)
!
...
!     FREQ_SKY   -- Reference frequency at the upper band(MHz) .
!     FREQ_SKY_S -- Reference at the lower band
...
!     IONO_APR_DEL     - A priori ionospheric path delay
