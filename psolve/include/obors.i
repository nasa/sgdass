...
!
!     BEGIN 'OBSFIL' SPECIFICATION  2026.04.01_16:42:00
!
      INTEGER*2    S_OBORG_IFILL_LEN
      PARAMETER  ( S_OBORG_IFILL_LEN =  103 ) ! Length of secondary space
!
      REAL*8 &
     &         S_FJD, &
     &         S_FRACT, &
...
!                           ! to theoretical delay rate (dimensionless)!!
     ...
     &         S_IONO_APR_DEL,    &    ! Error in the differential total electron contents  (in TEC units)
     &         S_FREQ_SKY_S,      &    ! Sky frequency at the low band
     &         S_MOD_ATM_PRES(2), &    ! Atmospheric pressure from the output of numerical weather model (Pa)
     &         S_MOD_WPR_PRES(2)       ! Partial pressure of the water wapor from the output of numerical weather model (Pa)
!
...
!
     ...
     &             S_PIND_OBS     ! Internal PIMA observation index 
!
        INTEGER*2 &
     &             S_IFIRST_OBORG_I2, &
     &             S_ISITE(2), &
