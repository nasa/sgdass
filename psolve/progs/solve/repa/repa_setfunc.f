...
! *                                                                      *
! *  ### 02-DEC-2004  REPA_SETFUNC  v1.5 (d)  L. Petrov  07-NOV-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!!   write  ( 6,  * ) ' j3 = ',j3, ' feed= ', REP%OBS(J3)%FEED_ANG(2); call pause ( 'repad_setfunc 180' ) ! %%%%
                 ...
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%ARG_GOO(IND_PLT) = REP%OBS(J3)%DTEC
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_GOO(IND_PLT) = REP%OBS(J3)%DTEC_APR
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'GrIon_Dl' ) THEN
                   CALL REPA_GET_IONO ( 'GrIon_Dl', J3, REP, &
     &                                   REP%PLT(IND_BAS)%ARG_GOO(IND_PLT), ERR )
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_GOO(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
              END IF
     ...
     &                                                 REP%OBS(J3)%SPAMB_GR_S
                   REP%PLT(IND_BAS)%ERR_GOO(IND_PLT) = 1.D-12
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%VAL_GOO(IND_PLT) = REP%OBS(J3)%DTEC
                   REP%PLT(IND_BAS)%ERR_GOO(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_err' ) THEN
                   REP%PLT(IND_BAS)%VAL_GOO(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                   REP%PLT(IND_BAS)%ERR_GOO(IND_PLT) = 1.D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_apr' ) THEN
                   REP%PLT(IND_BAS)%VAL_GOO(IND_PLT) = REP%OBS(J3)%DTEC_APR
                   REP%PLT(IND_BAS)%ERR_GOO(IND_PLT) = 1.D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%VAL_GOO(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
                   REP%PLT(IND_BAS)%ERR_GOO(IND_PLT) = 1.D-12
...
!
              ...
              IF ( REP__CH_ARG(ARG_IND) .EQ. 'Time    ' ) THEN
                 ...
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%ARG_BAD(IND_PLT) = REP%OBS(J3)%DTEC
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_BAD(IND_PLT) = REP%OBS(J3)%DTEC_APR
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'GrIon_Dl' ) THEN
                   CALL REPA_GET_IONO ( 'GrIon_Dl', J3, REP, &
     &                                   REP%PLT(IND_BAS)%ARG_BAD(IND_PLT), ERR )
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_BAD(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
              END IF
              IF ( REP__CH_VAL(VAL_IND) .EQ. 'Delay   ' ) THEN
...
!@                   REP%PLT(IND_BAS)%VAL_BAD(IND_PLT) = REP%OBS(J3)%EL(2)/DEG__TO__RAD
     ...
     &                                  REP%PLT(IND_BAS)%ERR_BAD(IND_PLT)  )
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%VAL_BAD(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
                   REP%PLT(IND_BAS)%ERR_BAD(IND_PLT) = 1.D-12
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'PhIon_Dl' ) THEN
                   CALL REPA_GET_IONO ( 'PhIon_Dl', J3, REP, &
     &                                  REP%PLT(IND_BAS)%VAL_BAD(IND_PLT), &
     &                                  REP%PLT(IND_BAS)%ERR_BAD(IND_PLT)  )
     ...
     &                                                 REP%OBS(J3)%SPAMB_GR_S
                   REP%PLT(IND_BAS)%ERR_BAD(IND_PLT) = 1.D-15
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%VAL_BAD(IND_PLT) = REP%OBS(J3)%DTEC
                   REP%PLT(IND_BAS)%ERR_BAD(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_apr    ' ) THEN
                   REP%PLT(IND_BAS)%VAL_BAD(IND_PLT) = REP%OBS(J3)%DTEC_APR
                   REP%PLT(IND_BAS)%ERR_BAD(IND_PLT) = 1.0D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_err' ) THEN
                   REP%PLT(IND_BAS)%VAL_BAD(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                   REP%PLT(IND_BAS)%ERR_BAD(IND_PLT) = 1.D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND)(1:3) .EQ. 'Cal' .AND. &
     &                     REP__CH_VAL(VAL_IND)(5:7) .EQ. '_St'       ) THEN
                   CALL CHIN ( REP__CH_VAL(VAL_IND)(4:4), ICAL )
                   CALL CHIN ( REP__CH_VAL(VAL_IND)(8:8), ISTA )
...
!
              ...
              IF ( REP__CH_ARG(ARG_IND) .EQ. 'Time    ' ) THEN
                 ...
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%ARG_UNR(IND_PLT) = REP%OBS(J3)%DTEC
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Dtec_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_UNR(IND_PLT) = REP%OBS(J3)%DTEC_APR
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'GrIon_Dl' ) THEN
                   CALL REPA_GET_IONO ( 'GrIon_Dl', J3, REP, &
     &                                   REP%PLT(IND_BAS)%ARG_UNR(IND_PLT), ERR )
                 ELSE IF ( REP__CH_ARG(ARG_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%ARG_UNR(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
              END IF
              IF ( REP__CH_VAL(VAL_IND) .EQ. 'Delay   ' ) THEN
     ...
     &                                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT)  )
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Iono_apr' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%IONO_APR_DEL
     ...
     &                                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT)  )
                 ...
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'SpAmb_Ps' ) THEN
                   ...
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = 1.0D-15
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'GrAmb_Gx' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%NAMB_GR_X* &
     &                                                 REP%OBS(J3)%SPAMB_GR_X 
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = 1.0D-12
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'GrAmb_Gs' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%NAMB_GR_S* &
     &                                                 REP%OBS(J3)%SPAMB_GR_S 
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = 1.0D-12
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec    ' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%DTEC
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_apr' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%DTEC_APR
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = 1.0D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND) .EQ. 'Dtec_err' ) THEN
                   REP%PLT(IND_BAS)%VAL_UNR(IND_PLT) = REP%OBS(J3)%DTEC_ERR
                   REP%PLT(IND_BAS)%ERR_UNR(IND_PLT) = 1.0D-6
                 ELSE IF ( REP__CH_VAL(VAL_IND)(1:3) .EQ. 'Cal' .AND. &
     &                     REP__CH_VAL(VAL_IND)(5:7) .EQ. '_St'       ) THEN
                   CALL CHIN ( REP__CH_VAL(VAL_IND)(4:4), ICAL )
...
!
      DO 450 J5=1,REP__M_ARG
         ...
         IF ( REP__CH_ARG(J5) .EQ. 'Time    ' ) THEN
            ...
            ELSE IF ( REP__CH_ARG(J5) .EQ. 'Dtec    ' ) THEN
              REP%CH_ARG(J5) = 'Differential TEC from fringe fitting'
            ELSE IF ( REP__CH_ARG(J5) .EQ. 'Dtec_apr' ) THEN
              REP%CH_ARG(J5) = 'A priori differential TEC'
            ELSE IF ( REP__CH_ARG(J5) .EQ. 'GrIon_Dl' ) THEN
              REP%CH_ARG(J5) = 'Group ionospheric delay'
            ELSE IF ( REP__CH_ARG(J5) .EQ. 'Iono_apr' ) THEN
              REP%CH_ARG(J5) = 'A priori ionospheric delay'
         END IF
 450  CONTINUE 
!
...
!
      DO 460 J6=1,REP__M_VAL
         ...
         IF ( REP__CH_VAL(J6) .EQ. 'Delay   ' ) THEN
            ...
            ELSE IF ( REP__CH_VAL(J6) .EQ. 'Iono_apr' ) THEN
              REP%CH_VAL(J6) = 'A priori ionospheric delay'
            ...
            ELSE IF ( REP__CH_VAL(J6) .EQ. 'Dtec    ' ) THEN
              REP%CH_VAL(J6) = 'Differential TEC from fringe fitting'
            ELSE IF ( REP__CH_VAL(J6) .EQ. 'Dtec_err' ) THEN
              REP%CH_VAL(J6) = 'Differential TEC error'
            ELSE IF ( REP__CH_VAL(J6) .EQ. 'Dtec_apr' ) THEN
              REP%CH_VAL(J6) = 'A prori differential TEC'
