...
! *                                                                      *
! * ### 07-DEC-2004   REPA_INQLINE   v1.2 (d) L. Petrov 05-NOV-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &                                      REP%OBS(IND_OBS)%SPAMB_GR_S*1.D9
           CALL CHASHL ( STR  ) 
           LINE(IND_CINQ) = LINE(IND_CINQ)(1:I_LEN(LINE(IND_CINQ)))//': '//STR
         ELSE IF ( REP__CH_VAL(REP%CNF%VAL_IND) == 'Iono_apr' ) THEN
           WRITE ( UNIT=STR, FMT='(F8.3)' ) REP%OBS(IND_OBS)%IONO_APR_DEL
         ELSE IF ( REP__CH_VAL(REP%CNF%VAL_IND) == 'Dtec    ' ) THEN
           WRITE ( UNIT=STR, FMT='(F9.2)' ) REP%OBS(IND_OBS)%DTEC
         ELSE IF ( REP__CH_VAL(REP%CNF%VAL_IND) == 'Dtec_apr' ) THEN
           WRITE ( UNIT=STR, FMT='(F8.3)' ) REP%OBS(IND_OBS)%DTEC_APR
         ELSE IF ( REP__CH_VAL(REP%CNF%VAL_IND) == 'Dtec_err' ) THEN
           WRITE ( UNIT=STR, FMT='(F9.2)' ) REP%OBS(IND_OBS)%DTEC_ERR
