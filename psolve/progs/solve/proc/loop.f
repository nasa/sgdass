...
!
     ...
     &          I_BAS, IL, NE, NA, N_WEI, N_DTEC, N_EXT_ERR, MJD_1ST
...
!
     ...
     &            STAT_ACM(M_ACM)*8, STR*80, STR1*160, STR2*80
!
     ...
     &           RATE_THR, METEO_VAL, TEMP_VAL, TAI_OBS, BAS_LEN, &
...
!   pet  2025.12.12  Added support of MEAN_BAS_EFF_FREQ
!   pet  2026.06.30  Added support of SNGL_OUT value of SOLVE_DEBUG
!
...
!
     ...
     &           R_BAS, RIS_BAS(MO_BAS), N_METEO(MAX_ARC_STA)
     ...
     &           STR_DTEC_SBA_ERR_SCL*32, REG*3, SOLVE_WRONG_FREQ_PRINT*5
      CHARACTER, ALLOCATABLE ::  BUF_METEO(:,:)*128
     ...
     &                       SUPR_INQ
      ...
      INTEGER*4, EXTERNAL :: ILEN, I_LEN, GET_UNIT, LTM_DIF, NSTBA
...
!
      ...
      IF  ( STR == 'YES' .OR. STR == 'LOOP' .OR. STR == 'loop' .OR. STR == 'yes' ) THEN
            FL_DEBUG_SBA = .TRUE.
         ELSE
            FL_DEBUG_SBA = .FALSE.
      END IF 
...
!
      ...
      IF ( ILEN(STR_DTEC_SBA_ERR_SCL) .NE. 0 ) THEN
           ...
           END IF
      END IF
      CALL GETENVAR ( 'SOLVE_WRONG_FREQ_PRINT', SOLVE_WRONG_FREQ_PRINT )
      CALL TRAN  ( 11, SOLVE_WRONG_FREQ_PRINT, SOLVE_WRONG_FREQ_PRINT )
      IF ( SOLVE_WRONG_FREQ_PRINT(1:1) == 'Y' .OR. &
     &     SOLVE_WRONG_FREQ_PRINT(1:1) == 'S' .OR. &
     &     SOLVE_WRONG_FREQ_PRINT(1:1) == 'L'      ) THEN
...
!
     ...
     &                    IOSTAT=IER )
              END IF
              IF ( SOLVE_DEBUG(1:8) == 'SNGL_OUT' ) THEN
                   FINAM_DEBUG = SOLVE_DIR_DEBUG(1:I_LEN(SOLVE_DIR_DEBUG))//'/'// &
     &                           TRIM(DBNAME_CH)//'_sngl.txt'
                   OPEN ( UNIT=LUN_DEBUG, FILE=FINAM_DEBUG, STATUS='UNKNOWN', &
     &                    IOSTAT=IER )
              END IF
...
!
              IF ( GIM_COLLECT_INFO ) THEN
                ...
                ELSE IF ( SOLVE_DEBUG(1:8) == 'SNGL_OUT' ) THEN
                   WRITE ( LUN_DEBUG, '(A)' ) '# VLBI experiment information excerpt SNGL_OUT. Format version of 2026.06.30'
                ELSE
     ...
     &                                             '  Nobs: ', IDEND - IDBGN + 1, ' Exp_code: '//EXP_CODE
              ...
              IF ( SOLVE_DEBUG(1:1) == '3'  ) THEN
                ...
                ELSE IF ( SOLVE_DEBUG(1:2) == '46' ) THEN
                   ...
                   WRITE ( LUN_DEBUG, '(A)' ) '# 161:175 D15.7 -- Observation reciprocal weight. Units: s'
                   WRITE ( LUN_DEBUG, '(A)' ) '# 178:192 D15.7 -- Original group delay uncertainity at the upper band. Units: s'
                   WRITE ( LUN_DEBUG, '(A)' ) '#'
                   WRITE ( LUN_DEBUG, '(A)' ) '# Ind Date Tai                Sta1     Sta2     Sou name  F   u-coord         v-coord          d tau/d alpha   d tau/d delta      d tau/atm_1     d tau/atm_2    recipr. weight   original error'
                   WRITE ( LUN_DEBUG, '(A)' ) '#'
                ELSE IF ( SOLVE_DEBUG(1:2) == '66' ) THEN
                   WRITE ( LUN_DEBUG, '(A)' ) '#'
                   WRITE ( LUN_DEBUG, '(A)' ) '#   1:5   I5   Source index'
                   WRITE ( LUN_DEBUG, '(A)' ) '#   7:22  A16  Scan name'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  24:46  A23  TAI observation time'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  48:55  A8   The first  station of a baseline'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  57:64  A8   The second station of a baseline'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  67:74  A8   Source name'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  86:86  L1   Flag whether this observation is used in soltuion: T: yes, F: not'
                   WRITE ( LUN_DEBUG, '(A)' ) '#  97:105 F9.2 Structure group delay in ps'
                   WRITE ( LUN_DEBUG, '(A)' ) '# 120:128 F9.2 Group delay uncertainty after re-weighting in ps'
                   WRITE ( LUN_DEBUG, '(A)' ) '# 143:151 F9.6 Uncalbrated fringe amplitude'
                   WRITE ( LUN_DEBUG, '(A)' ) '# 162:168 F7.2 UV projection in Mlambda'
                   WRITE ( LUN_DEBUG, '(A)' ) '# '
                   WRITE ( LUN_DEBUG, '(A)' ) '# Obs Scan name        Observation date/time   Baseline           Source          Flag    Structure delay     Group delay error               Corr. Amp.          UV-projection'
                   WRITE ( LUN_DEBUG, '(A)' ) '# '
...
!
                ...
                ELSE IF ( SOLVE_DEBUG(1:8) == 'OUT_ELEV' ) THEN
                   ...
                   WRITE ( LUN_DEBUG, '(A)' ) '# '
                ELSE IF ( SOLVE_DEBUG(1:8) == 'SNGL_OUT' ) THEN
                   WRITE ( LUN_DEBUG, '(A)' ) '# NOBS        7:12  I6     Observation index'
                   WRITE ( LUN_DEBUG, '(A)' ) '# TIM        19:41  A23    TAI time tag of the observation'
                   WRITE ( LUN_DEBUG, '(A)' ) '# STA1       48:55  A8     Name of the 1st station of a baseline'
                   WRITE ( LUN_DEBUG, '(A)' ) '# STA2       57:64  A8     Name of the 2nd station of a baseline'
                   WRITE ( LUN_DEBUG, '(A)' ) '# SOU        71:78  A8     Source name'
                   WRITE ( LUN_DEBUG, '(A)' ) '# EL1        86:93  F8.5   Elevation of first  station in radians'
                   WRITE ( LUN_DEBUG, '(A)' ) '# EL2        95:102 F8.5   Elevation of second station in radians'
                   WRITE ( LUN_DEBUG, '(A)' ) '# AZ1       108:115 F8.5   Azimuth   of first  station in radians'
                   WRITE ( LUN_DEBUG, '(A)' ) '# AZ2       102:109 F8.5   Azimuth   of second station in radians'
                   WRITE ( LUN_DEBUG, '(A)' ) '# FUS       135:135 L1     Flag whether the observation should be used'
                   WRITE ( LUN_DEBUG, '(A)' ) '# TOC       145:163 D19.12 Delay observatum minus calculatum, in sec'
                   WRITE ( LUN_DEBUG, '(A)' ) '# TERR      174:183 D10.4  Rescaled observation uncertainty, in sec'
                   WRITE ( LUN_DEBUG, '(A)' ) '# ADER      195:204 D10.4  Applied additive reweighting parameter'
                   WRITE ( LUN_DEBUG, '(A)' ) '# DUT1      215:228 D14.7  Partial derivative of delay over UT1, dimensionless'
                   WRITE ( LUN_DEBUG, '(A)' ) '# DZE1      239:252 D14.7  Partial derivative of delay over zenith path delay at the 1st station'
                   WRITE ( LUN_DEBUG, '(A)' ) '# DZE2      254:267 D14.7  Partial derivative of delay over zenith path delay at the 2nd station'
                   WRITE ( LUN_DEBUG, '(A)' ) '# UT1_M_TAI 280:289 F10.6  UT1 minus TAI'
                   WRITE ( LUN_DEBUG, '(A)' ) '# '
              END IF
              FL_TPD_READ = .FALSE.
         END IF
...
!
     ...
     &                           ' Procedure '//EDC%HEA%PRC_NAME(1:I_LEN(EDC%HEA%PRC_NAME))
        END IF
!
        IF ( INDEX ( SOLVE_EXPORT, 'print_meteo' ) > 0 ) THEN
             ALLOCATE ( BUF_METEO(NOBS,NUMSTA) )
             N_METEO = 0
        END IF
!
...
!
                FL_OBSFIL_UPDATE = .TRUE.
           END IF
           IF ( SOLVE_DEBUG(1:1) == 'R'  ) THEN
                IF ( .NOT. BTEST ( AUTO_SUP, INT4(INIT__SPS) ) ) WRITE ( 6, * ) 'No INIT_SPS Exp '//DBNAME_CH
           END IF
!
...
!
                IUNW = 0
                CALL SBIT ( SUPSTAT,  IUNW__SPS,  0     )
                USER_SUP = IBCLR ( USER_SUP, IDATYP     )
                USER_SUP = IBCLR ( USER_SUP, G_GXS__DTP )
                USER_SUP = IBCLR ( USER_SUP, GX__DTP    )
                USER_SUP = IBCLR ( USER_SUP, GS__DTP    )
                USER_SUP = IBCLR ( USER_SUP, GOOD__SPS  )
                USER_SUP = IBCLR ( USER_SUP, CBAD__SPS  )
                FL_OBSFIL_UPDATE = .TRUE.
...
!
! ------------- NB: FAMB is in seconds, whiule PHAMI8 is in microseconds
!
                DOBS   = DOBS   - FAMB*1.D6*NUMAMB
                DOBSXS = DOBSXS - FAMB*1.D6*NUMAMB_S
                DPH    = DPH    - PHAMI8   * NPHAM4
                DPHXS  = DPHXS  - PHAMI8_S * NPHAM4_S
!
                NUMAMB   = 0
                NUMAMB_S = 0
                NPHAM4   = 0
...
!
           ...
           IF ( .NOT. FL_EQUAL_EFF_FREQ ) THEN
              ...
              ELSE
                ...
                FREQ_PH_X    = MEAN_EFF_FREQ(2)*1.D6
                FREQ_RATE_X  = MEAN_EFF_FREQ(3)*1.D6
                FREQ_GR_S    = MEAN_EFF_FREQ(4)*1.D6
                FREQ_PH_S    = MEAN_EFF_FREQ(5)*1.D6
                FREQ_RATE_S  = MEAN_EFF_FREQ(6)*1.D6
                I_BAS = ABS ( NSTBA ( INT4(ISITE(1)), INT4(ISITE(2)) ) )
                IF ( I_BAS .LE. MAX4_OBS_BSL ) THEN
                     FREQ_GR_X    = MEAN_BAS_EFF_FREQ(I_BAS,1)*1.D6
                     FREQ_GR_S    = MEAN_BAS_EFF_FREQ(I_BAS,2)*1.D6
                END IF
           END IF
!
! -------- Preserve original group delay error and delay reweigting.
...
!
           IF ( DATYP_INQ ( IDATYP, FUSED__DTP ) ) THEN
!
! ------------- In a case of FUSED data type if the effective frequency 
! ------------- at of the bands of both bands is missing, take from the 
! ------------- averaged frqequency
!
                I_BAS = ABS ( NSTBA ( INT4(ISITE(1)), INT4(ISITE(2)) ) )
                IF ( FREQ_GR_X < (1.0D0 + EPS_FREQ)*FREQ__SOLVE_MIN ) THEN
                     IF ( I_BAS .LE. MAX4_OBS_BSL ) THEN
                          FREQ_GR_X = MEAN_BAS_EFF_FREQ(I_BAS,1)*1.D6
                        ELSE
                          FREQ_GR_X = MEAN_EFF_FREQ(1)*1.D6
                ...
                END IF
!
                IF ( FREQ_GR_S < (1.0D0 + EPS_FREQ)*FREQ__SOLVE_MIN ) THEN
                     IF ( I_BAS .LE. MAX4_OBS_BSL ) THEN
                          FREQ_GR_S = MEAN_BAS_EFF_FREQ(I_BAS,2)*1.D6
                        ELSE
                          FREQ_GR_S = MEAN_EFF_FREQ(4)*1.D6
...
!
     ...
     &                                TAU_OC )
           END IF
!%   write ( 6, * ) 'LOOP 2585: nobs= ', nobs, ' cor_tau = ', sngl(cor_tau), ' tau_oc= ', tau_oc, ' tau_e= ', sngl(tau_e) ! %%%%%%%%%%%%%
!
...
!          write ( 6, * ) ' PROC(loop)-2348  NOBS= ', NOBS ; call flush ( 6 ) ! %%%%%
 ...
 232            FORMAT ( I5, 1X, A, 1X, A, 1X, A8,'/',A8, 2X, A, ' || ', &
     ...
     &                   ' AZ_1: ', 0PF7.2 )
             ELSE IF ( SOLVE_DEBUG(1:2) == '65' ) THEN
                STR = MJDSEC_TO_DATE ( MJD_UTC_OBS, UTC_OBS, -2 )
                WRITE ( LUN_DEBUG, 235 ) NOBS, SCAN_NAME, STR(1:23), &
     &                      ISITN_CHR(ISITE(1)), ISITN_CHR(ISITE(2)), &
     &                      ISTRN_CHR(ISTAR), FL_USED, TAUGR_OBS_X, TAUPH_OBS_X, &
     &                      NPHAM4, PHAMI8, FREQ_SKY
 235            FORMAT ( I5, 1X, A, 1X, A, 1X, A8,'/',A8, 2X, A, ' || ', &
     &                        ' Used: ', L1, ' TAUGR_OBS_X= ', 1PD20.12, &
     &                        ' TAUPH_OBS_X= ', 1PD20.12, ' NPHAM_X= ', I8, &
     &                        ' PHAMI8 = ', 1PD15.7,  ' REFFR= ', 1PD15.7 )
             ELSE IF ( SOLVE_DEBUG(1:2) == '66' ) THEN
                STR = MJDSEC_TO_DATE ( MJD_UTC_OBS, UTC_OBS, -2 )
 ...
 237                 FORMAT ( I5, 1X, A, 1X, A, 1X, A8,'/',A8, 2X, A, ' || ', &
     ...
     &                        ' ps  Err_del: ', F9.2, ' ps  Cor_amp: ', F9.6, &
...
!
 ...
 272            FORMAT ( I5, 1X, A, 1X, A, 1X, A8, '/', A8, 1X, A8, 1X, &
     ...
     &                   2X, 1PD14.7, 1X, 1PD14.7 )
              ELSE IF ( SOLVE_DEBUG(1:2) == '42' ) THEN
...
!
                CALL CLRCH ( STR1 )
                IL = 1
                IF ( BTEST ( AUTO_SUP, INT4(INIT__SPS) ) ) STR1 = STR1(1:IL)//' INIT' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(NOFX__SPS) ) ) STR1 = STR1(1:IL)//' NOFX' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(NOFS__SPS) ) ) STR1 = STR1(1:IL)//' NOFS' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(BQCX__SPS) ) ) STR1 = STR1(1:IL)//' BQCX' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(BQCS__SPS) ) ) STR1 = STR1(1:IL)//' BQCS' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(DSBS__SPS) ) ) STR1 = STR1(1:IL)//' DSBS' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(DSSO__SPS) ) ) STR1 = STR1(1:IL)//' DSSO' ; IL = ILEN(STR1)
                IF ( BTEST ( AUTO_SUP, INT4(FURE__SPS) ) ) STR1 = STR1(1:IL)//' FURE' ; IL = ILEN(STR1)
!
!                IF ( BTEST ( AUTO_SUP, INT4(CUEL__SPS) ) ) STR1 = STR1(1:IL)//' CUEL' ; IL = ILEN(STR1)
!                IF ( BTEST ( AUTO_SUP, INT4(LSNR__SPS) ) ) STR1 = STR1(1:IL)//' LSNR' ; IL = ILEN(STR1)
!                IF ( BTEST ( AUTO_SUP, INT4(EXTS__SPS) ) ) STR1 = STR1(1:IL)//' EXTS' ; IL = ILEN(STR1)
!                IF ( BTEST ( AUTO_SUP, INT4(GOOD__SPS) ) ) STR1 = STR1(1:IL)//' GOOD' ; IL = ILEN(STR1)
!                IF ( BTEST ( AUTO_SUP, INT4(CBAD__SPS) ) ) STR1 = STR1(1:IL)//' CBAD' ; IL = ILEN(STR1)
!
                CALL CLRCH ( STR2 )
                IL = 1
                IF ( BTEST ( USER_SUP, INT4(GRPRAT__DTP) ) ) STR2 = STR2(1:IL)//' GRPRAT' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(PHSRAT__DTP) ) ) STR2 = STR2(1:IL)//' PHSRAT' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(SNBRAT__DTP) ) ) STR2 = STR2(1:IL)//' SNBRAT' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(GRPONL__DTP) ) ) STR2 = STR2(1:IL)//' GRPONL' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(PHSONL__DTP) ) ) STR2 = STR2(1:IL)//' PHSONL' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(SNBONL__DTP) ) ) STR2 = STR2(1:IL)//' SNBONL' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(RATONL__DTP) ) ) STR2 = STR2(1:IL)//' RATONL' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( G_GXS__DTP) ) ) STR2 = STR2(1:IL)//' G_GXS'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(PX_GXS__DTP) ) ) STR2 = STR2(1:IL)//' PX_GXS' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(PS_GXS__DTP) ) ) STR2 = STR2(1:IL)//' PS_GXS' ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( PX_GX__DTP) ) ) STR2 = STR2(1:IL)//' PX_GX'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( PX_GS__DTP) ) ) STR2 = STR2(1:IL)//' PX_GS'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( PS_GX__DTP) ) ) STR2 = STR2(1:IL)//' PS_GX'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( PS_GS__DTP) ) ) STR2 = STR2(1:IL)//' PS_GS'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( P_PXS__DTP) ) ) STR2 = STR2(1:IL)//' P_PXS'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(    GX__DTP) ) ) STR2 = STR2(1:IL)//' GX'     ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(    GS__DTP) ) ) STR2 = STR2(1:IL)//' GS'     ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(    PX__DTP) ) ) STR2 = STR2(1:IL)//' PX'     ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4(    PS__DTP) ) ) STR2 = STR2(1:IL)//' PS'     ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( SNG_X__DTP) ) ) STR2 = STR2(1:IL)//' SNG_X'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( SNG_S__DTP) ) ) STR2 = STR2(1:IL)//' SNG_S'  ; IL = ILEN(STR2)
                IF ( BTEST ( USER_SUP, INT4( FUSED__DTP) ) ) STR2 = STR2(1:IL)//' FUSED'  ; IL = ILEN(STR2)
!
                WRITE ( UNIT=LUN_DEBUG, FMT=273 ) NOBS, STR(1:23), &
     &                                            ISITN_CHR(ISITE(1)), ISITN_CHR(ISITE(2)), &
     &                                            FL_USED_GXS, FL_USED_GX, FL_USED_GS, &
     &                                            AUTO_SUP, USER_SUP, &
     &                                            TRIM(STR1(2:)), TRIM(STR2(2:))
 273            FORMAT ( 'NOBS: ', I5, 2X, A, 2X, A, ' / ', A, 2X, L1, 1X, L1, 1X, L1, &
     &                   ' AUTO_SUP: ', B32, ' USER_SUP: ', B32, ' Auto_Flags: ', A, &
     &                   ' Sup_flags: ', A )
...
!
 ...
 256            FORMAT ( I5, 1X, A, 1X, A8,'/',A8, 1X, A, 2X, L1, &
     ...
     &                   2X, 1PD15.8, 1X, 1PD15.8, 2X, 1PD15.8, 2X, 1PD15.8 )
...
!
 ...
 380            FORMAT ( 'Ind_obs: ', I6, ' IoX_del: ', 1PD13.6, 1X, L1 )
     ...
     &                                                  (TAI_OBS - TAI_1ST), &
     &                                                  ISITN_CHR(ISITE(1)), ISITN_CHR(ISITE(2)), ELEV, AZ
 385            FORMAT ( 'Nobs: ', I6, ' Tim= ', F8.1, ' Sta= ', A, 1X, A, &
     &                   ' Elev= ', F8.5, 1X, F8.5, ' Az= ', F8.5, 1X, F8.5 )
             ELSE IF ( SOLVE_DEBUG(1:8) == 'SNGL_OUT' ) THEN
                STR = MJDSEC_TO_DATE ( MJD_TAI_OBS, TAI_OBS, -2 )
                WRITE ( UNIT=LUN_DEBUG, FMT=386 ) NOBS, STR(1:23), &
     &                                            ISITN_CHR(ISITE(1)), ISITN_CHR(ISITE(2)), &
     &                                            ISTRN_CHR(ISTAR), &
     &                                            ELEV, AZ, &
     &                                            FL_USED, TAU_OC, TAU_E, ADDERR_GR_TAU, &
     &                                            ROTP(3,1), AP(1,1), AP(2,1), UT1_M_TAI
 386            FORMAT ( 'Nobs: ', I6, ' Dat= ', A23, ' Sta= ', A, 1X, A, ' Sou= ', A, &
     &                   ' Elev= ', F8.5, 1X, F8.5, ' Az= ', F8.5, 1X, F8.5, &
     &                   ' Fl_used= ', L1, ' Tau_oc= ', 1PD19.12, 1X, &
     &                   ' Tau_e= ', 1PD11.4, ' Add_err= ', 1PD11.4, &
     &                   ' Der_UT1= ', 1PD14.7, ' Der_Atm= ', 1PD14.7, 1X, 1PD14.7, &
     &                   ' UT1_M_TAI= ', 0PF10.6 )
...
!
 ...
 387                 FORMAT ( 'Obs: ', I6, ' Tim: ', A, ' Sta: ', A, 1X, A, &
     ...
     &                        ' || Tec_apr: ', F6.1, 1X, F6.1, ' Dtec_adj= ', F6.1, &
     &                        ' Del_bias: ', 1PD13.6, ' sec Iono_apr_del: ', F8.3, ' ns' )
               END IF
           END IF
!
           IF ( INDEX ( SOLVE_EXPORT, 'print_meteo' ) > 0 ) THEN
                STR = MJDSEC_TO_DATE ( MJD_UTC_OBS, UTC_OBS, -2 )
                IF ( ATMPR(1) > 0.0 ) THEN
                     VALS(1) = 100.0*ATMPR(1)
                   ELSE
                     VALS(1) = -1.0D0
                END IF 
                IF ( TEMPC(1) > -90.0 ) THEN
                     VALS(2) = TEMPC(1) + 273.15
                   ELSE
                     VALS(2) = -1.0D0
                END IF
                IF ( RELHU(1) >  0.0 ) THEN
                   ...
                   ELSE
                     VALS(3) = -1.0
                END IF
                WRITE ( STR1, 262 ) DBNAME_CH, ISITN_CHR(ISITE(1)), STR(1:19), &
     &                              VALS(1:3), MOD_ATM_PRES(1), MOD_WPR_PRES(1)
262             FORMAT ( A, 2X, A, 2X, A, '  Obs: ', F8.0, 1X, F5.1, 1X, F5.1, &
     &                                    '  Mod: ', F8.0, 1X, F8.0 )
                IF ( N_METEO(ISITE(1)) > 0 ) THEN
                     IF ( STR1 == BUF_METEO(N_METEO(ISITE(1)),ISITE(1)) ) THEN
                          CONTINUE 
                        ELSE
                          N_METEO(ISITE(1)) = N_METEO(ISITE(1)) + 1
                          BUF_METEO(N_METEO(ISITE(1)),ISITE(1)) = STR1
                     END IF
                   ELSE 
                     N_METEO(ISITE(1)) = N_METEO(ISITE(1)) + 1
                     BUF_METEO(N_METEO(ISITE(1)),ISITE(1)) = STR1
...
!
                IF ( ATMPR(2) > 0.0 ) THEN
                     VALS(1) = 100.0*ATMPR(2)
                   ELSE
                     VALS(1) = -1.0D0
                END IF 
                IF ( TEMPC(2) > -90.0 ) THEN
                     VALS(2) = TEMPC(2) + 273.15
                   ELSE
                     VALS(2) = -1.0D0
                END IF
                IF ( RELHU(2) >  0.0 ) THEN
                ...
                END IF
!
                WRITE ( STR1, 262 ) DBNAME_CH, ISITN_CHR(ISITE(2)), STR(1:19), &
     &                              VALS(1:3), MOD_ATM_PRES(2), MOD_WPR_PRES(2)
                IF ( N_METEO(ISITE(2)) > 0 ) THEN
                     IF ( STR1 == BUF_METEO(N_METEO(ISITE(2)),ISITE(2)) ) THEN
                          CONTINUE 
                        ELSE
                          N_METEO(ISITE(2)) = N_METEO(ISITE(2)) + 1
                          BUF_METEO(N_METEO(ISITE(2)),ISITE(2)) = STR1
                     END IF
                   ELSE 
                     N_METEO(ISITE(2)) = N_METEO(ISITE(2)) + 1
                     BUF_METEO(N_METEO(ISITE(2)),ISITE(2)) = STR1
...
!
           IF ( FL_USED ) THEN
                IF ( FL_WRONG_FREQ_PRINT ) THEN
                     IF ( FREQ_GR_X  .GE. 1.0D6 .AND. &
     &                    FREQ_GR_S  .GE. 1.0D6 .AND. &
     &                    FREQ_SKY   .GE. 1.0D6 .AND. &
     &                    FREQ_SKY_S .GE. 1.0D6 .AND. &
     &                    .NOT. ( FREQ_GR_X .GE. FREQ_SKY   .AND. FREQ_GR_X .LE. 8.8D9  .AND. &
     &                            FREQ_GR_S .GE. FREQ_SKY_S .AND. FREQ_GR_S .LE. 2.4D9        )  ) THEN
                          LUN_FREQ = GET_UNIT()
                          OPEN ( UNIT =LUN_FREQ, FILE='/tmp/wrong_freq.txt', STATUS='UNKNOWN', ACCESS='APPEND' )
                                 WRITE ( LUN_FREQ, 144 ) DBNAME_CH, NOBS, FREQ_GR_S, FREQ_GR_X, FL_EQUAL_EFF_FREQ
 144                             FORMAT ( 'Exp: ',A, ' Ind_Obs: ', I7, ' Freqs= ', 1PD13.6, 1X, 1PD13.6, 1X, L1 )
                          CLOSE ( UNIT=LUN_FREQ )
                          IF ( SOLVE_WRONG_FREQ_PRINT(1:1) .NE. 'L' ) THEN
!
! ---------------------------- Unless long, print only once
!
                               FL_WRONG_FREQ_PRINT = .FALSE.
                         END IF
                     END IF
           ...
           END IF
!
!!   write ( 6, * ) 'LOOP 3467: nobs= ', nobs, ' cor_tau = ', sngl(cor_tau), ' tau_oc= ', tau_oc, ' tau_e= ', sngl(tau_e), ' bas= ', isitn_chr(isite(1))//'/'//isitn_chr(isite(2)), ' FL_USED= ', FL_USED ! %%%%%%%%%%%%%
           IF ( .NOT. FL_USED ) THEN
!
...
!
          IF ( INDEX ( SOLVE_EXPORT, 'print_meteo' ) > 0 ) THEN
               LUN_DEBUG = GET_UNIT()
               FINAM_DEBUG = SOLVE_EXPORT_DIR(1:I_LEN(SOLVE_EXPORT_DIR))//'/'// &
     &                       TRIM(DBNAME_CH)//'_meteo.info'
               OPEN ( UNIT=LUN_DEBUG, FILE=FINAM_DEBUG, STATUS='UNKNOWN', &
     &                IOSTAT=IER, IOMSG=STR )
               IF ( IER .NE. 0 ) THEN
                    CALL ERR_LOG ( 8377, IUER, 'LOOP', 'Error in opening  '// &
     &                  'output file for meteo data '//TRIM(FINAM_DEBUG)// &
     &                  ' -- '//STR )
                    RETURN 
               END IF
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) METEO__LABEL
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) '#' 
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) '# Experiment: '//DBNAME_CH
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) '#' 
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) '# Generated on '//GET_CDATE()
               WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) '#' 
               DO 490 J9=1,NUMSTA
                  DO 4100 J10=1,N_METEO(J9)
                     WRITE ( UNIT=LUN_DEBUG, FMT='(A)' ) TRIM(BUF_METEO(J10,J9))
 4100             CONTINUE 
 490           CONTINUE 
               CLOSE ( LUN_DEBUG )
               DEALLOCATE ( BUF_METEO )
          END IF
!
! ------- Save them in glbc4
!
...
!@    write ( 6, * ) 'loop-3090 ' , nobs ; call flush ( 6 ) ! %%%%%%%%%%
     ...
     &                ISLTY2 .NE. 'I'                         ) THEN
!
! ------------ Remind: reparameterization is not supported for a non-fast mode
...
!
 1009     CONTINUE
     ...
     &                            ' for session '//DBNAME_CH
          ...
          IF ( SOLVE_DEBUG(1:8) == 'SNGL_OUT' ) THEN
               CLOSE ( LUN_DEBUG )
               WRITE ( 6, '(A)' ) 'Wrote a file with single baseline information '// &
     &                            FINAM_DEBUG(1:I_LEN(FINAM_DEBUG))// &
     &                            ' for session '//DBNAME_CH
          END IF
