     ...
     &                              IND_MET, ATM_PRES, AIR_TEMP, IUER )
! ************************************************************************
! *                                                                      *
...
! *                                                                      *
! * ### 21-NOV-2005 GETDB_FILL_OBORG v2.15 (d) L. Petrov 13-JAN-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      ...
      LOGICAL*4  FL_COMP_THEO, FL_SOU_USE_DB_IGNORE, FL_PIMA
      INTEGER*4  IND_MET(2,MAX_OBS)
     ...
     &           REFFR, REFFR_S, REFFR_VAR, TAU_GR(2), FREQ_EFF(2), FRQ_BAND_MIN, &
                 FRQ_BAND_MAX, FRQ_SKY_1(1024), FRQ_SKY_2(1024)
      INTEGER*4, ALLOCATABLE :: OBS_TAB(:,:)
      CHARACTER  STR*80, STR1*256, DESCR*256, STR1_ARR(32)*1, STR2_ARR(32)*2048, &
     &           FIL_IONO_DEBUG_FILE*128, SOLVE_EXPORT_DIR*128, C_BAS(MAX4_BSL)*17, &
     &           BAS_NAM*17, SOLVEVAR_PRINT_REF_FREQ*3
      INTEGER*4  M__ARR
      ...
      INTEGER*4  CLASS, CLASS_TEC_APR, TYP, DIMS(2), NUM_FIELDS, SEG_IND, LEN_REC, &
     &           LEN_DATA, MJD_UTC_OBS, ARR_I4(M__ARR), N_FRQ(2), N_AVBAND, &
     &           NBIT_SAMPLE, NUM_CHAN, L_FRQ(M__ARR), J1, J2, J3, J4, J5, J6, &
     &           N_EFF(2), NB_EFF(MAX4_OBS_BSL,2), IND_QUALCODE, LUN_DEB, L_BAS, I_BAS, IER
      INTEGER*4  CLASS_NGRAMB, CLASS_NPHAMB, CLASS_GDAMBSP, CLASS_IONGDEL, CLASS_EFFFRQ, &
     &           CLASS_DTEC, CLASS_DTEC_SIG
      ADDRESS__TYPE :: ADR_DATA 
      LOGICAL*2  FL_11, FL_SLV, FL_IONO, FL_DTEC, FL_IONO_FREQ, FL_NUSOLVE, &
     &           FL_ALL_READ, FL_GETDB_IONO_DEBUG, FL_REFFR_VAR
      INTEGER*2   INT2_ARG
      INTEGER*4   INT4
      ...
      INTEGER*4, EXTERNAL :: ADD_CLIST, GET_UNIT, ILEN, I_LEN, NSTBA
...
!
     ...
     &         'for observation table' )
      ...
      END IF
!
      FL_GETDB_IONO_DEBUG = .FALSE.
      CALL GETENVAR ( 'SOLVEVAR_GETDB_IONO_DEBUG', STR )
      IF ( STR(1:1) == 'y' .OR. STR(1:1) == 'Y' ) THEN
           FL_GETDB_IONO_DEBUG = .TRUE.
           CALL GETENVAR ( 'SOLVE_EXPORT_DIR',  SOLVE_EXPORT_DIR )
           IF ( ILEN(SOLVE_EXPORT_DIR) == 0 ) THEN
                FIL_IONO_DEBUG_FILE = '/tmp/'//DBNAME_CH//'_getdb_iono.txt'
              ELSE
                FIL_IONO_DEBUG_FILE = TRIM(SOLVE_EXPORT_DIR)//'/'// &
     &                                DBNAME_CH//'_getdb_iono.txt'
           END IF
           LUN_DEB = GET_UNIT()
           OPEN ( UNIT=LUN_DEB, FILE=FIL_IONO_DEBUG_FILE, STATUS='UNKNOWN', &
     &            IOMSG=STR1, IOSTAT=IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 8812, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &              'an attempt to open a debugging ionosphere file '// &
     &               TRIM(FIL_IONO_DEBUG_FILE)//' -- '//TRIM(STR1)//' please '// &
     &              'check environment variable SOLVE_EXPORT_DIR' )
                RETURN
           END IF
      END IF
!
      CALL GETENVAR ( 'SOLVEVAR_REFFR', STR )
      IF ( ILEN(STR) > 0 ) THEN
           READ ( UNIT=STR, FMT='(F15.8)' ) REFFR_VAR
           FL_REFFR_VAR = .TRUE.
         ELSE
           FL_REFFR_VAR = .FALSE.
      END IF
      CALL GETENVAR ( 'SOLVEVAR_PRINT_REF_FREQ', SOLVEVAR_PRINT_REF_FREQ )
      CALL TRAN ( 11, SOLVEVAR_PRINT_REF_FREQ, SOLVEVAR_PRINT_REF_FREQ )
!
...
!
      IF ( FUSED_STATUS == IONOV__LOADED ) THEN
           CALL ERR_PASS      ( IUER, IER )
           CALL GVH_INQ_LCODE ( GVH, 'TEC_APR ', DESCR, CLASS_TEC_APR, TYP, DIMS, &
     &                          NUM_FIELDS, SEG_IND, LEN_REC, LEN_DATA, ADR_DATA, &
     &                          IER )
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 8816, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &               'inquiring lcode TEC_APR' )
                RETURN
           END IF
           IF ( CLASS_TEC_APR == 0 ) THEN
                WRITE ( 6, * ) 'GETDB_FILL_OBORG: Experiment '//DBNAME_CH// &
     &                         ' has FUSED_STATUS loaded, but has no TEC_APR lcode'
           END IF
      END IF
!
      CALL ERR_PASS      ( IUER, IER )
      CALL GVH_INQ_LCODE ( GVH, 'DTEC_SIG', DESCR, CLASS_DTEC_SIG, TYP, DIMS, &
     &                     NUM_FIELDS, SEG_IND, LEN_REC, LEN_DATA, ADR_DATA, &
     &                     IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 8817, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &         'inquiring lcode DTEC_SIG' )
           RETURN
      END IF
!
      IF ( CLASS .NE. 0 ) THEN
...
!
      CALL ERR_PASS      ( IUER, IER )
      CALL GVH_INQ_LCODE ( GVH, 'DTEC    ', DESCR, CLASS_DTEC, TYP, DIMS, &
     &                     NUM_FIELDS, SEG_IND, LEN_REC, LEN_DATA, ADR_DATA, &
     &                     IER )
      IF ( IER .NE. 0 ) THEN
...
!
         ...
         BAS_NAM  = ISITN_CHR(ISITE(1))//'/'//ISITN_CHR(ISITE(2))
         I_BAS    = ABS ( NSTBA ( INT4(ISITE(1)), INT4(ISITE(2)) ) )
         L_BAS = MAX ( I_BAS, L_BAS )
...
!
         ...
         IF ( SOLVEVAR_PRINT_REF_FREQ(1:1) == 'Y' ) THEN
              WRITE ( 6, 210  ) J1, ARR_R8(1), ARR_R8(2) 
 210          FORMAT ( 'GETDB_FILL_OBORG-589 Obs: ', I6, &
     &                 ' Reference frequencies: ', F13.1, 1X, F13.1, ' Hz' ) 
         END IF
         REFFR   = ARR_R8(1)*1.D-6
         REFFR_S = ARR_R8(2)*1.D-6
         IF ( FL_REFFR_VAR ) THEN
              REFFR = REFFR_VAR*1.D-6
         END IF
...
!
     ...
     &                 'getting lcode DEL_RATE' )
              ...
              IF ( N_AVBAND == 1 ) ARR_R8(2) = 0.0D0
!
! ----------- Solve keeps the total phase in degrees -- transform to radians
! ----------- Here ARR_R8         is phase in raidians
! ----------- Here TOTPH, TOTPH_S is phase in degrees
!
...
!
              ...
              IF ( LQUAL_CHR .EQ. ' 0' ) THEN
                 ...
                 ELSE
!
! ---------------- NB: Units: microseconds
!
                   DPH_ORIG =   ARR_R8(1)/(PI2*REFFR)
                   DPH      = ( ARR_R8(1) + PI2*NPHAM4   )/(PI2*REFFR)
!!    write ( 6, * ) 'GETDB_FILL_OBORG-576 J1= ', J1, ' Pha_rad= ', ARR_R8(1), ' REFFR= ', REFFR, ' NPHAM4= ', NPHAM4 ! %%%%%%%%%%%
              END IF
              IF ( LQUAL_S_CHR .EQ. ' 0' ) THEN
                   DPH_ORIG_S = 0.0D0
                   DPH_S      = 0.0D0
...
!
            ...
            ELSE
              IUNWP = 0
         END IF
!!    write ( 6, '(A,I6,A,B32)' ) 'GETDB_FILL_OBORG-875 J1= ', J1, ' USER_SUP= ', USER_SUP ! %%%%%%%%%%%%%%
!
         IF ( N_AVBAND > 1 ) THEN
              IF ( LQUAL_S_CHR == ' 0' .OR. &
     &             LQUAL_S_CHR == ' A' .OR. &
...
!
         IF ( IER .EQ. 0 .AND. ARR_R8(1) > 2.0*1.D6*FRQ_DIF_LIM ) THEN
...
!
              ...
              MEAN_EFF_FREQ(2) = MEAN_EFF_FREQ(2) + REFFREQ
              MEAN_EFF_FREQ(3) = MEAN_EFF_FREQ(3) + PHEFFREQ
              FL_IONO_FREQ = .TRUE.
              IF ( I_BAS .LE. MAX4_OBS_BSL ) THEN
                   NB_EFF(I_BAS,1) = NB_EFF(I_BAS,1) + 1
                   MEAN_BAS_EFF_FREQ(I_BAS,1) = MEAN_BAS_EFF_FREQ(I_BAS,1) + EFFREQ
              END IF
            ELSE
              EFFREQ       = REFFR
              REFFREQ      = REFFR
         ...
         END IF
!
         IF ( IER .EQ. 0 .AND. ARR_R8(4) > 2.0*1.D6*FRQ_DIF_LIM ) THEN
              EFFREQ_XS    = ARR_R8(4)*1.0D-6
              PHEFFREQ_XS  = ARR_R8(5)*1.0D-6
              REFFREQ_XS   = ARR_R8(6)*1.0D-6
!
              N_EFF(2) = N_EFF(2) + 1
              MEAN_EFF_FREQ(4) = MEAN_EFF_FREQ(4) + EFFREQ_XS
              MEAN_EFF_FREQ(5) = MEAN_EFF_FREQ(5) + REFFREQ_XS
              MEAN_EFF_FREQ(6) = MEAN_EFF_FREQ(6) + PHEFFREQ_XS
              IF ( I_BAS .LE. MAX4_OBS_BSL ) THEN
                   NB_EFF(I_BAS,2) = NB_EFF(I_BAS,2) + 1
                   MEAN_BAS_EFF_FREQ(I_BAS,2) = MEAN_BAS_EFF_FREQ(I_BAS,2) + EFFREQ_XS
              END IF
              FL_IONO_FREQ = .TRUE.
            ELSE
              EFFREQ_XS    = REFFR_S
...
!
     ...
     &                 'getting lcode NUSEDCHN for the second station' )
              ...
              IF ( N_FRQ(1) > M__ARR .OR. N_FRQ(1) < 0 ) THEN
                   ...
                   CALL ERR_LOG ( 8868, IUER, 'GETDB_FILL_OBORG', 'Wrong value of '// &
     &                 'lcode NUSEDCHN(1): '//STR )
              ...
              IF ( N_FRQ(2) > M__ARR .OR. N_FRQ(2) < 0 ) THEN
                   ...
                   CALL ERR_LOG ( 8869, IUER, 'GETDB_FILL_OBORG', 'Wrong value of '// &
...
!
     ...
     &                          DIMS(1), DIMS(2), IND_CHN_I2, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8873, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
...
!
     ...
     &                              .TRUE., .FALSE., EFFREQ, PHEFFREQ, REFFREQ )
                   ...
                   END IF
              END IF
              IF ( FL_GETDB_IONO_DEBUG ) THEN
                   WRITE  ( UNIT=STR2_ARR(1), FMT=220 ) FRQ_BAND(1:N_FRQ(1))
                   WRITE  ( UNIT=STR2_ARR(2), FMT=230 ) NOAP(1,1:N_FRQ(1))
                   WRITE  ( UNIT=STR2_ARR(3), FMT=230 ) NOAP(2,1:N_FRQ(1))
                   WRITE  ( UNIT=STR2_ARR(4), FMT=240 ) NOSAMPLE_R8(1,1:N_FRQ(1))
                   WRITE  ( UNIT=STR2_ARR(5), FMT=240 ) NOSAMPLE_R8(1,1:N_FRQ(1))
                   WRITE  ( UNIT=STR2_ARR(6), FMT=250 ) AMP_PHS(1,1:N_FRQ(1))
 220               FORMAT ( 64(F9.3,  1X) )
 230               FORMAT ( 64(I4,    1X) )
 240               FORMAT ( 64(F12.0, 1X) )
 250               FORMAT ( 64(F8.6,  1X) )
                   WRITE ( UNIT=LUN_DEB, FMT=260 ) DBNAME_CH, J1, '1', FL_PIMA, &
     &                              REFFR, EFFREQ, SAMPLE_RATE(1), N_FRQ(1), &
     &                              TRIM(STR2_ARR(1)), &
     &                              TRIM(STR2_ARR(2)), &
     &                              TRIM(STR2_ARR(3)), &
     &                              TRIM(STR2_ARR(4)), &
     &                              TRIM(STR2_ARR(5)), &
     &                              TRIM(STR2_ARR(6))
 260               FORMAT ( A, 2X, I6, 1X, A1, 1X, L1, 2X, 2(F9.3, 1X), 2X, F9.0, &
     &                      2X, I2, 2X, 6(A, 2X) )
              END IF
            ELSE
!
...
!
     ...
     &                          DIMS(1), DIMS(2), ARR_I2, IER )
              IF ( IER .NE. 0 ) THEN
                   ...
                   CALL ERR_LOG ( 8874, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
...
!
     ...
     &                               DIMS(1), DIMS(2), FRQ_SKY_2, IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 8876, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
...
!
     ...
     &                          DIMS(1), DIMS(2), IND_CHN_I2, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR )
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 8878, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
...
!
     ...
     &                     DIMS(1), DIMS(2), ARR_R8(1), IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 8879, IUER, 'GETDB_FILL_OBORG', &
...
!
     ...
     &                     DIMS(1), DIMS(2), ARR_R8(2), IER )
                      IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 8880, IUER, 'GETDB_FILL_OBORG', &
     &                         'Error in getting lcode '//CAL(J4)%NAME// &
...
!
     ...
     &                         .TRUE., .FALSE., EFFREQ_S, PHEFFREQ_S, REFFREQ_S )
              END IF
!
              IF ( FL_GETDB_IONO_DEBUG .AND. N_FRQ(2) > 0 ) THEN
                   WRITE  ( UNIT=STR2_ARR(1), FMT=210 ) FRQ_BAND(1:N_FRQ(2))
                   WRITE  ( UNIT=STR2_ARR(2), FMT=220 ) NOAP(1,1:N_FRQ(2))
                   WRITE  ( UNIT=STR2_ARR(3), FMT=220 ) NOAP(2,1:N_FRQ(2))
                   WRITE  ( UNIT=STR2_ARR(4), FMT=230 ) NOSAMPLE_R8(2,1:N_FRQ(2))
                   WRITE  ( UNIT=STR2_ARR(5), FMT=230 ) NOSAMPLE_R8(2,1:N_FRQ(2))
                   WRITE  ( UNIT=STR2_ARR(6), FMT=240 ) AMP_PHS(1,1:N_FRQ(2))
                   WRITE  ( UNIT=LUN_DEB, FMT=250 ) DBNAME_CH, J1, '2', FL_PIMA, &
     &                                    REFFR_S, EFFREQ_S, SAMPLE_RATE(2), N_FRQ(2), &
     &                                    TRIM(STR2_ARR(1)), &
     &                                    TRIM(STR2_ARR(2)), &
     &                                    TRIM(STR2_ARR(3)), &
     &                                    TRIM(STR2_ARR(4)), &
...
!
         ...
         DEL_BIAS_UL = 0.0D0
         DTEC_FLG    = 0
         IF ( FUSED_STATUS == IONOV__LOADED .AND. CLASS_TEC_APR .NE. 0 ) THEN
              CALL ERR_PASS   ( IUER, IER )
...
!
     ...
     &                          DIMS(1), DIMS(2), DEL_BIAS_UL, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8883, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
...
!
     ...
     &                          DIMS(1), DIMS(2), DTEC_FLG, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8884, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &                 'getting lcodeDBDT_STS' )
                   RETURN
              END IF
         END IF
         IF ( CLASS_DTEC .NE. 0 ) THEN
              CALL GVH_GLCODE ( GVH, 'DTEC    ', J1, 1, 8, &
     &                          DIMS(1), DIMS(2), DTEC_ADJ, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8885, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &                 'getting lcode DTEC' )
                   RETURN
              END IF
         END IF
!
         IF ( CLASS_DTEC_SIG .NE. 0 ) THEN
              CALL GVH_GLCODE ( GVH, 'DTEC_SIG', J1, 1, 8, &
     &                          DIMS(1), DIMS(2), DTEC_ERR, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 8885, IUER, 'GETDB_FILL_OBORG', 'Error in '// &
     &                 'getting lcode DTEC_SIG' )
                   RETURN
              END IF
         END IF
!
...
!
 ...
 410  CONTINUE
      ...
      END IF
      IF ( N_EFF(1) > 0 ) THEN
           MEAN_EFF_FREQ(1) = MEAN_EFF_FREQ(1)/N_EFF(1)
           MEAN_EFF_FREQ(2) = MEAN_EFF_FREQ(2)/N_EFF(1)
           MEAN_EFF_FREQ(3) = MEAN_EFF_FREQ(3)/N_EFF(1)
      END IF
      IF ( N_EFF(2) > 0 ) THEN
           MEAN_EFF_FREQ(4) = MEAN_EFF_FREQ(4)/N_EFF(2)
           MEAN_EFF_FREQ(5) = MEAN_EFF_FREQ(5)/N_EFF(2)
           MEAN_EFF_FREQ(6) = MEAN_EFF_FREQ(6)/N_EFF(2)
      END IF
!
      DO 450 J5=1,L_BAS
         IF ( J5 .LE. MAX4_OBS_BSL ) THEN
              DO 460 J6=1,2
                  IF ( NB_EFF(J5,J6) > 0 ) THEN
                       MEAN_BAS_EFF_FREQ(J5,J6) = MEAN_BAS_EFF_FREQ(J5,J6)/NB_EFF(J5,J6)
                  END IF
 460          CONTINUE 
!!              WRITE ( 6, * ) 'GETDB_FILL_OBORG-1524 ', INT2(J5), ' Bas: ', C_BAS(J5), ' Eff_Freq: ', SNGL(MEAN_BAS_EFF_FREQ(J5,1:2)) ! %%%%%
         END IF
 450  CONTINUE 
      CALL ACS_OBSFIL ( 'C' )
!
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 17-NOV-2019  GETDB_QUALCODE_FIX v1.0 (d) L. Petrov 17-NOV-2019 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'gvh.i'
