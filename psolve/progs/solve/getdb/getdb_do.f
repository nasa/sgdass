...
! *                                                                      *
! *  ### 30-NOV-2005   GETDB_DO   v1.12 (d)  L. Petrov  18-NOV-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'solve.i'
...
!
      ...
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, J9, J10, IS, I, &
...
!
     ...
     &                    VCAT%CONF_FILE )
           ...
           GVF_ENV_DIR = VCAT%GVF_ENV_DIR(IND_REP)
           GVF_DB_DIR  = VCAT%GVF_DB_DIR(IND_REP)
           CALL GETDB_SELECT ( GVF_ENV_DIR, VCAT%GVF_REP_NAME(IND_REP), FILENV, IER )
           IF ( IER .NE. 0 ) THEN
...
!
      ...
      IF ( LEX ) THEN
           IS = UNLINK ( TRIM(HIST_FIL)//CHAR(0) )
      END IF
      IF ( ISEG_SL > 0 ) THEN
