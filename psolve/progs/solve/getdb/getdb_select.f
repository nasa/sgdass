      SUBROUTINE GETDB_SELECT ( GVF_ENV_DIR, REPO, FILENV, IUER )
! ************************************************************************
! *                                                                      *
...
! *                                                                      *
! *  ### 08-DEC-2005   GETDB_SELECT  v3.0 (d) L. Petrov  11-FEB-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      CHARACTER  GVF_ENV_DIR*(*), REPO*(*), FILENV*(*)
      INTEGER*4  IUER
      INTEGER*4  M_ENV
...
!@!
!@! -------- Check the hit key immediately
!@!
!@           CALL SENKRS_MN ( IX, IY, KEY )
!@!
!@! -------- If the entered key is one of CNTR/D, CNTR/X, or ESC -- exit immedately
!@!
!@           IF ( KEY_CH(4:4) == CHAR(27) .OR. &
!@     &          KEY_CH(4:4) == CHAR(4)  .OR. &
!@     &          KEY_CH(4:4) == CHAR(24) .OR. &
!@     &          KEY_CH(4:4) == '['           ) THEN
!@                GOTO 820
!@           END IF
!@           CALL SETCR_MN ( 0, 23 ) 
!@           CALL ADDSTR_F ( 'Database name >> '//KEY_CH(4:4) )
!@           CALL CLRCH ( STR ) 
!@           CALL GETSTR_MN ( STR  )
!@           STR = KEY_CH(4:4)//STR 
!
...
!
           IF ( INDEX ( STR, '['      ) > 0 .OR. &
     &          INDEX ( STR, ']'      ) > 0 .OR. &
     &          INDEX ( STR, CHAR(4)  ) > 0 .OR. &
     &          INDEX ( STR, CHAR(24) ) > 0 .OR. &
     &          INDEX ( STR, CHAR(27) ) > 0      ) THEN
!
! ------------- This is due to escape
!
                CALL SETCR_MN ( 0, 23 ) 
                CALL ADDSTR_F ( 'Database name >>                         ' )
                GOTO 900
...
!@                STR1 = STR
!@                CALL CLRCH ( STR )
!@                STR = STR1(1:I_LEN(STR1))//' not found in repo '//REPO//' ! '// &
!@     &                'Please try again'
!@                CALL ADDSTR_F ( STR(1:79) )
!@                CALL SETCR_MN ( 0, 22 ) 
!@                GOTO 920
 ...
 440            CONTINUE 
                IF ( ILEN(FILENV) > 0 ) THEN
                   ...
                   ELSE 
                     ...
                     STR1 = STR
                     CALL CLRCH ( STR )
                     STR = 'Database '//STR1(1:I_LEN(STR1))// &
     &                     ' is not found in repo '//REPO//'! '// &
     &                     'Please try again'
                     CALL ADDSTR_F ( STR(1:79) )
                     CALL SETCR_MN ( 0, 22 ) 
                     GOTO 920
