...
!                   POST2021
!   pet 2026.02.12  Added support of a new option for LISTING_OPTIONS SEG_STYLE 
!                   POST2026
!   pet 2026.02.15  Made APRIORI_ZENDEL to be always set .TRUE. when LISTING_OPTIONS is POST2026
!
     ...
     &                            'POST2021, POST2024 are acceptable' )
                      ...
                      ELSE IF ( TOKEN .EQ. 'SEG_STYLE' ) THEN
                         ...
                         IF ( TOKEN == 'PRE2005' ) THEN
                           ...
                           ELSE IF ( TOKEN == 'POST2026' ) THEN
                              SEG_LISTING_STYLE = SEG_POST2026_SPOOL__FMT 
                            ELSE 
...
!
     ...
     &                         ' while GIM_TIM_STEP was expected' )
                      ...
                      IF ( INDEX ( TOKEN, '.' ) < 1 ) TOKEN = TRIM(TOKEN)//'.'
                      IF ( IOS .NE. 0 ) THEN
     ...
     &                         'value of the qualifier GIM_TIM_STEP in IONOSPHERIC_MODEL: '// &
     &                         'an real numner in a range of [60, 90000] was expected, but got '//TOKEN )
                           STOP 'BATCH(goutpt)  abnormal termination'
                      END IF
!
...
!    
     ...
     &         ' already exists' )
      ...
      END IF
!
      IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
           APRIORI_ZENDEL = .TRUE.
      END IF
!
