...
!
      ...
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, J11, J12, KZEN, IXC, &
     &           L_PAR, L_PRP, IBC_BRK, IBS_BRK
      REAL*8     JDATE_APR, JDATE_APR_BEG, TOT_ZENDEL, WET_ZENDEL, &
     &           JAPR(MAX_OBS), JAPR_MIN, TOT_ZDEL(MAX_OBS), WET_ZDEL(MAX_OBS), &
     &           TOT_COEF(MAX_OBS), WET_COEF(MAX_OBS), WORK(MAX_OBS), SCALE_FACTOR
...
!
      IF ( KBATCH .AND. KGLOBALS ) THEN
           IF ( RCOND_CGM .GT. COND_WARNING .AND. COND_WARNING .GT. 1.0D0 ) THEN
                WRITE ( 23, 118 )
 118            FORMAT ( 'WARNING!!! Condition number of the CGM is too ', &
 ...
 113                           FORMAT ( I5, ". ", A, 1X, A2, I2, 1X, &
     &                                  A, 12X, (F15.3,A), 2(F13.3,A) )
                               IF ( IER .NE. 0 ) THEN
                                    ...
                                    RETURN 
                               END IF
                             ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                               CALL ERR_PASS ( IUER, IER )
                               WRITE ( STR, 113 ) NPARM, ISITN_CHR(ISTA),  &
     &                                 LET, J2, JD_TO_DATE ( JD_USE-32.184/86400.0D0, IER ), &
     &                                 CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
 115                           FORMAT ( I5, ". ", A, 1X, A2, I2, 1X, &
     &                                  A, 12X, (F15.3,A), 2(F13.3,A) )
                               IF ( IER .NE. 0 ) THEN
                                    WRITE ( 6, * ) ' JD_USE = ', JD_USE 
                                    CALL ERR_LOG ( 3917, IUER, 'A1JST', 'Wrong date' ) 
                                    RETURN 
                               END IF
                          END IF
...
!
     ...
     &                                            IER )
                  IF ( IER .NE. 0 ) THEN
                       CALL ERR_LOG ( 3918, IUER, 'A1JST', 'Error in '// &
...
!
     ...
     &                            CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
                        ...
                        ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                          CALL ERR_PASS ( IUER, IER )
                          WRITE ( STR, 115 ) NPARM, ISITN_CHR(ISTA),  &
     &                            LET, J2, &
     &                            JD_TO_DATE ( JDATE_CLO(J3)-32.184/86400.0D0, IER ), &
     &                            CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
                          IF ( IER .NE. 0 ) THEN
...
!
     ...
     &                                       CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
                                       IF ( IER .NE. 0 ) THEN
                                            WRITE ( 6, * ) ' FJDCL(KCLOCK) = ', FJDCL(KCLOCK)
                                            CALL ERR_LOG ( 3922, IUER, 'A1JST', 'Wrong date' ) 
                                            RETURN 
                                       END IF
                                  END IF
                                  WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
...
4202                                    FORMAT ( I5,". ",4A2,1X,"CL",I2," ", &
     ...
     &                                       CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
                                       IF ( IER .NE. 0 ) THEN
                                            WRITE ( 6, * ) ' FJDCL(KCLOCK) = ', FJDCL(KCLOCK)
                                            CALL ERR_LOG ( 3923, IUER, 'A1JST', 'Wrong date' ) 
                                            RETURN 
                                       END IF
                                     ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                                       LET = 'CL'
     ...
     &                                       CORR, UNIT, XSIG, UNIT, XSSIG, UNIT
                                       IF ( IER .NE. 0 ) THEN
                                            WRITE ( 6, * ) ' FJDCL(KCLOCK) = ', FJDCL(KCLOCK)
                                            CALL ERR_LOG ( 3924, IUER, 'A1JST', 'Wrong date' ) 
                                            RETURN 
                                       END IF
                                  ENDIF
                                  WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
...
!
                 ...
                 OPEN ( UNIT=61, FILE=FNAME, STATUS='OLD', IOSTAT=IOS4, IOMSG=STR )
                 I61 = IOS4
                 IF ( I61 .NE. 0 ) THEN
                      CALL ERR_LOG ( 3927, IUER, 'A1JST', 'Error '// &
     &                    'during attempt to open file '//FNAME(1:I_LEN(FNAME))// &
     &                    ' with a priori zenith '// &
     &                    'path delay created by CRES -- '//STR )
                      RETURN
                 END IF
!
! -------------- Skip the first line
...
!
     ...
     &                                                     TOT_ZENDEL, WET_ZENDEL
 117                FORMAT ( A8, 1X, F16.8, 1X, D14.6, 1X, D14.6 )
                    I61 = IOS4
                    IF ( I61 .EQ. -1 ) GOTO 844
                    IF ( I61 .NE.  0 ) THEN
                         WRITE ( 6, * ) ' J4=',J4,' J61=',I61
                         CALL ERR_LOG ( 3928, IUER, 'A1JST', &
     &                       'Error during attempt to read file '// &
     &                        FNAME(1:I_LEN(FNAME))//' with a priori zenith '// &
     &                       'path delay created by CRES' )
                         RETURN
                    END IF
                    IF ( STR(1:8) .EQ. ISITN_CHR(ISTA) .AND. &
     &                   DABS(TOT_ZENDEL) .GE. 1.D-14  .AND. &
     &                   DABS(WET_ZENDEL) .GE. 1.D-14        ) THEN
!
! ---------------------- The line is related to the station under investigation
!
                         IF ( KZEN .EQ. 0 ) THEN
...
!
                         ...
                         TOT_ZDEL(KZEN) = TOT_ZENDEL
                         WET_ZDEL(KZEN) = WET_ZENDEL
                         IF ( KZEN .GT. 1 ) THEN
!
...
!
                 CALL ERR_PASS ( IUER, IER )
                 CALL MAKE_SPLINE ( 1, KZEN, JAPR, TOT_ZDEL, 0.0D0, 0.D0, &
     &                              TOT_COEF, WORK, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 3929, IUER, 'A1JST', &
     &                    'Error during attempt to compute coefficients '// &
     &                    'of cubic spline for interpolation of a priori '// &
     &                    'total zenith path delay' )
                      RETURN
                 END IF
!
                 CALL ERR_PASS ( IUER, IER )
                 CALL MAKE_SPLINE ( 1, KZEN, JAPR, WET_ZDEL, 0.0D0, 0.D0, &
     &                              WET_COEF, WORK, IER )
                 IF ( IER .NE. 0 ) THEN
                      CALL ERR_LOG ( 3930, IUER, 'A1JST', &
     &                    'Error during attempt to compute coefficients '// &
     &                    'of cubic spline for interpolation of a priori '// &
     &                    'wet zenith path delay' )
                      RETURN
                 END IF
            END IF
!
...
!
     ...
     &                                    JDATE_ATM(J5)-JDATE_APR_BEG )
                            TOT_ZENDEL = FSPL8 ( JDATE_ATM(J5)-JDATE_APR_BEG, &
     &                                           KZEN, JAPR, TOT_ZDEL, IXC, TOT_COEF )
                            WET_ZENDEL = FSPL8 ( JDATE_ATM(J5)-JDATE_APR_BEG, &
     &                                           KZEN, JAPR, WET_ZDEL, IXC, WET_COEF )
!
                            CALL ERR_PASS ( IUER, IER )
                            STR1 = JD_TO_DATE ( JDATE_ATM(J5)-32.184/86400.0D0, IER )
                            IF ( IER .NE. 0 ) THEN
...
!
 ...
 1302                                 FORMAT ( I5, ". ", A8, 1X, A, I2, 1X, &
     ...
     &                                     2(F14.3,' ',A) )
                                   ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                                      IF ( DABS(TOT_ZENDEL)    < 9.99D-8 .AND. &
     &                                     DABS(WET_ZENDEL)    < 9.99D-8 .AND. &
     &                                     DABS(MAT(JB+NPARM)) < 9.99D5  .AND. &
     &                                     DABS(MAT(JS+NPARM)) < 9.99D5  .AND. &
     &                                     DABS(XSSIG)         < 9.99D5        ) THEN
!
                                           WRITE ( STR, 1303 ) NPARM, ISITN_CHR(ISTA),       &
     &                                                         'at', 0, STR1(1:23),          &
     &                                                         TOT_ZENDEL*1.D12,  UNIT(1:7), &
     &                                                         WET_ZENDEL*1.D12,  UNIT(1:7), &
     &                                                         MAT(JB+NPARM),     UNIT(1:7), &
     &                                                         MAT(JS+NPARM),     UNIT(1:7), &
     &                                                         XSSIG,             UNIT(1:7)
 1303                                      FORMAT ( I5, ". ", A8, 1X, A, I2, 1X, &
     &                                              A, 5(2X,F10.3, 1X, A) )
                                        ELSE
                                           WRITE ( STR, 1304 ) NPARM, ISITN_CHR(ISTA),       &
     &                                                         'at', 0, STR1(1:23),          &
     &                                                         TOT_ZENDEL*1.D12,  UNIT(1:7), &
     &                                                         WET_ZENDEL*1.D12,  UNIT(1:7), &
     &                                                         MAT(JB+NPARM),     UNIT(1:7), &
     &                                                         MAT(JS+NPARM),     UNIT(1:7), &
     &                                                         XSSIG,             UNIT(1:7)
 1304                                      FORMAT ( I5, ". ", A8, 1X, A, I2, 1X, &
     &                                              A, 5(2X,1PG10.3, 1X, A), ' NEW' )
                                     END IF
                                END IF
                                WRITE ( 23, '(A)' ) TRIM(STR)
                            END IF
...
!
                            ...
                            IF ( IER .NE. 0 ) THEN
                                 WRITE ( 6, * ) ' JDATE_ATM(J5) = ', JDATE_ATM(J5)
                                 CALL ERR_LOG ( 3933, IUER, 'A1JST', 'Wrong date' ) 
                                 RETURN 
                            END IF
!
                            CALL CLRCH ( STR )
     ...
     &                                   XSSIG,         UNIT(1:7)
                                 WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
                               ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT .AND. &
     &                                   .NOT. ( APRIORI_ZENDEL .AND. SEG_OUTPUT ) ) THEN
                                 CALL ERR_PASS ( IUER, IER )
                                 WRITE ( STR, 1305 ) NPARM, ISITN_CHR(ISTA), &
     &                                   LET, 0, STR1(1:23), &
     &                                   MAT(JB+NPARM), UNIT(1:7), &
     &                                   MAT(JS+NPARM), UNIT(1:7), &
     &                                   XSSIG,         UNIT(1:7)
  1305                           FORMAT ( I5, ". ", A8, 1X, A, I2, 1X, &
     &                                    A, 3(2X,F10.3, 1X, A) )
                                 WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
                            ENDIF
                       ENDIF
...
!
     ...
     &                                   XSSIG,         UNIT(1:7)
                                 WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
                               ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT .AND. &
     &                                   .NOT. APRIORI_ZENDEL ) THEN
                                 IF ( DABS(MAT(JS+NPARM)) < 9.99D5  .AND. &
     &                                DABS(XSSIG)         < 9.99D5        ) THEN
!
                                      WRITE ( STR, 1305 ) NPARM, ISITN_CHR(ISTA),   &
     &                                                    'at', 0, STR1(1:23),      &
     ...
     &                                                    XSSIG,         UNIT(1:7)
                                   ELSE
                                      WRITE ( STR, 1306 ) NPARM, ISITN_CHR(ISTA),   &
     &                                                    'at', 0, STR1(1:23),      &
     &                                                    MAT(JB+NPARM), UNIT(1:7), &
     &                                                    MAT(JS+NPARM), UNIT(1:7), &
     &                                                    XSSIG,         UNIT(1:7)
 1306                                 FORMAT ( I5, ". ", A8, 1X, A, I2, 1X, &
     &                                         A, 3(2X,1PG10.3, 1X, A), ' NEW' )
 ...
 1300                                  FORMAT ( I5, ". ",           &
     ...
     &                                     XSSIG,         UNIT(1:7)
                                       IF ( IER .NE. 0 ) THEN
                                            ...
                                            CALL ERR_LOG ( 3935, IUER, 'A1JST', 'Wrong date' ) 
                                            RETURN 
                                       END IF
                                       WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
                                  END IF
 ...
 1355                         FORMAT ( I5, ". ",A, 1X, A, 3X, &
     ...
     &                                 2(F14.3,' mm     '))
                              IF ( IER .NE. 0 ) THEN
                                   ...
                                   CALL ERR_LOG ( 3936, IUER, 'A1JST', 'Wrong date' ) 
                                   RETURN 
                              END IF
                            ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                              CALL ERR_PASS ( IUER, IER )
                              WRITE ( STR, 1356 ) NPARM, ISITN_CHR(ISTA), 'NG', &
     &                                JD_TO_DATE ( TGRAD(JATM)-32.184/86400.0D0, IER ), &
     &                                FULL_GRAD_STR(10:20), &
     &                                MAT(JB+NPARM), MAT(JS+NPARM), SCSIG(NPARM)
 1356                         FORMAT ( I5, ". ",A, 1X, A, 3X, &
     &                                 A, 2X, &
     &                                 A, 2X, F12.3,' mm     ', &
     &                                 2(F14.3,' mm     '))
                              IF ( IER .NE. 0 ) THEN
                                   WRITE ( 6, * ) ' TGRAD(JATM) = ', TGRAD(JATM)
                                   CALL ERR_LOG ( 3937, IUER, 'A1JST', 'Wrong date' ) 
                                   RETURN 
                              END IF
                         END IF
                         WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
...
!
     ...
     &                                MAT(JB+NPARM), MAT(JS+NPARM), SCSIG(NPARM)
                              IF ( IER .NE. 0 ) THEN
                                   WRITE ( 6, * ) ' TGRAD(JATM) = ', TGRAD(JATM)
                                   CALL ERR_LOG ( 3938, IUER, 'A1JST', 'Wrong date' ) 
                                   RETURN 
                              END IF
                            ELSE IF ( SEG_LISTING_STYLE == SEG_POST2026_SPOOL__FMT ) THEN
                              CALL ERR_PASS ( IUER, IER )
                              WRITE ( STR, 1356 ) NPARM, ISITN_CHR(ISTA), 'EG', &
     &                                JD_TO_DATE ( TGRAD(JATM)-32.184/86400.0D0, IER ), &
     &                                FULL_GRAD_STR(10:20), &
     &                                MAT(JB+NPARM), MAT(JS+NPARM), SCSIG(NPARM)
                              IF ( IER .NE. 0 ) THEN
                                   WRITE ( 6, * ) ' TGRAD(JATM) = ', TGRAD(JATM)
                                   CALL ERR_LOG ( 3939, IUER, 'A1JST', 'Wrong date' ) 
                                   RETURN 
                              END IF
                         END IF
                         WRITE ( 23, '(A)' ) STR(1:I_LEN(STR))
...
!
! --------- Correct insane values of ATMOS_VALUES and ATMOS_SIGMAS
!
            DO 470 J7=1,NUMSTA
               IF ( IS_R8_NAN ( ATMOS_VALUES(J1,J7) ) ) THEN
                    ATMOS_VALUES(J1,J7) = 1.D14
                  ELSE IF ( ATMOS_VALUES(J1,J7) > 1.D14 ) THEN
                    ATMOS_VALUES(J1,J7) = 1.D14
                  ELSE IF ( ATMOS_VALUES(J1,J7) < -1.D14 ) THEN
                    ATMOS_VALUES(J1,J7) = -1.D14
               END IF
!
               IF ( IS_R8_NAN ( ATMOS_SIGMAS(J1,J7) ) ) THEN
                    ATMOS_SIGMAS(J1,J7) = 1.D14
                  ELSE IF ( ATMOS_SIGMAS(J1,J7) > 1.D14 ) THEN
                    ATMOS_SIGMAS(J1,J7) = 1.D14
               END IF
 470        CONTINUE 
...
!
               ...
               IF ( DABS(PVAL) .GT. 1.D10 ) THEN
                    WRITE ( 6, * ) 'PVAL= ',PVAL
                    CALL ERR_PASS ( IUER, IER )
                    CALL ERR_LOG ( 3940, IER, 'A1JST', 'Error '// &
               ...
               IF ( SIND(1).GT.0  .AND.  SIND(2).GT.0 ) THEN
                    IF ( KSPOOL .AND. KPRINT(NPARM) ) THEN
                         ...
                         IF ( IER .NE. 0 ) THEN
                              ...
                              CALL ERR_LOG ( 3941, IUER, 'A1JST', 'Wrong date' ) 
                              RETURN 
                         END IF
                         IF ( SRC_LISTING_STYLE == SRC_POST2021_SPOOL__FMT .OR. &
     &                        SRC_LISTING_STYLE == SRC_POST2024_SPOOL__FMT      ) THEN
...
!
      IF ( KGLOBALS .AND. ( FL_VTD_GLB .OR.  FL_VTD_SES ) ) THEN
           ...
           IF ( IER .NE. 0 ) THEN
                CALL ERR_LOG ( 3942, IUER, 'A1JST', 'Error in an attempt '// &
...
!
! --------------------- Correlation between rignt ascension and 
...
!
! --------------------- Correlation between proper motion in declination and 
! ---------------------                     right ascension
!
...
!
! --------------------- Correlation between declination and 
! ---------------------                     proper motion in right ascension
!
...
!
     ...
     &                          MAT(JB+1), MAT(JA+1), IER )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 3943, IUER, 'A1JST', 'Error in attempt '// &
...
!
     ...
     &                          MAT(JB+1), MAT(JA+1), IER )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 3944, IUER, 'A1JST', 'Error in attempt '// &
...
!
                DO 480 J8=L_SAV,1,-1
                   CALL COPY_V ( 1, VAL_SAV(J8), %VAL(ADR_SAV(J8)) )
 480            CONTINUE
                L_SAV = 0
           END IF
!
...
!
     ...
     &                          INT4(NUMSTA), MAT(JB+1), MAT(JA+1), 1.0D0, IER )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 3945, IUER, 'A1JST', 'Error in attempt '// &
...
!
     ...
     &                          SIT_EST_EPOCH, IER )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 3946, IUER, 'A1JST', 'Error in attempt '// &
