...
!
     ...
     &          FATFIL*128, FINAM_DEBUG*128, SOLVE_DIR_DEBUG*128
      ...
      INTEGER*4 ITMP1, ITMP2, ITMP3, ITMP4, IBAS4, IS, I_BAS, &
     ...
     &          FL_USED_GXS, FL_USED_GS, FL_USED_GX
      ...
      INTEGER*4, EXTERNAL :: GET_UNIT, NSTBA
...
!   pet  2026.02.13  Amended format of ATZExx files: added putting there zenti wet path delay
!
!CCCCC
!
...
!
         ...
         IF ( ILEN(VTD_CONF_USE) > 0  .AND.  .NOT. FL_TPD_READ ) THEN
!
! ----------- Correct insane meteorological data
!
              IF ( IS_R8_NAN(ATMPR(1)) ) ATMPR(1) = 1013.5D0
              IF ( IS_R8_NAN(ATMPR(2)) ) ATMPR(2) = 1013.5D0
              IF ( ATMPR(1) < 100.0    ) ATMPR(1) = 1013.5D0
              IF ( ATMPR(2) < 100.0    ) ATMPR(2) = 1013.5D0
              IF ( ATMPR(1) > 1200.0   ) ATMPR(1) = 1013.5D0
              IF ( ATMPR(2) > 1200.0   ) ATMPR(2) = 1013.5D0
!
              IF ( IS_R8_NAN(TEMPC(1)) ) TEMPC(1) = 14.0D0
              IF ( IS_R8_NAN(TEMPC(2)) ) TEMPC(2) = 14.0D0
              IF ( TEMPC(1) < -99.0D0  ) TEMPC(1) = 14.0D0
              IF ( TEMPC(2) < -99.0D0  ) TEMPC(2) = 14.0D0
              IF ( TEMPC(1) >  70.0D0  ) TEMPC(1) = 14.0D0
              IF ( TEMPC(2) >  70.0D0  ) TEMPC(2) = 14.0D0
!
...
!
              IF ( I == 1 ) THEN
                   ...
                   TEM_AVG(J) = TEM_AVG(J) + TEMPC(1) 
                   PRS_AVG(J) = PRS_AVG(J) + 100.0*ATMPR(1) 
                 ELSE 
                   ...
                   TEM_AVG(J) = TEM_AVG(J) + TEMPC(2) 
                   PRS_AVG(J) = PRS_AVG(J) + 100.0*ATMPR(2) 
              END IF
!
              IF ( NAT(J) == 1 ) THEN
...
!
     ...
     IF ( FL_DEBUG_SBA ) write ( 6, * ) 'cres-11 nobs= ', nobs, ' bas: '//ISITN_CHR(ISITE(1)), ' / ', ISITN_CHR(ISITE(2)), ' freq= ', sngl(freq_gr_x), sngl(freq_gr_s) ! %%%%
         ELSE
...
!
     ...
     &                                    ATM_ZENDEL(J9), TROP_WZD(J9)
 110               FORMAT ( A8, 1X, F16.8, 1X, 1PD14.6, 1X, 1PD14.6 )
 490            CONTINUE
           END IF
!
...
!
                 IF ( FL_DEBUG_SBA ) write ( 6, * ) 'cres-22 DUAL: nobs= ', nobs, ' sou: ', istrn_chr(istar), ' idatyp= ', idatyp, ' cor_tau= ', sngl(cor_tau) ! %%%%
                 CONTINUE 
...
!
     ...
     &                               DEL_BIAS_UL*FREQ_GR_S**2/(FREQ_GR_X**2 - FREQ_GR_S**2)
                 ADDERR_GR_TAU = DSQRT ( ADDERR_GR_TAU**2 + (DTEC_ERR*VIO__CONST/FREQ_GR_X**2)**2 )
                     IF ( FL_DEBUG_SBA ) write ( 6, * ) 'cres-22 H:    nobs= ', nobs, ' sou: ', istrn_chr(istar), ISITN_CHR(ISITE(1)), ' / ', ISITN_CHR(ISITE(2)), ' idatyp= ', idatyp, &
...
!
     ...
     &                               DEL_BIAS_UL*FREQ_GR_X**2/(FREQ_GR_X**2 - FREQ_GR_S**2)
                 ...
                 IF ( FL_DEBUG_SBA ) write ( 6, * ) 'cres-22 L:    nobs= ', nobs, ' sou: ', istrn_chr(istar), ' idatyp= ', idatyp, ' cor_tau= ', sngl(cor_tau), ' dtec_adj= ', sngl(dtec_adj), ' tec_apr= ', sngl(tec_apr), ' DEL_BIAS_UL= ', sngl(del_bias_ul*freq_gr_x**2/(freq_gr_x**2 - freq_gr_s**2)), ' err= ', sngl(dtec_err*vio__const/freq_gr_s**2), ' frq_eff= ', sngl(freq_gr_x), sngl(freq_gr_s) ! %%%%
            END IF
...
!
           ...
           END IF
           CALL ERR_PASS ( IUER, IER )
           STR = MJDSEC_TO_DATE ( MJD_UTC_OBS, TAI_OBS, IER )
           IF ( IER .NE. 0 ) THEN
                WRITE ( 6, * ) 'SECND-2568 ier= ', ier
           END IF
...
!
              IF ( .NOT. IS_R8_NAN(SPR_AVG(J12)) .AND. .NOT. IS_R8_NAN(SPR_RMS(J12)) ) THEN
                   ...
                   TEM_AVG(J12) = TEM_AVG(J12)/NAT(J12)
                   PRS_AVG(J12) = PRS_AVG(J12)/NAT(J12)
