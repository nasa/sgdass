     ...
     &           L_TRP, IEXP_TRP, CSTA_TRP, MJD_TRP, TZEN_APR, WZEN_APR, &
...
! *                              Dimension: M_TRP.                       *
! *     TZEN_APR ( REAL*8   ) -- Array of apriori total atmospheric      *
! *                              zenith path delays in psec.             *
! *                              Dimension: M_TRP.                       *
! *     WZEN_APR ( REAL*8   ) -- Array of apriori wet atmospheric        *
! *                              zenith path delays in psec.             *
! *                              Dimension: M_TRP.                       *
...
! *                                                                      *
! *  ### 15-JUN-1999  GETPAR_PARSE  v17.1 (d) L. Petrov 30-MAR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           C_NPV(M_NPV)*128, STA_BAS(2)*8, COD_STA(MX_STA)*2
      REAL*8     RMS_VAL(M_SES), RMS_IND(M_SES), TZEN_APR(M_TRP), &
     &           WZEN_APR(M_TRP), ADJ_TRP(M_TRP), ERR_TRP(M_TRP), &
     &           TAI_STA_REF, TAI_SOU_REF
...
!
      CHARACTER  NAME*15, J_NAME*16, STR_NUM*32, STR*384, STR1*32, STR2*32, &
     &           STR_LAST*160, COMP*6, DBNAME_DATE*10, DBNAME_SUFFIX*2, &
     &           DATE_CHR*19, SPOOL_FMT_DATE*10, SOLVE_REV_DATE*10
     ...
     &           FL_GETPAR_NOSTA, FL_NO_NSCODE
      LOGICAL*4  LISTOPT_FL, FL_SEG_POST2005, FL_SEG_POST2026, &
     &           FL_SRC_POST2021, FL_SRC_POST2024, FL_APR_ZENDEL, &
...
!
         ...
         END IF
         IF ( INDEX ( STR, 'SEG_STYLE POST2026' ) > 0 ) THEN
              FL_SEG_POST2026 = .TRUE.
         END IF
         ...
         END IF
         IF ( INDEX ( STR, 'APRIORI_ZENDEL YES' ) > 0 ) THEN
              FL_APR_ZENDEL = .TRUE.
         END IF
!
...
!
                   CALL ERR_PASS ( IUER, IER )
                   CALL DFOR_MEN ( STR(58:71), TZEN_APR(L_TRP), IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR_NUM )
                        CALL INCH  ( J1, STR_NUM )
...
!
     ...
     &                           'the limit: '//STR_NUM )
                            RETURN
                        END IF
                        TZEN_APR(L_TRP) = 0.0D0
                        WZEN_APR(L_TRP) = 0.0D0
!
! --------------------- Transform the date
!
                        IF ( FL_SEG_POST2005 .OR. FL_SEG_POST2026 ) THEN
                             DATE_CHR = STR(22:44)
                           ELSE
                             DATE_CHR = '20'//STR(22:35)
                             CALL CHIN ( DATE_CHR(3:4), IVAL )
...
!
                   CALL ERR_PASS ( IUER, IER )
                   IF ( FL_SEG_POST2026 .AND. FL_APR_ZENDEL ) THEN
                        CALL DFOR_MEN ( STR(81:96), ADJ_TRP(L_TRP), IER )
                      ELSE IF ( FL_SEG_POST2026 .AND. .NOT. FL_APR_ZENDEL ) THEN
                        CALL DFOR_MEN ( STR(46:56), ADJ_TRP(L_TRP), IER )
                      ELSE 
...
!
                   IF ( FL_SEG_POST2026 .AND. FL_APR_ZENDEL ) THEN
                        CALL ERR_PASS ( IUER, IER )
                        CALL DFOR_MEN ( STR(46:56), TZEN_APR(L_TRP), IER )
                        IF ( IER .NE. 0 ) THEN
                             CALL CLRCH ( STR_NUM )
                             CALL INCH  ( J1, STR_NUM )
                             CALL ERR_LOG ( 4929, IUER, 'GETPAR_PARSE', 'Error '// &
     &                           'in reading total apriori zenith path delay on '// &
     &                           'line '//STR_NUM(1:I_LEN(STR_NUM))// &
     &                           ' -- wrong format: '//STR(58:71) )
                             RETURN
                        END IF
!
                        CALL ERR_PASS ( IUER, IER )
                        CALL DFOR_MEN ( STR(66:76), WZEN_APR(L_TRP), IER )
                        IF ( IER .NE. 0 ) THEN
                             CALL CLRCH ( STR_NUM )
                             CALL INCH  ( J1, STR_NUM )
                             CALL ERR_LOG ( 4930, IUER, 'GETPAR_PARSE', 'Error '// &
     &                           'in reading wet apriori zenith path delay on '// &
     &                           'line '//STR_NUM(1:I_LEN(STR_NUM))// &
     &                           ' -- wrong format: '//STR(58:71) )
                             RETURN
                        END IF
...
!
                   CALL ERR_PASS ( IUER, IER )
                   IF ( FL_SEG_POST2026 .AND. FL_APR_ZENDEL ) THEN
                        CALL DFOR_MEN ( STR(101:116), ERR_TRP(L_TRP), IER )
                      ELSE IF ( FL_SEG_POST2026 .AND. .NOT. FL_APR_ZENDEL ) THEN
                        CALL DFOR_MEN ( STR(61:76), ERR_TRP(L_TRP), IER )
                      ELSE 
                   ...
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR_NUM )
                        CALL INCH  ( J1, STR_NUM )
                        CALL ERR_LOG ( 4931, IUER, 'GETPAR_PARSE', 'Error '// &
...
!
                   IF ( ILEN(START(N_SES)) .EQ. 0 ) THEN
                        CALL ERR_LOG ( 4932, IUER, 'GETPAR_PARSE', 'No start '// &
...
!
                   ...
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 4933, IUER, 'GETPAR_PARSE', 'Error in '// &
...
!
                   ...
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 4934, IUER, 'GETPAR_PARSE', 'Error in '// &
                                ...
                                ELSE 
                                  IF ( FL_NOMASTER ) THEN
                                     ...
                                     ELSE
                                       WRITE ( 6, * ) 'L_STA = ', L_STA
                                       CALL ERR_PASS ( IUER, IER )
                                       CALL ERR_LOG ( 4936, IUER, 'GETPAR_PARSE', &
...
!
                        READ ( UNIT=STR(24:35), FMT='(F12.5)', IOSTAT=IER, IOMSG=STR1 ) MJD_EOP(N_SES)
                        IF ( IER .NE. 0 ) THEN
                             CALL CLRCH (     STR2 )
                             CALL INCH  ( J1, STR2 )
                             CALL ERR_LOG ( 4937, IUER, 'GETPAR_PARSE', 'Error in parsing '// &
     &                                     'line '//TRIM(STR2)//' -- '//STR1 )
                             RETURN
                        END IF
                        READ ( UNIT=STR(66:77), FMT='(F12.5)', IOSTAT=IER, IOMSG=STR1 ) MJD_NUT(N_SES)
                        IF ( IER .NE. 0 ) THEN
                             CALL CLRCH (     STR2 )
                             CALL INCH  ( J1, STR2 )
                             CALL ERR_LOG ( 4938, IUER, 'GETPAR_PARSE', 'Error in parsing '// &
     &                                     'line '//TRIM(STR2)//' -- '//STR1 )
                             RETURN
                        END IF
                 END IF
...
!
                   IF ( N_LSO .GT. M_LSO ) THEN
                        WRITE ( 6, * ) ' M_LSO = ',M_LSO
                        CALL ERR_LOG ( 4939, IUER, 'GETPAR_PARSE', &
     &                                'M_LSO is too small' )
                        RETURN
                   END IF
!
...
!
                   ...
                   IF ( N_LST .GT. M_LST ) THEN
                        WRITE ( 6, * ) ' M_LST = ',M_LST
                        CALL ERR_LOG ( 4940, IUER, 'GETPAR_PARSE', 'M_LST is '// &
     &                                'too small' )
                        RETURN
                   END IF
...
!
 ...
 810  CONTINUE
      N_LIN = J1-1
      IF ( FL_NO_NSCODE ) THEN
           CALL ERR_LOG ( 4941, IUER, 'GETPAR_PARSE', 'Did not find one '// &
     &         'or more stations in ns-codes.txt file' )
           RETURN 
      END IF
!
      IF ( L_SOU .GT. 0 ) THEN
