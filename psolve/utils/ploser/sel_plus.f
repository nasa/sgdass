      FUNCTION SELSTA_PLUS ( MES, DBOBJ, ISTA_LAST, ISTA, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine  SELSTA_PLUS allows user to chose the station uging DiaGI  *
...
! *                            SELSTA_PLUS or 0 if this call of          *
! *                            SELSTA_PLUS is the first call.            *
! *                                                                      *
! * _________________________ Output parameters: _______________________ *
! *                                                                      *
! * <SELSTA> ( LOGICAL*4 ) -- If .TRUE. -- user has chosen the station.  *
...
! *                                   case of error.                     *
! *                                                                      *
! *  ###  02-JUN-98   SELSTA_PLUS  v1.0  (c)  L. Petrov  02-JUN-98  ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
!!      INCLUDE    '../include/solve.i'
      INTEGER*4  MAX_ARC_SRC, MAX_ARC_STA, MAX_OBS
!@      PARAMETER  ( MAX_ARC_SRC = 600 )
      ...
      LOGICAL*4  SELSTA_PLUS
      INTEGER*4  COL_STA(3,2), ICOL_STA(2), ICOL_SNM, ICOL_NMR, N1$ARG, N2$ARG
!
! --- Define colors
!
     ...
     &           XLF, XRF, YBF, YTF, XCP, YCP, SIFR_H, SIFR_V
      ...
      PARAMETER  ( YBF = 0.02, YTF = 0.91 ) ! Right angle of the frame coord.
      PARAMETER  ( XCP = 0.90, YCP = 0.95 ) ! "Page" text coordinates
      LOGICAL*4  FL_SAVE_1
      CHARACTER  MES*(*), STR*80, CH*1
      INTEGER*4  I1, I2, IPAGE, NPAGE, ID_XW, IR, IRB, IRE, J1, ID, NB, IS, &
     &           INEW_ROW, IOLD_ROW, IER
      INTEGER*4  DIAGI_INBOX, PGOPEN, I_LEN, IDIV
...
!
      ...
      IF ( ID_XW .LE. 0 ) THEN
           CALL CLRCH   (        STR )
           CALL INCH    ( ID_XW, STR )
           CALL ERR_LOG ( 6261, IUER, 'SELSTA_PLUS', 'Error in openning '// &
...
! --- current page (IPAGE)
!
      NPAGE = IDIV ( DBOBJ%U_STA, MBX )
      IPAGE = 1
      IF ( ISTA_LAST .GT. 0  .AND.  ISTA_LAST .LE. DBOBJ%L_STA ) THEN
!
! -------- If the previous station was selected then try to find appropriate
! -------- page whete it was located
!
           IPAGE = IDIV ( ISTA_LAST, MBX )
      END IF
 910  CONTINUE
         CALL PGBBUF()
         CALL PGSAVE() ! 1
...
!
! ------ Setting the DIAGI_Ss of the station rectangulars
!
...
!
         IRB  = 1
         IRE  = DBOBJ%U_STA - (IPAGE-1)*MBX
         IF ( IRE .GT. MBX ) IRE = MBX
 920     CONTINUE
!
...
!
            ...
            CALL PGSLW   ( 5   )
!
! --------- Printing name of the station
!
            CALL PGPTXT  ( 0.125, 0.2, 0.0, 0.0, DBOBJ%C_STA(IS)(1:8) )
!
            CALL CLRCH   ( STR )
            CALL INCH    ( DBOBJ%KU_STA(IS), STR )
            STR='('//STR(1:I_LEN(STR))//')'
            CALL PGSCH   ( 0.6 )
            CALL PGSLW   ( 4   )
            CALL PGSCI   ( ICOL_SNM )
            CALL PGPTXT  ( 0.98, 0.10, 0.0, 1.0, STR(1:I_LEN(STR)) )
!
            CALL CLRCH   ( STR )
            CALL INCH    ( IS, STR )
            CALL PGSCI   ( ICOL_NMR )
            CALL PGPTXT  ( 0.02, 0.67, 0.0, 0.0, STR(1:I_LEN(STR)) )
!
...
! ------------------------------------------------------------------------
!
      SUBROUTINE SELSOU_PLUS ( MES, DBOBJ, IMODE, LSEL_SOU, ISR_LAST, ISR, IUER)
! ************************************************************************
! *                                                                      *
! *   Routine  SELSOU  allows user to chose the source(s) uging          *
...
! *                           causes printing the table of seleceted     *
! *                           sources. If then user click <Left_mouse>   *
! *                           or <Middle_mouse> SELSOU terminates its    *
! *                           work and retuen array LSEL_SOU which holds *
! *                           selection sstatus of the sources. If user  *
...
! *                           k-th source from the general list of       *
! *                           sources has been selscted previously.      *
...
! *                                   case of error.                     *
! *                                                                      *
! *  ###  26-MAY-98     SELSOU     v1.0  (c)  L. Petrov  27-MAY-98  ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
!#      INCLUDE    'solve.i'
...
!@      PARAMETER  ( MAX_ARC_SRC = 768 )
      ...
      INTEGER*4  COL_SOU(3,2), ICOL_SOU(2), ICOL_SNM, ICOL_NMR, N1$ARG, N2$ARG
!
! --- Define colors
!
     ...
     &           XLF, XRF, YBF, YTF, SIFR_H, SIFR_V
      ...
      INTEGER*4  J0, J1, J2, J3, I1, I2, ID, IS, ISS, NBX, IUSE_SOU, IDEV
!
! --- Plotting constants
!
      PARAMETER  ( XCT = 0.50, YCT = 0.95 ) ! Title coordinates
...
!
      INTEGER*4, EXTERNAL :: DIAGI_INBOX, IFIND_PL, PGOPEN, ILEN, I_LEN
      INTEGER*4  IDIV
      IDIV ( I1, I2 ) = I1/I2 + MIN ( 1, MOD ( I1, I2 ) )
!
! --- Setting screen size
!
      ...
      IF ( ILEN(STR) .GT. 0 ) THEN
           ...
           IF ( STR(1:4) .EQ. 'HUGE' ) THEN
                IDEV = 1
              ELSE IF ( STR(1:1) .EQ. '1'     ) THEN
                IDEV = 1
              ELSE IF ( STR(1:3) .EQ. 'BIG' ) THEN
                IDEV = 2
              ELSE IF ( STR(1:1) .EQ. '2'     ) THEN
                IDEV = 2
              ELSE IF ( STR(1:5) .EQ. 'SMALL' ) THEN
                IDEV = 3
              ELSE IF ( STR(1:1) .EQ. '3'     ) THEN
                IDEV = 3
              ELSE
                CALL ERR_LOG  ( 4145, IUER, 'DIAGI_DEF', 'Erroneous '// &
     &              'value of the environment variable DIAGI_SCREEN: '// &
     &               STR(1:I_LEN(STR))//'  -- one of "BIG", "SMALL", '// &
     &               '"HUGE" or "1", "2", "3" was expected' )
                RETURN
           END IF
         ELSE
!
...
!
      ...
      IF ( ID_XW .LE. 0 ) THEN
           CALL CLRCH   (        STR )
           CALL INCH    ( ID_XW, STR )
           CALL ERR_LOG ( 6221, IUER, 'SELSOU_PLUS', 'Error in openning '// &
     &         'the graphic device '//DIAGI_S%DEVICE//' IER='//STR )
           RETURN
      END IF
!
...
!
         ...
         DO 410 J1=ICB,ICE
            DO 420 J2=IRB,IRE
!
! ------------ Find indices of the next source among all sourcees (ISB) and
...
!
                  ...
                  CALL CLRCH   ( STR )
                  CALL INCH    ( DBOBJ%KU_SOU(ISB), STR )
                  STR='('//STR(1:I_LEN(STR))//')'
!
                  CALL PGSCH   ( 0.7 )
                  CALL PGSLW   ( 4   )
...
!
         ...
         END IF
!
         IF ( ( ICHAR(CH) == 220  .OR. YC .GT. YTF ) .AND. IPAGE .GT. 1 ) THEN
              IPAGE = IPAGE - 1
              GOTO 910
         END IF
!
         IF ( ( ICHAR(CH) == 221  .OR. YC .LT. YBF ) .AND. IPAGE .LT. NPAGE ) THEN
              IPAGE = IPAGE + 1
              GOTO 910
         END IF
!
...
! ----------- the box with ISR-th source should be only repainted.
!
              IUSE_SOU = ISR
!
              IF ( ISS .LT. IMODE  .AND.  ISS .LT. MBR ) THEN
