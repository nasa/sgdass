      SUBROUTINE BACKWARD_FILTER ( fjd,z_meas,cov_meas,num_eop,num_meas, &
     &    x_filt,cov_filt,num_parm,proj,kmon)
!
!
! get filter estimates at each epoch, starting from end.
!
...
! now propagate forward until we are done.
      write(*,*) "Doing backward Filter Pass.  Number to do:",num_meas
      do imeas=num_meas,1,-1
...
! Propagate current best guess to next epoch, if there is one.
        IF(imeas .gt. 1) then
          ...
          call propagate(x_filt(1,imeas),cov_filt(1,imeas), &
     &         num_parm, delta_T, x_filt_pred,cov_filt_pred )
        endif
      end do
!
      RETURN
