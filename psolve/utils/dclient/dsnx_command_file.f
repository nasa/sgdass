      SUBROUTINE DSNX_COMMAND_FILE ( DCLIENT, F_CONF, SINEX_FILE, &
     &                               COMMAND_FILE, MESSAGE_FILE, &
     &                               DATA_TYPE, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine  DSNX_COMMAND_FILE  is a part of dclient software.         *
! *   It creates                                                         *
! *   a) a body of e-mail message and write it in a temporary file;      *
! *   b) a command file in the form of C-Shell program which will be     *
! *      executed for processing a request for Sinex File transferring   *
! *      to the Data Server using dserver. It writes this program in     *
! *      a temporary file.                                               *
! *                                                                      *
...
! *    DSNX_FILE ( CHARACTER ) -- Full path name which corresponds to    *
! *                               the listing in Sinex format.           *
! * COMMAND_FILE ( CHARACTER ) -- File name with a C-Shell program for   *
! *                               processing a request for a Sinex file  *
! *                               submission.                            *
! * MESSAGE_FILE ( CHARACTER ) -- File name with a message to be sent to *
! *                               dserver.                               *
! *    DATA_TYPE ( CHARACTER ) -- Either "DSNX" or "ISNX".               *
! *                                                                      *
...
! *                                   case of error.                     *
! *                                                                      *
! * ### 06-JUN-2002 DSNX_COMMAND_FILE v1.2 (c) L. Petrov 21-DEC-2007 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'dclient.i'
      TYPE ( DCLIENT__STRU ) ::  DCLIENT
      CHARACTER  SINEX_FILE*(*), COMMAND_FILE*(*), MESSAGE_FILE*(*), &
     &           DATA_TYPE*(*)
      LOGICAL*4  F_CONF
      INTEGER*4  IUER
      INTEGER*4  M_MAI, MIND
      PARAMETER  ( M_MAI = 16, MIND=16 )
      INTEGER*4  L_MAI, LIND, IND(2,MIND), I11, IB, IE, IP, PID, J1, J2
      CHARACTER  CIS_MAI(M_MAI)*256, OUT*256, DELIM*4, USER_NAME*256, &
     &           USER_REALNAME*256, USER_E_ADDRESS*256, ANS*3, PID_STR*6
      CHARACTER  GET_CDATE*19
      PARAMETER  ( DELIM = CHAR(0)//CHAR(32)//CHAR(9)//',' )
      INTEGER*4, EXTERNAL :: GETPID, ILEN, I_LEN, LINDEX
!
...
!
     ...
     &                         'are you really going to submit '
           WRITE ( 6, '(A$)' ) ' Sinex file '//SINEX_FILE(IB:IE)// &
     &                         ' to the Data Center '// &
...
!
      ...
      IF ( I11 .NE. 0 ) THEN
           CALL ERR_LOG ( 5171, IUER, 'DSNX_COMMAND_FILE', 'Error in '// &
...
! --- Data type
!
      WRITE ( 11, '(A)' ) 'DATA_TYPE:     '//DATA_TYPE(1:I_LEN(DATA_TYPE))
!
! --- URL(s)
!
      WRITE ( 11, '(A)' ) 'URL:           '// &
...
!
                   ...
                   IF ( IP .LE. 0 ) THEN
                        CALL ERR_LOG ( 5172, IUER, 'DSNX_COMMAND_FILE', &
...
!
     ...
     &                THEN
                 ...
                 ELSE
                   ...
                   IF ( IP .LE. 0 ) THEN
                        CALL ERR_LOG ( 5173, IUER, 'DSNX_COMMAND_FILE', &
     &                      'Error in generating a message: field '// &
...
!
      ...
      IF ( I11 .NE. 0 ) THEN
           CALL ERR_LOG ( 5174, IUER, 'DSNX_COMMAND_FILE', 'Error in '// &
...
!
      ...
      WRITE ( 11, '(A)' ) '##################################################'
      WRITE ( 11, '(A)' ) '#'
      WRITE ( 11, '(A)' ) 'set infile = '//SINEX_FILE(1:I_LEN(SINEX_FILE))
...
!
     ...
     &                    ' -s "Submission of '// &
     &                    SINEX_FILE(IB:IE)//' by '//DCLIENT__LABEL// &
     &                    '" '// &
     ...
     &             ' < '//MESSAGE_FILE(1:I_LEN(MESSAGE_FILE))
      ...
      WRITE ( 11, '(A)' ) 'endif'
      WRITE ( 11, '(A)' ) '#'
      WRITE ( 11, '(A)' ) 'echo "=== Request for submitting the Sinex file '// &
     &                     SINEX_FILE(1:I_LEN(SINEX_FILE))//' is sent to '// &
     &                   DCLIENT%DATA_CENTER(1:I_LEN(DCLIENT%DATA_CENTER))//'"'
      WRITE ( 11, '(A)' ) 'exit 0'
!
      CLOSE ( UNIT=11 )
!
      CALL ERR_LOG ( 0, IUER )
      RETURN
      END  SUBROUTINE  DSNX_COMMAND_FILE  !#!#
