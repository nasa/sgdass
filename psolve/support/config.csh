...
# *                                                                      *
# *  ### 17-OCT-2017 config.csh   v1.14 (d)  L. Petrov  17-NOV-2025 ###  *
# *                                                                      *
# ************************************************************************
#
setenv LC_ALL C
setenv LANG   C
set config_revision_date = "2025.11.17"
#
...
if ( -d $FOURPACK_PREFIX == 0 ) then
     echo "psolve config: fourpack directory $FOURPACK_PREFIX was not found"
     exit 1
endif
if ( -d $FOURPACK_PREFIX/lib == 0 ) then
     echo "psolve config: fourpack directory $FOURPACK_PREFIX/lib was not found"
     exit 1
endif
if ( -d $FOURPACK_PREFIX/bin == 0 ) then
     echo "psolve config: fourpack directory $FOURPACK_PREFIX/bin was not found"
     exit 1
endif
if ( -f $FOURPACK_PREFIX/bin/fourpack_vars ) then
      ...
      if ( $FOURPACK_VERSION < $FOURPACK_VERSION_MIN ) then
           echo "psolve config: you have too old library fourpack version $FOURPACK_VERSION"
           echo "psolve config: while $FOURPACK_VERSION_MIN version is needed. Please upgrade"
           exit 1
      endif
endif
$ECHO "ok"
$ECHO "config.csh: Check fftw ... \c" | tee -a  $CONF_LOG
if ( -d $SOLVE_FFTW_DIR == 0 ) then
     echo "psolve config: fftw directory $SOLVE_FFTW_DIR was not found. Please check --with-fftw argument"
     exit 1
endif
if ( -d $SOLVE_FFTW_DIR/lib == 0 ) then
     echo "psolve config: fftw directory $SOLVE_FFTW_DIR/lib was not found. Please check --with-fftw argument"
     exit 1
endif
if ( -d $SOLVE_FFTW_DIR/include == 0 ) then
     echo "psolve config: fftw directory $SOLVE_FFTW_DIR/include was not found. Please check --with-fftw argument"
     exit 1
endif
if ( -f $SOLVE_FFTW_DIR/include/fftw3.f == 0 ) then
...
# --- Check TLE library
#
$ECHO "config.csh: Check tle library... \c" | tee -a  $CONF_LOG
#
if ( -d  $SOLVE_TLE_DIR/include == 0 ) then
     echo "psolve config: tle include directory $SOLVE_TLE_DIR/include was not found"
     echo "psolve config: Please install tle"
     exit 1
endif
#
if ( -f  $SOLVE_TLE_DIR/include/tle_local.i == 0 ) then
     echo "psolve config: tle version file $SOLVE_TLE_DIR/include/tle_local.i was not found"
     echo "psolve config: Please install tle"
     exit 1
endif
set TLE_VERSION = `cat $SOLVE_TLE_DIR/include/tle_local.i | grep "TLE__VERSION   = " | awk '{print substr($5,2,8)}'`
if ( "$TLE_VERSION" < "$TLE_VERSION_MIN" ) then
     echo "Eeeeh. You have tle version $TLE_VERSION, but $TLE_VERSION_MIN is needed."
     echo "Please upgrade."
     exit 1
endif
#
echo "ok" | tee -a  $CONF_LOG
$ECHO "config.csh: Check compiling/linking tle library... \c" | tee -a  $CONF_LOG
$PSOLVE_ROOT/support/check_tle_version.csh >>& $CONF_LOG
if ( $status != 0 ) then
     echo "Failed to link against tle library"     | tee -a  $CONF_LOG
     echo "Please look at temp/conf.log for hints" | tee -a  $CONF_LOG
     echo "SOLVE_TLE_DIR = $SOLVE_TLE_DIR"         | tee -a  $CONF_LOG
     echo "Check file temp.conf for detail"        | tee -a  $CONF_LOG
     exit 1
endif
make -f $PSOLVE_ROOT/support/check_tle_version.mak clean 
echo "ok" | tee -a  $CONF_LOG
#
# Check GVH library
# 
...
if ( $status == 0 ) then
     if ( -f ${SOLVE_SAVE_DATA}/vcat.conf == 0 ) then
          cp -p $SOLVE_ROOT/support/vcat.conf ${SOLVE_SAVE_DATA}/vcat.conf 
          echo "ok vcat.conf is generated from a template" 
        else
#
# ------- vcat file exists. Check whether upgrade is needed
#
...
$ECHO "TLE_LIB            =      $SOLVE_TLE_DIR/lib"            >> $out_file
$ECHO "TLE_INC            =      $SOLVE_TLE_DIR/include"        >> $out_file
...
$ECHO "SOLVE_LIB_VTD      = ${qt}-L$SOLVE_VTD_DIR/lib -lvtd -L$SOLVE_SPD_CLIENT_DIR/lib -lspc -L$SOLVE_NERS_DIR/lib -L$SOLVE_TLE_DIR/lib -ltle -lners${qt}"  >> $out_file
...
$ECHO "FOURPACK_PREFIX    = ${qt}$FOURPACK_PREFIX${qt}"         >> $out_file
$ECHO "SOLVE_FFTW_DIR     = ${qt}$SOLVE_FFTW_DIR${qt}"          >> $out_file
...
$ECHO "setenv TLE_LIB                 $SOLVE_TLE_DIR/lib"        >> $out_vars
$ECHO "setenv TLE_INC                 $SOLVE_TLE_DIR/include"    >> $out_vars
...
$ECHO "setenv SOLVE_LIB_VTD      ${qt}-L$SOLVE_VTD_DIR/lib -lvtd -L$SOLVE_SPD_CLIENT_DIR/lib -lspc -L$SOLVE_NERS_DIR/lib -L$SOLVE_TLE_DIR/lib -ltle -lners${qt}"  >> $out_vars
...
$ECHO "setenv FOURPACK_PREFIX    ${qt}$FOURPACK_PREFIX${qt}"     >> $out_vars
$ECHO "setenv SOLVE_FFTW_DIR     ${qt}$SOLVE_FFTW_DIR${qt}"      >> $out_vars
$ECHO "setenv SOLVE_FFTW_INC     ${qt}-I$SOLVE_FFTW_DIR/include${qt}"  >> $out_vars
$ECHO "setenv SOLVE_FFTW_LIB     ${qt}-L$SOLVE_FFTW_DIR/lib -lfftw3 -lfftw3_omp -lfftw3f -lfftw3f_omp${qt}" >> $out_vars
