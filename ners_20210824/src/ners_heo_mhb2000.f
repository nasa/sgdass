      SUBROUTINE NERS_HEO_MHB2000 ( IPAR, TARG_TDB, UT1_M_TDB, E1, E2, DPSI, DEPS, &
     &                         E1_RATE, E2_RATE, DPSI_RATE, DEPS_RATE, &
     &                         CROSS_NUT_E3  )
! ************************************************************************
! *                                                                      *
! *   Routine  NERS_HEO_MHB2000   computes angles of a small rotation of *
! *   the coordinate transformation from a celestial coordinate system   *
! *   to the terrestrial coordinate system: E1, E2, as well as nutation  *
...
! *                             Unit: rad.                               *
! *                                                                      *
! * ## 06-OCT-2003  NERS_HEO_MHB2000   v3.1 (c) L. Petrov 05-SEP-2006 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'nut_const.i'
...
!
! ----------- Compute the argument of the perturbed rotation
!
...
!
              DCOS_ARG = DCOS(ARG)
              DSIN_ARG = DSIN(ARG)
!!              CALL SINCOS ( %VAL(ARG), DSIN_ARG, DCOS_ARG ) 
!
...
!
              E1 = E1 &
     &           + (PMC_NUT(J1) + 1.D-12*PMC_RATE_NUT(J1)*TARG_TDB)*DCOS_ARG &
     &           + (PMS_NUT(J1) + 1.D-12*PMS_RATE_NUT(J1)*TARG_TDB)*DSIN_ARG
              E2 = E2 &
     &           + (PMC_NUT(J1) + 1.D-12*PMC_RATE_NUT(J1)*TARG_TDB)*DSIN_ARG &
     &           - (PMS_NUT(J1) + 1.D-12*PMS_RATE_NUT(J1)*TARG_TDB)*DCOS_ARG
!
! ----------- ... and their rate of change
!
              E1_RATE = E1_RATE &
     &          -VEL*(PMC_NUT(J1) + 1.D-12*PMC_RATE_NUT(J1)*TARG_TDB)*DSIN_ARG &
     &          + 1.D-12*PMC_RATE_NUT(J1)*DCOS_ARG                             &
     &          +VEL*(PMS_NUT(J1) + 1.D-12*PMS_RATE_NUT(J1)*TARG_TDB)*DCOS_ARG &
     &          + 1.D-12*PMS_RATE_NUT(J1)*DSIN_ARG
!
              E2_RATE = E2_RATE &
     &          +VEL*(PMC_NUT(J1) + 1.D-12*PMC_RATE_NUT(J1)*TARG_TDB)*DCOS_ARG &
     &          + 1.D-12*PMC_RATE_NUT(J1)*DSIN_ARG                             &
     &          +VEL*(PMS_NUT(J1) + 1.D-12*PMS_RATE_NUT(J1)*TARG_TDB)*DSIN_ARG &
     &          - 1.D-12*PMS_RATE_NUT(J1)*DCOS_ARG
...
!
     ...
     &                    MAS_TO_RAD
           E1 = E1*1.D-12   &
     &          + ( PRC_IN  + PRC_IN_RATE*TARG_TDB  )*DCOS_ARG  &
     &          + ( PRC_OUT + PRC_OUT_RATE*TARG_TDB )*DSIN_ARG
           E2 = E2*1.D-12   &
     &          + ( PRC_IN  + PRC_IN_RATE*TARG_TDB  )*DSIN_ARG  &
     &          - ( PRC_OUT + PRC_OUT_RATE*TARG_TDB )*DCOS_ARG
!
           E1_RATE = E1_RATE*1.D-12 &
     &               - VEL*( PRC_IN  + PRC_IN_RATE*TARG_TDB  )*DSIN_ARG  &
     &               + PRC_IN_RATE*TARG_TDB*DCOS_ARG                     &
     &               + VEL*( PRC_OUT + PRC_OUT_RATE*TARG_TDB )*DCOS_ARG  &
     &               + PRC_OUT_RATE*TARG_TDB*DSIN_ARG
!
           E2_RATE = E2_RATE*1.D-12 &
     &               + VEL*( PRC_IN  + PRC_IN_RATE*TARG_TDB  )*DCOS_ARG  &
     &               + PRC_IN_RATE*DSIN_ARG                              &
     &               + VEL*( PRC_OUT + PRC_OUT_RATE*TARG_TDB )*DSIN_ARG  &
     &               - PRC_OUT_RATE*DCOS_ARG
...
!
      RETURN
      END  SUBROUTINE  NERS_HEO_MHB2000   !#!#
