...
! *  character array BUF.                                                *
! *                                                                      *
! *  ###  30-JUN-1991   RD_TEXT   v7.3  (C) Petrov L.  30-DEC-2014 ###   *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      ...
      INTEGER*4  IOS, IS, J1, IER, LEN_STR, LUN, OFFSET, UNIX_DATE, POS1, POS2
...
! *   in the file FINAM.                                                 *
! *                                                                      *
! *  ###  30-JAN-98     WR_TEXT    v2.0  (c)  L. Petrov 16-AUG-2001 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INTEGER*4  LBUF, IUER
...
!
      IF ( FINAM .EQ. '-' ) THEN
           LUN = 6
         ELSE
           LUN = GET_UNIT () ! Get free Fotran I/O unit
...
!
      DO 410 J1=1,LBUF
         WRITE ( UNIT=LUN, FMT='(A)', IOSTAT=IO ) BUF(J1)(1:I_LEN(BUF(J1)))
         IF ( IO .NE. 0 ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( J1, STR )
              CALL CLRCH ( STR1 )
