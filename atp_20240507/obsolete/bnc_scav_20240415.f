...
! ----------------------------------------------------------------------
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! --- Check if the bnc file and the scav file are at least from the same
!     experiment
!     The current naming conversion is such:
!     + for SDE, the binary file may take the name format ssSSSS_orig.bnc
!       where ss is station id and SSSS is the experiment count. In some
!       instances you might tag the pid to the end of the file name, e.g.,
!       ssSSSS_orig.bnc_09999999. The scav file is generally named
!       ssSSSS_scav.anc, with the option to have the pid tagged as well.
!     + for vlbi, we have XXXXXXyy_orig.bnc, and XXXXXXyy_scav.anc, for
!       the binary and scav file, respectively. These also could be tagged
!       with a pid. XXXXXX is the session name, and yy is the station id.
!
      CALL CLRCH ( CBNC  ) 
      CALL CLRCH ( CSCAV )
      IF ( CH_EXP == 'sde' ) THEN
         CALL EXWORD ( FIL_BNC, MIND, LIND, IND, DELIM1, IER )
         CBNC = FIL_BNC( IND(1,LIND):IND(1,LIND) + 5)
! ------
         CALL EXWORD ( FIL_SCAV, MIND, LIND, IND, DELIM1, IER )
         CSCAV = FIL_SCAV( IND(1,LIND):IND(1,LIND) + 5)
! ------
         IF ( CBNC .NE. CSCAV ) THEN
            IUER = -1
            CALL ERR_LOG ( 6013, IUER, 'BNC_SCAV',                      &
     &              'Expected the .bnc and scav files to be from '//    &
     &              'to be from the same experiment. not '//CBNC//      &
     &              ' and '//CSCAV//' ,respectively'  )
            CALL EXIT ( 1 )
         END IF
!
! ---
!
      ELSE
         CALL EXWORD ( FIL_BNC, MIND, LIND, IND, DELIM1, IER )
         CBNC = FIL_BNC( IND(1,LIND):IND(1,LIND) + 7)
! ------
         CALL EXWORD ( FIL_SCAV, MIND, LIND, IND, DELIM1, IER )
         CSCAV = FIL_SCAV( IND(1,LIND):IND(1,LIND) + 7)
! ------
         IF ( CBNC .NE. CSCAV ) THEN
            IUER = -1
...
!#@@#@@!         END IF
!#@@#@@!!
!#@@#@@!! ------ Define path to scav file. We assume that the path is always
!#@@#@@!!        /anc/sde/scav/ssssss_scav.anc
!#@@#@@!!
!#@@#@@!         FIL_PATH = "/anc/sde/scav/"//TRIM(STR1(2))//"_scav.anc"
!#@@#@@!         INQUIRE ( FILE=FIL_PATH, EXIST=LEX )
!#@@#@@!         IF ( .NOT. LEX ) THEN
!#@@#@@!            IUER = -1
!#@@#@@!            CALL ERR_LOG ( 6011, IUER, 'BNC_SCAV',                      &
!#@@#@@!     &              'Scav file is not at '//TRIM(FIL_PATH) )
!#@@#@@!            CALL EXIT ( 1 )
!#@@#@@!         END IF
!#@@#@@!!     
...
!#@@#@@!!
!#@@#@@!! ------ Define path to scav file. We assume that the path is always
!#@@#@@!!        /anc/sde/vlbi/vvvvvvVV_scav.anc
!#@@#@@!!
!#@@#@@!         FIL_PATH = "/anc/vlbi/scav/"//TRIM(STR1(2))//"_scav.anc"
!#@@#@@!         INQUIRE ( FILE=FIL_PATH, EXIST=LEX )
!#@@#@@!         IF ( .NOT. LEX ) THEN
!#@@#@@!            IUER = -1
!#@@#@@!            CALL ERR_LOG ( 6011, IUER, 'BNC_SCAV',                      &
...
! ---------  
 ...
 513        CONTINUE
         ...
         ELSE
!
! --------- Make changes to the provencance block
!
