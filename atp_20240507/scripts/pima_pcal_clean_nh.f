...
! *                                                                         *
! *       IND_STA    =  Station index                      { INT*4 }        *
! *                                                                         *
! *       TIM        =  TIme array                         { REAL*8 }       *
! *                                                                         *
...
! *                                                                         *
! *   OUTPUT:                                                               *
! *       PHAS_AMB   =  Phase ambiguity                    { REAL*8 }       *
! *                                                                         *
! *  ### 8-JUL-2022  PIMA_PCAL_CLEAN    v1.0 (c)  J. SKEENS  8-JUL-2022 ### *
! *                                                                         *
! ***************************************************************************
!
      IMPLICIT   NONE
      ...
      REAL*8,    ALLOCATABLE :: PHAS_SDEL(:,:), PHAS(:,:), AMPL(:,:)
      ...
      INTEGER*4  I1, I2, I3, I4, I5, NP, IND_POL, IND_PLT
      INTEGER*4  M_TONES, IND_TONE, IND_FREQ, N_TONES, I_TONE, IND(2,32)
      INTEGER*4  NO_TON, NPCL, IND_MOD, ITYP, KCHN, NT_USED, I_ABOVE
      INTEGER*4  B_FRQ, E_FRQ, L_FRQ, I_FRQ, U_FRQ, U_FRG, I_FRG
      INTEGER*4  KP, LAST_TON, ITURN, IND_OBS
      INTEGER*4, ALLOCATABLE :: IND_FRQ(:), IND_TON(:)
      INTEGER*4, EXTERNAL :: ILEN, I_LEN, LTM_DIF
      REAL*4,    EXTERNAL :: PIMA_PC_GDEL
      REAL*8     TIM(PIM__MOBS), PIM%STA(IND_STA)%PCAL(1)%NPOI)
      REAL*8     PHAS_AMB(PIM%STA(IND_STA)%PCAL(1)%NO_TONES * PIM%NFRQ
      REAL*8     FREQ_1ST(PIM__MFRQ), PHAS_LAST
!
...
!
     ...
     &     PIM%CONF%ACT_CODE == PIMA__PDPL_CODE            ) THEN 
         IND_POL = 1
      ELSEIF ( ( PIM%CONF%POLAR == PIMA__POLAR_LL .OR.                  &
     &            PIM%CONF%POLAR == PIMA__POLAR_VV      ) .AND.         &
     &          PIM%NPOL == 2 ) THEN
         IND_POL = 2
      ELSEIF ( PIM%CONF%POLAR == PIMA__POLAR_LL .AND.                   &
     &         PIM%NPOL == 1 ) THEN
         IND_POL = 1
      ELSE 
         CALL ERR_LOG ( 8857, IUER, 'PIMA_PCAL_CLEAN',                  &
     &           'Polarization code '//TRIM(PIM%CONF%POLAR)//           &
     &           ' is not supported for plotting pcal. Supported '//    &
     &           'codes: RR, LL, HH, VV' )  
         RETURN 
      END IF
!
...
!
         ...
         ALLOCATE ( AMPL_SDEL(PIM%STA(IND_STA)%PCAL(1)%NO_TONES,        &
     &              PIM%NFRQ), STAT=IER )
         ALLOCATE ( PHAS(NPCL,PIM%STA(IND_STA)%PCAL(1)%NPOI), STAT=IER )
         ALLOCATE ( PHAS_SDEL(PIM%STA(IND_STA)%PCAL(1)%NO_TONES,        &
     &              PIM%NFRQ), STAT=IER )
         ALLOCATE ( FREQ(NPCL), STAT=IER )
         ALLOCATE ( FREQ_SDEL(PIM%STA(IND_STA)%PCAL(1)%NO_TONES,        &
     &              PIM%NFRQ), STAT=IER )
...
!
               FREQ_1ST(I_FRQ) = PIM%STA(IND_STA)%PCAL(U_FRG)%          &
     &                               FREQ(1,U_FRQ,I1) 
               DO 330 I3=1, N_TONES
                  KP = KP + 1
                  FREQ(KP)            = PIM%STA(IND_STA)%PCAL(U_FRG)%   &
     &                                      FREQ(I3,U_FRQ,I1) 
                  FREQ_SDEL(I3,I_FRQ) = PIM%STA(IND_STA)%PCAL(U_FRG)%   &
     &                                      FREQ(I3,U_FRQ,I1) 
                  PHAS(KP,I1)         = PIM%STA(IND_STA)%PCAL(U_FRG)%   &
     &                                      PHAS(I3,U_FRQ,I1,IND_POL)
                  PHAS_SDEL(I3,I_FRQ) =  PIM%STA(IND_STA)%PCAL(U_FRG)%  &
     &                                       PHAS(I3,U_FRQ,I1,IND_POL)
                  AMPL(KP,I1)         = PIM%STA(IND_STA)%PCAL(U_FRG)%   &
     &                                      AMPL(I3,U_FRQ,I1,IND_POL)
                  AMPL_SDEL(I3,I_FRQ) = PIM%STA(IND_STA)%PCAL(U_FRG)%   &
     &                                      AMPL(I3,U_FRQ,I1,IND_POL)
                  IND_FRQ(KP) = I_FRQ
                  IND_TON(KP) = I3
330            CONTINUE 
! ------------
               ...
               PC_GDEL_ARR(I_FRQ) = PIMA_PC_GDEL( N_TONES,              &
     &                                            FREQ_SDEL(1,I_FRQ),   &
     &                                            PHAS_SDEL(1,I_FRQ),   &
     &                                            AMPL_SDEL(1,I_FRQ),   &
     &                                            IER )
               IF ( IER .NE. 0 ) THEN
                  CALL ERR_LOG ( 8859, IUER, 'PIMA_PCAL_CLEAN',         &
     &                    'Failure in computing group delay'    )
                  RETURN
               ENDIF 
320         CONTINUE 
!
...
!
            CALL ERR_PASS ( IUER, IER )
            CALL PIMA_PC_SDEL ( PIM%STA(IND_STA)%PCAL(U_FRG)%NO_TONES,  &
     &                          I_FRQ, FREQ_SDEL, PHAS_SDEL, AMPL_SDEL, &
     &                          PC_GDEL_ARR, PC_SDEL, PC_SDEL_ARR, IER )
! ---------
            IF ( IER .NE. 0 ) THEN
               CALL ERR_LOG ( 8860, IUER, 'PIMA_PCAL_CLEAN',            &
     &                 'Failure in computing single-band group delays' )
               RETURN
            ENDIF     
! ---------
            IF ( PIM%CONF%DEBUG_LEVEL .GE. 6 ) THEN
               WRITE ( 6, 210 ) PIM%C_STA(IND_STA), I1, TIM(IND_STA),   &
                                PC_GDEL_ARR(I_FRQ), PC_SDEL
 210           FORMAT ( 'PIMA_PCAL_CLEAN sta: ', A, ' Epc: ', I5,       &
     &                  ' Tim: ', F8.2, '  PC_Gdel= ', 1PD12.5,         &
     &                  ' PC_Sdel= ', 1PD12.5 )
            END IF
!
...
!
               PHAS_AMB(I4,I1) = PHAS(I4,I1) - PI2*(FREQ(I4) -          &
     &                           FREQ_1ST(I_FRQ))*                      &
     &                           PC_SDEL_ARR(IND_FRQ(I4))
!
! ------------ Resolve phase ambiguity
!
               PHAS_AMB(I4,I1) = PHAS_AMB(I4,I1) -                      &
     &                           PI2*IDNINT(PHAS_AMB(I4,I1)/PI2)
! ------------
               IF ( AMPL(I4,I1) > 0.0 ) THEN
!
! --------------- Now we try to resolve remaining ambiguity
! --------------- N.B.: Pcal is not masked out
!
...
!
      ...
      CHARACTER  STR*32, STR1*32
      INTEGER*4  MODE, IND_STA, ISTA_LAST, ICODE, IFRQ, NC, NR, NFRQ
      INTEGER*4  J1, J2, J3, I1, I2, I3, I4, I5, NP, IND_POL, IND_PLT
      INTEGER*4  M_TONES, IND_TONE, IND_FREQ, N_TONES, I_TONE, IND(2,32)
      INTEGER*4  I_ABOVE, NO_TON, NPCL, IND_MOD, ITYP, KCHN, NT_USED
      INTEGER*4  B_FRQ, E_FRQ, L_FRQ, I_FRQ, U_FRQ, U_FRG, I_FRG, NPOI
      INTEGER*4  IND_OBS, KP, LAST_TON, ITURN, NPOI_ACCUM, NUM_ACCUM
      INTEGER*4, ALLOCATABLE :: IND_FRQ(:), IND_TON(:)
      INTEGER*4, EXTERNAL :: ILEN, I_LEN, LTM_DIF
      REAL*4,    EXTERNAL :: PIMA_PC_GDEL
      REAL*8     TIM(PIM__MOBS)
      REAL*8     PHAS_AMB(PIM%STA(IND_STA)%PCAL(1)%NO_TONES * PIM%NFRQ
      REAL*8     PIM%STA(IND_STA)%PCAL(1)%NPOI), FREQ_1ST(PIM__MFRQ)
      REAL*8     PHAS_LAST, AMP_MIN, T8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     P8(PIM%NPCT,MP,PIM%NFRQ), A8(PIM%NPCT,MP,PIM%NFRQ)
! ---
      CALL CHIN ( PIM%CONF%PHAS_CAL_CODE, IND_TONE ) ! Set IND_TONE
!
      IND_FREQ = PIM%CONF%BEG_FRQ
     ...
     &          PIM%NPOL == 1 ) THEN
         IND_POL = 1
      ELSE 
         CALL ERR_LOG ( 8862, IUER, 'PIMA_PCAL_CLEAN8',                 &
     &           'Polarization code '//TRIM(PIM%CONF%POLAR)//           &
     &           ' is not supported for plotting pcal. Supported '//    &
     &           'codes: RR, LL, HH, VV' )  
            RETURN 
...
! ------------
     ...
     &                                             IER )
               IF ( IER .NE. 0 ) THEN
                  CALL ERR_LOG ( 8864, IUER, 'PIMA_PCAL_CLEAN8',        &
     &                    'Failure in computing group delay' )
                  RETURN
               ENDIF 
320         CONTINUE 
...
! ---------
            IF ( IER .NE. 0 ) THEN
               CALL ERR_LOG ( 8865, IUER, 'PIMA_PCAL_CLEAN8',           &
     &                 'Failure in computing single-band group delays' )
               RETURN
            ENDIF    
! ---------
            IF ( PIM%CONF%DEBUG_LEVEL .GE. 8 ) THEN
               WRITE ( 6, 210 ) PIM%C_STA(IND_STA), I1, TIM(IND_STA),   &
     &                          PC_GDEL_ARR(I_FRQ), PC_SDEL
 210           FORMAT ( 'PIMA_PCAL_CLEAN8 sta: ', A, ' Epc: ', I5,      &
     &                  ' Tim: ', F8.2, '  PC_Gdel= ', 1PD12.5,         &
     &                  ' PC_Sdel= ', 1PD12.5 )
            END IF
!
...
!
     ...
     &                           NPOI_ACCUM, FREQS, IND_TONE_FREQ,      &
     &                           T8, P8, A8, Y8, IUER )
...
! *                         Default, IUER = -1                                  *
! *                                                                             *
! *  ### 8-JUL-2022  PIMA_PCAL_AVG         v1.0 (c)  J. SKEENS  8-JUL-2022 ###  *
! *                                                                             *
! *******************************************************************************
      ...
      REAL*8     AMP_MIN, T8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     P8(PIM%NPCT,MP,PIM%NFRQ), A8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     Y8(PIM%NPCT,MP,PIM%NFRQ), PHS_DIF, T_ABOVE(NPOI_ACCUM)
      REAL*8     Y_ABOVE(NPOI_ACCUM), Y_BACKUP(NPOI_ACCUM)
      REAL*8     T_POS(NPOI_ACCUM), PHASE_POS(NPOI_ACCUM)
      CHARACTER  PREF_NAME*128,MES_SELSTA*80, STA_STR*8
      CHARACTER  STR*32, STR1*32
      INTEGER*4  MODE, ISTA_LAST, ICODE, IFRQ, NC, NR, NFRQ
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, J9, K1, K2, NP, IND_POL
      INTEGER*4  M_TONES, IND_TONE, IND_FRQ, N_TONES, I_TONE, IND(2,32)
      INTEGER*4  I_ABOVE, I_NONZERO, LIND, IER, NPT, I_POS, REF_FREQ
      INTEGER*4  NPOI_ACCUM, NUM_ACCUM, NPOI, IND_TONE_FREQ(PIM__MTON)
...
! ---
     ...
     &          PIM%NPOL == 1 ) THEN
         IND_POL = 1
      ELSE
         CALL ERR_LOG ( 8866, IUER, 'PIMA_PCAL_AVG',                    &
     &           'Polarization code '//TRIM(PIM%CONF%POLAR)//           &
     &           ' is not supported for plotting pcal. Supported '//    &
     &           'codes: RR, LL, HH, VV' )  
         RETURN 
...
! ---------------
                  IF ( J2 == 1 ) THEN
                     FREQS(I_TONE,IFRQ) = PIM%STA(IND_STA)%             &
     &                                        PCAL(PIM%CONF%FRQ_GRP)%   &
     &                                        FREQ(J3,IND_FRQ,J2)
                  END IF
!
...
!
                  IF ( NUM_ACCUM .EQ. 0 ) THEN ! No averaging
                     T8(I_TONE,NP,IFRQ) = PIM%STA(IND_STA)%             &
     &                                        PCAL(PIM%CONF%FRQ_GRP)%   &
     &                                        TIME_MID_R8(J2)
                     P8(I_TONE,NP,IFRQ) = PIM%STA(IND_STA)%             &
     &                                       PCAL(PIM%CONF%FRQ_GRP)%    &
     &                                       PHAS(J3,IND_FRQ,J2,IND_POL)
                     A8(I_TONE,NP,IFRQ) = PIM%STA(IND_STA)%             &
     &                                       PCAL(PIM%CONF%FRQ_GRP)%    &
     &                                       AMPL(J3,IND_FRQ,J2,IND_POL)
                  ELSE IF ( NP * NUM_ACCUM <  NPOI ) THEN ! Averaging
                     POS_INDS = PIM%STA(IND_STA)%                        &
     &                           PCAL(PIM%CONF%FRQ_GRP)%                 &
     &                           AMPL(J3,IND_FRQ,                        &
     &                                (J2-1)*NUM_ACCUM + 1:J2*NUM_ACCUM, &
     &                                IND_POL) > PIMA__AMP_MIN
                     T8(I_TONE,NP,IFRQ) = SUM(PIM%STA(IND_STA)%         &
     &                                         PCAL(PIM%CONF%FRQ_GRP)%  &
     &                                         TIME_MID_R8(             &
     &                                              (J2-1)*NUM_ACCUM +  &
     &                                              1:J2 * NUM_ACCUM))  &
...
! ------------------
                     IF ( I_POS > 0 ) THEN
                        CALL ERR_PASS ( IUER, IER )
                        CALL AMBIG_RESOLVE ( ARA2__PHD, I_POS, T_POS,   &
     &                                       PHASE_POS, IER)
                        IF ( IER .NE. 0 ) THEN
...
! ------------------
                     DO 450 J5=1,NUM_ACCUM
                        IF ( POS_INDS(J5) .EQV. .TRUE. ) THEN
                           I_POS = I_POS + 1
                           P8(I_TONE,NP,IFRQ) = P8(I_TONE,NP,IFRQ) +    &
     &                                          PHASE_POS(I_POS) 
                           A8(I_TONE,NP,IFRQ) =                         &
                                        A8(I_TONE,NP,IFRQ) +            &
     &                                  PIM%STA(IND_STA)%               &
...
! ------------------
                  ...
                  ELSE          ! NP * NUM_ACCUM >=  NPOI - special logic in last averaging interval
                     POS_INDS(1:NPOI - (J2-1) * NUM_ACCUM) =            &
     &                                  PIM%STA(IND_STA)%               &
     &                                    PCAL(PIM%CONF%FRQ_GRP)%       &
     &                                    AMPL(J3,IND_FRQ,              &
...
!
                  IF ( IFRQ /= REF_FREQ .AND.                           &
     &                 A8(1,NP,IFRQ) > PIMA__AMP_MIN .AND.              &
     &                 A8(1,NP,1)    > PIMA__AMP_MIN      ) THEN
                     Y8(I_TONE,NP,IFRQ) =  P8(I_TONE,NP,IFRQ) -         &
     &                                     P8(I_TONE,NP,REF_FREQ) 
                     IF ( NP > 1 ) THEN
                        PHS_DIF = Y8(I_TONE,NP,IFRQ) -                  &
     &                            Y8(I_TONE,NP-1,IFRQ) 
                        PHS_DIF = PHS_DIF - PI2*IDNINT(PHS_DIF/PI2)
                        Y8(I_TONE,NP,IFRQ) = Y8(I_TONE,NP-1,IFRQ) +     &
     &                                       PHS_DIF
                     ENDIF
                  ENDIF
    430        CONTINUE
...
!
            Y_ABOVE = 0.0D0
!
! --------- Unwrapping must be done very carefully here, too, as jumps
!           are identified in PIMA_GEPM, and tones will be deactivated
!           if too many are present Using similar logic to POS_INDS
!           above, INDS_ABOVE finds valid epochs.
!
            I_TONE = 0
            DO 431 J3 = 1, M_TONES
               I_TONE = I_TONE + 1
! ------------
               IF ( IFRQ /= REF_FREQ ) THEN
                  I_ABOVE = 0
                  INDS_ABOVE = A8(I_TONE,1:NPOI_ACCUM,IFRQ) >           &
     &                         PIMA__AMP_MIN
! ---------------
     ...
     &                                 Y_ABOVE, IER )
!
! --------------- Use a backup method of unwrapping if the first 
!                 try fails
!
...
! ---------------
     ...
     &                       'Failure in resolving time unwrapped '//   &
     &                       'phase ambiguity')
                     RETURN
                  ENDIF
!
...
&                                  NUM_ACCUM, NPOI_ACCUM, FREQS, T8, P8, &
&                                  A8, BPAS_PHASE, IUER )
! ************************************************************************
! *                                                                      *
...
! *     BPAS_PHASE   =
! *                                                                         *
! *  ### 8-JUL-2022  PIMA_PCAL_BPASS   v1.0 (c)  J. SKEENS  8-JUL-2022 ###  *
! *                                                                         *
! ***************************************************************************
!
      IMPLICIT   NONE
      ...
      REAL*8     BPAS_AMP(PIM__MTON), AMP_MIN, T8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     P8(PIM%NPCT,MP,PIM%NFRQ), A8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     PHS_DIF
      CHARACTER  PREF_NAME*128, STR*32, STR1*32
      CHARACTER  MES_SELSTA*80, STA_STR*8
      INTEGER*4  MODE, ISTA_LAST, ICODE, IFRQ, NC, NR, NFRQ
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, J9, NP, IND_POL
      INTEGER*4  M_TONES, IND_TONE, IND_FRQ, N_TONES, I_TONE, IND(2,32)
      INTEGER*4  I_NONZERO, LIND, IER, NPT, POS_NUM, I_ABOVE
      INTEGER*4  KP, NPOI_ACCUM, NUM_ACCUM, NPOI
...
! ---
     ...
     &         PIM%NPOL == 1 ) THEN
         IND_POL = 1
      ELSE
         CALL ERR_LOG ( 8871, IUER, 'PIMA_PCAL_BPASS',                  &
     &           'Polarization code '//TRIM(PIM%CONF%POLAR)//           &
     &           ' is not supported for plotting pcal. Supported '//    &
     &           'codes: RR, LL, HH, VV' )  
         RETURN 
...
!
         DO 410 J1=1,NPOI_ACCUM
            NP = NP + 1
            BPAS_PHASE(1:M_TONES) = BPAS_PHASE(1:M_TONES) +             &
     &                              P8(1:M_TONES,NP,IFRQ)
            BPAS_AMP(1:M_TONES)   = BPAS_AMP(1:M_TONES)   +             &
...
!
         ...
         BPAS_PHASE(1:M_TONES) = BPAS_PHASE(1:M_TONES) -                &
     &                           SUM(BPAS_PHASE(1:M_TONES))/M_TONES
         BPAS_AMP(1:M_TONES)   = BPAS_AMP(1:M_TONES)/NP
         BPAS_AMP(1:M_TONES)   = BPAS_AMP(1:M_TONES) -                  &
     &                           SUM(BPAS_AMP(1:M_TONES))/M_TONES
         NP = 0
         DO 420 J2=1,NPOI_ACCUM
            NP = NP+1
!
...
!
            IF ( ALL ( A8(1:M_TONES,NP,IFRQ) >  AMP_MIN ) ) THEN
               P8(1:M_TONES,NP,IFRQ) = P8(1:M_TONES,NP,IFRQ) -          &
     &                                 BPAS_PHASE(1:M_TONES)
               A8(1:M_TONES,NP,IFRQ) = A8(1:M_TONES,NP,IFRQ) -          &
     &                                 BPAS_AMP(1:M_TONES)
!
! --------- Identify which amplitude values are below threshold, exclude
!
            ELSE
               I_ABOVE = 0
 ...
 430           CONTINUE
            ENDIF
 420     CONTINUE
         !IF ( IND_STA == 7 .AND. IFRQ == 4) THEN
         !   CALL DIAGI_1(M_TONES,FREQS(1,IFRQ),BPAS_PHASE,IER)
         !   CALL DIAGI_1(M_TONES,FREQS(1,IFRQ),BPAS_AMP,IER)
         !   CALL DIAGI_2(NPOI_ACCUM,T8(2,1:NPOI_ACCUM,IFRQ),P8(2,1:NPOI_ACCUM,IFRQ)-BPAS_PHASE(2),&
         !&             NPOI_ACCUM,T8(3,1:NPOI_ACCUM,IFRQ),P8(3,1:NPOI_ACCUM,IFRQ)-BPAS_PHASE(3),IER)
         !ENDIF 
      ENDIF
      CALL ERR_LOG ( 0, IUER )
      RETURN
