...
! *                           model and some important intermediate      *
! *                           quantities. Dimension: DER__NDER.          *
! *                           Symbolic names and meaning of the indexes  *
! *                           of the elements of DER_DEL are defined     *
! *                           in vtd.i .                                 *
! *  DER_RAT ( REAL*8    ) -- Vector of partial derivatives of phase     *
! *                           delay rate with respect to parameters of   *
! *                           the model and rate of change of some       *
...
! *                                                                      *
! *  ### 27-JAN-2004   VTD_DELAY v3.21 (d)   L. Petrov  31-MAR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &                    VTD%STA(ISTA(J3))%IVS_NAME//' for epoch '// &
     &                    STR(1:30)//' TAI'  )
                      RETURN
                 END IF
!
...
!
     ...
     &                  VTD%STA(ISTA(J3))%IVS_NAME//' on epoch '//STR(1:30)// &
     &                  ' TAI' )
                   RETURN
              END IF
            ELSE
...
!
! -------- Vector to the northen pole of the celestial coordinate system
!
           Z_VEC(1) =  VTD%MOM%YPL
           Z_VEC(2) = -VTD%MOM%XPL
           Z_VEC(3) =  1.0D0
...
!
      ...
      DER_DEL(VTD__TEC1)     = VTD%STA(ISTA(1))%TEC * VTD%STA(ISTA(1))%IONO_MAP
      DER_DEL(VTD__TEC2)     = VTD%STA(ISTA(2))%TEC * VTD%STA(ISTA(2))%IONO_MAP
!
