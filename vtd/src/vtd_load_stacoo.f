...
! *                                                                      *
! * ### 26-JAN-2004  VTD_LOAD_STACOO  v2.6 (d) L. Petrov 11-AUG-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'vtd.i'
     ...
     &           '$$  SIT-MODFILE Format 2001.09.26 ' )
      LOGICAL*4  LEX
      REAL*8     TAI_REF, TIM
      INTEGER*4  NBUF, J1, J2, J3, J4, J5, J6, IOS, IYR, I_STA, &
     &           USE_STA(VTD__M_STA), NUM_ERR, MJD_REF, I_FMT, IER
      INTEGER*4, EXTERNAL :: ILEN, I_LEN
!
      IF ( VTD%L_STA == 0 ) THEN
           IF ( VTD%STA(1)%STA_INP == 'F' ) THEN
                CONTINUE 
              ELSE
...
!
              ...
              I_STA = 0
              IF ( VTD%STA(1)%STA_INP .NE. 'F' ) THEN
 ...
 420               CONTINUE 
                 ELSE
!
! ---------------- A special case: station list is not defined and is taken from
! ---------------- the station position file
!
                   VTD%L_STA = VTD%L_STA + 1
                   I_STA     = VTD%L_STA 
                   VTD%STA(VTD%L_STA)%IVS_NAME = BUF(J1)(11:18) 
              END IF
!
...
!
     ...
     &                                 VTD%STA(I_STA)%MJD_EPC(VTD%STA(I_STA)%N_EPC), &
     &                                 VTD%STA(I_STA)%TAI_EPC(VTD%STA(I_STA)%N_EPC), IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL CLRCH ( STR ) 
                        CALL INCH  ( J1, STR ) 
...
!
     ...
     &               VTD%STA(I_STA)%COO_TRS(1,VTD%STA(I_STA)%N_EPC)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2149, IUER, 'VTD_LOAD_STACOO', 'Error '// &
     &                 'in parsing X-coordinate field of line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
...
!
     ...
     &                 'in parsing Y-coordinate field of line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
...
!
     ...
     &                 'in parsing Z-coordinate field of line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
...
!
              IF ( ILEN(BUF(J1)(205:214)) > 0 ) THEN
                   CALL ERR_PASS ( IUER, IER )
                   CALL DATE_TO_TIME ( BUF(J1)(205:214), VTD%STA(I_STA)%MJD_DA_BEG, &
     &                                 TIM, IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 2152, IUER, 'VTD_LOAD_STACOO', 'Error '// &
     &                      'in parsing Date_beg field of line '//STR(1:I_LEN(STR))// &
     ...
     &                       VTD%CONF%FINAM_STACOO )
              ...
              END IF
!
              IF ( ILEN(BUF(J1)(226:235)) > 0 ) THEN
                   CALL ERR_PASS ( IUER, IER )
                   CALL DATE_TO_TIME ( BUF(J1)(226:235), VTD%STA(I_STA)%MJD_DA_END, &
     &                                 TIM, IER )
                   IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 2153, IUER, 'VTD_LOAD_STACOO', 'Error '// &
     &                      'in parsing Date_end field of line '//STR(1:I_LEN(STR))// &
     &                      ' of the station coordinate file '// &
     &                       VTD%CONF%FINAM_STACOO )
                        RETURN 
                   END IF
...
! ----------- Parsing X-error
!
              READ ( UNIT=BUF(J1)(50:59), FMT='(F10.3)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(1)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2154, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing X-error filed in line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
              READ ( UNIT=BUF(J1)(84:93), FMT='(F10.3)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(2)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2155, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing Y-error filed in line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
              READ ( UNIT=BUF(J1)(118:126), FMT='(F10.2)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(3)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2156, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing Z-error filed in line '//STR(1:I_LEN(STR))// &
     &                 ' of the station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
!
! ----------- Transform errors from mm to m
!
              VTD%STA(I_STA)%ERR_VEC(1:3) = VTD%STA(I_STA)%ERR_VEC(1:3)*0.001D0
...
!
     ...
     &                            VTD%STA(I_STA)%TAI_EPC(VTD%STA(I_STA)%N_EPC), IER )
              IF ( IER .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR ) 
                   CALL ERR_LOG ( 2157, IUER, 'VTD_LOAD_STACOO', &
...
!
     ...
     &               VTD%STA(I_STA)%COO_TRS(1,VTD%STA(I_STA)%N_EPC)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2158, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
...
!
     ...
     &               VTD%STA(I_STA)%COO_TRS(2,VTD%STA(I_STA)%N_EPC)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2159, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the '// &
     &                 'station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
...
!
     ...
     &               VTD%STA(I_STA)%COO_TRS(3,VTD%STA(I_STA)%N_EPC)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2160, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the '// &
     &                 'station coordinate file '//VTD%CONF%FINAM_STACOO )
                   RETURN 
              END IF 
              VTD%STA(I_STA)%COR_COO_VEL = 0.0D0
         END IF
         IF ( ( I_FMT .EQ. 1 .OR. I_FMT .EQ. 2 ) .AND. &
     &        BUF(J1)(1:8) .EQ. 'STA_CRL:'             ) THEN
!
! ----------- Process correlations between station positions and velocities
!
              I_STA = 0
              DO 440 J4=1,VTD%L_STA
                 IF ( BUF(J1)(11:18) .EQ. VTD%STA(J4)%IVS_NAME ) THEN
                      I_STA = J4
                 END IF
 440          CONTINUE 
              IF ( I_STA == 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2161, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the '// &
     &                 'station coordinate file '//TRIM(VTD%CONF%FINAM_STACOO)// &
     &                 ' -- station '//BUF(J1)(11:18)//' was not defined before' )
                   RETURN 
              END IF
              DO 450 J5=1,15
                 READ ( UNIT=BUF(J1)(30+(J5-1)*7:36+(J5-1)*7), FMT='(F6.3)', IOSTAT=IOS ) &
     &                  VTD%STA(I_STA)%COR_COO_VEL(J5)
                 IF ( IOS .NE. 0 ) THEN
                      CALL CLRCH ( STR ) 
                      CALL INCH  ( J1, STR )
                      CALL ERR_LOG ( 2162, IUER, 'VTD_LOAD_STACOO', 'Error in '// &
     &                    'parsing line '//STR(1:I_LEN(STR))//' of the '// &
     &                    'station coordinate file '//TRIM(VTD%CONF%FINAM_STACOO)// &
     &                    ' -- erro in reading correlation '//BUF(J1)(30+(J5-1)*7:36+(J5-1)*7) )
                      RETURN 
                 END IF
 450          CONTINUE 
         END IF
...
!
      ...
      DO 460 J6=1,VTD%L_STA
         IF ( USE_STA(J6) .EQ. 0  ) THEN
              IF ( VTD%STA(J6)%STA_TYP == VTD__OR ) THEN
                   VTD%STA(J6)%COO_TRS = 0.0D0
                 ELSE 
                   CALL ERR_PASS ( IUER, IER )
                   CALL ERR_LOG ( 2163, IER, 'VTD_LOAD_STACOO', 'Station '// &
     &                  VTD%STA(J6)%IVS_NAME//' was not found in the '// &
...
!
      IF ( NUM_ERR .GT. 0 ) THEN
           ...
           CALL ERR_LOG ( 2164, IUER, 'VTD_LOAD_STACOO', STR(1:I_LEN(STR))// &
     &         ' stations were not found in station coordinate catalogue'// &
     &         ' file '//VTD%CONF%FINAM_STACOO )
           RETURN 
      END IF
!
      IF ( MJD_REF == 0 ) THEN
           CALL ERR_LOG ( 2165, IUER, 'VTD_LOAD_STACOO', ' Reference '// &
