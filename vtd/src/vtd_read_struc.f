...
! *                                                                      *
! * ## 27-FEB-2004  VTD_READ_STRUC   v2.2 (d)  L. Petrov  07-MAR-2026 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
     ...
     &                ' -- wrong format of the date_end field' )
             ...
             END IF
!
             IF ( (VTD%STRUC(VTD%L_STR)%MJD_BEG*86400.0D0 + VTD%STRUC(VTD%L_STR)%SEC_BEG) .LE.       &
     &            (VTD%MJD_BEG*86400.0D0 + VTD%TAI_BEG)                                        .AND. &
     &            (VTD%STRUC(VTD%L_STR)%MJD_END*86400.0D0 + VTD%STRUC(VTD%L_STR)%SEC_END) .GE. &
     &            (VTD%MJD_END*86400.0D0 + VTD%TAI_END)                                              ) THEN
!
! --------------- Source map is within the range of observations
!
                  CONTINUE 
                ELSE
!
! --------------- The Source map out within the range of observations
!
                  VTD%L_STR = VTD%L_STR - 1
                  GOTO 410
             END IF
!
     ...
     &               VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(1)
             IF ( IOS .NE. 0 ) THEN
                  CALL CLRCH ( STR ) 
                  CALL INCH  ( J1, STR )
     ...
     &                ' -- wrong format of the first pixel phase center field' )
                  ...
                  RETURN 
             END IF
             VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(1) = MAS__TO__RAD*VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(1)
!
             READ ( UNIT=BUF(J1)(81:88), FMT='(F8.2)', IOSTAT=IOS ) &
     &              VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(2)
             IF ( IOS .NE. 0 ) THEN
                  CALL CLRCH ( STR ) 
                  CALL INCH  ( J1, STR )
     ...
     &                ' -- wrong format of the second pixel phase center field' )
                  ...
                  RETURN 
             END IF
             VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(2) = MAS__TO__RAD*VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(2)
!       
...
!
     ...
     &                    ' for more than two bands. Maps for bands '// &
     &                    VTD%SOU(J3)%IMAGE_BAND(1)//' '// &
     &                    VTD%SOU(J3)%IMAGE_BAND(2)//' have already been '// &
     &                   'loaded' )
                      RETURN 
                 END IF
                 VTD%SOU(J3)%PHASE_CENTER_OFFS(1,IBND) = VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(1)
                 VTD%SOU(J3)%PHASE_CENTER_OFFS(2,IBND) = VTD%STRUC(VTD%L_STR)%PHASE_CENTER_OFFS(2)
!
#ifdef VTD__NO_STRUC
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 21-AUG-2007  MAP_PROCESS  v1.0 (d)  L. Petrov  21-AUG-2007 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
