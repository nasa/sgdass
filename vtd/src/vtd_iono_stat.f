...
! *                                                                      *
! *  ### 10-OCT-2010 VTD_IONO_STAT v1.1 (d)  L. Petrov  14-JUN-2018 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
                CALL CLRCH ( STR ) 
                WRITE ( UNIT=STR(1:15) , FMT='(1PD15.7)' ) OBS_TYP%FRQ_ION_EFF(1)
                CALL ERR_LOG ( 2282, IUER, 'VTD_IONO_STAT', 'Trap of '// &
     &              'internal control: bad upper band ionosphere frequency '// &
     &               STR  )
                RETURN 
           END IF
...
!
                CALL CLRCH ( STR ) 
                WRITE ( UNIT=STR(1:15) , FMT='(1PD15.7)' ) OBS_TYP%FRQ_ION_EFF(2)
                CALL ERR_LOG ( 2283, IUER, 'VTD_IONO_STAT', 'Trap of '// &
     &              'internal control: bad lower ionosphere frequency '// &
     &               STR )
                RETURN 
           END IF
