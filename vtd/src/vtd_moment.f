...
! *                                                                      *
! *  ### 27-JAN-2004   VTD_MOMENT  v5.5 (d)  L. Petrov  11-APR-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
           ...
           IF ( KNOT .EQ. -1 ) THEN
                STR = MJDSEC_TO_DATE( VTD%UEOP%MJD_BEG, VTD%UEOP%TAI_BEG, IER )
...
!
           IF ( KNOT .EQ. -2 ) THEN
                ...
                IER  = 0
                STR1 = MJDSEC_TO_DATE ( VTD%UEOP%MJD_BEG, VTD%UEOP%TAI_BEG, IER )
                IER  = 0
                STR2 = MJDSEC_TO_DATE ( MJD, TAI, IER ) 
                CALL ERR_LOG ( 2225, IUER, 'VTD_MOMENT', 'Requested '// &
     &              'moment of time '//TRIM(STR2)// &
     &              ' is too late. The file with the EOP series '// &
     &              TRIM(VTD%CONF%FINAM_EOP)//' ends on '//TRIM(STR1)// &
...
!
     ...
     &               TRIM(STR)//' from ERM file '//VTD%CONF%FINAM_ERM )
                RETURN
           END IF
!
           IF ( VTD%UEOP%STATUS .NE. UEOP__LOADED ) THEN
...
!
     ...
     &            (TIM_ARG - VTD%NZO(IND_NZO)%TAI_BEG)
...
!
              TARG = (MJD        - VTD%NZO(IND_NZO)%MJD_BEG)*86400.0D0 + &
     &               (VTD%MOM%TAI - VTD%NZO(IND_NZO)%TAI_BEG)
              KNOT = IXMN8 ( VTD%NZO(IND_NZO)%NOD_SPL, &
     &                       VTD%NZO(IND_NZO)%TIM_ARR(1), TARG )
              IF ( KNOT .EQ. -1 ) THEN
                  STR(1:28)  = MJDSEC_TO_DATE ( MJD, TAI, IER )
