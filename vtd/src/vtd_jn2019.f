...
! *                                                                      *
! *  ###  18-APR-2024  VTD_JN2019  v2.0 (d)  L. Petrov  19-OCT-2023 ###  *
! *                                          J. Skeens                   *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
!
     ...
     &          (VTD%MOM%TAI - VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%TAI_BEG)
     ...
     &                                   VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%TAI_BEG, IER )
     ...
     &                                   VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%TAI_BEG + &
...
!
 ...
 410  CONTINUE
      IF ( VTD%CONF%IVRB .GE. 2 ) THEN
           WRITE ( 6, 210 ) VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%NAME, VTD%MOM%MJD, &
     &                      VTD%MOM%TAI, SAT_POS_TRS, SAT_VEL_TRS
 210       FORMAT ( 'Satellite ', A, ' MOM_MJD/MOM_TAI = ', I5, ' Tai= ', F12.5/ &
     &              ' Sat_pos = ', 3(F15.5, 1X)/ &
     &              ' Sat_vel = ', 3(F15.5, 1X) )
      END IF
      IF ( VTD%CONF%IVRB .GE. 4 ) THEN
           WRITE ( 6, 220 ) VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%NAME, &
     &                      VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%MJD_BEG, &
     &                      VTD%NZO(VTD%SOU(ISOU)%IND_NZO)%TAI_BEG, &
     &                      ISOU, VTD%SOU(ISOU)%IND_NZO
 220       FORMAT ( 'Satellite ', A, ' Sat_MJD_BEG/Sat_TAI_BEG = ', I5, ' Tai= ', F12.5, &
     &              ' I_SOU: ', I4, ' IND_NZO: ', I3 )
      END IF
!
...
!
     ...
     &             'to to compute phase center offset/variation for station '// &
     &              VTD%STA(ISTA1)%IVS_NAME )
               RETURN 
          END IF
...
!
     ...
     &         'to get phase center offset for satellite '// &
...
! 
     ...
     &         'to the celestial coordinate system on '//STR(1:24) )
           RETURN 
      END IF
!
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 19-APR-2023  NERS_AZELHA_SAT  v1.0 (d) L. Petrov 19-APR-2023 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'astro_constants.i'
