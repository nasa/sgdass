...
! *                                                                      *
! *  ### 27-FEB-2004    VTD_STRUC  v2.4 (d)  L. Petrov  26-JUL-2023 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
              ...
              GR_DEL_HIGH = VTD_GR_DEL_STRUC ( BX, BY, &
     &                                 VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_HIGH), &
     &                                 FREQ_GR_HIGH, &
...
!
              CALL ERR_PASS ( IUER, IER )
              GR_DEL_LOW = VTD_GR_DEL_STRUC ( BX, BY, &
     &                                VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_LOW), &
     &                                FREQ_GR_LOW, &
...
!
              IF ( VTD%CONF%TEST(1) == 2  .AND. VTD%CONF%TEST(2) == 0 ) THEN
                   IER = -1
                   PH_DEL_HIGH = VTD_PH_DEL_STRUC ( BX, BY, &
     &                                      VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_HIGH), &
     &                                      FREQ_GR_HIGH, &
     &                                      VTD%SOU(ISOU)%MAP(IND_MAP_HIGH), IER )
...
!
                   IER = -1
                   PH_DEL_LOW  = VTD_PH_DEL_STRUC ( BX, BY, &
     &                                      VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_LOW), &
     &                                      FREQ_GR_LOW, &
     &                                      VTD%SOU(ISOU)%MAP(IND_MAP_LOW), IER )
                   IF ( IER .NE. 0 ) CALL EXIT ( 1 )
!
     ...
     &               (             FREQ_GR_HIGH**2 -            FREQ_GR_LOW**2 ) 
                 ELSE IF ( VTD%CONF%TEST(1) == 2  .AND. VTD%CONF%TEST(2) == 1 ) THEN
                   IER = -1
                   DELAY_STR = VTD_PH_DEL_STRUC ( BX, BY, &
     &                                    VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_HIGH), &
     &                                    FREQ_GR_HIGH, &
     &                                    VTD%SOU(ISOU)%MAP(IND_MAP_HIGH), IER )
                   IF ( IER .NE. 0 ) CALL EXIT ( 1 )
...
!
              CALL ERR_PASS ( IUER, IER )
              CORR_FD(IND_MAP_HIGH) = VTD_CORR_AMP ( BX, BY, &
     &                                               VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_HIGH), &
     &                                               FREQ_GR_HIGH, &
     &                                               VTD%SOU(ISOU)%MAP(IND_MAP_HIGH), IER )
              IF ( IER .NE. 0 ) THEN
...
!
              CALL ERR_PASS ( IUER, IER )  
              CORR_FD(IND_MAP_LOW)  = VTD_CORR_AMP ( BX, BY, &
     &                                               VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_LOW), &
     &                                               FREQ_GR_LOW, &
     &                                               VTD%SOU(ISOU)%MAP(IND_MAP_LOW), IER )
              IF ( IER .NE. 0 ) THEN
...
!
                   CALL ERR_PASS ( IUER, IER )
                   DELAY_STR = VTD_GR_DEL_STRUC ( BX, BY, &
     &                                            VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_LOW), &
     &                                            OBS_TYP%FRQ_ION_EFF(IND_MAP_LOW) , &
     &                                            VTD%SOU(ISOU)%MAP(IND_MAP_LOW), IER )
                   IF ( IER .NE. 0 ) THEN
...
!
                   ...
                   IF ( OBS_TYP%DELAY_TYPE == VTD__MH__DTP ) THEN
                        DELAY_STR = VTD_GR_DEL_STRUC ( BX, BY, &
     &                                         VTD%SOU(ISOU)%PHASE_CENTER_OFFS(1,IND_MAP_HIGH), &
     &                                         OBS_TYP%FRQ_ION_EFF(IND_MAP_HIGH), &
     &                                         VTD%SOU(ISOU)%MAP(IND_MAP_HIGH), IER )
