...
! ************************************************************************
! *                                                                      *
! *   Auxiliary routine VTD_LOAD_STAVEL reads the file with station      *
...
! *                                                                      *
! * ### 26-JAN-2004  VTD_LOAD_STAVEL  v2.3 (d) L. Petrov 11-AUG-2026 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'vtd.i'
...
!
              VTD%STA(I_STA)%VEL_TRS(1:3) = VTD%STA(I_STA)%VEL_TRS(1:3)*0.001D0/ &
     &                                      JUL_YEAR__TO__SEC
!
! ----------- Parsing velocity X-error
!
              READ ( UNIT=BUF(J1)(37:44), FMT='(F8.3)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(4)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2156, IUER, 'VTD_LOAD_STAVEL', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the station '// &
     &                 'description file '//TRIM(VTD%CONF%FINAM_STADESC)// &
     &                 'with velocity X-error' )
                   RETURN 
              END IF 
!
! ----------- Parsing velocity Y-error
!
              READ ( UNIT=BUF(J1)(63:70), FMT='(F8.3)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(5)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2156, IUER, 'VTD_LOAD_STAVEL', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the station '// &
     &                 'description file '//TRIM(VTD%CONF%FINAM_STADESC)// &
     &                 'with velocity Y-error' )
                   RETURN 
              END IF 
!
! ----------- Parsing velocity Z-error
!
              READ ( UNIT=BUF(J1)(89:96), FMT='(F8.3)', IOSTAT=IOS ) &
     &               VTD%STA(I_STA)%ERR_VEC(6)
              IF ( IOS .NE. 0 ) THEN
                   CALL CLRCH ( STR ) 
                   CALL INCH  ( J1, STR )
                   CALL ERR_LOG ( 2156, IUER, 'VTD_LOAD_STAVEL', 'Error in '// &
     &                 'parsing line '//STR(1:I_LEN(STR))//' of the station '// &
     &                 'description file '//TRIM(VTD%CONF%FINAM_STADESC)// &
     &                 'with velocity Z-error' )
                   RETURN 
              END IF 
!
! ----------- Transform velocity errors from mm/yr to m/s
!
              VTD%STA(I_STA)%ERR_VEC(4:6) = VTD%STA(I_STA)%ERR_VEC(4:6)*0.001D0/ &
     &                                      JUL_YEAR__TO__SEC
