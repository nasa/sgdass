...
! *                                                                      *
! * ### 18-DEC-2002  VTD_SET_BINDISP  v1.5 (d) L. Petrov 22-JUN-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           STA_TRS_COO(3,L_STA)
      INTEGER*2  BIT_SUM_I2
      INTEGER*4  BIT_SUM_COMP
      LOGICAL*4  FL_WARN
...
!
      ...
      INTEGER*4, EXTERNAL :: GET_UNIT, I_LEN, IXMN8, IXMN8_S, BIT_CHECKSUM
!
! --- Initialization
!
...
!
     ...
     &                         POSVAR_FIL(1:I_LEN(POSVAR_FIL))
                        IF ( USE_MOD .EQ. PSV__REQ ) THEN
                             CALL ERR_LOG ( 2822, IUER, 'VTD_SET_BINDISP', &
     &                           'Station '//STA_NAM(J2)//' was not found '// &
...
!
     ...
     &                                     BDSFSC(I_STA(J2)), IER )
                   IF ( USE_MOD .EQ. PSV__REQ ) THEN
                        CALL ERR_LOG ( 2823, IUER, 'VTD_SET_BINDISP', 'Station '// &
...
!
     ...
     &                                'a session which has nominal '
              ...
              END IF
              IF ( USE_MOD .EQ. PSV__REQ ) THEN
                   CALL ERR_LOG ( 2824, IUER, 'VTD_SET_BINDISP', 'Station '// &
     &                  STA_NAM(J2)//' has position variation data '// &
...
!
              IF ( IREC_END .LT. BDSNSA(I_STA(J2)) ) THEN
                 ...
                 ELSE IF ( IREC_BEG .GT. 1 ) THEN
                   ...
                   L_BDS = L_BDS + 1
                 ELSE
                   CALL ERR_LOG ( 2825, IUER, 'VTD_SET_BINDISP', 'Trap of '// &
     &                 'internal control' )
                   RETURN
              END IF
...
!
     ...
     &                            ' BDSNSA(I_STA(J2)) = ',BDSNSA(I_STA(J2))
                   CALL ERR_LOG ( 2826, IUER, 'VTD_SET_BINDISP', 'Trap of '// &
     &                 'internal control' )
                   RETURN
              END IF
         END IF
...
!
              ...
              CALL ERR_LOG ( 2827, IUER, 'VTD_SET_BINDISP', 'Sampling rate of '// &
...
!
     ...
     &          IOSTAT=IOS )
         IF ( IOS .NE. 0 ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( IOS, STR )
              CALL ERR_LOG ( 2828, IUER, 'VTD_SET_BINDISP', 'Error '// &
     &             STR(1:I_LEN(STR))//' in opening file '//BDSFIL(I_STA(J2)) )
              RETURN
         END IF
         READ ( UNIT=LUN, REC=2 ) HDR2
!
     ...
     &                           (J4-1)*BDSSAM(I_STA(J2)) 
            IF ( BDS_ENDIAN .EQ. 'L'  .AND.  ENDIAN__FMT .EQ. 'B' ) THEN
               ...
               ELSE IF ( BDS_ENDIAN .NE. ENDIAN__FMT ) THEN
                 CALL ERR_LOG ( 2829, IUER, 'VTD_SET_BINDISP', 'Unsupported '// &
     &               'ENDIAN flag: '//BDS_ENDIAN//' in file position '// &
     &               'variation file '//BDSFIL(I_STA(J2)) )
                 RETURN
            END IF
            IF ( HDR2%MJD_FMT == BINDISP__MJD_FMT ) THEN
!
! -------------- Extract displacement extension. The extenion sign 
! -------------- is in bits 12-14 (0-base). Extenion occupies 
! -------------- bits 0-3, 4-8, 8-11
!
                 IF ( BTEST ( BDS%EXT_DSP, 12 ) ) THEN
                      KX = -1
                    ELSE
                      KX =  0
                 END IF
                 IF ( BTEST ( BDS%EXT_DSP, 13 ) ) THEN
                      KY = -1
                   ELSE
                      KY =  0
                 END IF
                 IF ( BTEST ( BDS%EXT_DSP, 14 ) ) THEN
                      KZ = -1
                   ELSE
                      KZ =  0
                 END IF
!     
                 CALL MVBITS ( BDS%EXT_DSP,  0, 4, KX,  0 )
                 CALL MVBITS ( BDS%EXT_DSP,  4, 4, KY,  0 )
                 CALL MVBITS ( BDS%EXT_DSP,  8, 4, KZ,  0 )
                 CALL MVBITS ( BDS%EXT_DSP, 15, 1, BIT_SUM_I2,  0 )
                 BIT_SUM_COMP = BIT_CHECKSUM ( BDS%X_DSP, 1, 63 )
                 IF ( BIT_SUM_COMP .NE. BIT_SUM_I2 ) THEN
                      CALL CLRCH ( STR )
                      CALL INCH  ( M__HDR+J4, STR )
                      CALL ERR_LOG ( 2830, IUER, 'VTD_SET_BINDISP', 'Bit sum '// &
     &                    'check is wrong in record '//TRIM(STR)// &
     &                    ' when processing file '//BDSFIL(I_STA(J2)) )
                      RETURN
                 END IF
              ELSE IF ( HDR2%MJD_FMT == BINDISP__MJD_FMT_01 ) THEN
!
! -------------- Extract displacement extension. The extenion sign 
...
!
     ...
     &                           0.0D0, 0.0D0, SPL_BDS(1,1), WORK_SPL, IER )
              IF ( IER .NE. 0 ) THEN
                   CALL ERR_LOG ( 2831, IUER, 'VTD_SET_BINDISP', 'Error in '// &
...
!
     ...
     &                                             BDSFSC(I_STA(J2)), IER )
                 CALL ERR_LOG ( 2832, IUER, 'VTD_SET_BINDISP', 'Trap of '// &
...
!
      CALL ERR_LOG ( 0, IUER )
      RETURN
      END  SUBROUTINE  VTD_SET_BINDISP  !#!#
