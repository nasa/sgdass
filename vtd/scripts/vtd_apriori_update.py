...
# *                                                                      *
# * # 29-OCT-2018 vtd_apriori_update.py v 3.33 (d) L. Petrov 12-AUG-2026 # *
# *                                                                      *
# ************************************************************************
"""
...
vau__label = "vtd_apriori_update.py  20260804"
fmt__label = "# Configuration of file vtd_apriori update.  Format version of 2025.06.19"
config__num_par = 31 # The number of configuration parameters
...
viono_show_exe    = vtd_local.vtd_prefix  + "/bin/viono_show_header"
...
#
       ...
       self.merra2_geosit_atm    = None
       self.merra2_geosit_lws    = None
...
#
   ...
   for line in conf_buf:
       ...
       elif  ( line.split()[0]           == "LOG_WGET_FILE:" ):
               config.log_wget_file       = line.split()[1]
               num_par = num_par + 1
       ...
       elif  ( line.split()[0]           == "MERRA2_GEOSIT_ATM:" ):
               config.merra2_geosit_atm = line.split()[1].lower()
               num_par = num_par + 1

       elif  ( line.split()[0]           == "MERRA2_GEOSIT_LWS:" ):
               config.merra2_geosit_lws = line.split()[1].lower()
               num_par = num_par + 1
...
#
    for year in range(iono_year_beg,year_now+1):
        ...
        if ( "ftp" in config.iono_url ):
#
# ========== OLD code of ftp site!!
#
...
# ---------- Current code
#
             tmp_iono_dir_lis_file  = "%s_%d.dir" % ( tmp_iono, year ) 
             if ( os.path.isfile(tmp_iono_dir_lis_file) ):
                  os.unlink ( tmp_iono_dir_lis_file )
#
# ---------- Get the directory with listing of the remote site
#
             comstr = 'cd  ' + iono_year_dir + '; ' + \
                      'wget ' + \
                      '-q '   + \
                      '-a ' + config.log_wget_file + " " + \
                      '-O %s' % tmp_iono_dir_lis_file + " " + \
                      '-l 1 '   + \
                      '--no-parent ' + \
                      '-c '   + \
                      '-N '   + \
        ...
        if ( fl_new_year == 1 or year >= year_now-1 ):
             ...
             ( ret, out ) = exe ( config, comstr )
             if ( ret != 0 ):
                  print_log      ( config, "Failure in downloading iono directory listing" )
                  print_log      ( config, "Failed command %s" % comstr )
                  for line in out:
                      print_log_verb ( config, line )
         
                  exit ( 1 )        
             print_log ( config, "Downloaded iono listing for %d in file %s " % \
                         ( year, tmp_iono_dir_lis_file ) )
             if ( os.path.isfile(tmp_iono_dir_lis_file) ):
                  com = "chmod 666 " + tmp_iono_dir_lis_file
                  ( ret, out ) = exe ( config, comstr )

        if ( not "ftp" in config.iono_url ):
#
# ---------- Make a list of local ionospheric files
#
             iono_loc_dir = []
             for path, dirs, files in os.walk(iono_year_dir):
                 for file in files:
                     if ( len(file) > 5 ):
                          if ( "#" in file ): continue
                          if ( "~" in file ): continue
                          if ( "GIM.INX" in file ):
                               if ( config.verb >= 5 ):
                                    print ( "loc_iono_file= ", file )
                               iono_loc_dir.append ( file )

#
# ---------- Read the file with the contents of the remote sideto the geven year
#
             if ( os.path.isfile(tmp_iono_dir_lis_file) ):
                  iono_rem_dir = read_file_to_buffer ( tmp_iono_dir_lis_file )
                  os.unlink ( tmp_iono_dir_lis_file )
             else:
                  continue
             if ( iono_rem_dir == None ): continue

#
# ---------- Get the list of URL to download
#
             url_list = []
             for line in iono_rem_dir:
#
# -------------- Extract a line with the url
#
                 if ( '<td><a href="https' in line ):
                      url = line.split()[1].replace('"','').replace("href=","")
                      id = url.rfind("/")
                      if ( id < 1 ): contune
                      iono_file = url[id+1:]
#
...
#
                      if ( "COD0OPSFIN_" in iono_file and \
                           "_GIM.INX.gz" in iono_file     ):
                           if ( not iono_file in iono_loc_dir ):
#
# ----------------------------- Aga! This remote file with GIM is not 
# ----------------------------- in the local directory
#
                                if ( config.verb >= 5 ):
                                    print ( "rem_iono_file= %s url: %s", ( iono_file, url ) )
                                url_list.append ( url )
...
#
                  ...
                  for url in url_list:
#
# ------------------- Download the GIM file that corresponds to the given URL
#
                      comstr = 'cd  ' + iono_year_dir + '; ' + \
                               'wget ' + \
                               '-q '   + \
                      ...
                      ( ret, out ) = exe ( config, comstr )
                      if ( ret != 0 ):
                           print_log ( config, "Failure in downloading iono file from URL  %s" % url )
                           print_log ( config, "Failed command %s" % comstr )
                           for line in out:
                               print_log_verb ( config, line )
                      ...
                      if ( id < 1 ): contune
                      iono_file = url[id+1:]
                      print_log ( config, "Downloaded iono from %s to file %s " % ( url, iono_file ) )
...
#
             ...
             ( ret, out ) = exe ( config, comstr )
             if ( ret != 0 ):
                  print_log ( config, "Failure in downloading slant path delays from %s to %s " % \
                              ( config.spd_url, spd_dir ) )
                  for line in out:
                      print_log_verb ( config, line )
                  return ( 1 )
...
#
        ...
        if ( ret != 0 ):
             print_log ( config, "Failure in downloading bspd_summary.txt file with command %s " % \
                         comstr )
             print     ( config, "Failure in downloading bspd_summary.txt file with command %s " % 
                         comstr )
             for line in out:
                 print_log_verb ( config, line )
             return ( 1 )        
...
#
        if ( os.path.isfile(loc_bspd_summary_file) ):
...
#
                     ...
                     print_log_verb ( config, "The lenght of local and summary files is different for %s" % \
                                      loc_bspd_dir )
                     return 1
        else:
             loc_sum_buf = []
...
#
        ...
        if ( ivrb >= 2 ):
             print ( "vau_spd REM beg: ", rem_mjd_beg, rem_tai_beg, \
                     " end: ", rem_mjd_end, rem_tai_end, rem_dat_end, \
             ...
             print ( "vau_spd LOC beg: ", loc_mjd_beg, loc_tai_beg, \
                     " end: ", loc_mjd_end, loc_tai_end, loc_dat_end, \
...
#
                 ...
                 if ( ivrb >= 3 ):
                      ...
                      print ( "loc_num_epochs= ", loc_num_epochs )
...
#
        ...
        if ( len(loc_sum_buf) == 0 ):
#
# ---------- Case when we download entier directory from remote
# ---------- host to the local host
#
             rem_prefix = loc_prefix      
             for line in rem_sum_buf:
                 if ( len(line.split()) < 10 ): continue
                 if ( line.split()[0] == "Station_name:" ):
                      sta_nam = line.split()[1].lower()
...
#
# ----- Prepare the read lock to the directory with slant path delay in binary format
#
        if ( pyvers >= "0312000" ):
             tim_now = datetime.now().replace(tzinfo=None)
        else:
...
#
        ...
        ( ret, out ) = exe ( config, comstr )
        if ( ret != 0 ):
             print_log ( config, "Failure in moving the summary file with command %s" % \
                         comstr )
             for line in out:
                 print_log_verb ( config, line )
                 return ( 1 )        
...
#
        ...
        if ( config.verb >= 2 ):
             print ( "vau_bds REM beg: ", rem_mjd_beg, rem_tai_beg, \
                     " end: ", rem_mjd_end, rem_tai_end, " rem_dat_end: ", rem_dat_end, \
                     " num_epc: ", rem_num_epochs, " rem_num_sta: ", rem_num_sta,  \
                     " rem_tim_stp: ", rem_tim_stp )

             print ( "vau_bds LOC beg: ", loc_mjd_beg, loc_tai_beg, \
                     " end: ", loc_mjd_end, loc_tai_end, " loc_dat_end: ", loc_dat_end, \
                     " num_epc: ", loc_num_epochs, " loc_num_sta: ", loc_num_sta,  \
...
#                 if ( config.verb >= 3 ):
#                      print ( 'vau_bds: loc_fil_len= ', loc_fil_len, ' rem_fil_len = ', rem_fil_len )
#
# -------------- Download the trailer of the remote file
#
                 comstr = curl_exe + " " + \
...
#
                 ...
                 f.close()
                 if ( config.verb >= 2 ):
                      print_log ( config, "vau_bds Wrote temporary loading file %s" % tmp_bds_file )
...
#
        ...
        print_log_verb ( config, "About to start renaming files for %s" % bds_dir )
#
...
#
        ...
        print_log ( config, "Updated " + bds_dir + " loading. Last epoch: " + \
                             rem_dat_end )
        print_log_verb ( config, "Finished renaming files for %s" % bds_dir )
    return 3
#
# ------------------------------------------------------------------------
#
def vau_bme ( config, loa_typ ):
    if ( loa_typ == "merra2_geosit_atm" ):
         outdir = config.load_bds_dir  + "/atm/merra2_geosit"
         comstr = bindisp_merge_exe   +                    " " + \
                  config.load_bds_dir  + "/atm/merra2/"   + " " + \
                  config.load_bds_dir  + "/atm/geosit/" + " " + \
                  outdir
    elif ( loa_typ == "merra2_geosit_lws" ):
         outdir = config.load_bds_dir  + "/lws/merra2_geosit"
         comstr = bindisp_merge_exe   +                    " " + \
                  config.load_bds_dir  + "/lws/merra2/"   + " " + \
                  config.load_bds_dir  + "/lws/geosit/" + " " + \
                  outdir
    elif ( loa_typ == "merra2_geosfpit_atm" ):
         outdir = config.load_bds_dir  + "/atm/merra2_geosfpit"
         comstr = bindisp_merge_exe   +                    " " + \
                  config.load_bds_dir  + "/atm/merra2/"   + " " + \
                  config.load_bds_dir  + "/atm/geosfpit/" + " " + \
                  outdir
    elif ( loa_typ == "merra2_geosfpit_lws" ):
         ...
         print_log ( config, "vau_bme: unknown loa_typ %s" % loa_typ )
         exit ( 1 )        
...
def main():
    ...
    opts.add_option ( "-V", "--vers", action="store_true", \
                      dest="version", \
                      default=False, \
                      metavar="NAME", \
...
#
# - Check whether another vtd_apriori_update.py process is running
#
    my_pid = os.getpid()
    com = "ps -eaf | grep vtd_apriori_update.py | grep -v grep"
    (ret,out) = exe ( config, com )
    if ( len(out) > 0 ):
         for line in out:
             pid = int(line.split()[1])
             if ( pid != my_pid ):
                  print ( "Another vtd_apriori_update.py is running with PID %d" % pid )
                  print ( "You can either wait for completion %d process (preferred) or terminate it with command kill -9 %d" % \
                          ( pid, pid ) )
                  exit  ( 1 )
...
#
         ...
         if ( config.iono_update == "yes" ):
              vau_download_iono  ( config, opts.ivrb )
    ...
    if ( config.merra2_geosit_atm == "yes" ):
         ret = vau_bme ( config, "merra2_geosit_atm" )
         if ( ret != 0 ): 
              print_log ( config, "Error in attempt to update merra2_geosit_atm time series" )
         else:
              print_log ( config, "Updated merra2_geosit_atm time series" )
      
    if ( config.merra2_geosit_lws == "yes" ):
         ret = vau_bme ( config, "merra2_geosit_lws" )
         if ( ret != 0 ): 
              print_log ( config, "Error in attempt to update merra2_geosit_lws time series" )
         else:
              print_log ( config, "Updated merra2_geosfpit_lws time series" )
    if ( config.merra2_geosfpit_atm == "yes" ):
         ret = vau_bme ( config, "merra2_geosfpit_atm" )
         if ( ret != 0 ): 
    ...
    if ( config.merra2_geosfpit_lws == "yes" ):
         ret = vau_bme ( config, "merra2_geosfpit_lws" )
         if ( ret != 0 ): 
