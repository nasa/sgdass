...
# *                                                                      *
# *  ###  27-JUL-2022  config.csh  v1.9  (d)  L. Petrov  15-AUG-2025 ### *
# *                                                                      *
# ************************************************************************
#
...
if ( -f $ATP_SPD_CLIENT_DIR/lib/libspc.a == 0 ) then
     echo "vtd config: spd_client file $ATP_SPD_CLIENT_DIR/lib/libspc.a  was not found"
     echo "vtd config: Please install spd_client"
     exit 1
endif
if ( -f $ATP_SPD_CLIENT_DIR/include/spd.i == 0 ) then
     echo "vtd config: spd_client file $ATP_SPD_CLIENT_DIR/include/spd.i was not found"
     echo "vtd config: Please install spd_client"
     exit 1
endif
if ( -f $ATP_SPD_CLIENT_DIR/include/spd_local.i == 0 ) then
     echo "vtd config: spd_client file $ATP_SPD_CLIENT_DIR/include/spd_local.i was not found"
     echo "vtd config: Please install spd_client"
     exit 1
endif
if ( `cat $ATP_SPD_CLIENT_DIR/include/spd_local.i | grep "SPD__PETOOLS = " | grep yes` == "" && \
     `cat $ATP_SPD_CLIENT_DIR/include/spd_local.i | grep "SPC__PETOOLS = " | grep yes` == "" && ) then
     echo "vtd config: spd_client was installed without petools"
     echo "vtd config: Please re-install spd_client with petools"
     exit 1
endif
set ATP_SPD_CLIENT_VERSION = `cat $ATP_SPD_CLIENT_DIR/include/spd_local.i | grep "SPD__VERSION =" | awk '{print substr($5,2,8)}'`
if ( "$ATP_SPD_CLIENT_VERSION" < "$SPD_CLIENT_VERSION_MIN" ) then
     echo "Eeeeh. You have spd_client version $ATP_SPD_CLIENT_VERSION, but $SPD_CLIENT_VERSION_MIN is needed."
     echo "Please upgrade."
...
$ECHO "ok"
#
# --- Generate tle_local.i from template
#
set DATE_ISO    = `date "+%Y.%m.%d_%H:%M:%S"`
set root_len    = `echo $ATP_ROOT         | awk '{print length($1)}'`
set pref_len    = `echo $ATP_PREFIX       | awk '{print length($1)}'`
set version_len = `echo $ATP_VERSION      | awk '{print length($1)}'`
cat $ATP_ROOT/include/atp_local_templ.i | \
    sed "s@%%@Automatically generated on $DATE_ISO from atp_local_templ.i@g" | \
    sed "s@%ROOT_LEN%@$root_len@g"            | \
    sed "s@%PREFIX_LEN%@$pref_len@g"          | \
    sed "s@%VERSION_LEN%@$version_len@g"      | \
    sed "s@%ROOT_VAL%@$ATP_ROOT@g"            | \
    sed "s@%PREFIX_VAL%@$ATP_PREFIX@g"        | \
    sed "s@%VERSION_VAL%@$ATP_VERSION@g"       \
    > $ATP_ROOT/include/atp_local.i 
#
#
if ( `uname` == "Linux" ) then
...
$ECHO "SPD_CLIENT_PREFIX  = $ATP_SPD_CLIENT_DIR"               >> $out_file
$ECHO "PETOOLS_LIB        = $PETOOLS_LIB"                      >> $out_file
$ECHO "SPD_CLIENT_LIB     = -L $ATP_SPD_CLIENT_DIR/lib -lspc"  >> $out_file
...
$ECHO "setenv SPD_CLIENT_PREFIX  $ATP_SPD_CLIENT_DIR"                        >> $out_vars
$ECHO "setenv PETOOLS_LIB        ${qt}$PETOOLS_LIB${qt}"                     >> $out_vars
$ECHO "setenv SPD_CLIENT_LIB     ${qt}-L $ATP_SPD_CLIENT_DIR/lib -lspc${qt}" >> $out_vars
