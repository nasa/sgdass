...
!
     ...
     &               %VAL(1) )
!
!@       FILIN  =  '/f2/anc/k2x048_orig.anc'
 !@      FILOUT =  '/f2/anc/k2x048_scav.anc'
  !@     FILBNC =  '/f2/anc/k2x048_orig.bnc'
!
!     
       FILIN  = '/f2/anc/vo3278gs_orig.anc'
       FILOUT = '/f2/anc/vo3278gs_scav.anc'
       FILBNC = '/f2/anc/vo3278gs_orig.bnc'

       Print *, "%%%%%%%%%%% WE ARE HERE %%%%%%"
       PRINT *, "OUR ANC FILE IS:  ", TRIM(FILIN)
       PRINT *, "OUR SCAV FILE IS: ", TRIM(FILOUT)
       Print *, " "
       IUER = -1
       CALL ANC_PARSE_SIM ( FILIN, FILOUT, ANC, IUER )
       IF ( IUER .NE. 0 ) THEN
          CALL ERR_LOG ( 5035, IUER, 'ANC_PARSE_SIM_MAIN_TESTER',       &
     &            'Failed to parse anc file data from '//FILIN )
       END IF

      PRINT *, "NUM_PRV:  ", ANC%NUM_PRV 
      PRINT *, "NUM_DOO:  ", ANC%NUM_DOO
      PRINT *, "NUM_MET:  ", ANC%NUM_MET
      PRINT *, "NUM_TPS:  ", ANC%NUM_TPS
      PRINT *, "NUM_TSYS: ", ANC%NUM_TSYS
      PRINT *, "NUM_TPI:  ", ANC%NUM_TPI
      PRINT *, "NUM_PCS:  ", ANC%NUM_PCS
      PRINT *, "NUM_PCAL: ", ANC%NUM_PCAL
      PRINT *, "NUM_TGPS: ", ANC%NUM_TGPS
      PRINT *, "NUM_GPS:  ", ANC%NUM_GPS
      PRINT *, "NUM_SEFD: ", ANC%NUM_SEFD
      PRINT *, "UTC_MTAI: ", ANC%UTC_MTAI
      PRINT *, " -------- "
!@      DO J1 = 1, ANC%NUM_TPS
  !@       PRINT *, "J1: ", J1, "CTAG:  ", ANC%TPS_TAG(J1),               &
    !@ &                        "I8TAG: ", ANC%I8TPS(J1),                 &
!@     &                        "I8DIF = ", ANC%I8TPS(J1) - ANC%I8TPS(1)
  !@    END DO

!@@!      J2 = ANC%NUM_TPS + 10
    !@@!  DO J1 = 1, J2
        !@@! PRINT *, "J1: ", J1, "I8TAG: ", ANC%I8TPS(J1)
!@@!      END DO
...
!
       WRITE ( 6, 112 ) TFINISH - TSTART
!
 112   FORMAT ( 'Total Runtime = ', F25.10, ' seconds' )
...
! ----------------------------------------------------------------------------
!
      SUBROUTINE ANC_PARSE_SIM ( FILIN, FILOUT, ANC, IUER )
!     
! ************************************************************************
! *                                                                      *
...
! ------
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 294
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(1), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------ 
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 313
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(2), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 332
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(3), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 352
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(4), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
         CALL CPU_TIME ( TS1 )     ! %%%%%%%%%%% ANC_PARSE_SIM - 371
!
...
!
            CALL SORT2_I8 ( K8_TPS, I8TPS_ARR, K8_TPS_ARR )
 
         CALL CPU_TIME ( TF1 )     ! %%%%%%%%%%% ANC_PARSE_SIM - 406
         PRINT *,  "Runtime for allocating TSYS:", TF1 - TS1

! ------            
         ELSEIF ( TRIM(BUFL2) == 'NUM_PC_SENSOR:' ) THEN

         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 389
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(5), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 408
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(6), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )

         CALL CPU_TIME ( TS1 )     ! %%%%%%%%%%% ANC_PARSE_SIM - 439
!
...
!                mean that the int*8 will also be ordered.
!
            CALL SORT2_I8 ( K8_PCS, I8PCS_ARR, K8_PCS_ARR )
 
         CALL CPU_TIME ( TF1 )     ! %%%%%%%%%%% ANC_PARSE_SIM - 475
         PRINT *,  "Runtime for allocating PCAL:", TF1 - TS1
! ------
         ELSEIF ( TRIM(BUFL2) == 'NUM_FMT2GPS_TIMER:' ) THEN

         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 445
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(7), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 465
          CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(8), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
! ------            
         ...
         J7 = 0                    ! %%%%%%%%%%% ANC_PARSE_SIM - 510
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECTS(9), " = ", TF - TS
         IF ( J7 == 0 ) CALL CPU_TIME (TS )
...
!
         ...
         CALL CPU_TIME ( TF )
         PRINT *,  "Runtime of ", SECT_ID, " = ", TF - TS
...
!
                        !@@@@@@@@@@@TPSLIST @@@@@@@@@@@@!
              IND4_CNT = LTM_DIF ( 0, ANC%NUM_TPS, ANC%TPS_TAG,         &
...
!
          ...
          IF ( IND4_CNT .NE. IND_TPS ) THEN 
             PRINT *, "TPS_ICNT:",K_TSYS,"CHA:",IND4_CNT,"INT8:",IND_TPS, "|| INT8_og:", IND8_TPS, "tmp:", IND8_TMP
          END IF
...
!
               ! @@@@@@@@@@@@@@@  PCSLIST @@@@@@@@@@@@@!
               IND4_CNT2 = LTM_DIF ( 0, ANC%NUM_PCS, ANC%PCS_TAG,         &
     &                             BUFL(IND(1,4):IND(2,4))   )
               IF ( IND4_CNT2 < 1 ) THEN
!@!@               IND_PCS = LTM_DIF ( 0, ANC%NUM_PCS, ANC%PCS_TAG,         &
    !@!@ &                             BUFL(IND(1,4):IND(2,4))   )
        !@!@       IF ( IND_PCS < 1 ) THEN
                  CALL ERR_LOG ( 5048, IUER, 'ANC_PARSE_SIM',           &
     &                    'Tag '//BUFL(IND(1,4):IND(2,4))//             &
...
! ---
      ...
      CLOSE ( UNIT = 112 )
! ---
      PRINT *, " %%%%%%%% WE ARE PRINT COMPARING THE IND_TPS "
