...
! *                                                                      *
! * ### 27-SEP-2022   PCAL_AVER   v2.0  (d)    N. Habana   30-SEP-2022 ###  *
! *                                                                         *
! ***************************************************************************
!
      IMPLICIT   NONE 
...
! ------
         ...
         DO 432 J3 = 1, NP
            IF ( IV(J3) .NE. 0 ) THEN
                PCAL_AMP_RMS = PCAL_AMP_RMS + (ABS(PCAL(J3) - PCAL_AVR))**2
                PCAL_PHA_RMS = PCAL_PHA_RMS + (PHAS_CMPL_R4(PCAL(J3) - PCAL_AVR))**2
             END IF
 432     CONTINUE
...
! ------
! ------
!@         NSIG_MAX = CMPLX(-1.0D0)
 !@        DO 430 J3 = 1, NP
   !@         IF ( IV(J3) > 0 ) THEN
     !@          NSIG_VAL = CDABS ( (PCAL(J3) - PCAL_AVR)/PCAL_RMS )
!
! ------------ Compare only the real part
!

       !@        IF ( REAL(REAL(NSIG_VAL)) > REAL(REAL(NSIG_MAX)) ) THEN
         !@         NSIG_MAX = NSIG_VAL
           !@       IND_NSIG = J3
             !@  END IF
!@            END IF
 ...
 !@430     CONTINUE 
! ------
   !@      IF ( REAL(REAL(NSIG_MAX)) > REAL(REAL(NSIG_LIM)) ) THEN
     !@       IV(IND_NSIG) = 0
       !@     RMS_OLD  = (PCAL_RMS**2 + PCAL_AVR**2)*KP
         !@   TIM_AVR  = (TIM_AVR*KP  - TIM(IND_NSIG) )/(KP-1)
           !@ PCAL_AVR = (PCAL_AVR*KP - PCAL(IND_NSIG))/(KP-1)
!@            PCAL_RMS = CDSQRT( (RMS_OLD - PCAL(IND_NSIG)**2)/(KP-1) -    &
  !@   &                        PCAL_AVR**2 )
    !@     ELSE
      !@      GOTO 810 
        !@ END IF
