...
! *   Usage: -exp                                 sde|vlbi                             *
! *          -i                                   input_file                           *      
! *          -frq_ind|-frq_val|-tim_ind|-tim_rng  tot|index|freq|index|time_range      *
! *          -pol                                 polarization                         *
! *          -ord                                 plot_ordinate                        *
! *          -var                                 plot_variable                        * 
! *          -del                                 tim_dif_max                          *
...
! *       if -frq_val -- frequency || tot                   { real*8 } [Hz]            *
...
! *       sefd -- system equivalent flux density                        [kJy]          *
...
! *                          -- Tsys [K] vs Freq [GHz] at given time                   *
! *                 IF ( -ord == phas ) THEN                                           *
! *                     IF ( -var == time ) THEN                                       *
...
! *                                                                      *
! *  ### 25-AUG-2021     BNC_PLOT            v7.2 (d)    N. Habana   14-MAY-2025 ###   *
! *                                                                                    *
! **************************************************************************************
!
      IMPLICIT   NONE
      ...
      CHARACTER  CH_FRQ*16, CH_BND, DIR_PLT*64, TITS(2)*64
      CHARACTER  CH_BRD*8, CH_BRD_TOT*(ANC__MTGPS)
      ...
      LOGICAL*1  FL_IND_ALL
      CHARACTER  ID_ARR(ANC__MTPS)*4, MODE*6, CH_IND_ALL*12
      INTEGER*4  ME, NS, NF
      PARAMETER  ( ME = 16 ) 
      REAL*8     ELEV_TATM(ME), VAL_TATM(ME), SPL_TATM(ME), TATM
      INTEGER*4  IND_ARR(ANC__MEPC), IND_SCA(ANC__MEPC), IARR(ANC__MEPC)
      ...
      INTEGER*4  IP, KP, LP, NP, JP, JN, KB
      ...
      REAL*8     FMGT_TOT(ANC__MTGPS,ANC__MEPC)
      REAL*8     FMPT_TOT(ANC__MTGPS,ANC__MEPC)
      REAL*8     T1_TOT(ANC__MTGPS,ANC__MEPC), NP_TOT(ANC__MTGPS)
      REAL*8     SEFD(ANC__MNPOL,ANC__MEPC)
      REAL*8     SEFD_FRQ(ANC__MNPOL,ANC__MEPC)
      INTEGER*4  NP_SEFD(ANC__MNPOL)
...
! ----
      ...
      INTEGER*4  NPOL, IND_POLS(ANC__MNPOL, ANC__MTPS)
...
!
      ...
      FL_IND_ALL     = .FALSE.   ! Should we plot all the indices?
      PLOTORD        = 'tsys'
      PLOTVAR        = 'time'    ! or 'elev'  or 'spectr' or 'azim' or 'azel'
      TIM_DIF_MAX    = 9.5D0 
      PRINTMOD       = 1
!     
! --- Check if the hostname is sagitta, and if so, change the plots_dir
!      
      CALL GET_ENVIRONMENT_VARIABLE ( "HOST", HOSTNAME )
      IF ( TRIM(HOSTNAME) == 'sa'                          .OR.         &
     &     TRIM(HOSTNAME) == "gs61a-sagitta.ndc.nasa.gov"  .OR.         &
     &     TRIM(HOSTNAME) == "gs61a-sagitta"                    ) THEN
         DIR_PLT   = '/t0/anc/plots/'
      ELSE
...
!
     ...
     &                           'tot|index|freq|index|time_range '//   &
     &                  '-pol  polarization '//                         &
     &                  '-ord  plot_ordinate '//                        &
     &                  '-var  plot_variable '//                        &
     &                  '-del  tim_dif_max '//                          &
...
!
! --------- Check if the index is "tot"
!
            CH_IND_ALL = CH_IND
            CALL TRAN ( 11, CH_IND_ALL, CH_IND_ALL )
            IF ( CH_IND_ALL == 'TOT') FL_IND_ALL = .TRUE.
!     
! --------- If we are plotting specific freq. then Check if the index
!           is an integer or real number
! --------- N.B: A real number is when there a decimal point on the input ==> -frq_val
!
            IF ( .NOT. FL_IND_ALL ) THEN 
...
!
     ...
     &           PLOTORD == 'fmpt' .OR. PLOTORD == 'sefd'      ) THEN              !
               CONTINUE
            ELSE 
               IUER = -1
               CALL ERR_LOG ( 5008, IUER, 'BNC_PLOT',                   &
     &                 'Unsupported mode '//PLOTORD//'. Expected '//    &
     &                 ' -- tsys, phas, ampl, fmgt, fmpt, or sefd' )
               CALL EXIT ( 1 )
            END IF ! plotord
!
! --------- Arg. 11
...
!
                  IF ( CH_FLG3 .NE. '-tim_ind' ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 5055, IUER, 'BNC_PLOT',             &
     &                       'Switch the flag "'//TRIM(CH_FLG3)//       &
     &                       '" to "-tim_ind" ' )
                  END IF
!     
...
!
                  IF ( IND_FRQ .NE. ATP__INIT ) THEN !
                     IND_TIM = IND_FRQ
                  ELSE
                     CALL ERR_LOG ( 5056, IUER, 'BNC_PLOT',             &
     &                    'When plotting SEFD vs Spectrum, an integer'  &
     &                    //' for time index is expected' )
                     CALL EXIT ( 1 )
                  END IF ! ind_frq
               ELSE
                  IUER = -1
                  CALL ERR_LOG ( 5054, IUER, 'BNC_PLOT',                &
     &                    'sefd plot unsupported mode '//      &
     &                     PLOTVAR//' -- time, or spectr expected' )
                  CALL EXIT ( 1 )
               END IF ! plotvar (given plotord = sefd )
            END IF              ! plotord 
!
! --------- Arg. 13
...
!
                  IF ( IND_FRQ .NE. ATP__INIT ) THEN !
                     IND_TIM = IND_FRQ
                  ELSE
                     CALL ERR_LOG ( 5058, IUER, 'BNC_PLOT',        &
     &                    'When plotting SEFD vs Time, an integer '//   &
     &                    'for time index is expected' )
                     CALL EXIT ( 1 )
                  END IF ! ind_frq
! ------------
               ELSE
                  IUER = -1
                  CALL ERR_LOG ( 5057, IUER, 'BNC_PLOT',                &
     &                    'sefd plot unsupported mode '//      &
     &                     PLOTVAR//' -- time, or spectr expected' )
                  CALL EXIT ( 1 )
               END IF ! plotvar (given plotord = sefd )            
...
!
! --------- Sometime SDE files have 8 letter long names. Let's try
!           that version as well
!
            CALL CLRCH ( STR1(1) )
            STR1(1) = FIL_BNC ( LBNC-16:LBNC-9 )
! ---------
            IF ( TRIM(STR1(1)) .NE. TRIM(ANC%EXP_CODE) ) THEN
! ------------
               IUER = -1
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
         FIL_STA_LST = '/progs/atp_20230928/share/vlbi_station.names'
         CALL GET_STA_ID ( FIL_STA_LST, STA_NAM, STA_ID, IUER )
      END IF
! ---
...
!
     ...
     &             'Error setting default values for the plot' )
      ...
      END IF
!
! --- If we are printing everything, then the frequency index should be
!     equal to zero.
!
      IF ( FL_IND_ALL ) THEN      
!
...
! ---
         ...
         WRITE ( 6 ,* ) 'Num_tps:   ', ANC%NUM_TPS,  'Num_SEFD: ', ANC%NUM_SEFD
         WRITE ( 6 ,* ) 'Plot_type: ', PLOTORD, ' ( ', PLOTVAR, ' ) '
         WRITE ( 6, * ) '______________________________________________'
!     
! ------ Dealing with Tsys
!
         IF ( PLOTORD == 'tsys' ) THEN
!
! --------- Plot the non-spectrum plots
! --------- Deal with Tsys vs Time or the look angles: time; elev;
!           azim; azel
!
            IF ( PLOTVAR .NE. 'spectr' ) THEN
!
! ------------ Go through each frequncy and generate the plot for it, 
!              if it exists. 
!
               DO 210 K1 = 1, ANC%NUM_TPS
!
! --------------- Accept only Tsys elements with defined frequencies
!
                  DELTS = ABS( ANC%TPS(K1)%SKY_FRQ - ANC__FILLER_R8 ) 
                  IF ( DELTS .LT. DEL_MIN  ) GOTO 210
!     
! --------------- Get the frequency index and the polarizarion
!
                  IND_FRQ = K1
...
!
! --------------- Convert sky frequency to string, shift to the left 
!                 incase of inital blanks.
!
                  WRITE ( CH_FRQ, 111 ) ANC%TPS(IND_FRQ)%SKY_FRQ*1.D-6
...
!
     ...
     &                       ANC%TPS(IND_FRQ)%SKY_FRQ*1.D-6,            & ! Converted to MHz for title
     &                       ANC__POL(ANC%TPS(IND_FRQ)%POL),            &
     &                       ANC%TPS(IND_FRQ)%ID
! ---------------
                  WRITE ( 6, 106 ) K1, ANC%NUM_TPS, CHAR(13)
!     
...
!
                  IUER = -1
                  CALL TSYS_TIME_FILTER_RAW ( ANC, IND_FRQ, NP, T1, X1, &
     &                                  T11, T12, IUER )
!
...
!
                  IUER = -1
                  CALL TSYS_TIME_FILTER_SCAN ( ANC, IND_FRQ,            &
     &                                   TIM_DIF_MAX, NP, T1, X1, NS,   &
...
!
                     DO 1410 J1 = 1, NP !LP
                        T1(J1) = ( T1(J1) + ANC%TAI_TTO +              &
     &                             ANC%UTC_MTAI)/3.6D3 - 24.D0
 1410                CONTINUE
!
...
! ------------------------ Plotting values
!
                           T2(JP)  = ( TIM_AVR(J2) + ANC%TAI_TTO +     &
     &                               ANC%UTC_MTAI )/3600.D0 - 24.D0        ! Time in hrs from the first 00:00 UTC
                           T3(JP)  = TIM_AVR(J2) + ANC%TAI_TTO             ! Time in seconds 
                           T22(JP) = EL_SCA(JP)
                           T33(JP) = AZ_SCA(JP)
...
!
                           IF ( FL_APPLY_TATM ) THEN
                              ...
                              TATM = FSPL8 ( T22(JP), LE, ELEV_TATM,    &
     &                                 VAL_TATM, IP, SPL_TATM )
                              X2(JP) = X2(JP) - TATM
                              E2(JP) = E2(JP) - TATM
                           END IF
                        END IF
...
!
                     IF ( JP == 0 ) THEN
                        IUER = -1
                        CALL ERR_LOG ( 5048, IUER, 'BNC_PLOT',          &
...
! ------------------------
     ...
     &                                 ANC%PCS(IND_FRQ)%SKY_FRQ*1.D-6,            & ! Converted to MHz for title
...
!
 210           CONTINUE
!
! --------- Plot the spectrum plots (Tsys)
! --------- N.B: The spectrum plots one polarization at a time, so
!                you have to run it twice.
!
            ELSE
!
! ------------ To reduce runtime, we will limit the number of time plots 
!              based on maximum number os scans.
!
               IVAR_FLAG = 1
               IUER      = -1
               CALL ATP_SCANS ( ANC, TIM_DIF_MAX, IVAR_FLAG, K2,        &
     &                          IARR, IUER )
! ------------               
               DO 211 K1 = 1, K2 !ANC%NUM_TSYS
!
! --------------- Get the time index and make room for later correcting the
!                 polarization
!
                  IND_TIM = K1                  
                  CALL INCH ( IND_TIM, CH_IND )
                  IN_POL  = IN_POL
! ---------------
                  WRITE ( 6, 106 ) K1, K2, CHAR(13)
!     
! --------------- Generate plots
!
                  IF ( CH_PLOT == 'raw' )  THEN
...
!                    e.g. mg0024_tsys_spectr_tim_ind_198_V_raw.gif =
!                                    {exp_name}_{ord}_{var}_tim_ind_{tim_ind}_{pol}_{raw}.gif
!                    e.g. vo3250gs_phas_spectr_sca_ind_3_V_ave.gif =
!                                    {exp_name}{sta_id}_{ord}_{var}_sca_ind_{sca_ind}_{pol}_{ave}.gif
!
                     CALL CLRCH ( FNAM_PLT(1) )
                     IF ( CH_EXP == 'SDE' ) THEN
                        FNAM_PLT(1) = TRIM(DIR_PLT)//                   &
...
!
                     IUER = -1
                     CALL TSYS_FREQ_FILTER_RAW ( ANC, IND_TIM, IN_POL,  &
     &                                   NP, FRQ, TSYS_ARR, IUER )
!     
...
! --------------------- Get seconds since J2000 and (MJD_TTO, TAI_TTO)
!
                        TSINCE(1) = MJDSEC_TO_TIM ( ANC%MJD_TTO,        &
     &                                              ANC%TAI_TTO )
!
! --------------------- Get seconds since J2000 and time of computation
!
                        TSINCE(1) = TSINCE(1) + ANC%TTO(IND_TIM)%TIM
!
...
!%%                           PRINT *, "%%%%% BNC_PLOT_2 - 1635 %%%%%%%%%%"
   !%%                        IUER = -1
      !%%                     CALL DIAGI_1 ( NP, F1, X1, IUER )
                     ...
                     ELSE
                        IUER = -1
                        CALL ERR_LOG ( 5049, IUER, 'BNC_PLOT',          &
     &                          'Not enough points to make a '//        &
     &                          'Tsys raw spectrum for time index '//   &
     &                          TRIM(CH_IND) )
                     END IF
! ------------------
                     IF ( CH_PLOT == 'tot' ) GOTO 1812
! ------------------
                  ELSEIF ( CH_PLOT == 'ave' ) THEN
! ------------------
 1812                CONTINUE
...
!                    e.g. mg0024_tsys_spectr_tim_ind_198_V_raw.gif =
!                                    {exp_name}_{ord}_{var}_tim_ind_{tim_ind}_{pol}_{raw}.gif
!                    e.g. vo3250gs_tsys_spectr_sca_ind_3_V_ave.gif =
!                                    {exp_name}{sta_id}_{ord}_{var}_sca_ind_{sca_ind}_{pol}_{ave}.gif
!
                     CALL CLRCH ( FNAM_PLT(2) )
                     IF ( CH_EXP == 'SDE' ) THEN
     ...
     &                       CH_POL(1:1)//'_ave.gif'
                     END IF
!     
! ------------------ Get the scan averages and RMS for Tsys for this 
!                    particular scan.
!     
                     IUER = -1
...
! --------------------- Get seconds since J2000 and (MJD_TTO, TAI_TTO )
!
                        TSINCE(2) = MJDSEC_TO_TIM (ANC%MJD_TTO, ANC%TAI_TTO )
!
! --------------------- Get seconds since J2000 and time of computation
!
...
!%%                           PRINT *, "%%%%% BNC_PLOT_2 - 1766 %%%%%%%%%%"
   !%%                        IUER = -1
      !%%                     CALL DIAGI_1E ( NF, F1, X1, E1, IUER )
                     ...
                     ELSE
                        IUER = -1
                        CALL ERR_LOG ( 5050, IUER, 'BNC_PLOT',          &
     &                          'Not enough scans to make a Tsys '//    &
     &                          'ave spectr plot for time index '//     &
     &                          TRIM(CH_IND) )
                     END IF
! ------------------
                     IF ( CH_PLOT == 'tot' ) GOTO 1813
! ------------------
                  ELSE
! ------------------
                     GOTO 1811
...
! ---------------
 211           CONTINUE
            END IF
!
! ------ Dealing with Phase and Amplitude
!
         ELSEIF ( PLOTORD == 'phas' .OR. PLOTORD == 'ampl' ) THEN
!     
! --------- Plot the non-spectrum plots
!
            IF ( PLOTVAR == 'time' ) THEN
!
! ------------ Go through each frequency and generate the plot for it,
!              if it exists
!     
               DO 212 K1 = 1, ANC%NUM_PCS
!
! --------------- Accept only Tsys elements with defined frequencies
!
                  DELTS = ABS( ANC%PCS(K1)%SKY_FRQ - ANC__FILLER_R8 ) 
                  IF ( DELTS .LT. DEL_MIN  ) GOTO 212
!
! --------------- Get the frequency index and the polarizarion
!
                  IND_FRQ = K1
...
!                 incase of inital blanks.
!
                  WRITE ( CH_FRQ, 111 ) ANC%PCS(IND_FRQ)%SKY_FRQ*1.D-6
                  CH_FRQ = CH_FRQ(1:I_LEN(CH_FRQ)-2)
                  CALL CHASHL( CH_FRQ )
...
!
     ...
     &                      ANC%PCS(IND_FRQ)%ID
! ---------------
                  WRITE ( 6, 106 ) K1, ANC%NUM_PCS, CHAR(13)
!
! --------------- Generate plot file names for both raw and ave data
!                 if SDE:
!                 e.g. mg0024_tsys_time_198_7416Va_ave.gif =
...
!
                  IUER = -1
                  CALL PCAL_TIME_FILTER_RAW ( ANC, IND_FRQ, NP, T1,     &
     &                                        CX1, IUER )
!
...
!
                  IUER = -1
                  CALL PCAL_TIME_FILTER_SCAN ( ANC, IND_FRQ,            &
     &                                   TIM_DIF_MAX, NP, T1, CX1, NS,  &
     &                                   TIM_AVR, PCAL_AVR,             &
...
!
                        IF ( JP == 0 ) THEN
                           IUER = -1
                           CALL ERR_LOG ( 5051, IUER, 'BNC_PLOT',             &
     &                             'Error computing scan averages for '//     &
     &                             TRIM(FIL_BNC)//' tsys at freq ind '//        &
     &                             TRIM(CH_IND))
                        END IF
...
! --------- Plot the spectrum 
! --------- N.B: The spectrum plots one polarization at a time, so
!                you have to run for each polarization
!     
            ELSEIF (PLOTVAR == 'spectr' ) THEN
!@@@@@@@@@@@@@@@@@@@@@@
!
! ------------ To reduce runtime, we will limit the number of time plots 
!              based on maximum number of scans.
!
               IVAR_FLAG = 2
               IUER      = -1
               CALL ATP_SCANS ( ANC, TIM_DIF_MAX, IVAR_FLAG, K2,        &
...
!
                     IUER = -1
                     CALL PCAL_FREQ_FILTER_RAW ( ANC, IND_TIM, IN_POL,  &
     &                                     NP, FRQ, CX1, IUER )
!
! ------------------ Do we have at least 1 point to plot?
!
...
!
                        TSINCE(1) = MJDSEC_TO_TIM ( ANC%MJD_PCAL,       &
     &                                              ANC%TAI_PCAL)
!
! --------------------- Get seconds since J2000 and time of computation
!
...
!
                           ...
                           X1 = 0.D0
                           DO 1681 J1 =  1, NP
                              F1(J1) =  FRQ(J1)/1.D9                 ! Frequency [GHz]
...
! ---------------------
                        ...
                        DIA(1)%NCLR      = 1
!
! --------------------- Different colour of ampl and phas plots
!
                        IF ( PLOTORD == 'phas' ) THEN
                           DIA(1)%ICOL(1)   = ICL2
                        ELSE
...
! ---------------------
                     ...
                     ELSE
                        IUER = -1
                        CALL ERR_LOG ( 5052, IUER, 'BNC_PLOT',          & 
     &                          'Not enough points to make a '//        &
     &                          PLOTORD//' raw spectr for time index '  &
     &                          //TRIM(CH_IND) )
                     END IF
! ------------------
                     IF ( CH_PLOT == 'tot' ) GOTO 1672
! ------------------
                  ELSEIF ( CH_PLOT == 'ave' ) THEN
! ------------------
 1672                 CONTINUE               
...
!
! --------------------- Different color of ampl and phas plots
!
                        IF ( PLOTORD == 'phas' ) THEN
                           DIA(2)%ICOL(1)   = ICL2
                        ELSE
...
! ---------------------
                     ELSE
                        IUER = -1
                        CALL ERR_LOG ( 5053, IUER, 'BNC_PLOT',           &
     &                          'Not enough scans to make a '//PLOTORD// &
     &                          ' ave spectr for time index '//          &
     &                           TRIM(CH_IND) )
                     END IF
! ------------------
                     IF ( CH_PLOT == 'tot' ) GOTO 1673
! ------------------
                  ELSE
                     GOTO 1671
                  END IF
...
!@@@@@@@@@@@@@@@@@@@@@@               
            END IF
         ELSEIF ( PLOTORD == 'fmpt' .OR. PLOTORD == 'fmgt' ) THEN
!
! --------- Define all the boards
!
            DO 711 J1 = 1, ANC%NUM_TGPS
               CH_BRD_TOT(J1:J1) = ANC%TGPS(J1)%BOARD
 711        CONTINUE
!
! --------- Define Title of all boards
!
            CALL CLRCH  ( TIT )
            WRITE ( TIT, 115 ) STR_DO_DATE, PLOTORD, TRIM(CH_BRD_TOT)
!     
! --------- Generate file name
!           if SDE:
!           e.g. mgx054_fmgt_time__a_raw.gif =
!                      {exp_name}_{ord}_{var}_{tot}_{raw}.gif
!           if VLBI:
!           e.g. vo3250mg_fmgt_time_1_a_raw.gif =
!                      {exp_name}{stn_id}_{ord}_{var}_{tot}_{raw}.gif
!
            CALL CLRCH ( FNAM_PLT(1) )
            IF ( CH_EXP == 'SDE' ) THEN
               FNAM_PLT(1) = TRIM(DIR_PLT)//                            &
     ...
     &                       TRIM(CH_BRD_TOT)//'_raw.gif'
            END IF
!
! --------- Get the raw data for each board
!
            DO 721 J1 = 1, ANC%NUM_TGPS
! ------------
               IUER = -1
               T1 = 0.D0
               X1 = 0.D0
               X2 = 0.D0
               CALL FMTGPS_TIME_FILTER_RAW(ANC,J1,NP,T1,X1,X2,IUER)
! ------------
               IF ( NP < 0 ) GOTO 721
!
! ------------ Write to total matrices
! ------------ Convert time to hours from midnight UTC, and scale the 
!              clock differences
!
               NP_TOT  ( J1 )        = NP
               DO 722 J2 = 1, NP
                  T1_TOT  ( J1, J2 )  = ( T1(J2) + ANC%TAI_GPS +             &
     &                                    ANC%UTC_MTAI )/3.6D3 - 24.D0
                  FMGT_TOT( J1, J2 )  = X1(J2)*1.D6
                  FMPT_TOT( J1, J2 )  = X2(J2)*1.D9
 722           CONTINUE
 721        CONTINUE
!     
! --------- Proceed only if there are actual points to use
!
            IF ( NP > 0 ) THEN
!
! ------------ Populate the Diagi derived type that relates to raw plots
!
               CALL NOUT ( SIZEOF(DIA(1)), DIA(1) )
!
               DIA(1)%IDEV      = 10
...
! ---------------
                  DIA(1)%NPOI(J1)   = NP
                  DIA(1)%ADR_X8(J1) = LOC( T1   )
                  DIA(1)%ADR_Y8(J1) = LOC( FMGT )
                  DIA(1)%LER(J1)    = .FALSE.                  
 731           CONTINUE
! ------------
               DIA(1)%XMIN    = T1( 1 )     - X_RNG*DIAGI_FIE
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
         ...
         END IF ! plotord == tsys (for all plots)
      ELSE
!@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@!
! ------ Plot one frequency/time slot
!@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@#$@!
...
!
! ------------ If dealing with SEFD
!
               ELSEIF ( PLOTORD == 'sefd' ) THEN
                  IUER = -1
                  CALL ERR_LOG ( 5059, IUER, 'BNC_PLOT',                &
     &                    'Ordinate SEFD only takes integers for '//    &
     &                    'index. Not '//CH_IND )
               ELSEIF ( PLOTORD == 'fmgt' .OR. PLOTORD == 'fmpt' ) THEN
                  IUER = -1
                  CALL ERR_LOG ( 5060, IUER, 'BNC_PLOT',                &
     &                    'Ordinate'//PLOTORD//' does not take '//    &
     &                    'frequency values. Just times.' )
               END IF
!
...
!
!@@               Print *, "%%%%%%%%%%%%%%%%% BNC_PLOT - 2479 %%%%%%%%%%"
   !@@            PRINT *, "IND_FRQ= ", IND_FRQ, "CH_IND = ", CH_IND
...
!
               ELSEIF ( PLOTORD == 'sefd' ) THEN
!
! --------------- Get the number of polarizations and what they are
! --------------- N.B: We assume there will be a maximum of two polarizations
!
                  IUER = -1
                  CALL GET_POL_TPS ( ANC, NPOL, IPOLS, IUER )

                  IF ( NPOL .GE. 2 ) THEN
                     CH_POL = ANC__POL(IPOLS(1))//ANC__POL(IPOLS(2))                  
                  ELSEIF ( NPOL == 1 ) THEN
                     CH_POL = ANC__POL(IPOLS(1))
                  ELSE
                     IUER = -1
                     CALL ERR_LOG ( 5061, IUER, 'BNC_PLOT',             &
     &                       'There are no polarization(s) to plot' )
                  END IF
! 
! ------------ if dealing with forter clocks
!
               ELSEIF ( PLOTORD == 'fmgt' .OR. PLOTORD == 'fmpt' ) THEN
                  CALL CLRCH ( CH_POL )
               END IF           ! plotord == tsys
            END IF ! ind_frq == atp_init
         END IF ! pltvar .ne. spectr
! ------
         IUER = 0
         STR_DO_DATE = MJDSEC_TO_DATE ( ANC%MJD_DOO, ANC%TAI_DOO +      &
     &                                  ANC%UTC_MTAI, IUER )
! ---
         ...
         WRITE ( 6 ,* ) 'Num_tgps: ', ANC%NUM_TGPS, 'Num_GPS:  ', ANC%NUM_GPS
         WRITE ( 6 ,* ) 'Num_tps:  ', ANC%NUM_TPS,  'Num_SEFD: ', ANC%NUM_SEFD
         WRITE ( 6 ,* ) 'Plot_type: ',PLOTORD,' ( ',PLOTVAR, ' ) ind: ', IND_FRQ
         WRITE ( 6, * ) '______________________________________________'
!
...
! ------
         ELSEIF ( PLOTORD == 'fmgt' .OR. PLOTORD == 'fmpt' ) THEN
!
! --------- Write Plot Title to "TIT"
!
            CALL CLRCH  ( TIT )
            CH_BRD = ANC%TGPS(IND_FRQ)%BOARD
            WRITE ( TIT, 115 ) STR_DO_DATE, PLOTORD, CH_BRD
         ENDIF
...
!
               IUER = -1
               CALL TSYS_TIME_FILTER_SCAN ( ANC, IND_FRQ, TIM_DIF_MAX,   &
     &                                      NP, T1, X1, NS,              &
     &                                      TIM_AVR, TSYS_AVR, TSYS_RMS, &
     &                                      IND_SCA, IND_ARR, IUER )
!
...
!
                  DO 410 J1 = 1, NP !LP
                     T1(J1) = (T1(J1) + ANC%TAI_TTO + ANC%UTC_MTAI)/   &
     &                        3.6D3 - 24.D0
 410              CONTINUE
!
! --------------- Filter out any zero values from the average values
...
!
     ...
     &                    TSYS_AVR(J2) < ANC__TSYS_MAX       ) THEN
!
! --------------------- Skip elevations that are below the minimum 
!                       elevation
! --------------------- This is the Elevation where this scan ends
!
                        IF ( PLOTVAR == 'elev' ) THEN
...
!
     ...
     &                                      ANC__POL(IN_POL), IND_TIM
!
! ------------------ Convert to frequency from Hz to GHz
!
                     F1 = 0.D0
                     X1 = 0.D0
                     DO 810 J1 =  1, NP
...
!     
               IUER = -1
               CALL PCAL_TIME_FILTER_SCAN ( ANC, IND_FRQ, TIM_DIF_MAX,  &
     &                                      NP, T1, CX1, NS, TIM_AVR,   &
...
!     
                  DO 510 J1 = 1, NP !LP
                     T1(J1) = (T1(J1) + ANC%TAI_PCAL + ANC%UTC_MTAI)/   &
     &                        3.6D3 - 24.D0
 510              CONTINUE
!
! --------------- Plot for the phase
...
! ------ N.B: The time diffeerences are only plotted as a function of
!            time
!     
         ELSEIF ( PLOTORD == 'fmgt' .OR. PLOTORD == 'fmpt' ) THEN
!
...
!           if SDE:
!           e.g. mgx054_fmgt_time_1_a_raw.gif =
!                      {exp_name}_{ord}_{var}_{ind}_{board}_{raw}.gif
!           if VLBI:
!           e.g. vo3250mg_fmgt_time_1_a_raw.gif =
!                      {exp_name}{stn_id}_{ord}_{var}_{ind}_{board}_{raw}.gif
!
            CALL CLRCH ( FNAM_PLT(1) )
            IF ( CH_EXP == 'SDE' ) THEN
               FNAM_PLT(1) = TRIM(DIR_PLT)//                            &
...
!
               DO 710 J1 = 1, NP
                  T1(J1)   = ( T1(J1) + ANC%TAI_GPS +                   &
     &                         ANC%UTC_MTAI )/3.6D3  - 24.D0
                  FMGT(J1) = FMGT(J1)*1.D6
                  FMPT(J1) = FMPT(J1)*1.D9
 710           CONTINUE
!     
! ------------ Populate the diagi derived type that relates to raw plots
!
...
!
 ...
 730           CONTINUE
!     
! ------------ Plot for the formatter minus GPS for 1 formatter
!
               IF ( PLOTORD == 'fmgt' ) THEN
! ---------------
                  Y_TEMP = 0.D0
...
! ---------------
                  ...
                  DIA(1)%STATUS    = DIA__DEF
!
! ------------ Plot for the formatter minus GPS
!
               ELSE
                  Y_TEMP = 0.D0
                  Y_TEMP = FMPT
...
!
         ELSEIF ( PLOTORD == 'sefd' ) THEN
            ...
            IUER = -1
            CALL SEFD_FREQ_FILTER_RAW ( ANC, NPOL, IPOLS, IND_TIM,      &
     &                                  NP_SEFD, SEFD_FRQ, SEFD, IUER )
           
            PRINT *, "%%%%% BNC_PLOT_2 - 4053 %%%%%%%"
      !@      PRINT *, "NPOL: ", NPOL, "IPOLS: ", (IPOLS(J2), J2=1,NPOL), "IND_TIM", IND_TIM

        !@    PRINT *, "NPTS: ", (NP_SEFD(J2), J2=1,NPOL)
          !@ T1 = SEFD_FRQ(1,1:NP_SEFD(1))
!@           T2 = SEFD_FRQ(2,1:NP_SEFD(2))
  !@         X1 = SEFD(1,1:NP_SEFD(1))
    !@       X2 = SEFD(2,1:NP_SEFD(2))


!@           PRINT *, "POL", ANC__POL(IPOLS(1))
  !@         DO J1 = 1, NP_SEFD(1)
    !@          PRINT *, T1(J1), X1(J1)
      !@     END DO
           
        !@   IUER = -1
  !@         CALL DIAGI_2( NP_SEFD(1), T1, X1, NP_SEFD(2), T2, X2, IUER )
...
!@!##!@!     &         ' POL= ', A1,' ID= ', A4 )
 110  FORMAT ( A22, ' PLOT=',A4 ,' FRQ=',F7.1,'POL=', A2,' ID=', A7)
 111  FORMAT ( F8.1 )
