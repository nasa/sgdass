...
! *                                                                      *
! *  ### 07-MAY-2021  ANC_PARSE   v3.2 (d)  N. Habana  22-SEP-2025  ###  *
! *                                                                      *
! ************************************************************************
!
      IMPLICIT   NONE 
      ...
      INTEGER*4  LIND2, IND2(2,MIND)
      INTEGER*4  IND_DOO, IND_MET, IND_CBL
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, JP1
...
!
! --- Set the default experiment type to VLBI
!
      ANC%EXP_TYPE = 'VLBI'
!
...
!
      DO 470 JP1 = 1, ANC__MVPV
         ANC%PRV(JP1)%N_TEXT = 0
         DO 480 J8=1,ANC__MPRV
            CALL CLRCH ( ANC%PRV(JP1)%TEXT(J8) )
 480     CONTINUE
 470  CONTINUE
!
      IND_PRV_CUR  =  0
...
!
     ...
     &           'Unsupported version of ANTCAL FMT'//TRIM(BUF(1)) )
      ...
      END IF
!
! --- If the format is recognised, parse the file to ANC%FILIN
!
      ANC%FILIN = TRIM(FILIN)
!     
! --- Which version of the anc file are we dealing with?
!
...
!
     ...
     &                   ANC%TTO(K_TTO)%EL                            
                  ...
                  ANC%TTO(K_TTO)%SOU_NAM = CUR_LINE(75:82)              ! Source Name (74:82)
                  ANC%TTO(K_TTO)%SCA_NAM = CUR_LINE(85:100)             ! Scan name (85:95)
               END IF
!
...
!
     ...
     &                   ANC%PCAL(K_PCAL)%IND_SCA
                  ...
                  ANC%PCAL(K_PCAL)%SCA_NAM = CUR_LINE(81:96)            ! Scan name (81:91)
               END IF
!
...
!
               ...
               READ ( UNIT=CUR_LINE(60:67), FMT='(F8.5)' ) RPHAS ! Phase angle [rad]
!
...
!
     ...
     &                   ANC%TPI(K_TPI)%EL                            
                  ...
                  ANC%TPI(K_TPI)%SCA_NAM = CUR_LINE(95:110)             ! Scan name (95:105)
               END IF
!
...
!
            IF ( CUR_LINE(IND(1,1):IND(2,1)) == 'STATION:' ) THEN
               ANC%STA_NAM  =  TRIM( CUR_LINE ( IND(1,2):IND(2,2) ) )
!
! --------- Get the experiment code
!
            ELSEIF ( CUR_LINE(IND(1,1):IND(2,1)) == 'EXP_CODE:' ) THEN
               ANC%EXP_CODE = CUR_LINE(IND(1,2):IND(2,2))
!
! --------- UTC_MTAI
!
...
!
! --------- How many PROVENANCEs are there?
!
...
!                     "PROVENANCE: 1"
!
               ANC%PRV(IPRV)%N_TEXT = ANC%PRV(IPRV)%N_TEXT + 1
               IF ( LIND .GE. 3 ) THEN
                    ANC%PRV(IPRV)%TEXT(ANC%PRV(IPRV)%N_TEXT) = CUR_LINE( IND(1,3):IND(2,LIND) )
                  ELSE
                    CALL CLRCH ( ANC%PRV(IPRV)%TEXT(ANC%PRV(IPRV)%N_TEXT) )
               END IF
               ANC%NUM_PRV_LINES  = IPRV*IPRV_CNT
!     
...
! ---------
     ...
     &                    'Error in parsing DATA_ON date '//            &
     &                    CUR_LINE(IND(1,4):IND(2,4))//' on line'//     &
     &                    TRIM(STR) )
                  DEALLOCATE ( BUF )
                  RETURN
               END IF
...
!
         DO 411 J1=2,NBUF
            PREV_LINE = CUR_LINE
            CUR_LINE = BUF(J1)
            IF ( CUR_LINE(1:1) == '#' ) GOTO 411 ! Skip comments
!
! --------- First process the performance critical lines: TSYS, PCAL, 
!           TPI, FMTGPS, OPACITY, TATM, and SEFD using the most efficent way
!
!==============================================================================
!
...
! --------------- Azimuth [rad]     
!
                  READ ( UNIT=CUR_LINE(54:63),  FMT='(F10.4)' )         &
     &                   ANC%TTO(K_TTO)%AZ
...
! --------------- Source Name
!
                  ANC%TTO(K_TTO)%SOU_NAM = CUR_LINE(75:82)              ! (74:82)
!
! --------------- Scan Name
!
                  ANC%TTO(K_TTO)%SCA_NAM = CUR_LINE(85:100)             ! (85:75)
               END IF
!
! ------------ Is the sensor tag in TTO block available in list of tags
!              from TPS_SENSOR block
...
! --------------- Azimuth [rad]     
!
                  IF ( CUR_LINE(74:82) == '*********' ) CUR_LINE(74:82) = ' 0.0     '
...
! --------------- Elevation [rad]
!
                  IF ( CUR_LINE(85:91) == '*******' ) CUR_LINE(85:91) = ' 0.0   '
...
! --------------- Scan name
!
                  ANC%TPI(K_TPI)%SCA_NAM = TRIM(CUR_LINE(104:119))
               END IF
!
! ------------ Is the sensor tag in TPI block available in list of tags
!              from TPS_SENSOR block
...
!
     ...
     &                ANC%TPI(K_TPI)%TPIZERO(IND_TPS)
! ------------
               GOTO 411
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
            ELSEIF ( CUR_LINE(1:5) == 'PCAL:' ) THEN
...
! --------------- Check for overflow
!
                  IF ( K_PCAL > ANC%NUM_EPO_PCAL ) THEN
                     CALL CLRCH (     STR )
                     CALL INCH  ( J1, STR )
                     CALL IINCH ( K_PCAL, STR1 )
                     CALL IINCH ( ANC%NUM_PCAL, STR2 )
...
! --------------- Source name
!
                  ANC%PCAL(K_PCAL)%SOU_NAM = CUR_LINE(70:79)
!
! --------------- Scan name
!
                  ANC%PCAL(K_PCAL)%SCA_NAM = CUR_LINE(81:96)
               END IF
!
! ------------ Is the sensor tag in PCAL block available in list of tags
! ------------ from PCS_SENSOR block
...
!
               ...
               READ ( UNIT = CUR_LINE(58:66), FMT='(F9.5)' ) RPHAS ! Phase angle [rad]
!
               IF ( ABS(RAMPL - ANC%FILLER_R8) > EPS ) THEN
                  RAMPL = RAMPL*ANC__AMP_SCA
               ELSE
...
! --------------- Scan name
!
                  ANC%GPS(K_GPS)%SCA_NAM = CUR_LINE(100:115)
               END IF
!     
...
!
 ...
 !                 PRINT *, "%%%%%%%%%%%%% ANC_PARSE - 2439 %%%%%%%%"
  !                PRINT *, "j1_",J1,"_"
   !               PRINT *, "ind_",K_TGPS,"_"
    !              PRINT *, "cur_",CUR_LINE(41:47),"_"
     !             PRINT *, "anc_",ANC%TGPS_TAG(K_TGPS+1),"_"
                  
                  IF ( CUR_LINE(41:47)==ANC%TGPS_TAG(K_TGPS+1) ) THEN
                     K_TGPS = K_TGPS + 1
                  ELSE
                     K_TGPS = LTM_DIF (0, ANC%NUM_TGPS, ANC%TGPS_TAG, &
     &                                   TRIM(CUR_LINE(41:47)) )
                  END IF
               ELSE
                  K_TGPS = LTM_DIF ( 0, ANC%NUM_TGPS, ANC%TGPS_TAG,   &
...
! --------------- Check for overflow
!
                  IF ( K_SEFD > ANC%NUM_EPO_SEFD ) THEN
                     CALL CLRCH (     STR )
                     CALL INCH  ( J1, STR )
                     CALL IINCH ( K_SEFD, STR1 )
                     CALL IINCH ( ANC%NUM_SEFD, STR2 )
...
!              coincides with the index of the sensor tag in TP_SENSOR 
!              Block. Units = K
!OPACITY:      1 2025.04.24-17:30:00.15  TP_003  0.0133    -9.5442 29.9191   1803+78  114-1730a  
!
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!
! --------- Parse information from the Total Atmospheric Brightness
!           Temperature (TATM) block
! --------- Note that TATM is computed from the Tsys data, ergo
!           there is no need to parse the date. Just take the same
!           epoch from ANC%TTO, and find corresponding TPS
!     
...
!-----------------------------------------------------------------------------
!
! --------- Less critical blocks parsed below
! --------- Parse information from the TP_SENSOR block
!
! --------- Extract words
!
...
!
            IF ( CUR_LINE(1:8) == 'STATION:' ) THEN
               ANC%STA_NAM = CUR_LINE(IND(1,2):IND(2,2))
!
! --------- Get the Experiment code
!
...
!
            ELSEIF ( CUR_LINE(1:9) == 'UTC_MTAI:' ) THEN
              READ ( UNIT=CUR_LINE(IND(1,2):IND(2,2)), FMT='(F6.1)' )  &
     &                ANC%UTC_MTAI
!
...
!
            ELSEIF ( CUR_LINE(1:8) == 'FILLERS:' ) THEN
! ------------
               IF ( LIND < 5 ) THEN
!
...
!
            ELSEIF ( CUR_LINE(1:15) == 'NUM_PROVENANCE:' ) THEN
               READ ( UNIT=CUR_LINE(IND(1,2):IND(2,2)), FMT='(I16)' )    &
     &                ANC%NUM_PRV
!
! --------- When we enter the part of the file that has 'DIMENSION'
!           it is time to check where the dimension belongs to
!     
            ELSEIF ( CUR_LINE(1:10) == 'DIMENSION:' ) THEN
!     
! ------------ How many DATA_ONs are there?
...
! --------------
                  READ ( UNIT=CUR_LINE(IND(1,3):IND(2,3)), FMT='(I6)')  &
     &                   ANC%NUM_DOO
!
! --------------- Allocate the number to ANC%DOO
!
...
!                 N.B: NUM_RECORDS .LE. NUM_TPS*NUM_EPO_TTO
!
                  IL = I_LEN(BUF(J1+1))
                  READ ( UNIT = BUF(J1+1)(19:IL), FMT='(I8)' )    &
     &                ANC%NUM_TSYS
                  ANC%NUM_TTO = ANC%NUM_TSYS
!
...
!                 N.B: NUM_RECORDS .LE. NUM_TPS*NUM_EPO_TPI
!
                  IL = I_LEN(BUF(J1+1))
                  READ( UNIT=BUF(J1+1)(18:IL), FMT='(I16)' ) ANC%NUM_TPI
!
! --------------- Allocate number to ANC%TPI
!
...
!                 N.B: NUM_RECORDS .LE. NUM_PCS*NUM_EPO_PCAL
!
                  IL = I_LEN(BUF(J1+1))
                  READ( UNIT=BUF(J1+1)(19:IL),FMT='(I16)' ) ANC%NUM_PCAL
!
! --------------- Allocate number to ANC%PCAL
!
...
!                 N.B: NUM_RECORDS .LE. NUM_FMTGPS*NUM_EPO_GPS
!
                  IL = I_LEN(BUF(J1+1))
                  READ( UNIT=BUF(J1+1)(21:IL), FMT='(I16)' ) ANC%NUM_GPS
!     
...
!
                  IL = I_LEN(BUF(J1+1))
                  READ( UNIT=BUF(J1+1)(22:IL), FMT='(I16)' ) ANC%NUM_OPA
!
!
! --------------- Allocate NUM_TPS in each time stamp's OPA
!
                  DO 570 J2=1,ANC%NUM_EPO_TTO
...
!
                  IL = I_LEN(BUF(J1+1))
                  READ( UNIT=BUF(J1+1)(19:IL),FMT='(I16)' ) ANC%NUM_TATM
!
! --------------- Allocate NUM_TPS in each time stamp's TATM
!
                  DO 580 J2=1,ANC%NUM_EPO_TTO
...
!
! ------------ Is the Block defined as expected?
!
               IF ( K_TPS == 0 ) THEN
!
! --------------- Are there the right number of words?
!
                  IF ( LIND < 11 ) THEN ! No less than 11 words in this block
                     CALL CLRCH (     STR )
                     CALL INCH  ( J1, STR )
                     CALL ERR_LOG ( 6978, IUER, 'ANC_PARSE',            &
     &                       'Too few words on TP_SENSOR block line '// &
...
!
               READ (UNIT=CUR_LINE(IND(1,3):IND(2,3)), FMT='(F8.2)')    &
     &                ANC%TPS(K_TPS)%IF_FRQ
               ANC%TPS(K_TPS)%IF_FRQ  = ANC%TPS(K_TPS)%IF_FRQ*1.D6
!
! ------------ Local Oscilator Freq. [Hz]
!              Use the fact of positions of the values around it
!
               READ ( UNIT=CUR_LINE(IND(1,4):IND(2,4)), FMT='(F10.2)' ) &
     &                ANC%TPS(K_TPS)%LO_FRQ
               ANC%TPS(K_TPS)%LO_FRQ  = ANC%TPS(K_TPS)%LO_FRQ*1.D6
!
! ------------ Sky Freq. [Hz]
!              Use the fact of positions of the values around it
!
               READ ( UNIT=CUR_LINE(IND(1,5):IND(2,5)), FMT='(F9.2)')   &
     &                ANC%TPS(K_TPS)%SKY_FRQ
               ANC%TPS(K_TPS)%SKY_FRQ = ANC%TPS(K_TPS)%SKY_FRQ*1.D6
!
! ------------ Bandwidth [Hz]
!              Use the fact of positions of the values around it
!
               READ ( UNIT=CUR_LINE(IND(1,6):IND(2,6)), FMT='(F7.2)')   &
     &                ANC%TPS(K_TPS)%BDW
...
!     
               READ ( UNIT = CUR_LINE(IND(1,9):IND(2,9)), FMT='(I4)' )              &
     &                ANC%TPS(K_TPS)%IF_IND
!
! ------------ Sensor Sideband Code
!
...
! ------------
     ...
     &                    CUR_LINE(IND(1,11):IND(2,11))//' (line '//    &
...
! --------- Parse information from the PC_SENSOR
!
            ELSEIF ( CUR_LINE(1:10) == 'PC_SENSOR:' ) THEN
!
! ------------ Is the Block defined as expected?
!
               IF ( K_PCS == 0 ) THEN
...
! --------------- Are there the right number of words?
!
                  IF ( LIND < 8 ) THEN ! No less than 11 words in this block
                     CALL CLRCH (     STR )
                     CALL INCH  ( J1, STR )
                     CALL ERR_LOG ( 6979, IUER, 'ANC_PARSE',            &
     &                       'Too few words on PC_SENSOR block line '// &
     &                       TRIM(STR)//' of the antenna calibration'// &
     &                       ' file '//TRIM(FILIN) )
                     DEALLOCATE ( BUF )
                     RETURN
...
!
                  IF ( .NOT. ASSOCIATED ( ANC%PCS ) ) THEN
...
! ------------ Sky Frequency [Hz]
!
               READ ( UNIT=CUR_LINE(IND(1,3):IND(2,3)), FMT='(F13.2)' ) &
     &                ANC%PCS(K_PCS)%SKY_FRQ
...
!
     ...
     &                    'polarization code. We expected R, L, H, '//  &
     &                    'V, X, or Y. See the antenna calibration '//  &
     &                    'file '//TRIM(FILIN) )
                  DEALLOCATE ( BUF )
                  RETURN
               END IF
...
! ------------ Intermediate Frequency Index
!
               READ ( UNIT=CUR_LINE(IND(1,6):IND(2,6)), FMT='(I6)' )    &
     &                ANC%PCS(K_PCS)%IF_IND
!
! ------------ Sensor Subband Code
!
               IF ( CUR_LINE(IND(1,7):IND(2,7))     == 'USB'   ) THEN
                  ANC%PCS(K_PCS)%SSB = ANC__USB
               ELSEIF ( CUR_LINE(IND(1,7):IND(2,7)) == 'LSB'   ) THEN
                  ANC%PCS(K_PCS)%SSB = ANC__LSB
               ELSEIF ( CUR_LINE(IND(1,7):IND(2,7)) == 'n/a'   ) THEN
                  ANC%PCS(K_PCS)%SSB = ANC__NA
               ELSEIF ( CUR_LINE(IND(1,7):IND(2,7)) == 'DUMMY' ) THEN
                  ANC%PCS(K_PCS)%SSB = ANC__NA
               ELSE
                  CALL CLRCH (     STR )
...
! ------------ Net Subband Code
!
               IF ( CUR_LINE(IND(1,8):IND(2,8)) == 'USB' ) THEN
                  ANC%PCS(K_PCS)%NSB = ANC__USB
               ELSEIF ( CUR_LINE(IND(1,8):IND(2,8)) == 'LSB' ) THEN
                  ANC%PCS(K_PCS)%NSB = ANC__LSB
               ELSEIF ( CUR_LINE(IND(1,8):IND(2,8)) == 'n/a' ) THEN
                  ANC%PCS(K_PCS)%NSB = ANC__NA
               ELSEIF ( CUR_LINE(IND(1,8):IND(2,8)) == 'DUMMY' ) THEN
                  ANC%PCS(K_PCS)%NSB = ANC__NA
               ELSE
                  CALL CLRCH (     STR )
                  CALL INCH  ( J1, STR )
                  CALL ERR_LOG ( 6884, IUER, 'ANC_PARSE',                  &
     &                 CUR_LINE(IND(1,8):IND(2,8))//' in line '//        &
     &                 TRIM(STR)//' is not a supported subband code.'// &
     &                 ' We expected USB, LSB, n/a, or DUMMY. See '//   &
     &                 'the antenna calibration file '//TRIM(FILIN) )
                  DEALLOCATE ( BUF )
...
! --------------- Are there the right number of words?
!
                  IF ( LIND < 3 ) THEN ! No less than 11 words in this block
                     CALL CLRCH (     STR )
                     CALL INCH  ( J1, STR )
                     CALL ERR_LOG ( 6981, IUER, 'ANC_PARSE',            &
     &                       'Too few words on FMT2GPS_TIME block '//   &
     &                       'line '//TRIM(STR)//' of the antenna '//   &
