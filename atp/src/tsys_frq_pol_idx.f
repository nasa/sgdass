      SUBROUTINE TSYS_FRQ_POL_IDX ( ANC, FRQ_VAL, IPOL, IND_FRQ, IUER )
!
! **************************************************************************************
! *                                                                                    *
! *   Routine  TSYS_FRQ_POL_IDX                                                        *
...
!
      ALLOCATE (ARR1(ANC%NUM_TPS), ARR2(ANC%NUM_TPS), ARR3(ANC%NUM_TPS))
!
      DO 300 J0 = 1, ANC%NUM_TPS
         ARR1(J0) = ANC%TPS(J0)%SKY_FRQ
         ARR2(J0) = REAL(ANC%TPS(J0)%POL, 8)
         ARR3(J0) = REAL(J0,8)
 300  CONTINUE
!
! --- Sort these in order of increasing frequency
!
      CALL SORT83 ( ANC%NUM_TPS, ARR1, ARR2, ARR3 )
!
...
!
            IDX1 = IXMN8 ( ANC%NUM_TPS, ARR1, FRQ_VAL )
!@@!            IF ( IDX1 == -1 ) IDX1 = 1                          ! Smaller than minimum freq.
!@@!            IF ( IDX1 == -2 ) IDX1 = ANC%NUM_TPS                    ! Larger than minimum freq.
!
! --------- Does this index match the polarization?
!
!@@!            IF ( ARR2(IDX1) == REAL(IPOL,8) ) THEN
               IND_FRQ_TMP = IDX1
               GOTO 700
 !@@!           END IF
         ELSE
...
!
            IDX2 = IDX1 + 1
!
            IDX1 = IXMN8_S (IDX2, ANC%NUM_TPS, ARR1, FRQ_VAL )
!@@!            IF ( IDX1 == -1 ) IDX1 = IDX2                          ! Smaller than minimum freq.
!@@!            IF ( IDX1 == -2 ) IDX1 = ANC%NUM_TPS                    ! Larger than minimum freq.
!
! --------- Does this index match the polarization?
!
...
! ------ 
         IUER  = -1
         CALL ERR_LOG ( 2912, IUER, 'TSYS_FRQ_POL_IDX',              &
...
! ---
      CALL ERR_LOG ( 0, IUER )
      RETURN
      END SUBROUTINE TSYS_FRQ_POL_IDX !#!1
!
