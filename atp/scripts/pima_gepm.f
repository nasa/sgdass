      SUBROUTINE PIMA_GEPM ( PIM, STA_NAM, T_ACCUM, GEPM_OVR, TIME_THRESH, DIFF_THRESH, MAX_COUNT, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine PIMA_GEPM 
! *                                                                      *
...
! *                                                                      *
! *  ### 21-JUN-2022  PIMA_GEPM   v1.0 (d)              21-JUN-2022 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
     ...
     &           FAIL_Y8DIFF, NUM_SUM, MAX_COUNT
      REAL*8     EPS, T_ACCUM, SIGMA_DETX, SIGMA_DETY, VAR_THRESH, T_SHIFT, &
     &           TIME_THRESH, DIFF_THRESH, BPASS_THRESH
      PARAMETER  ( MPB    =   7 )
     ...
     &     PHASE_ABOVE(PIM__MTON), AMP_ABOVE(PIM__MTON), BPASS_PHASE(PIM__MTON), &
     &     BPASS_SORTED(PIM__MTON-1), DIFF_PHASE(PIM__MTON), AVE_PHASE, AVE_AMP, &
     ...
     &     BPASS_DIFF(PIM__MTON), BPASS_MEDIAN
...
!
                 DO 430 J3=1,M_TONES-1
                    IF ( ABS ( BPASS_DIFF(J3) ) > BPASS_THRESH*BPASS_MEDIAN ) THEN
                        ! One tone logic
                        IF ( J3 == 1 .AND. ABS ( BPASS_DIFF(J3+1) ) < BPASS_THRESH*BPASS_MEDIAN ) THEN
                            PIM%PCAL_MASK(J3,J1,IND_STA) = 0
                            FAIL_ARR(J3,IFRQ,IND_STA) = FAIL_CPDIFF
                        ELSEIF ( J3 == M_TONES-1 ) THEN
                            IF ( ABS ( BPASS_DIFF(J3-1) ) < BPASS_THRESH*BPASS_MEDIAN ) THEN 
                                PIM%PCAL_MASK(J3+1,J1,IND_STA) = 0
                                FAIL_ARR(J3+1,IFRQ,IND_STA) = FAIL_CPDIFF
                            ENDIF
                        ELSEIF ( J3 /= M_TONES-1 ) THEN
                            IF ( ABS ( BPASS_DIFF(J3+1) ) > BPASS_THRESH*BPASS_MEDIAN .AND. &
        &                        SIGN ( BPASS_DIFF(J3), BPASS_DIFF(J3+1) ) /= BPASS_DIFF(J3) ) THEN
                                PIM%PCAL_MASK(J3+1,J1,IND_STA) = 0
                                FAIL_ARR(J3+1,IFRQ,IND_STA) = FAIL_CPDIFF
                            ENDIF
                        ENDIF
 ...
 430             CONTINUE
                 IF ( TWO_TONE_LOGIC .EQV. .TRUE. ) THEN
                     DO 440 J4 = 1,M_TONES-1
                         IF ( ABS( BPASS_DIFF(J4) ) < BPASS_THRESH*BPASS_MEDIAN ) THEN
                            IF ( J4 == 1 .AND. ABS ( BPASS_DIFF(J4+1) ) > BPASS_THRESH*BPASS_MEDIAN &
        &                        .AND. ABS ( BPASS_DIFF(J4+2) ) < BPASS_THRESH*BPASS_MEDIAN ) THEN
                                PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                                FAIL_ARR(J4:J4+1,IFRQ,IND_STA) = FAIL_CPDIFF
                            ELSEIF ( J4 == M_TONES-1 .AND. &
        &                            ABS ( BPASS_DIFF(J4-1) ) > BPASS_THRESH*BPASS_MEDIAN &
        &                            .AND. ABS ( BPASS_DIFF(J4-2) ) < BPASS_THRESH*BPASS_MEDIAN ) THEN 
                                PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                                FAIL_ARR(J4:J4+1,IFRQ,IND_STA) = FAIL_CPDIFF
                            ELSEIF ( J4 > 2 .AND. J4 <  M_TONES-2 ) THEN
                                IF ( ABS ( BPASS_DIFF(J4+1) ) > BPASS_THRESH*BPASS_MEDIAN &
        &                            .AND. ABS ( BPASS_DIFF(J4-1) ) > BPASS_THRESH*BPASS_MEDIAN & 
        &                            .AND. ABS ( BPASS_DIFF(J4+2) ) < BPASS_THRESH*BPASS_MEDIAN & 
        &                            .AND. ABS ( BPASS_DIFF(J4-2) ) < BPASS_THRESH*BPASS_MEDIAN & 
        &                            .AND. SIGN ( BPASS_DIFF(J4-1), BPASS_DIFF(J4+1) ) /= BPASS_DIFF(J4-1) ) THEN
                                    PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                                    FAIL_ARR(J4:J4+1,IFRQ,IND_STA) = FAIL_CPDIFF
                                ENDIF
                            ENDIF
...
!
                 DO 450 J5 = 1,M_TONES
                    ...
                    AMP_MED_TONE = TONE_SORT( CEILING ( NPOI_ACCUM*1.0D0 / 2 ))
                    IF ( AMP_MED_TONE < PIMA__AMP_MIN .AND. FREQS(J5,IFRQ) > PIMA__MIN_FRQ ) THEN
