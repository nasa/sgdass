#!/usr/bin/env python3
"""
Program l2b_sub_exe.py converts an experiment log file to binary
format as defined in the atp library.
It also generates and updates the reduced data anc file.

This is an executable version of l2b_sub.py

15-APR-2024  l2b_sub_exe.py    v1.1 (c) N. Habana
...
#
#def l2b_sub_exe  ( exp_type, tim_delt, fil_log_path, pid ):
if ( len(sys.argv)-1 < 4 ):
   print ( "Usage: l2b_sub_exe.py sde|vlbi tim_delt fil_log_path pid " )
   exit ( 1 )
else:
   exp_type     = sys.argv[1]; exp_type     = exp_type.lower()
   tim_delt     = sys.argv[2]; tim_delt     = tim_delt.lower()
   fil_log_path = sys.argv[3]; fil_log_path = fil_log_path.lower()
   pid          = sys.argv[4]; pid          = int(pid)
#
...
#
      if ( not "sde" in fil_log_path ):
         ...
         print ( ret, out )
#
# --- The path to SDE logs is /sde/ssSSSS/ssSSSS.log or sometimes
#     /sde/ssSSSSss/ssSSSSss.log where ss is the station id
#
      stn_id  = fil_log[0:2]
#
...
#
      if ( os.path.isdir("/f0/anc/temp/sde") ):
         dir_anc_temp = "/f0/anc/temp/sde"
      else:
...
#
      if ( not "sde" in fil_log_path ):
         ...
         print ( ret, out )
#
# --- The path to SDE logs is /sde/ssSSSS/ssSSSS.log or
#     /sde/ssSSSSss/ssSSSSss.log where ss is the station id
#
      stn_id  = fil_log[0:2]
#
...
#     access to the NVME fast directories
#
      if ( os.path.isdir("/f0/anc/temp/vlbi") ):
         dir_anc_temp = "/f0/anc/temp/vlbi"
      else:
...
# ----
   ...
   print ( date_iso1 )
#
   com = "log2ant -t dat -t met -t tsys -t tpi -t sefd -t phc -t fmt -t cbl -a -u -o " + \
