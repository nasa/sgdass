...
! *                                                                      *
! *  ### 21-JUN-2022  PIMA_GEPM      v1.0 (d)  J. SKEENS   21-JUN-2022 ###  *
! *                                                                         *
! ***************************************************************************
!
      IMPLICIT   NONE
...
!
      ...
      END IF      
!
! --- is the configuration file polarization acceptable?
!
     ...
     &           PIM%NPOL == 1 ) THEN
         IND_POL = 1
       ELSE
         CALL ERR_LOG ( 8829, IUER, 'PIMA_GEPM',                        &
     &           'Polarization code '//TRIM(PIM%CONF%POLAR)//           &
     &           ' is not supported for plotting pcal. '//              &
     &           'Supported codes: RR, LL, HH, VV' )
         RETURN
      END IF
!
! --- get indices of station list.
!
...
!
! ------ NB: this logic does not support merged and combined
!            frequency groups       !!
!
         IF ( PIM%STA(IND_STA)%PCAL(1)%PCAL_AVAIL .AND.                 &
     &        PIM%STA(IND_STA)%PCAL(1)%PCAL_USE ) THEN
!
            NPCL = PIM%STA(IND_STA)%PCAL(1)%NO_TONES * PIM%NFRQ          ! No. of Phase Calibrations
!
! --------- Memory allocation
!
...
! ---------
            ...
            FREQS = 0.0D0
!
! --------- Get the number of points and time at each point
!           N.B: The TDiff is not necessarily consistent, so to get the
!                scan length you need sum of differences between all points
!                points.  
!
            NPOI =  PIM%STA(IND_STA)%PCAL(1)%NPOI              
 ...
 350        CONTINUE
!
! --------- Actual computational average
!
            T_ACCUM_AVG = TIM_DIFF / (NPOI - 1)
!
! --------- What if the actual computational average is less than the
!           the computational value you had declared
!
...
! ---------
            ...
            ALLOCATE ( TONE_SORT(NPOI_ACCUM) )
!
! --------- Set up phase, time, amplitude, differenced phase arrays and
!           average to the given time step
!
            CALL ERR_PASS ( IUER, IER )
...
!
     ...
     &                    ' in PIMA_PCAL_BPASS' )
               ...
               ENDIF
!                 
! ------------ Analyze BPASS phase and amplitude                 
!
               DO 420 J2=1,M_TONES-1
                  BPASS_DIFF(J2) = BPASS_PHASE(J2+1) - BPASS_PHASE(J2)
 420           CONTINUE
!
! ------------ Compute median
!
               BPASS_SORTED = ABS ( BPASS_DIFF(1:PIM__MTON-1) )
               CALL SORT_R8 ( PIM__MTON-1, BPASS_SORTED )
!
               IF ( MOD ( PIM__MTON, 2 ) == 1 ) THEN
                  BPASS_MEDIAN = BPASS_SORTED(PIM__MTON/2 + 1)
               ELSE
                  BPASS_MEDIAN = 0.5 * (BPASS_SORTED(PIM__MTON/2) +     &
     &                           BPASS_SORTED(PIM__MTON/2+1))
               ENDIF
!
! ------------ Turn off points with jump of > threshold*median diff
!
               DO 430 J3 = 1, M_TONES - 1
                  IF ( ABS( BPASS_DIFF(J3) ) .GT.                       &
     &                 BPASS_THRESH*BPASS_MEDIAN  ) THEN
!
! ------------------ One tone logic
!
                     IF ( J3 == 1 .AND. ABS ( BPASS_DIFF(J3+1) ) <      &
                          BPASS_THRESH*BPASS_MEDIAN ) THEN
                        PIM%PCAL_MASK(J3,J1,IND_STA) = 0
                        FAIL_ARR(J3,IFRQ,IND_STA)    = FAIL_CPDIFF
!
! ------------------ 
!
                     ELSEIF ( J3 == M_TONES-1 ) THEN
                        IF ( ABS ( BPASS_DIFF(J3-1) ) <                 &
     &                       BPASS_THRESH*BPASS_MEDIAN ) THEN 
                           PIM%PCAL_MASK(J3+1,J1,IND_STA) = 0
                           FAIL_ARR(J3+1,IFRQ,IND_STA) = FAIL_CPDIFF
                        ENDIF
!
! ------------------ 
!
                     ELSEIF ( J3 /= M_TONES-1 ) THEN
                        IF ( ABS ( BPASS_DIFF(J3+1) ) >                 &
     &                       BPASS_THRESH*BPASS_MEDIAN                  &
     &                                .AND.                             &
     &                       SIGN(BPASS_DIFF(J3), BPASS_DIFF(J3+1)) /=  &
     &                       BPASS_DIFF(J3) ) THEN
                           PIM%PCAL_MASK(J3+1,J1,IND_STA) = 0
                           FAIL_ARR(J3+1,IFRQ,IND_STA) = FAIL_CPDIFF
                        ENDIF
...
! ------------ 
!
               IF ( TWO_TONE_LOGIC .EQV. .TRUE. ) THEN
                  DO 440 J4 = 1,M_TONES-1
                     IF ( ABS( BPASS_DIFF(J4) ) <                       &
     &                    BPASS_THRESH*BPASS_MEDIAN ) THEN
                        IF ( J4 == 1                     .AND.          &
     &                       ABS ( BPASS_DIFF(J4+1) ) >                 &
     &                       BPASS_THRESH*BPASS_MEDIAN   .AND.          &
     &                       ABS ( BPASS_DIFF(J4+2) ) <                 &
     &                       BPASS_THRESH*BPASS_MEDIAN        ) THEN
                           PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                           FAIL_ARR(J4:J4+1,IFRQ,IND_STA)  = FAIL_CPDIFF
                        ELSEIF ( J4 == M_TONES-1            .AND.       &
     &                           ABS ( BPASS_DIFF(J4-1) ) >             &
     &                           BPASS_THRESH*BPASS_MEDIAN  .AND.       &
     &                           ABS ( BPASS_DIFF(J4-2) ) <             &
     &                           BPASS_THRESH*BPASS_MEDIAN       ) THEN
                           PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                           FAIL_ARR(J4:J4+1,IFRQ,IND_STA) = FAIL_CPDIFF
                        ELSEIF ( J4 > 2 .AND. J4 <  M_TONES-2 ) THEN
                           IF ( ABS ( BPASS_DIFF(J4+1) ) >              &
     &                          BPASS_THRESH*BPASS_MEDIAN  .AND.        &
     &                          ABS ( BPASS_DIFF(J4-1) ) >              &
     &                          BPASS_THRESH*BPASS_MEDIAN  .AND.        & 
     &                          ABS ( BPASS_DIFF(J4+2) ) <              &
     ...
     &                          ABS ( BPASS_DIFF(J4-2) ) <              &
     &                          BPASS_THRESH*BPASS_MEDIAN  .AND.        &
     &                          SIGN ( BPASS_DIFF(J4-1),                &
     &                                 BPASS_DIFF(J4+1) ) /=            &
     &                          BPASS_DIFF(J4-1)                ) THEN
                              PIM%PCAL_MASK(J4:J4+1,J1,IND_STA) = 0
                              FAIL_ARR( J4:J4+1,IFRQ,IND_STA) =         &
     &                                  FAIL_CPDIFF
                           ENDIF
                        ENDIF
                     ENDIF
...
!
               DO 450 J5 = 1,M_TONES
                  TONE_SORT(1:NPOI_ACCUM) = A8(J5,1:NPOI_ACCUM,IFRQ)
                  CALL SORT_FAST_R8( INT8 ( NPOI_ACCUM ), TONE_SORT )
                  AMP_MED_TONE = TONE_SORT( CEILING(NPOI_ACCUM*1.D0/2) )
!
! --------------- 
!
                  IF ( AMP_MED_TONE < PIMA__AMP_MIN .AND.               &
     &                 FREQS(J5,IFRQ) > PIMA__MIN_FRQ ) THEN
                     DIFF_MASK(J5,1:NPOI_ACCUM,IFRQ) = 0
                     P8(J5,1:NPOI_ACCUM,IFRQ) = 0.0D0
                     A8(J5,1:NPOI_ACCUM,IFRQ) = 0.0D0
                     FAIL_ARR(J5,IFRQ,IND_STA) = FAIL_AMP
                  ENDIF
 450           CONTINUE
!
! ------------ Loop over tones within the frequency group
...
!
 ...
 1010             CONTINUE
!
! --------------- Remove residual group delay and unwrap
!
                  IF ( RECURSIVE == 1 ) THEN !
                     I_ABOVE = 0
                     INDS_ABOVE(1:M_TONES) = A8(1:M_TONES,NP,IFRQ) >    &
     &                                       PIMA__AMP_MIN
                     DO 530 K3 = 1,M_TONES
...
!
     ...
     &                          'Failure in resolving residual phase '  &
     &                          //'ambiguity' )
                        RETURN
                     ENDIF
                     I_ABOVE = 0
...
!
                  IF ( ALL ( A8(1:M_TONES,NP,IFRQ) >                    &
     &                       PIMA__AMP_MIN )          ) THEN
                     IF ( M_TONES > 4 ) THEN
                        KNOT_LOC(1) =  FREQS(1,IFRQ) - EPS
                        KNOT_LOC(2) =  FREQS(M_TONES,IFRQ) + EPS
                        CALL ERR_PASS ( IUER, IER )
                        CALL EBSPL_LSQ_CNS3 ( M_TONES, FREQS(1,IFRQ),   &
     &                                        P8(1,NP,IFRQ), N_KNOT, 3, &
     &                                        KNOT_LOC, P8_SPL_COEFF,   &
     &                                        0.D0, 5.D-7, 1.D-1,       &
     &                                        P8_SPL_RMS, IER )
                        IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 8835, IUER, 'PIMA_GEPM',      &
     &                             'Failure in determining phase '//    &
     &                             'spline coefficients' )
                           RETURN
                        ENDIF
                        CALL ERR_PASS ( IUER, IER )
                        CALL EBSPL_LSQ_CNS3 ( M_TONES, FREQS(1,IFRQ),   &
     &                                        A8(1,NP,IFRQ), N_KNOT, 3, &
     &                                        KNOT_LOC, A8_SPL_COEFF,   &
     &                                        0.D0, 5.D-7, 1.D-1,       &
     &                                        A8_SPL_RMS, IER )
                        IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 8836, IUER, 'PIMA_GEPM',      &
     &                             'Failure in determining amplitude'// &
     &                             'spline coefficients')
                           RETURN
                        ENDIF
                     ENDIF
...
!
                     IF ( I_ABOVE > 4 ) THEN
                        ...
                        CALL ERR_PASS ( IUER, IER )
                        CALL EBSPL_LSQ_CNS3 ( I_ABOVE, FREQ_ABOVE,      &
     &                                        PHASE_ABOVE, N_KNOT, 3,   &
     &                                        KNOT_LOC, P8_SPL_COEFF,   &
     &                                        0.D0, 5.D-7, 1.D-1,       &
     &                                        P8_SPL_RMS, IER )
                        IF ( IER .NE. 0 ) THEN
...
!
     ...
     &                             //' spline coefficients' )
                  ...
                  ENDIF
!
! --------------- Evaluate splines on complex plane against
!                 corresponding data to find spurious signals
!
                  NUM_NONZERO = COUNT ( FREQS(1:M_TONES,IFRQ) >         &
     &                                  PIMA__MIN_FRQ )
                  IF ( NUM_NONZERO > 4) THEN
                     AMPMAX = MAXVAL(A8(1:M_TONES,NP,IFRQ))
                     IF ( AMPMAX == 0 ) THEN
                        DIFF_MASK(1:M_TONES,NP,IFRQ) = 0
                     ELSEIF ( ALL ( A8(1:M_TONES,NP,IFRQ) >             &
     &                              PIMA__AMP_MIN )           .OR.      &
     &                        I_ABOVE > 3                   ) THEN
                        DO 560 K6 = 1, M_TONES
                           IF ( A8(K6,NP,IFRQ) > PIMA__AMP_MIN ) THEN
                              CALL ERR_PASS ( IUER, IER )
                              P8_SPL(K6) = EBSPL_VAL_R8(N_KNOT, MDEG,   &
     &                                                  FREQS(K6,IFRQ), &
     &                                                  KNOT_LOC,       &
     &                                                  P8_SPL_COEFF )
                              A8_SPL(K6) = EBSPL_VAL_R8(N_KNOT, MDEG,   &
     &                                                  FREQS(K6,IFRQ), &
     &                                                  KNOT_LOC,       &
     &                                                  A8_SPL_COEFF )
                              CP_SPL(K6) = A8_SPL(K6) *                  &
     &                                     COMPLEX( COS(P8_SPL(K6)),     &
     &                                              SIN(P8_SPL(K6)) )
                              CP_DATA(K6) = A8(K6,NP,IFRQ) *             &
     &                                      COMPLEX(COS(P8(K6,NP,IFRQ)), &
     &                                              SIN(P8(K6,NP,IFRQ)))
                              CP_DIFF(K6) = SQRT(                        &
     &                                   ( REALPART(CP_DATA(K6))-        &
     &                                     REALPART(CP_SPL(K6)))**2      &
     &                                 + ( IMAGPART(CP_DATA(K6))-        &
     &                                     IMAGPART(CP_SPL(K6)))**2)/    &
     &                                   AMPMAX
                           ELSEIF (FREQS(K6,IFRQ) > PIMA__MIN_FRQ) THEN
                              CP_DIFF(K6) = 0
 ...
 560                    CONTINUE
                        ...
                        IF ( CP_DIFF(CP_MAXLOC) > DIFF_THRESH ) THEN
                           ...
                           P8(CP_MAXLOC,NP,IFRQ) = 0.D0
                           A8(CP_MAXLOC,NP,IFRQ) = 0.D0
                           RECURSIVE = 1
                           GOTO 1010
                        ENDIF
                     ELSE
         ...
         DO 570 K7 = PIM%CONF%BEG_FRQ+1,PIM%CONF%END_FRQ
            ...
            DO 580 K8=1,M_TONES
               ...
               DO 590 K9 = 1,NPOI_ACCUM-1
                  IF ( ABS ( Y8(I_TONE,K9 + 1,IFRQ) ) > 0.D0 .AND.      &
     &                 ABS ( Y8(I_TONE,K9,IFRQ - 1) ) > 0.D0  ) THEN
                     DIFF_Y8(K9) = ( Y8(I_TONE,K9 + 1,IFRQ) -           &
     &                               Y8(I_TONE,K9    ,IFRQ) ) **2
...
!
                  IF ( ABS ( Y8(I_TONE,K9 + 1,IFRQ - 1) ) > 0.D0 .AND.  &
     &                 ABS ( Y8(I_TONE,K9,IFRQ - 1) ) > 0.D0 ) THEN
                     ...
                     IF ( DIFF_Y8(K9) > DIFF_Y8_THRESHX .AND.           &
     &                    K9 > N_SHIFT + 1              .AND.           &
     &                    K9 < NPOI_ACCUM - N_SHIFT - 1       ) THEN
                        IF ( ( Y8(I_TONE,K9 + N_SHIFT, IFRQ - 1) -      &
     &                         Y8(I_TONE,K9 - N_SHIFT, IFRQ - 1 ))**2   &
     &                         > DIFF_Y8_THRESHY )                      &
     &                     COUNT_DIFF = COUNT_DIFF + 1
                     ENDIF
                  ENDIF
 591           CONTINUE
               ...
               IF ( COUNT_DIFF  > MAX_COUNT .AND.                       &
     &              FAIL_ARR(I_TONE,IFRQ,IND_STA) /= FAIL_AMP) THEN
                  PIM%PCAL_MASK(I_TONE,K7,IND_STA) = 0
                  FAIL_ARR(I_TONE,IFRQ,IND_STA)    = FAIL_Y8DIFF
               ENDIF
               IF ( ALL ( FAIL_ARR(I_TONE,2:IFRQ,IND_STA) > 0 ) .AND.   &
     &              FAIL_ARR(I_TONE,1,IND_STA) /= FAIL_AMP ) THEN
...
!
 ...
 730                 CONTINUE
                     ...
                     IF ( MEAN_MASK/NPT < 1-TIME_THRESH ) THEN
                        ...
                        ENDIF
                     ELSEIF ( FAIL_ARR(L2,IFRQ,IND_STA) .NE.            &
     &                        FAIL_Y8DIFF )     THEN
 ...
 710        CONTINUE
         ...
         IF ( ALL ( FAIL_ARR(1:M_TONES,1:IFRQ,IND_STA) > 0  ) ) THEN
            WRITE ( 6, * ) 'All pcal tones of station ',                &
     &                     PIM%C_STA(IND_STA), ' are noise. ',          &
     &                     'Recommend adding to PCAL: not_to_use:  ',   &
     &                     'in control file'
         ENDIF
 610  CONTINUE
!
...
!
      CALL ERR_PASS ( IUER, IER )
      CALL PIMA_PCST ( PIM, STA_NAM, T_ACCUM, FREQS, T8, P8, A8, Y8,    &
     &                 FAIL_ARR, IND_POL, IER )
      IF ( IER .NE. 0 ) THEN
         CALL ERR_LOG ( 8840, IUER, 'PIMA_GEPM',                        &
     &           'Failure in computing phase calibration statistics' )
         RETURN
      ENDIF
      
      CALL ERR_PASS ( IUER, IER )
      CALL WRI_PCAL_MASK ( PIM, PIM%CONF_FILE,                          &
     &                     PIM%CONF%PCAL_MASK_FILE, IER )
      IF ( IER .NE. 0 ) THEN
         CALL ERR_LOG ( 8841, IUER, 'PIMA_GEPM',                        &
     &            'Failure in writing phase calibration mask' )
         RETURN
      ENDIF
!
      CALL ERR_PASS  ( IUER, IER )
      ...
      IF ( IER .NE. 0 ) THEN
         CALL ERR_LOG ( 8842, IUER, 'PIMA_GEPM',                        &
     &           'Failure in writing phase calibration report' )
         RETURN
      ENDIF
!
      CALL ERR_LOG ( 0, IUER )
