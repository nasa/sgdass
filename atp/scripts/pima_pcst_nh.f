...
! *       IND_POL        =
! *                                                                             *      
! *  ### 21-JUN-2022  PIMA_PCST          v1.1 (c)  J. SKEENS   26-AUG-2022 ###  *
! *                                                                             *
! *******************************************************************************
!
      IMPLICIT   NONE
      ...
      CHARACTER  STA_NAM*(*)
      INTEGER*4  IUER
      INTEGER*4  MPB, MP, NN, MDEG, N_KNOT, SIG_NUM, FAIL_AMP
      INTEGER*4  FAIL_CPDIFF, NUM_SUM, BIN_DIFF
      ...
      REAL*4     G_DEL
      REAL*8,    ALLOCATABLE :: FREQ_SDEL(:,:), AMPL_SDEL(:,:)
      REAL*8,    ALLOCATABLE :: TIM(:), PHAS(:,:), PHAS_AMB(:,:)
      REAL*8,    ALLOCATABLE :: TONE_SORT(:), RMS_FREQ(:,:) 
      REAL*8,    ALLOCATABLE :: ROOT_SQUARE(:), PHASE_SMOOTH(:)
      REAL*8,    ALLOCATABLE :: TIME_DET(:), PHASE_CHIX(:)
      REAL*8,    ALLOCATABLE :: DIFF_PHASE_PLOT(:), DEL_PHASE_PLOT(:)
      REAL*8,    ALLOCATABLE :: PHAS_SDEL(:,:), AMPL(:,:), T_NONZERO(:)
      REAL*8,    ALLOCATABLE :: RMS_TIME(:,:,:), ROOT_SQUARE_TIME(:)
      REAL*8,    ALLOCATABLE :: PHASE_UNWRAPPED(:), PHASE_CHIY (:)
      REAL*8     FRQ_DIF, FREQS(PIM__MTON,PIM__MFRQ), P8_SPL(PIM__MTON)
      REAL*8     P8_SPL_COEFF(1-MDEG:PIM__MTON), P8_SPL_RMS, AVE_PHASE
      REAL*8     A8_SPL_COEFF(1-MDEG:PIM__MTON), A8_SPL_RMS, AVE_AMP
      REAL*8     CP_DIFF(PIM__MTON), MEAN_MASK, AMPMAX, TIM_DIFF
      REAL*8     PHASE_ABOVE(PIM__MTON), AMP_ABOVE(PIM__MTON)
      REAL*8     BPAS_AMP(PIM__MTON), DIFF_PHASE(PIM__MTON)
      REAL*8     DIFF_AMP(PIM__MTON), FREQ_1ST(PIM__MFRQ), T_ACCUM_AVG
      REAL*8     FRQ_DIF_MIN, PCAL_FRQ_STEP, PHAS_LAST, SIG, T8_DIFF
      REAL*8     AMP_MED_TONE, ROOT_SQUARE_FREQ(PIM__MTON), SHIFT, DRIFT
      REAL*8     ABS_PHASE_DEL_RATE, PHS_DIF, SIGMA_DETX, SIGMA_DETY
      REAL*8     T8(PIM%NPCT,MP,PIM%NFRQ), P8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     Y8(PIM%NPCT,MP,PIM%NFRQ), DEL_RATE_THRESH
      REAL*8     SIGMA_THRESHX, SIGMA_THRESHY, SIGMA_DIFF
      REAL*8     PHASE_DEL_HIST(MP), PHASE_DIFF_HIST(MP)
      REAL*8     RMS_AVG, RMS_VAR, BPASS_PHASE(PIM__MTON), TIME_AVG
      REAL*8     KNOT_LOC(N_KNOT), A8_SPL(PIM__MTON), TIME_AVG_STA
      REAL*8     FREQ_ABOVE(PIM__MTON), BPAS_PHASE(PIM__MTON)
      REAL*8     SIGMA_DIFF_USE, SIGMA_DIFF_MIN,A8(PIM%NPCT,MP,PIM%NFRQ)
      REAL*8     DIFF_RATE_THRESH, AVG_DIFF, AVG_DEL
      REAL*8     DEL_ARG_HIST(MP), DIFF_ARG_HIST(MP)
      REAL*8     NORMAL_DIST_DEL(MP), NORMAL_DIST_DIFF(MP)
      ...
      INTEGER*4  NP, IND_POL, IND_PLT
      INTEGER*4  J1, J2, J3, J4, J5, J6, J7, J8, J9
      INTEGER*4  I1, I2, I3, I4, I5
      INTEGER*4  K1, K2, K3, K4, K5, K6
      INTEGER*4  L1, L2, L3
      INTEGER*4  M1, M2, M3, M4, M5, M6, M7, M8
      INTEGER*4  N1, N2, N3, N4
      INTEGER*4  N, COUNT, COUNT_SIG, N_SHIFT, COUNT_HIST
      INTEGER*4  M_TONES, IND_TONE, IND_FREQ, N_TONES, I_TONE, IND(2,32)
      INTEGER*4  I_NONZERO, LIND, IER, NPT, BEG_STA, END_STA, PREFIX
      INTEGER*4  NO_TON, NPCL, IND_MOD, ITYP, KCHN, NT_USED, KP
      INTEGER*4  NUM_NONZERO, NPOI_ACCUM, NUM_ACCUM, N_AVG, N_AVG_STA
      INTEGER*4  NUM_STA, IND_TONE_FREQ(PIM__MTON), IFRQ_DIFF, TONE_DIFF
      INTEGER*4  FAIL_ARR(PIM__MTON,PIM%NFRQ,PIM%NSTA), RMS_NUMEL
      INTEGER*4  I_ABOVE, CP_MAXLOC, RECURSIVE, NPOI, STA_LIST(PIM%NSTA)
      INTEGER*4  N_SINCE_DETECT
...
! ---------
            ...
            ENDIF
!
! --------- Set up phase, time, amplitude, differenced phase arrays 
! --------- and average to the given time step
!
            CALL ERR_PASS  ( IUER, IER )
            CALL PIMA_PCAL_AVG ( PIM, IND_STA, PIMA__AMP_MIN, NUM_ACCUM, &
     &                           NPOI_ACCUM, FREQS, IND_TONE_FREQ, T8,   &
     &                           P8, A8, Y8, IER )
            IF ( IER .NE. 0 ) THEN
...
! ------ 
         ...
         DO 410 J1 = PIM%CONF%BEG_FRQ, PIM%CONF%END_FRQ
            ...
            NP = 0
            KP = 0
            IF ( PIM%STA(IND_STA)%PCAL(PIM%CONF%FRQ_GRP)%PCAL_AVAIL     &
     &                          .AND.                                   &
...
!
                  IF ( ANY(PIM%PCAL_MASK(1:M_TONES,J1,IND_STA)<1)) THEN 
                     ...
                     I_ABOVE = 0
                     INDS_ABOVE(1:M_TONES) =                            &
                                A8(1:M_TONES,NP,IFRQ) >                 &
     &                          PIMA__AMP_MIN .AND.                     &
...
! ------------------
                     CALL ERR_PASS  ( IUER, IER )
                     G_DEL = PIMA_PC_GDEL( M_TONES, FREQS(1,IFRQ),      &
     &                                     P8(1,NP,IFRQ), A8(1,NP,IFRQ),&
     &                                     IER )
                     IF ( IER .NE. 0 ) THEN
                        CALL ERR_LOG ( 8848, IUER, 'PIMA_PCST',         &
     &                          'Failure in computing residual '//      &
     &                          'group delay' )
                        RETURN
                     ENDIF
!
! ------------------ PIMA_PC_GDEL will work with non-consecutive freqs
!                    (but signals an error)
!     
                     PHASE_ABOVE(1:I_ABOVE) = PHASE_ABOVE(1:I_ABOVE) -  &
     &                                        G_DEL * PI2 *             &
     &                                        ( FREQ_ABOVE(1:I_ABOVE) - &
     &                                          FREQ_ABOVE(1) )
                     CALL ERR_PASS  ( IUER, IER )
...
! ------
 470     CONTINUE
! ------
         IF ( LTM_DIF ( 0, PIM%CONF%L_PUS, PIM%CONF%PCAL_USE_STA,       &
     &                  PIM%C_STA(IND_STA) ) == 0 .OR.                  &
     &        PIM%CONF%L_PUS == 0  ) THEN
!
! --------- Find jitter of non-masked pcal tones in the frequency 
!           direction this is evaluated as
!           sqrt(phase - average phase)^2       
!
            DO 910 N1 = 1,NP
               IFRQ = 0
               IF ( PIM%STA(IND_STA)%PCAL(PIM%CONF%FRQ_GRP)%PCAL_AVAIL  &
...
! ------------------
                     IF ( I_ABOVE > 0 ) THEN 
                        PHASE_ABOVE(1:I_ABOVE) =                        &
     &                          PHASE_ABOVE(1:I_ABOVE) -                &
     &                          SUM (PHASE_ABOVE(1:I_ABOVE)) / I_ABOVE
! ---------------------
                        DO 940 N4=1,I_ABOVE
                           ROOT_SQUARE_FREQ(N4) =                       &
     &                             SQRT(PHASE_ABOVE(N4)** 2 )  
 940                    CONTINUE
! ---------------------
                        RMS_FREQ(IFRQ,IND_STA) =                        &
     &                         SUM ( ROOT_SQUARE_FREQ  ) / I_ABOVE
                     ELSE
 ...
 910        CONTINUE
!
! --------- Find jitter of non-masked pcal tones in the time direction 
!           wrt a smoothed curve redo pcal accumulation b/c phase
!           correction in freq dir. will cause jumps in time dir.
!
            T8    = 0.0D0
            P8    = 0.0D0
            A8    = 0.0D0
...
! ---------
            ALLOCATE ( PHASE_SMOOTH(PIM%STA(IND_STA)%PCAL(1)%NPOI),     &
     &                 STAT=IER )
            ALLOCATE ( PHASE_UNWRAPPED(PIM%STA(IND_STA)%PCAL(1)%NPOI),  &
     &                 STAT=IER )
            ALLOCATE ( ROOT_SQUARE_TIME(PIM%STA(IND_STA)%PCAL(1)%NPOI), &
     &                 STAT=IER )
            ALLOCATE ( T_NONZERO(PIM%STA(IND_STA)%PCAL(1)%NPOI),        &
     &                 STAT=IER )
            ALLOCATE ( INDS_NONZERO(PIM%STA(IND_STA)%PCAL(1)%NPOI),     &
     &                 STAT=IER )
! ---------
            IF ( IER .NE. 0 ) THEN
               CALL ERR_LOG ( 8850, IUER, 'PIMA_PCST',                  &
     &                 'Failure to allocate arrays for clock '//        &
     &                 'break investigation' )
               RETURN
            ENDIF
! ---------
...
!
                     IF ( PIM%PCAL_MASK(M2,M1,IND_STA) .EQ. 1 .AND.     &
     &                    FREQS(M2,IFRQ) .GT. 0                ) THEN
!
...
!
     ...
     &                                       T_NONZERO, PHASE_UNWRAPPED, &
     &                                       IER )
                        IF ( IER .NE. 0 ) THEN
                           CALL ERR_LOG ( 8851, IUER, 'PIMA_PCST',      &
     &                             'Failure in resolving time '//       &
     &                             'unwrapped phase ambiguity')
                           RETURN
                        ENDIF
!
...
!
     ...
     &                                       PHASE_SMOOTH )
!
! --------------------- Evaluate rms in time direction as
!                       sqrt( (phase - smoothed phase)^2 )
!
                        DO 840 M4=1,I_NONZERO
                           ROOT_SQUARE_TIME(M4) =                       &
     &                           SQRT( ( PHASE_SMOOTH(M4) -             &
     &                                   PHASE_UNWRAPPED(M4) )**2 )
 840                    CONTINUE
! ---------------------
                        IF ( I_NONZERO > 0 ) THEN 
                           RMS_TIME(M2,IFRQ,IND_STA) =                  &
     &                           SUM (ROOT_SQUARE_TIME(1:I_NONZERO)) /  &
     &                           I_NONZERO
                        ELSE
...
! ---------------------
 ...
 810        CONTINUE 
!
! --------- Calculate average, variance of time RMS values, turn off
!           tone if RMS too high
!
...
! ---------------
                  DO 860 M6 = 1, M_TONES
! ------------------
                     IF ( PIM%PCAL_MASK(M6,M5,IND_STA) .EQ. 1 .AND.     &
     &                    FREQS(M6,IFRQ) .GT. 0               ) THEN
                        RMS_VAR = RMS_VAR + ( RMS_TIME(M6,IFRQ,IND_STA) &
     &                                        - RMS_AVG ) ** 2
                     ENDIF
 860              CONTINUE
               ENDIF
...
! ------------
                  DO 880 M8=1,M_TONES
                     ...
                     IF ( PIM%PCAL_MASK(M8,M7,IND_STA) .EQ. 1 .AND.     &
     &                    FREQS(M8,IFRQ) .GT. 0              ) THEN
! ------------------
                        IF ( PIM%CONF%DEBUG_LEVEL >= 8 ) THEN
                           WRITE (6,*) 'TONE: ',    M8
                           WRITE (6,*) 'IFRQ: ',    IFRQ
                           WRITE (6,*) 'RMS_TIME:',                     &
     &                                 RMS_TIME(M8,IFRQ,IND_STA)
                           WRITE (6,*) 'RMS_LIMIT', RMS_AVG +           &
     &                                 SQRT(RMS_VAR) *  RMS_NSIGMA
                        ENDIF
! ---------------------
                        IF ( RMS_TIME(M8,IFRQ,IND_STA) >                &
     &                       RMS_AVG + SQRT(RMS_VAR)*RMS_NSIGMA ) THEN
...
! ------
         ...
         ALLOCATE ( FIND_IND_FREQ(PIM%CONF%END_FRQ - PIM%CONF%BEG_FRQ), &
     &              STAT=IER ) 
! ------
         IF ( IER .NE. 0 ) THEN
...
! ---------------
                  ...
                  END IF
!                   
! --------------- Calculate chi-squared test statistic by adding 
!                 epoch-by-epoch differenced relative phase (PHASE_CHI)
!                 SIGMA_DIFF is the variance of the noisiest tone --> 
!                 this is necessary because some tones can be 
!                 significantly noisier and spoil the chi-squared test
!                 In practice, clock breaks will still be detected
!
                  IF ( J1 .NE. IND_FREQ .AND.                           &
     &                 MAXVAL( ABS ( Y8(I_TONE,1:NPOI_ACCUM,IFRQ) ) )   &
     &                  < PHASE_MAX           ) THEN
! ------------------
                     COUNT_SIG = 0
                     SIGMA_DIFF = 0.0D0
! ------------------
                     DO 1040 J4=1,NPOI_ACCUM-1
                        PHASE_CHIX(J4) = PHASE_CHIX(J4) +               &
     &                                   ( Y8(I_TONE,J4+1,IFRQ) -       &
     &                                     Y8(I_TONE,J4,IFRQ)     )**2
! ---------------------
                        IF ( J4 - N_SHIFT >= 1 .AND.                    &
     &                       J4 + N_SHIFT <= NPOI_ACCUM ) THEN
                           PHASE_CHIY(J4) =                             &
     &                             PHASE_CHIY(J4) +                     &
     &                             ( Y8(I_TONE,J4+N_SHIFT,IFRQ) -       &
     &                               Y8(I_TONE,J4-N_SHIFT,IFRQ)   ) ** 2
                        ENDIF 
! ---------------------
 ...
 1040                CONTINUE
! ------------------
                     IF ( COUNT_SIG > 1 ) SIGMA_DIFF = SIGMA_DIFF /     &
     &                                    (COUNT_SIG - 1)
! ------------------
...
!--------SIGMA_DET: polynomial fit of chi-squared CDF by degree of
!                   freedom for P_A = 10^-6
!--------SIGMA_THRESH: threshold for chi-squared test
!
         SIGMA_DETX = 22.780665613542475                  +             &
     &                2.692011253135052        * COUNT    -             &
     &                0.03627826253410943      * COUNT**2 +             &
     &                0.0004955576943092998    * COUNT**3 -             &
     &                0.0000025870752232632106 * COUNT**4
         SIGMA_DETY = 18.423071715064864                   +            &
     &                 2.55002691257416         * COUNT    -            &
     &                 0.03364748902821395      * COUNT**2 +            &
     &                 0.00046147263166883837   * COUNT**3 -            &
     &                 0.0000024137416822531333 * COUNT**4
...
! ------
         DO 1070 J7=1,NPOI_ACCUM - 1
            IF ( ( PHASE_CHIX(J7) > SIGMA_THRESHX ) .AND.               &
     &           ( N_SINCE_DETECT >=  5)                 ) THEN
! ------------
               IF ( J7 < NPOI_ACCUM - NUM_SUM - 1 .AND.                 &
     &              J7 > NUM_SUM + 1                    ) THEN
! ---------------
                  IF ( PHASE_CHIY(J7)  > SIGMA_THRESHY ) THEN
                     WRITE (6,*) 'For station ',                        &
     &                           PIM%C_STA(IND_STA),                    &
     &                           ' likely clock break near t = ',       &
     &                           TIME_DET(J7), ' sec'
!                         ELSE
...
! ---------------
            ...
            ENDIF
! ---------
            IF ( ABS(Y8(TONE_DIFF,J7+1,IFRQ_DIFF) -                     &
     &               Y8(TONE_DIFF,J7,IFRQ_DIFF)  ) < 6 * SIGMA_DIFF_USE &
     &         ) THEN

               COUNT_HIST = COUNT_HIST + 1 
               DIFF_PHASE_PLOT(COUNT_HIST) =                            &
     &                     Y8(TONE_DIFF,J7+1,IFRQ_DIFF) -               &
     &                     Y8(TONE_DIFF,J7,IFRQ_DIFF)
            ENDIF
 1070    CONTINUE
!
! ------ Develop histogram of relative phase differences to test
!        whether noise is really thermal (Gaussian)
! ------ Note that this is includes many tones of potentially
!        different variance, so differences may not be Gaussian even
!        if each individual underlying tone is a Gaussian random walk
!
         DIFF_ARG_HIST = 0.0D0
...
! ---------
            CALL HIST ( COUNT_HIST, DIFF_PHASE_PLOT, BIN_DIFF,          &
     &                  -6 * SQRT ( SIGMA_DIFF_MIN ),                   &
     &                   6 * SQRT ( SIGMA_DIFF_MIN ), DIFF_ARG_HIST,    &
     &                  PHASE_DIFF_HIST, 1 )
            NORMAL_DIST_DIFF(1:BIN_DIFF) =                              &
     &                   1. / (SQRT (SIGMA_DIFF_MIN)  *                 &
     &                   SQRT ( 2 * PI__NUM ) ) *                       &
     &                   EXP ( -1. / 2 * ( DIFF_ARG_HIST(1:BIN_DIFF) /  &
     &                   SQRT ( SIGMA_DIFF_MIN ) ) ** 2 )
! ---------
            CALL DIAGI_1 ( NPOI_ACCUM, TIME_DET,                        &
     &                     Y8(TONE_DIFF,1:NPOI_ACCUM,IFRQ_DIFF), IER )
            CALL DIAGI_1 ( COUNT_HIST, TIME_DET,                        &
     &                     ABS ( DIFF_PHASE_PLOT ), IER )
            CALL DIAGI_2 ( BIN_DIFF, DIFF_ARG_HIST, PHASE_DIFF_HIST,    &
     &                     BIN_DIFF, DIFF_ARG_HIST, NORMAL_DIST_DIFF,   &
     &                     IER )
            CALL DIAGI_2 ( NPOI_ACCUM-1, TIME_DET, PHASE_CHIX,          &
     &                     NPOI_ACCUM, TIME_DET,                        &
     &                     TIME_DET*0 + SIGMA_THRESHX, IER )
            CALL DIAGI_2 ( NPOI_ACCUM-1, TIME_DET, PHASE_CHIY,          &
     &                     NPOI_ACCUM, TIME_DET,                        &
     &                     TIME_DET*0 + SIGMA_THRESHY, IER )
         ENDIF 
 610  CONTINUE 
! ---
      IF ( N_AVG .NE. 0 ) THEN
...
!
      CALL ERR_PASS ( IUER, IER )
! ---
      CALL WRI_PCAL_RMS ( PIM, PIM%CONF_FILE, RMS_TIME, RMS_FREQ,       &
     &                    TIME_AVG, STA_NAM, IER ) 
      IF ( IER .NE. 0 ) THEN
         CALL ERR_LOG ( 8856, IUER, 'PIMA_PCST',                        &
     &           'Failure in writing RMS file')
         RETURN
      ENDIF
! ---
      CALL ERR_LOG ( 0, IUER )
