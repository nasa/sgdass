...
!
      PIM%NSTA     = 1
      PIM%C_STA(1) = ANC%STA_NAM

!     
! --- Pass the phase call information from ANC to PIMA
!
      IF ( ANC%NUM_PCAL .GT. 0 ) THEN
         PIM%STA(1)%PCAL_AVAIL = .TRUE.
      ELSE
         PIM%STA(1)%PCAL_AVAIL = .FALSE.
!
         CALL INCH ( ANC%NUM_PCAL, STR )
         IUER = -1
         CALL ERR_LOG ( 1201, IUER, 'ATP_TO_PIM',                       &        
     &        'There is no PCAL section in ATP to pass to PIMA for '//         &
     &        TRIM(ANC%STA_NAM)//' in experiment '//ANC%EXP_CODE )
