...
def vsdc_parse_config ( vsdc ):
   ...
   if ( conf_buf[0][0:len(vsdc__config_label)] != vsdc__config_label[0:len(vsdc__config_label)] and
        conf_buf[0][0:len(vsdc__config_label)] != vsdc__config_label[0:len(vsdc__config_label_104)] ):
...
#
   ...
   for line in conf_buf:
       ...
       if ( len ( line.split() ) < 2 and \
            ...
            return 1
              ...
              num_par = num_par + 1
       elif ( line.split()[0]     == "UPLOAD_COMMAND:" ):
              vsdc.upload_command  = line.split()[1]
              num_par = num_par + 1
       elif ( line.split()[0]     == "UPLOAD_MANIFEST:" ):
              vsdc.upload_manifest = line.split()[1]
              num_par = num_par + 1
            
   if ( conf_buf[0][0:len(vsdc__config_label)] == vsdc__config_label_v104[0:len(vsdc__config_label_v104)] ):
        vsdc.upload_command  = "no"
        vsdc.upload_manifest = "no"
        num_par = num_par + 3
   ...
   if ( not os.path.isfile ( vsdc.netrc_file ) and vsdc.netrc_file != "no" ):
