...
!
!       gvf_db.i  2007.10.14  v 3.5  Leonid Petrov  2025.11.04_20:09:42
!
...
!
     ...
     &     'N_PHAMB ',  GVH__IGN, & ! 198  Whmf2 wet partial deriv. def.
     &     'OBCLFLGS',  GVH__IGN, & ! 199  Bit set indicate that calibration is recommended for observations
     &     'OBCLLIST',  GVH__IGN, & ! 200  Available obs dependent calibrations (poletide, earthdide, ?)
     &     'OCE_CFLG',  GVH__IGN, & ! 201  Ocean load flow control mess def
     &     'OCE_CONT',  GVH__IGN, & ! 202  Obs dependent ocean loading
     &     'OCE_DELD',  GVH__IGN, & ! 203  Ocean load site dependent displace
     &     'OCE_HORZ',  GVH__IGN, & ! 204  Site-dep ocean cont - horizontal
     &     'OCE_MESS',  GVH__IGN, & ! 205  Ocean loading message definition
     &     'OCE_OLD ',  GVH__IGN, & ! 206  Add to Cal-OceanLoad to get Cal10 OceanLoading
     &     'OCE_STAT',  GVH__IGN, & ! 207  Ocean loading station status.
     &     'OCE_VERT',  GVH__IGN, & ! 208  Site-dep ocean cont - vertical
     &     'OPTLCOEF',  GVH__IGN, & ! 209  Ocean Pole Tide loading Coefficients
     &     'OPTLCONT',  GVH__IGN, & ! 210  Ocean Pole Tide Load Contribution
     &     'PAN_MESS',  GVH__IGN, & ! 211  Feedhorn rot. angle mod. ident.
     &     'PARANGLE',  GVH__IGN, & ! 212  Feedhorn rot. angle
     &     'PEP_MESS',  GVH__IGN, & ! 213  PEP Utility Message Definition
     &     'PHCAMP_1',  GVH__IGN, & ! 214  Phase cal amplitudes by channel by station in band 1
     &     'PHCAMP_2',  GVH__IGN, & ! 215  Phase cal amplitudes by channel by station in band 2
     &     'PHCFRQ_1',  GVH__IGN, & ! 216  Phase cal freqs by channel by station in band 1
     &     'PHCFRQ_2',  GVH__IGN, & ! 217  Phase cal freqs by channel by station in band 2
     &     'PHCOFF_1',  GVH__IGN, & ! 218  Phase cal offsets (-18000 to 18000) by channel by station in band 1
     &     'PHCOFF_2',  GVH__IGN, & ! 219  Phase cal offsets (-18000 to 18000) by channel by station in band 2
     &     'PHCPHS_1',  GVH__IGN, & ! 220  Phase cal phases (-18000 to 18000) by channel by station in band 1
     &     'PHCPHS_2',  GVH__IGN, & ! 221  Phase cal phases (-18000 to 18000) by channel by station in band 2
     &     'PHC_RATE',  GVH__IGN, & ! 222  PC rate by sta ( us per s)
     &     'PLX1PSEC',  GVH__IGN, & ! 223  Parallax partial/contr  1 parsec
     &     'PLX_CFLG',  GVH__IGN, & ! 224  Parallax flow control mess def
     &     'PLX_MESS',  GVH__IGN, & ! 225  Parallax message definition
     &     'PLX_PART',  GVH__IGN, & ! 226  Parallax partial deriv. def.
     &     'POLAR_XY',  GVH__IGN, & ! 227  Polar motion X & Y for obs (rad)
     &     'PRE_DATA',  GVH__IGN, & ! 228  Precession constant (asec/cent).
     &     'PTDXYPAR',  GVH__IGN, & ! 229  Pole Tide Partials w.r.t. X & Y
     &     'PTD_CFLG',  GVH__IGN, & ! 230  Pole tide flow control mess def
     &     'PTD_CONT',  GVH__IGN, & ! 231  Pole tide contributions def.
     &     'PTD_MESS',  GVH__IGN, & ! 232  Pole tide message definition
     &     'PTOLDCON',  GVH__IGN, & ! 233  Old Pole Tide Restorer Contrib.
     &     'QBFACTOR',  GVH__IGN, & ! 234  Measure of uniformity of data.
     &     'RATRESID',  GVH__IGN, & ! 235  Rate resid (sec per sec)
     &     'RATUFLAG',  GVH__IGN, & ! 236  Delay rate unweight flag
     &     'REL_CFLG',  GVH__IGN, & ! 237  Relativisitc bending use status
     &     'REL_DATA',  GVH__IGN, & ! 238  Relativity mod data (gamma).
     &     'SBRESID ',  GVH__IGN, & ! 239  Single band delay residual
     &     'SCAN_UTC',  GVH__IGN, & ! 240  Nominal scan time YMDHMS.
     &     'SITEZENS',  GVH__IGN, & ! 241  Site zenith path delays (nsec).
     &     'SITHSOAM',  GVH__IGN, & ! 242  Horz south ocean loading ampltudes (m)
     &     'SITHSOPH',  GVH__IGN, & ! 243  Horz south ocean loading phases (rad).
     &     'SITHWOAM',  GVH__IGN, & ! 244  Horz west ocean loading ampltudes (m)
     &     'SITHWOPH',  GVH__IGN, & ! 245  Horz west ocean loading phases (rad).
     &     'SITOCAMP',  GVH__IGN, & ! 246  Vert ocean loading ampltudes (m)
     &     'SITOCPHS',  GVH__IGN, & ! 247  Vert ocean loading phases (rad).
     &     'SIT_MESS',  GVH__IGN, & ! 248  Site Module Message Definition
     &     'SIT_PART',  GVH__IGN, & ! 249  Site partials: dtau/dr_1=-dtau/dr_2
     &     'SRCHPAR ',  GVH__IGN, & ! 250  FRNGE/Fourfit search parameters
     &     'STARELEV',  GVH__IGN, & ! 251  Elev angles calc by COREL
     &     'STARTOFF',  GVH__IGN, & ! 252  Offset nominal start time (sec).
     &     'STARTSEC',  GVH__IGN, & ! 253  Start time in sec past hour
     &     'STAR_REF',  GVH__IGN, & ! 254  Source a priori coordinates reference
     &     'STOP_OFF',  GVH__IGN, & ! 255  Offset nominal stop time (sec).
     &     'STOP_SEC',  GVH__IGN, & ! 256  Stop  time in sec past hour
     &     'STR_CFLG',  GVH__IGN, & ! 257  Parallax flow control mess def
     &     'STR_MESS',  GVH__IGN, & ! 258  Star module message definition
     &     'STR_PART',  GVH__IGN, & ! 259  Star partial derivatives def.
     &     'SUN2CONT',  GVH__IGN, & ! 260  High order bending contrib.(sec)
     &     'SUN_CONT',  GVH__IGN, & ! 261  Consensus bending contrib. (sec)
     &     'SUN_DATA',  GVH__IGN, & ! 262  Solar geocentric coordinates and velocities
     &     'TAPQCODE',  GVH__IGN, & ! 263  Tape quality code
     &     'TECTPLNM',  GVH__IGN, & ! 264  4-char tectonic plate names
     &     'THE_MESS',  GVH__IGN, & ! 265  Theory module identification
     &     'TIDALUT1',  GVH__IGN, & ! 266  Flag for tidal terms in UT1 sers
     &     'TILTRMVR',  GVH__IGN, & ! 267  Axis Tilt Contribution Remover
     &     'URVR    ',  GVH__IGN, & ! 268  Rate derivatives mHz per asec
     &     'UT1EPOCH',  GVH__IGN, & ! 269  TAI - UT1 epoch value definition
     &     'UT1INTRP',  GVH__IGN, & ! 270  Message for UT1 interp. scheme
     &     'UT1LIBRA',  GVH__IGN, & ! 271  Hi Freq UT1 Libration Contribution
     &     'UT1ORTHO',  GVH__IGN, & ! 272  ORTHO_EOP Tidal UT1 contribution
     &     'UT1_-TAI',  GVH__IGN, & ! 273  UT1 time of day for this obsvr.
     &     'UT1_CFLG',  GVH__IGN, & ! 274  UT1 control flag message def.
     &     'UT1_MESS',  GVH__IGN, & ! 275  UT1 Module message definition
     &     'UT1_PART',  GVH__IGN, & ! 276  UT1 partial derivatives def.
     &     'UTCM_TAG',  GVH__IGN, & ! 277  UTC at central epoch YMDHMS.
     &     'UVF/ASEC',  GVH__IGN, & ! 278  U V in FR per arcsec from CALC per band
     &     'WOBEPOCH',  GVH__IGN, & ! 279  Interpolated wobble array def
     &     'WOBINTRP',  GVH__IGN, & ! 280  Interp. scheme for polar motion.
     &     'WOBLIBRA',  GVH__IGN, & ! 281  Hi Freq Wobble Libration Contribution
     &     'WOBORTHO',  GVH__IGN, & ! 282  ORTHO_EOP tidal wobble contribtn
     &     'WOBXCONT',  GVH__IGN, & ! 283  X Wobble contribution definition
     &     'WOBYCONT',  GVH__IGN, & ! 284  Y Wobble contribution definition
     &     'WOB_CFLG',  GVH__IGN, & ! 285  Wobble flow control mess def.
     &     'WOB_MESS',  GVH__IGN, & ! 286  Wobble message definition.
     &     'WOB_PART',  GVH__IGN, & ! 287  Wobble partial derivatives def.
     &     'ZDELAY  ',  GVH__IGN  & ! 288  Corel zenith atmos. delay (sec).
     &     /
!
