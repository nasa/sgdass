...
! *                                                                      *
! * ## 31-JUL-2007  GVF_TRANSFORM_MAIN v3.8 (d) L. Petrov 18-MAR-2024 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
     ...
     &           FIL_TRY*128, PID_STR*8
      ...
      LOGICAL*1  FL_DEBUG
      INTEGER*4, EXTERNAL :: GETPID, ILEN, I_LEN, LINDEX, LTM_DIF, RENAME, SYSTEM
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 31-JUL-2007  GVF_TRANSFORM  v3.5 (d) L. Petrov  01-OCT-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
#ifdef GVH_STANDALONE
     ...
     &           EXP_NAME*16, BGV_DISP*3, FIL_TMP*128, PID_STR*8
     ...
     &           IL, IL1, ID, IP, IS, MODE_OUT, IER
      LOGICAL*1  FL_DEBUG
      INTEGER*4, EXTERNAL :: GET_PID, ILEN, I_LEN, LINDEX, LTM_DIF, RENAME, SYSTEM
!
      FL_DEBUG = .TRUE.
      CALL CLRCH ( PID_STR ) 
      WRITE ( UNIT=PID_STR, FMT='(I8)' ) GETPID()
...
!
! ------------- Write down the envelope file first in the temporary file 
! ------------- on the same directory
!
                FIL_ENV_OUT = TRIM(GVF_ENV_DIR)//'/'//TRIM(FIL_ENV_OUT)//'.env'
                FIL_TMP = TRIM(FIL_ENV_OUT)//'__'//PID_STR
                CALL ERR_PASS ( IUER, IER )
                CALL WR_TEXT  ( L_ENV, BUF_ENV, FIL_TMP, IER )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 1830, IUER, 'GVF_TRANSFORM', 'Error in '// &
     &                   'attempt to write output database file '//FIL_TMP )
                     RETURN 
                END IF
!
! ------------- and then renaming it to the permanent name
!
                IS = RENAME ( TRIM(FIL_TMP)//CHAR(0), TRIM(FIL_ENV_OUT)//CHAR(0) )
                IF ( IER .NE. 0 ) THEN
                     CALL ERR_LOG ( 1831, IUER, 'GVF_TRANSFORM', 'Error in '// &
     &                   'attempt to rename the output database file from '// &
     &                   TRIM(FIL_TMP)//' to '//FIL_ENV_OUT )
                     RETURN 
                END IF
