...
! *                                                                      *
! *  ### 17-AUG-2002  MATVEC_TEST  v1.2 (d)  L. Petrov  30-JAN-2006 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 17-AUG-2002  MUL_MM_SANITY v1.2 (d) L. Petrov  20-JAN-2005 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      CHARACTER  FUNC*(*)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 17-AUG-2002  MUL_MM_COMPARE v1.0 (d)  L. Petrov 17-AUG-2002 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      CHARACTER  FUNC*(*)
...
!
      ...
      DO 410 J1=1,N1
         DO 420 J2=1,M1
            MAT1(J2,J1) = RGAUSS ( ISEED, 1.0D0 )
 420     CONTINUE
 410  CONTINUE
!
      DO 430 J3=1,N2
         DO 440 J4=1,M2
            MAT2(J4,J3) = RGAUSS ( ISEED, 1.0D0 )
 440     CONTINUE
 430  CONTINUE
...
!
     ...
     &                   MAT3(I1,I2), MAT4(I1,I2), (MAT3(I1,I2) - MAT4(I1,I2)), EPS
 110                  FORMAT ( 'Function: ',A,' M1=',I4,' N1=',I4,' M2=',I4, &
     &                     ' N2=',I4,' M3=',I4, ' N3=',I4,' i=',I4,' j=',I4/ &
     &                     ' GEN:  mat(i,j) = ', 1PG22.15, &
     &                     ' OPT:  mat(i,j) = ', 1PG22.15, &
     &                     ' DIFF           = ', 1PG22.15, ' EPS= ', 1PG22.15 )
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 17-AUG-2002  MUL_MM_COMPARE v1.1 (d)  L. Petrov 05-OCT-2002 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      CHARACTER  FUNC*(*)
...
!
      EPS_MAX = -1.0D0
      DO 430 J3=1,M3
         IF ( DABS(VEC2(J3)) .LT. VAL_MIN ) THEN
...
!
     ...
     &                  VEC2(J3), VEC3(J3), VEC2(J3) - VEC3(J3), EPS
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 17-AUG-2002  MUL_MM_TIMER  v1.1 (d) L. Petrov  20-JAN-2005 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      CHARACTER  FUNC*(*)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 17-AUG-2002   MUL_MM_TIM  v1.2 (d)  L. Petrov 06-AUG-2012  ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'matvec.i'
     ...
     &           J9, J10, J11, J12, J13, IER
      REAL*8     DNOP, MFLOP_OPT, MFLOP_GEN, TIM
      REAL*8,    EXTERNAL :: RGAUSS, CPU_TIMER
!
      ISEED = 32137
      DO 410 J1=1,N1
         DO 420 J2=1,M1
...
!
           TIM = CPU_TIMER ( %VAL(0) ) ! start timing
           DO 450 J5=1,N_REP
              IS = OPT_FUNC ( M1, N1, MAT1, N2, MAT2, M3, MAT3, IER )
 450       CONTINUE
           TIM = CPU_TIMER ( %VAL(2) ) ! end timing
           IF ( TIM <  0.01 ) THEN
                WRITE ( 6, 110 ) FUNC, M1, N1, N3, TIM
 110            FORMAT ( 1X,' Warning: fuction ',A,' M1=',I4,' N1=',I4, &
     &                      ' N3= ', I4, '  TIM_DIF = ',F8.3 )
                MFLOP_OPT = 1.0
              ELSE
                MFLOP_OPT = N_REP*DNOP*1.D-6/TIM
           END IF
         ELSE IF ( FUNC .EQ. 'sv_v' ) THEN
!
...
!
 ...
 4110      CONTINUE
     ...
     &             FUNC .EQ. 'ti_s'                                 ) THEN
           IF ( INT_DIM < 2 ) N_REP = N_REP/4
           IF ( INT_DIM < 3 ) N_REP = N_REP/10
!
...
!
 ...
 130       FORMAT ( A,'  dims: ',I4,', ',I4, '  speed: ', F8.1, ' Mflops' )
         ELSE
!          write ( 6, * ) ' n_rep = ',n_re
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 18-AUG-2002     INV_CHECK   v1.0 (d)  L. Petrov 18-AUG-2002 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      CHARACTER  FUNC_NAME*(*)
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 17-AUG-2002    INV_TIM    v1.2 (d)  L. Petrov  06-AUG-2012 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
      INCLUDE   'matvec.i'
...
!
 ...
 120       FORMAT ( A,'  dims: ',I4, '  speed: ', F8.1, ' Mflops' )
         ELSE
           TIM = CPU_TIMER ( %VAL(0) ) ! start timing
           DO 470 J7=1,N_REP
              IER = -1
 ...
 470       CONTINUE
           ...
           IF ( TIM < 0.01 ) THEN
                WRITE ( 6, 110 ) FUNC, N, TIM
           END IF
           MFLOP_GEN = N_REP*DNOP*1.D-6/TIM
           WRITE ( 6, 130 ) FUNC, N, MFLOP_GEN
 130       FORMAT ( A,'  dims: ',I4, '  gen_speed: ',F8.1,' Mflops  ' )
      END IF
!
      CALL ERR_LOG ( 0, IUER )
      RETURN
