...
# *                                                                      *
# * ### 20-JUN-1998  config.csh  v8.20  (d) L. Petrov   28-MAR-2025  ### *
# *                                                                      *
# ************************************************************************
#
setenv LANG   C
setenv LC_ALL C
set config_revision_date = "2025.03.28"
#
...
if ( "$PETOOLS_BLAS" == "NO" )  then
     ...
     echo "ok"
#
     $ECHO "config.csh: Check linking against $blas library... \c"
...
#
     ...
     $ECHO "setenv PNG_LIB_DIR         ${qt}$PNG_LIB_DIR${qt}"         >> $out_vars
     $ECHO "setenv PNG_INC_DIR         ${qt}$PNG_INC_DIR${qt}"         >> $out_vars
  else
...
$ECHO "Making  petools_local.o \c" | tee -a  $CONF_LOG
#
set PETOOLS__VERSION = `echo $PETOOLS_VERSION | awk '{printf "%s%s%s\n", substr($0,0,4), substr($0,5,2), substr($0,7,2)}'`
set PETOOLS__LABEL   = `echo $PETOOLS_VERSION | awk '{printf "petools %s%s%s\n", substr($0,0,4), substr($0,5,2), substr($0,7,2)}'`
set petools_version_len     = `echo $PETOOLS__VERSION  | awk '{print length($1)}'`
set petools_label_len       = `echo $PETOOLS__LABEL    | awk '{print length($0)}'`
set petools_prefix_len      = `echo $PETOOLS_PREFIX    | awk '{print length}'`
set petools_root_len        = `echo $PETOOLS_ROOT      | awk '{print length($1)}'`
set petools_doc_len         = `echo $PETOOLS_ROOT/doc  | awk '{print length($1)}'`
#
cat ${PETOOLS_ROOT}/include/petools_local.templ          | \
    sed "s@PETOOLS__VERSION%%LEN@$petools_version_len@g" | \
    sed "s@PETOOLS__VERSION%%STR@$PETOOLS_VERSION@g"     | \
    sed "s@PETOOLS__LABEL%%LEN@$petools_label_len@g"     | \
    sed "s@PETOOLS__LABEL%%STR@$PETOOLS__LABEL@g"         | \
    sed "s@PETOOLS__PREFIX%%LEN@$petools_prefix_len@g"   | \
    sed "s@PETOOLS__PREFIX%%STR@$PETOOLS_PREFIX@g"       | \
    sed "s@PETOOLS__ROOT%%LEN@$petools_root_len@g"       | \
    sed "s@PETOOLS__ROOT%%STR@$PETOOLS_ROOT@g"           | \
    sed "s@PETOOLS__DOC%%LEN@$petools_doc_len@g"         | \
    sed "s@PETOOLS__DOC%%STR@$PETOOLS_ROOT/doc@g"        > \
    ${PETOOLS_ROOT}/include/petools_local.i
echo " done" | tee -a  $CONF_LOG
#
...
if ( $PETOOLS_PGPLOT == "YES" ) then
     ...
     $ECHO "PNG_LIB_DIR         =  $PNG_LIB_DIR"              >> $out_file
     $ECHO "PNG_INC_DIR         =  $PNG_INC_DIR"              >> $out_file
  else
