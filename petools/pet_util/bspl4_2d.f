      SUBROUTINE BSPL4_2D_CMP ( DEG, BC_CODE, M1, M2, &
...
! *                                                                      *
! *  ### 04-FEB-2008  BSPL4_2D_CMP  v2.0 (d) L. Petrov  19-APR-2009 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
      ...
      CALL BSPL4_2D_DIM_CMP ( 1, DEG, BC_CODE, M1, ARG_ARR_1, &
     &                        M1, M2, BCF, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 1831, IUER, 'BSPL4_2D_CMP', 'Failure in computing '// &
     &         'coefficients of B-spline expansion over the 1st dimension' )
           RETURN 
      END IF
!
      CALL ERR_PASS ( IUER, IER )
      CALL BSPL4_2D_DIM_CMP ( 2, DEG, BC_CODE, M2, ARG_ARR_2, &
     &                        M1, M2, BCF, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 1832, IUER, 'BSPL4_2D_CMP', 'Failure in computing '// &
...
!
      CALL ERR_LOG ( 0, IUER )
      RETURN
      END  SUBROUTINE BSPL4_2D_CMP  !#!  
!
! ------------------------------------------------------------------------
!
      SUBROUTINE BSPL4_2D_DIM_CMP ( DIM, DEG, BC_CODE, M, ARG, M1, M2, &
     &                              BCF, IUER )
! ************************************************************************
! *                                                                      *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ## 04-FEB-2008  BSPL4_2D_DIM_CMP  v2.3 (d) L. Petrov  29-DEC-2021 ## *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!$OMP&   PRIVATE ( J7, J8 ), SCHEDULE ( STATIC )
           DO 470 J7=1,M2
              CALL SGBTRS ( 'T', M+DEG-1, 1, 1, 1, MAT_B4, DEG+1, IPIV, BC(0,J7), M1+DEG, IER )
              DO 480 J8=1-DEG,M1-1
                 BCF(J8,J7) = BC(J8+DEG-1,J7)
 480          CONTINUE 
 470       CONTINUE 
...
!$OMP&     PRIVATE ( J9, IER ), SCHEDULE ( STATIC )
           DO 490 J9=1-DEG,M1
              CALL SGBTRS ( 'T', M+DEG-1, 1, 1, 1, MAT_B4, DEG+1, IPIV, BC(0,J9), M2+DEG, IER )
 490       CONTINUE 
!$OMP END PARALLEL DO
!$OMP      PARALLEL DO IF ( NTHR > 1 ), &
...
!
      CALL ERR_LOG ( 0, IUER )
      RETURN
      END  SUBROUTINE  BSPL4_2D_DIM_CMP  !#!#
!
! ------------------------------------------------------------------------
!
      FUNCTION VAL_2D_BSPL4 ( ARG_1, ARG_2, M1, M2, DEG, IND_1, IND_2, &
     &                        ARG_ARR_1, ARG_ARR_2, BCF )
! ************************************************************************
! *                                                                      *
! *   Routine VAL_2D_BSPL4  computes the value of the 2D spline of the   *
! *   DEG th degree at the point with coordinates (ARG_1, ARG_2).        *
! *                                                                      *
! * _________________________ Input parameters: ________________________ *
! *                                                                      *
! *     ARG_1 ( REAL*4    ) -- coordinate of the point along dimension 1 *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 01-FEB-2008  VAL_2D_BSPL4  v1.0 (d) L. Petrov  01-FEB-2008 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  IND_1, IND_2, M1, M2, DEG
      REAL*4     ARG_1, ARG_2, ARG_ARR_1(M1), ARG_ARR_2(M2), &
     &           BCF(1-DEG:M1,1-DEG:M2)
      REAL*4     VAL_2D_BSPL4 
      INTEGER*4  MDEG
      PARAMETER  ( MDEG = 16 )
      REAL*4     BSPL(-MDEG:0,2)
...
!
      DO 410 J1=-DEG,0
         BSPL(J1,1) = BSPL4_VAL ( M1, ARG_ARR_1, DEG, IND_1+J1, ARG_1 )
 410  CONTINUE 
      DO 420 J2=-DEG,0
         BSPL(J2,2) = BSPL4_VAL ( M2, ARG_ARR_2, DEG, IND_2+J2, ARG_2 )
 420  CONTINUE 
!
      VAL_2D_BSPL4 = 0.0
      DO 430 J3=-DEG,0
         DO 440 J4=-DEG,0
            VAL_2D_BSPL4 = VAL_2D_BSPL4 + BCF(IND_1+J4,IND_2+J3)* &
     &                                    BSPL(J4,1)*BSPL(J3,2)
 440     CONTINUE 
 430  CONTINUE 
!
      RETURN
      END  FUNCTION VAL_2D_BSPL4  !#!  
!
! ------------------------------------------------------------------------
!
      SUBROUTINE VAL_DER_2D_BSPL4 ( ARGS, DEG, DIMS, INDS, ARG_ARR_1, &
...
! *                                                                      *
! *       VAL ( REAL*4    ) -- Value of the 2D spline at the point with  *
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 02-MAR-2009  VAL_DER_2D_BSPL4 v1.0 (d) L. Petrov 02-MAR-2009 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           BCF(1-DEG:DIMS(1),1-DEG:DIMS(2))
      ...
      INTEGER*4  MDEG
      PARAMETER  ( MDEG = 16 )
      REAL*4     BSPL(-MDEG:0,2), DSPL(-MDEG:0,2)
...
!
      RETURN
      END  SUBROUTINE  VAL_DER_2D_BSPL4  !#!#
