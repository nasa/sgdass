      SUBROUTINE SET_READ_LOCK ( IO_LOCK_FILE, READ_LOCK_FILE, &
     &                           WRITE_LOCK_FILE, WAIT_TIME, &
     &                           FD_READ_LOCK, FD_WRITE_LOCK, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine SET_READ_LOCK waits for the write lock to be cleared and   *
! *   then sets the read lock to allow the process to block other        *
! *   processes from reading the data while it writes data.              *
! *                                                                      *
...
! *                                                                      *
! *  ### 16-OCT-2019 SET_READ_LOCK v1.2 (d)  L. Petrov  07-JUL-2026  ### *
! *                                                                      *
! ************************************************************************
#ifdef ADR_32BIT
...
#define  FUNC_OPEN OPEN
     ...
     &           EACCES, EDQUOT, EEXIST, ENOENT, EINTR, EPERM, &
     &           OPEN_READ_FLAGS, OPEN_WRITE_FLAGS, MODE_FLAGS, &
     &           MODE_FLAGS_NOISGID, OLD_UMASK, ITIMER_REAL, &
     &           SIGALRM, LOCK_EX, LOCK_SH, LOCK_NB, LOCK_UN, LOCK_SHARED
...
!
      ...
      CALL GET_SYSTEM_CONSTANT ( 'EACCES',      EACCES,        ARG_LEN )
      ...
      CALL GET_SYSTEM_CONSTANT ( 'EPERM',       EPERM,         ARG_LEN )
!
...
!
      ...
      MODE_FLAGS_NOISGID  = MODE_1 + MODE_2 + MODE_3 + MODE_4 + MODE_5 + MODE_6
      ...
      MODE_FLAGS_NOIGSID = MODE_1 .OR. MODE_2 .OR. MODE_3 .OR. MODE_4 .OR. MODE_5 .OR. MODE_6 
...
!
           ...
           IF ( FD_IO_LOCK < 0 ) THEN
                CALL CLRCH ( STR )
                CALL GERROR ( STR )
                CALL ERR_LOG ( 1111, IUER, 'SET_READ_LOCK', 'Error '// &
     ...
     &              'io lock file '//IO_LOCK_FILE )
                ...
                RETURN
           END IF
        ELSE IF ( FD_IO_LOCK < 0 .AND. IOS .EQ. EPERM ) THEN
          FD_IO_LOCK = FUNC_OPEN ( TRIM(IO_LOCK_FILE)//CHAR(0), %VAL(OPEN_WRITE_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
          IF ( FD_IO_LOCK < 0 ) THEN
               IOS = IERRNO()
             ELSE 
               IOS = 0
          ...
          IF ( FD_IO_LOCK < 0 .AND. IOS .EQ. EEXIST  ) THEN
!
! ------------- The file exist? Try to open for reading only
!
                FD_IO_LOCK = FUNC_OPEN ( TRIM(IO_LOCK_FILE)//CHAR(0), %VAL(OPEN_READ_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
          END IF
          IF ( FD_IO_LOCK < 0 ) THEN
               IOS = IERRNO()
             ELSE 
          ...
          IF ( FD_IO_LOCK < 0 ) THEN
               ...
               CALL GERROR ( STR )
               IS = CLOSE ( %VAL(FD_IO_LOCK) )
               CALL ERR_LOG ( 1112, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
     &             ' in an attempt to create the io lock file '//IO_LOCK_FILE )
        ...
        ELSE IF ( FD_IO_LOCK < 0 .AND. IOS .NE. EEXIST  ) THEN
           ...
           IS = FLOCK ( %VAL(FD_IO_LOCK), %VAL(LOCK_UN) )
           IS = CLOSE ( %VAL(FD_IO_LOCK) )
           CALL ERR_LOG ( 1113, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
     &          ' in an attempt to create the io lock file '//IO_LOCK_FILE )
           RETURN
      END IF
!
...
!
! --- Set exclusive blocking lock for the io lock file. Thought the lock will expire when
...
!
      ...
      IF ( IS < 0 ) THEN
           CALL CLRCH  ( STR )
           CALL GERROR ( STR )
           CALL ERR_LOG ( 1114, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
...
!
     ...
     &              'write lock file '//WRITE_LOCK_FILE )
                ...
                RETURN
           END IF
        ELSE IF ( FD_WRITE_LOCK < 0 .AND. IOS .EQ. EPERM  ) THEN
           FD_WRITE_LOCK = FUNC_OPEN ( TRIM(WRITE_LOCK_FILE)//CHAR(0), %VAL(OPEN_WRITE_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
           IF ( FD_WRITE_LOCK < 0 ) THEN
                IOS = IERRNO()
              ELSE 
                IOS = 0
...
! ------------- The file exist? Try to open for reading only
!
                FD_WRITE_LOCK = FUNC_OPEN ( TRIM(WRITE_LOCK_FILE)//CHAR(0), %VAL(OPEN_READ_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
           END IF
           IF ( FD_WRITE_LOCK < 0 ) THEN
                CALL CLRCH ( STR )
                CALL GERROR ( STR )
                CALL ERR_LOG ( 1116, IUER, 'SET_READ_LOCK', 'Error '// &
     &               TRIM(STR)//' in an attempt to open for reading the '// &
     &              'write lock file '//WRITE_LOCK_FILE )
                IS = FLOCK ( %VAL(FD_IO_LOCK), %VAL(LOCK_UN) )
                IS = CLOSE ( %VAL(FD_IO_LOCK) )
           ...
           END IF
           IS = UMASK ( %VAL(OLD_UMASK) )
        ELSE IF ( FD_WRITE_LOCK < 0 ) THEN
           CALL CLRCH  ( STR )
           CALL GERROR ( STR )
           IS = FLOCK ( %VAL(FD_IO_LOCK), %VAL(LOCK_UN) )
           IS = CLOSE ( %VAL(FD_IO_LOCK) )
           CALL ERR_LOG ( 1117, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
     &          ' in an attempt to create the write lock file '//WRITE_LOCK_FILE )
           RETURN
      END IF
!
! --- Set shared non-blocking lock for the write lock file
!
      IS = FLOCK   ( %VAL(FD_WRITE_LOCK), %VAL(LOCK_SHARED) )
!
...
!
     ...
     &              'read lock file '//READ_LOCK_FILE )
                ...
                RETURN
           END IF
        ELSE IF ( FD_READ_LOCK < 0 .AND. IOS .EQ. EPERM ) THEN
           FD_READ_LOCK = FUNC_OPEN ( TRIM(READ_LOCK_FILE)//CHAR(0), %VAL(OPEN_WRITE_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
           IF ( FD_READ_LOCK < 0 ) THEN
                IOS = IERRNO()
              ELSE 
                IOS = 0
...
! ------------- The file exist? Try to open for reading only
!
                FD_READ_LOCK = FUNC_OPEN ( TRIM(READ_LOCK_FILE)//CHAR(0), %VAL(OPEN_READ_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
           END IF
           IF ( FD_READ_LOCK < 0 ) THEN
                IOS = IERRNO()
              ELSE 
           ...
           IF ( FD_READ_LOCK < 0 ) THEN
                ...
                CALL GERROR ( STR )
                IS = CLOSE ( %VAL(FD_WRITE_LOCK) )
                CALL ERR_LOG ( 1119, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
     &          ' in an attempt to create the WRITE lock file '//READ_LOCK_FILE )
               RETURN
           END IF
           IS = UMASK ( %VAL(OLD_UMASK) )
        ELSE IF ( FD_READ_LOCK < 0 .AND. IOS .NE. EEXIST  ) THEN
           ...
           IS = CLOSE ( %VAL(FD_WRITE_LOCK) )
           IS = CLOSE ( %VAL(FD_IO_LOCK) )
           CALL ERR_LOG ( 1120, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
...
!
      ...
      IS = SETITIMER ( %VAL(ITIMER_REAL), TIMER, TIMER )
!
! --- Set exclusive blocking lock for the read lock file. Thought the lock will expire when
! --- the ALARM signal will be triggered
!
      IS = FLOCK   ( %VAL(FD_READ_LOCK), %VAL(LOCK_EX) )
!
! --- Restore the alarm handler to the previous state
!
#ifdef LINUX
      ...
      IF ( IS < 0 ) THEN
           CALL CLRCH  ( STR )
           CALL GERROR ( STR )
           CALL ERR_LOG ( 1118, IUER, 'SET_READ_LOCK', 'Error '//TRIM(STR)// &
     &          ' in an attempt to set exclusive lock to the read lock file '// &
     &          READ_LOCK_FILE )
