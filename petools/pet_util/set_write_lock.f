...
! *                                                                      *
! *  ### 16-OCT-2019 SET_WRITE_LOCK v1.2 (d)  L. Petrov  07-JUL-2026 ### *
! *                                                                      *
! ************************************************************************
#ifdef ADR_32BIT
...
#define  FUNC_OPEN OPEN
     ...
     &           EACCES, EDQUOT, EEXIST, EINTR, ENOENT, EPERM, &
     &           OPEN_READ_FLAGS, OPEN_WRITE_FLAGS, OPEN_WRITE_FLAGS_NOEXC, &
     &           MODE_FLAGS, MODE_FLAGS_NOISGID, OLD_UMASK, ITIMER_REAL, &
...
!
      ...
      OPEN_WRITE_FLAGS_NOEXC = O_CREAT_FLAG + O_RDWR_FLAG 
      ...
      MODE_FLAGS_NOSGID      = MODE_1 .OR. MODE_2 .OR. MODE_3 .OR. MODE_4 .OR. MODE_5 .OR. MODE_6
      LOCK_SHARED = LOCK_SH .OR. LOCK_NB
#endif
!
! --- Try to open the io lock file in the exclusive creation mode
!     ==========================================-================
!
...
!
! ------- Operation is not permitted. Try to open it again, but this time without ISGID bit 
!
...
!
                FD_READ_LOCK = FUNC_OPEN ( TRIM(IO_LOCK_FILE)//CHAR(0), %VAL(OPEN_READ_FLAGS), %VAL(MODE_FLAGS_NOISGID) )
           END IF
           IF ( FD_IO_LOCK < 0 ) THEN
                IOS = IERRNO()
              ELSE 
                IOS = 0
           END IF
           IF ( FD_IO_LOCK < 0 .AND. IOS .NE. EEXIST  ) THEN
                CALL CLRCH  ( STR )
                CALL GERROR ( STR )
                IS = CLOSE ( %VAL(FD_IO_LOCK) )
                CALL ERR_LOG ( 1012, IUER, 'SET_WRITE_LOCK', 'EE Error '//TRIM(STR)// &
...
!
      ...
      IF ( IS < 0 ) THEN
           CALL CLRCH  ( STR )
           CALL GERROR ( STR )
           CALL ERR_LOG ( 1014, IUER, 'SET_WRITE_LOCK', 'Error '//TRIM(STR)// &
...
!
           ...
           IF ( FD_READ_LOCK < 0 ) THEN
                CALL CLRCH ( STR )
                CALL GERROR ( STR )
                CALL ERR_LOG ( 1015, IUER, 'SET_WRITE_LOCK', 'Error '// &
     &               TRIM(STR)//' in an attempt to open for reading the '// &
...
!
           ...
           IF ( FD_READ_LOCK < 0 ) THEN
                ...
                IS = FLOCK ( %VAL(FD_IO_LOCK), %VAL(LOCK_UN) )
                IS = CLOSE ( %VAL(FD_IO_LOCK) )
                CALL ERR_LOG ( 1016, IUER, 'SET_WRITE_LOCK', 'Error '//TRIM(STR)// &
...
!
           ...
           IF ( FD_WRITE_LOCK < 0 ) THEN
                CALL CLRCH ( STR )
                CALL GERROR ( STR )
                CALL ERR_LOG ( 1018, IUER, 'SET_WRITE_LOCK', 'Error '// &
     &               TRIM(STR)//' in an attempt to open for reading the '// &
...
!
           ...
           IF ( FD_WRITE_LOCK < 0 ) THEN
              ...
              ELSE 
               IOS = 0
           END IF
           IF ( FD_WRITE_LOCK < 0 .AND. IOS .NE. EEXIST  ) THEN
                CALL CLRCH  ( STR )
                CALL GERROR ( STR )
                IS = CLOSE ( %VAL(FD_WRITE_LOCK) )
                CALL ERR_LOG ( 1019, IUER, 'SET_WRITE_LOCK', 'EE Error '//TRIM(STR)// &
     &          ' in an attempt to create the WRITE lock file '//WRITE_LOCK_FILE )
        ...
        ELSE IF ( FD_WRITE_LOCK < 0 .AND. IOS .NE. EEXIST  ) THEN
           ...
           IS = CLOSE ( %VAL(FD_READ_LOCK) )
           IS = CLOSE ( %VAL(FD_IO_LOCK) )
           CALL ERR_LOG ( 1020, IUER, 'SET_WRITE_LOCK', 'Error '//TRIM(STR)// &
     &          ' in an attempt to create the WRITE lock file '//WRITE_LOCK_FILE )
           RETURN
      END IF
!
...
!
      ...
      IF ( IS < 0 ) THEN
           CALL CLRCH  ( STR )
           CALL GERROR ( STR )
           CALL ERR_LOG ( 1021, IUER, 'SET_WRITE_LOCK', 'Error '//TRIM(STR)// &
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 18-OCT-2019 LIFT_WRITE_LOCK  v1.0 (d)  L. Petrov 18-OCT-2019 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 18-OCT-2019 LIFT_READ_LOCK  v1.0 (d)  L. Petrov 18-JUL-2021 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 18-OCT-2019 LIFT_WRITE_LOCK  v1.0 (d)  L. Petrov 18-JUL-2021 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
