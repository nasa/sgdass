...
! *                                                                      *
! * ### 24-MAR-2010  EBSPL_LSQ_CNS  v1.2 (d)  L. Petrov  24-NOV-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
      IF ( MF < 2 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( MF, STR )
           CALL ERR_LOG ( 1411, IUER, 'EBSPL_LSQ_CNS', 'Parameter MF '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
      IF ( DEG < 0 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( DEG, STR )
           CALL ERR_LOG ( 1412, IUER, 'EBSPL_LSQ_CNS', 'Parameter DEG '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
      IF ( MF < MS + DEG ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( MS, STR )
           CALL ERR_LOG ( 1413, IUER, 'EBSPL_LSQ_CNS', 'Parameter MF '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPAR, STR )
           CALL ERR_LOG ( 1414, IUER, 'EBSPL_LSQ_CNS', 'Failure to allocate '// &
     ...
     &          'temporary array EQU' )
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPA2, STR )
           CALL ERR_LOG ( 1415, IUER, 'EBSPL_LSQ_CNS', 'Failure to allocate '// &
     &          STR(1:I_LEN(STR))//' bytes of dynamic memory for '// &
     &          'temporary array NOR_MAT' )
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPAR, STR )
           CALL ERR_LOG ( 1416, IUER, 'EBSPL_LSQ_CNS', 'Failure to allocate '// &
     &          STR(1:I_LEN(STR))//' bytes of dynamic memory for '// &
...
!
      DO 410 J1=1,MF
         ...
         IF ( J1 == 1 ) THEN
!
! ----------- The first knot. Check undershort or overshot
!
              IF ( ARG_VEC(2) - ARG_VEC(1) > 0.0D0 .AND. PIV_IND == -2 ) THEN
!
! ---------------- Ascending order and ARG_VEC(1) < ARG_NOD(1). Check for 
! ---------------- undershort
!
                   PIV_IND = IXMN8 ( MS, ARG_NOD, ARG_VEC(J1) + EPS*(ARG_VEC(2) - ARG_VEC(1)) )
              END IF
              IF ( ARG_VEC(2) - ARG_VEC(1) < 0.0D0 .AND. PIV_IND == -1 ) THEN
!
! ---------------- Descending order and ARG_VEC(1) < ARG_NOD(1). Check for 
! ---------------- overshort
!
                   PIV_IND = IXMN8 ( MS, ARG_NOD, ARG_VEC(J1) + EPS*(ARG_VEC(2) - ARG_VEC(1)) )
              END IF
           ELSE IF ( J1 == MF ) THEN
!
! ----------- The last knot. Check undershort or overshot
!
              IF ( ARG_VEC(MF) - ARG_VEC(MF-1) > 0.0D0 .AND. PIV_IND == -1 ) THEN
!
! ---------------- Ascending order and ARG_VEC(1) < ARG_NOD(1). Check for 
! ---------------- undershort
!
                   PIV_IND = IXMN8 ( MS, ARG_NOD, ARG_VEC(J1) - EPS*(ARG_VEC(MF) - ARG_VEC(MF-1)) )
              END IF
              IF ( ARG_VEC(MF) - ARG_VEC(MF-1) < 0.0D0 .AND. PIV_IND == -2 ) THEN
!
! ---------------- Descending order and ARG_VEC(1) < ARG_NOD(1). Check for 
! ---------------- undershort
!
         ...
         IF ( PIV_IND < 1 ) THEN
              CALL CLRCH   ( STR )
              CALL INCH    ( J1, STR )
              CALL ERR_LOG ( 1417, IUER, 'EBSPL_LSQ_CNS', 'The '//TRIM(STR)// &
     &            '-th index is out of range of the spline knot sequence' )
              DEALLOCATE ( NOR_VEC, NOR_MAT, EQU_VEC )
              RETURN 
         END IF
!
         CALL NOUT_R8 ( LPAR, EQU_VEC )
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 1418, IUER, 'EBSPL_LSQ_CNS', 'Failure '// &
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 25-MAR-2010  EBSPL_WLSQ_CNS v2.2 (d)  L. Petrov  22-NOV-2025 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INTEGER*4  MS, MF, DEG, IUER
...
!
      ...
      IF ( MF < 2 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( MF, STR )
           CALL ERR_LOG ( 1911, IUER, 'EBSPL_WLSQ_CNS', 'Parameter MF '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
      IF ( DEG < 0 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( DEG, STR )
           CALL ERR_LOG ( 1912, IUER, 'EBSPL_WLSQ_CNS', 'Parameter DEG '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
      IF ( MF < MS + DEG ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( MS, STR )
           CALL ERR_LOG ( 1913, IUER, 'EBSPL_WLSQ_CNS', 'Parameter MF '// &
     &         'is too small: '//STR )
           RETURN 
      END IF
!
...
!
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPAR, STR )
           CALL ERR_LOG ( 1914, IUER, 'EBSPL_WLSQ_CNS', 'Failure to allocate '// &
     &          STR(1:I_LEN(STR))//' bytes of dynamic memory for '// &
     &          'temporary array EQU' )
           RETURN 
      END IF
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPA2, STR )
           CALL ERR_LOG ( 1915, IUER, 'EBSPL_WLSQ_CNS', 'Failure to allocate '// &
     &          STR(1:I_LEN(STR))//' bytes of dynamic memory for '// &
     &          'temporary array NOR_MAT' )
           DEALLOCATE ( EQU_VEC )
           RETURN 
      ...
      IF ( IER .NE. 0 ) THEN
           CALL CLRCH ( STR )
           CALL IINCH ( LPAR, STR )
           CALL ERR_LOG ( 1916, IUER, 'EBSPL_WLSQ_CNS', 'Failure to allocate '// &
     &          STR(1:I_LEN(STR))//' bytes of dynamic memory for '// &
     &          'temporary array NOR_VEC' )
           DEALLOCATE ( NOR_MAT )
           DEALLOCATE ( EQU_VEC )
...
!
         ...
         IF ( PIV_IND < 1 ) THEN
              CALL CLRCH   ( STR )
              CALL INCH    ( J1, STR )
              WRITE ( 6, * ) 'ARG_VEC(J1) = ', ARG_VEC(J1)
              WRITE ( 6, * ) 'ARG_NOD(1)  = ', ARG_NOD(1)
              WRITE ( 6, * ) 'ARG_NOD(MS) = ', ARG_NOD(MS)
              CALL ERR_LOG ( 1917, IUER, 'EBSPL_WLSQ_CNS', 'The '//TRIM(STR)// &
     &            '-th index is out of range of the spline knot sequence' )
              DEALLOCATE ( NOR_VEC, NOR_MAT, EQU_VEC )
              RETURN 
         END IF
...
!
      ...
      CALL INVS ( LPAR, NOR_MAT, RC, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 1918, IUER, 'EBSPL_WLSQ_CNS', 'Failure to invert '// &
     &         'normal matrix' )
           DEALLOCATE ( NOR_VEC )
           DEALLOCATE ( NOR_MAT )
           DEALLOCATE ( EQU_VEC )
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! * ### 12-AUG-2016 DIAD_CVT_S_PIV  v1.0 (d) L. Petrov  12-AUG-2016 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
