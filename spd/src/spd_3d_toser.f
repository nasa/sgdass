...
! *                                                                      *
! *  ### 20-FEB-2009  SPD_3D_TOSER  v2.5 (d)  L. Petrov 15-JUN-2025 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'spd.i'
...
!
! ------------- No relevant BSPD files were found? 
!
                IF ( MODE == 'summary' ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 6505, IUER, 'SPD_3D_TOSER', 'Find no binary '// &
     &                   'files in '//TRIM(DIRBSPD)//' directory -- cannot '// &
     &                   'run command summary' )
                     CALL EXIT ( 1 )
                END IF
!
...
!
      ...
      DO 420 J2=1,1024*1024*1024
         ...
         IF ( LEV == 0 ) GOTO 820
         IF ( IS .NE. 0 ) THEN
              CALL ERR_LOG ( 6506, -2, 'SPD_3D_TOSER', 'Error in '// &
...
!
              ...
              IF ( IUER .NE. 0 ) THEN
                   IUER = -1
                   CALL ERR_LOG ( 6507, IUER, 'SPD_3D_TOSER', 'Error in '// &
     &                 'reading directory '//DIRIN(1:I_LEN(DIRIN))//' -- '// &
     &                  FILNAM )
                   CALL EXIT ( 1 )
              END IF
...
!
         ...
         IF ( L_FIL > M_FIL ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( M_FIL, STR )
              CALL ERR_LOG ( 6508, -2, 'SPD_3D_TOSER', 'Too many spd files '// &
...
!
      ...
      END IF
      IF ( IVRB .GE. 2 ) THEN
           WRITE ( 6, '(A,I7,A)' ) '  spd_3d_toser: Got ', L_FIL, ' ascii  files in the input directory '//TRIM(DIRIN)
           WRITE ( 6, '(A,I7,A)' ) '  spd_3d_toser: Got ', L_BSPD, ' binary files in the input directory '//TRIM(DIRBSPD)
      END IF
      IF ( MODE == 'summary' ) THEN
           IF ( L_BSPD == 0 ) THEN
                IUER = -1
                CALL ERR_LOG ( 6509, IUER, 'SPD_3D_TOSER', 'No bdsp '// &
     &              'files was found in directory '//DIRBSPD )
                CALL EXIT ( 1 )
              ELSE
                IF ( ILEN(PREF_OUT) == 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 6510, IUER, 'SPD_3D_TOSER', 'The '// &
...
!
                ...
                IF ( IUER .NE. 0 ) THEN
                     IUER = -1
                     CALL ERR_LOG ( 6512, IUER, 'SPD_3D_TOSER', 'Error in '// &
     &                   'an attempt to create BSPD summary' )
                     CALL EXIT ( 1 )
                END IF
!
                CALL EXIT ( 0 )
              ELSE IF ( MODE .NE. 'summary' ) THEN
                CALL ERR_LOG ( 6513, -2, 'SPD_3D_TOSER', 'No spd files were '// &
     &              'found in '//DIRIN(1:I_LEN(DIRIN))//' directory mode= '//MODE )
                CALL EXIT ( 1 )
           END IF
      END IF
      IF ( IVRB == 7 ) THEN
 ...
 150               FORMAT ( '  Copy file ', I6, ' ( ', I6, ' ) ', A, A,$ )
     ...
     &                       FILBSPD(J3)(1:I_LEN(FILBSPD(J3)))//'__'//POSTFIX//CHAR(0) )
               IF ( IS .NE. 0 ) THEN
                    CALL CLRCH   ( STR )
                    CALL GERROR  ( STR )
                    CALL ERR_LOG ( 6514, -2, 'SPD_3D_TOSER', 'Error in '// &
...
!
     ...
     &                      MOD_TEXT, M_INP, L_INP, INP_TEXT, IUER )
         IF ( IUER .NE. 0 ) THEN
              IUER = -1
              CALL ERR_LOG ( 6515, IUER, 'SPD_3D_TOSER', 'Error in reading '// &
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
           IUER = -1
           CALL ERR_LOG ( 6519, IUER, 'SPD_3D_TOSER', 'Error in '// &
     &         'an attempt to create BSPD summary' )
           CALL EXIT ( 1 )
      END IF
!
...
!
      ...
      IF ( IUER .NE. 0 ) THEN
           IUER = -1
           CALL ERR_LOG ( 6520, IUER, 'SPD_3D_TOSER', 'Check of directory '// &
