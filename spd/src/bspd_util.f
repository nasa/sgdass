...
! *                                                                      *
! *  ### 18-NOV-2024   BSPD_CHECK  v1.3 (c)  L. Petrov  18-MAY-2026 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
     ...
     &           NUM_EPC_MIN_STR*9, NUM_EPC_MAX_STR*9, STR*128, &
     &           NUM_EPOCH_FIELD*12
     ...
     &           SUM_NEPC, DAT_NEPC, STA_MJD_BEG, STA_MJD_END, UNIX_DATE, &
     &           N_ERR, N_NFD, J1, J2, J3, J4, J5, NUM_EPC, NUM_EPC_MIN, &
     &           NUM_EPC_MAX, NUM_EPOCH_FROM_DATES, IER
...
!
      ...
      DO 410 J1=1,NB
         ...
         IF ( BUF(J1)(IND(1,1):IND(2,1)) == 'Num_Epoch:' ) THEN
              NUM_EPOCH_FIELD = BUF(J1)(IND(1,2):IND(2,2))
         ...
         IF ( BUF(J1)(IND(1,1):IND(2,1)) == 'Sample_Interval:' ) THEN
              ...
              DAT_NEPC = IDNINT ( (SUM_TIM_END - SUM_TIM_BEG)/TIM_STEP ) + 1
         END IF
...
!
 ...
 210               FORMAT ( 'Wrong size of file ', A, ' Computed: ', I9, ' Actual: ', I9 )
              ...
              END IF
!
              NUM_EPOCH_FROM_DATES = NINT ( ( (SPD_3D_DEL%TIM%MJD_END - SPD_3D_DEL%TIM%MJD_BEG)*86400.0D0 + &
     &                                        (SPD_3D_DEL%TIM%TAI_END - SPD_3D_DEL%TIM%TAI_BEG) )/ &
     &                                       SPD_3D_DEL%TIM%TIM_STEP &
     &                                      ) + 1
              IF ( NUM_EPOCH_FROM_DATES .NE. SPD_3D_DEL%LAB%TOT_NUM_DEL ) THEN
                   FL_WRONG_SIZE_LEN = .TRUE.
                   N_ERR = N_ERR + 1
                   WRITE ( UNIT=C_ERR(N_ERR), FMT=220 ) TRIM(FIL_BSPD), SPD_3D_DEL%LAB%TOT_NUM_DEL, NUM_EPOCH_FROM_DATES 
 220               FORMAT ( 'Mismatch between TOT_NUM_DEL and beg/end dates for ',A, &
     &                      ' -- ', I6, ' and ', I6 )
                   GOTO 410
              END IF
         END IF
...
!
      STATUS_LINE = 'OK'
      IF ( MIN_STA_TIM_BEG == 0 .AND. MAX_STA_TIM_BEG == 0 ) then
           STATUS_LINE = 'Wrong prefix '//TRIM(FIL_PREFIX)
        ELSE IF ( DABS(MIN_STA_TIM_BEG - SUM_TIM_BEG) > EPS .OR. &
     &     DABS(MAX_STA_TIM_BEG - SUM_TIM_BEG) > EPS      ) THEN
           STATUS_LINE = 'Mismatch in the start date'
        ELSE IF ( DABS(MIN_STA_TIM_END - SUM_TIM_END) > EPS .OR. &
     &     DABS(MAX_STA_TIM_END - SUM_TIM_END) > EPS      ) THEN
           ...
           N_ERR = N_ERR + 1
           WRITE  ( UNIT=STATUS_LINE, FMT=230 ) NUM_EPC, NINT(MIN_STA_TIM_END/TIM_STEP), &
     &                                          TIM_TO_DATE ( MIN_STA_TIM_END, IER ), &
     &                                          STA_MJD_END, STA_TIM_END 
 230       FORMAT ( 'Mismatch in the end date at ', I9, ' epochs. ', &
     &              'Number of common epochs: ', I9, &
     &              ' Min epoch: ', A, 2X, I5, 2X, F7.1 )
        ELSE IF ( DAT_NEPC .NE. SUM_NEPC ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( DAT_NEPC, STR )
           N_ERR = N_ERR + 1
           C_ERR(N_ERR) = 'The field Num_Epoch '//TRIM(NUM_EPOCH_FIELD)// &
     &                    ' does not correspond to the actual number of '// &
     &                    'epochs from start and stop dates: '//STR
           STATUS_LINE = 'Wrong Num_Epoch field in the summary file'
      END IF
!
      WRITE ( 6, 110   ) TRIM(STATUS_LINE)
      IF ( MIN_STA_TIM_BEG .EQ. 0 .AND. MAX_STA_TIM_BEG .EQ. 0 ) THEN
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 30-SEP-2024    BSPD_INFO   v1.1 (d)  L. Petrov  2026.05.18  ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
      ...
      PARAMETER  ( EPS = 180.0D0 )
      REAL*8     TAI_TRUN
      INTEGER*4  J1, IP, NUM_EPOCH_FROM_DATES, IER
...
!
     ...
     &                      SPD_3D_DEL%TIM%NREC, NUM_EPOCH_FROM_DATES
 110       FORMAT ( 'MJD_BEG: ', I5, ' TAI_BEG: ', F8.1, ' BEG_DATE: ', A, &
     &                ' MJD_END: ', I5, ' TAI_END: ', F8.1, ' END_DATE: ', A, &
     &                ' TIM_STEP: ', F8.1, ' NUM_EPOCHS: ', I9, ' TIM_NREC: ', I4, &
     &                'NUM_EPOCH_FROM_DATES: ', I5  )
           IF (  NUM_EPOCH_FROM_DATES .NE. SPD_3D_DEL%LAB%TOT_NUM_DEL) THEN
                 WRITE ( 6, '(A)' ) 'ERROR: mismatch betwween fields TOT_NUM_DEL and dates!'
           END IF
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 30-SEP-2024 BSPD_TRUNCATE  v1.0 (d)  L. Petrov  19-NOV-2024 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
!
      IF ( NREC_TRUN == 0 ) THEN
           WRITE ( 6, '(A)' ) 'Nothing to truncate in file '//TRIM(FIL_BSPD)
           CALL ERR_LOG ( 0, IUER )
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 30-SEP-2024 BSPDIR_TRUNCATE  v1.0 (d) L. Petrov 24-NOV-2024 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 19-NOV-2024  BSPD_EXTRACT  v1.0 (d)  L. Petrov 21-NOV-2024 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
!
      IF ( NREC_EXTR == 0 ) THEN
           WRITE ( 6, '(A)' ) 'Nothing to extract in file '//TRIM(FIL_BSPD)
           CALL ERR_LOG ( 0, IUER )
...
! *   License: NASA Open Source Software Agreement (NOSA).               *
! *                                                                      *
! *  ### 19-NOV-2024  BSPD_EXTEND  v1.1 (d)  L. Petrov  28-FEB-2025  ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
      INCLUDE   'astro_constants.i'
...
!
     ...
     &              (EXTN_TAI_END - SPD_3D_DEL%TIM%TAI_END)       + EPS )/SPD_3D_DEL%TIM%TIM_STEP 
      IF ( NREC_EXTN*SPD_3D_DEL%LAB%LEN_DEL .NE. LEN_EXTN ) THEN
           ...
           WRITE ( 6, * ) 'NREC_EXTN= ', NREC_EXTN
           WRITE ( 6, * ) 'LEN_EXTN=  ', LEN_EXTN, ' SPD_3D_DEL%LAB%LEN_DEL = ', SPD_3D_DEL%LAB%LEN_DEL
