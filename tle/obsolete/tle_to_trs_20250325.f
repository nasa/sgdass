...
!
      CXP = DCOS(PARS_EOP(1))                                       ! cos(x_p)
      SXP = DSIN(PARS_EOP(1))                                       ! sin(x_p)
      CYP = DCOS(PARS_EOP(2))                                       ! cos(y_p)
      SYP = DSIN(PARS_EOP(2))                                       ! sin(y_p)
      CTG = DCOS(THETA_GMST)
      STG = DSIN(THETA_GMST)
!
...
! --- Compute the satellite's terestrial coordinates, 
!     See eq. 97 of memo in ./docs/
!
      X(1) =               ( CXP*CYP )*X_TEME(1)  +                     &
     &       ( CTG*CXP*SYP + STG*SXP )*X_TEME(2)  +                     &
     &       ( STG*CXP*SYP - CTG*SXP )*X_TEME(3)
      X(2) =  -SYP*X_TEME(1)  + CTG*CYP*X_TEME(2) + STG*CYP*X_TEME(3)
      X(3) =               ( SXP*CYP )*X_TEME(1)  +                     &
     &       ( CTG*SXP*SYP - STG*CXP )*X_TEME(2)  +                     &
     &       ( STG*SXP*SYP + CTG*CXP )*X_TEME(3)
!
...
!
     ...
     &          ( STG*SXP*SYP + CTG*CXP )*XDOT_TEME(3)
       ...
       PRINT *, "%%%%%%%%%%%%%%% TLE_TO_TRS - 271 %%%%%%%%"
       PRINT *, "X(TRS)=", X
       PRINT *, "XDOT(TRS)=", XDOT
       PRINT *, "%%%%%%%%%%%%%%% TLE_TO_TRS - 274 %%%%%%%%"
