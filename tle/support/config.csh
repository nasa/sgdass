...
# *                                                                      *
# *  ###  23-MAR-2023   config.csh  v1.2  (d)  L. Petrov 15-FEB-2025 ### *
# *                                                                      *
# ************************************************************************
#
...
if ( -d $SPC_PREFIX == 0 ) then
     echo "tle config: spd client directory $SPC_PREFIX was not found"
     exit 1
endif
if ( -d $SPC_PREFIX/lib == 0 ) then
     echo "tle config: spd client directory $SPC_PREFIX.lib was not found"
     exit 1
endif
if ( -d $SPC_PREFIX/include == 0 ) then
     echo "tle config: spd client directory $SPC_PREFIX/include was not found"
     exit 1
endif
if ( -f $SPC_PREFIX/lib/libspc.a == 0 ) then
     echo "tle config: spd client library $SPC_PREFIX/lib/libspc.a was not found"
     exit 1
endif
#
if ( -d  $NERS_PREFIX == 0 ) then
     echo "tle config: ners install directory $NERS_PREFIX was not found"
     echo "tle config: Please install ners"
     exit 1
endif
#
if ( -d  $NERS_PREFIX/include == 0 ) then
     echo "tle config: ners include directory $NERS_PREFIX/include was not found"
     echo "tle config: Please install ners"
     exit 1
endif
#
if ( -f  $NERS_PREFIX/include/ners_local.i == 0 ) then
     echo "tle config: ners version file $NERS_PREFIX/include/ners_local.i was not found"
     echo "tle config: Please install ners"
     exit 1
endif
...
if ( "$NERS_STANDALONE" == "YES" ) then
     echo "tle config: ners package was configured without petools"
     echo "tle config: Please re-configure and re-install ners with petools support"
     exit 1
endif
#
# --- Generae tle_local.i from template
#
set DATE_ISO    = `date "+%Y.%m.%d_%H:%M:%S"`
set root_len    = `echo $TLE_ROOT         | awk '{print length($1)}'`
set pref_len    = `echo $TLE_PREFIX       | awk '{print length($1)}'`
set version_len = `echo $TLE_VERSION      | awk '{print length($1)}'`
cat $TLE_ROOT/include/tle_local_templ.i | \
    sed "s@%%@Automatically generated on $DATE_ISO from tle_local_templ.i@g" | \
    sed "s@%ROOT_LEN%@$root_len@g"            | \
    sed "s@%PREFIX_LEN%@$pref_len@g"          | \
    sed "s@%VERSION_LEN%@$version_len@g"      | \
    sed "s@%ROOT_VAL%@$TLE_ROOT@g"            | \
    sed "s@%PREFIX_VAL%@$TLE_PREFIX@g"        | \
    sed "s@%VERSION_VAL%@$TLE_VERSION@g"       \
    > $TLE_ROOT/include/tle_local.i 
#
