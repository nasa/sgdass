...
!
     ...
     &         '". Acceptable only OLD, NEW or UNKNOWN' )
           RETURN
      END IF
!
...
!
     ...
     &         '". Acceptable only B1, I2, L2, I4, L4, R4, R8 ' )
           RETURN
      END IF
...
! ************************************************************************
! *                                                                      *
! *  ###   03-JAN-97   WRBIN_RECORD  v 2.2 (c) L. Petrov 04-MAY-2004 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
...
! ************************************************************************
! *                                                                      *
! *  ###   03-JAN-97  RDBIN_RECORD  V2.0  (c) L. Petrov   06-JAN-97 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT NONE
...
!
     ...
     &         'reading the prefix of the record: '//STR )
           RETURN
      END IF
      READ ( UNIT=PRF, FMT='(A4)' ) NBT
      IF ( NBT .LT. 0 ) THEN
           CALL CLRCH ( STR )
           CALL INCH  ( NBT, STR )
           CALL ERR_LOG ( 9032, IUER, 'RDBIN_RECORD', 'Record length is '// &
     &            'negative: '//STR )
           RETURN
      END IF
      IF ( NBT .GT. LA ) THEN
           ...
           CALL CLRCH ( STR1 )
           CALL INCH  ( NBT, STR1 )
           CALL ERR_LOG ( 9033, IUER, 'RDBIN_RECORD', 'Parameter LA is '// &
...
!
      ...
      IF ( IC .LT. 0 ) THEN
           ...
           CALL CLRCH   ( STR1     )
           CALL GERROR  ( STR1     )
           CALL ERR_LOG ( 9034, IUER, 'RDBIN_RECORD', 'Error during '// &
