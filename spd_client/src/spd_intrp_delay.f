      SUBROUTINE SPD_INTRP_DELAY ( STA_NAM, L_STA, SPD_DEL, ELEV, AZIM, &
...
! * SPD_DEL ( SPD_DEL__TYPE ) -- Array of objects with Slant Path        *
...
! *                                                                      *
! * ### 10-JUL-2014  SPD_INTRP_DELAY  v2.1 (d) L. Petrov 26-AUG-2014 ### *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
      ISTA = 0
      DO 410 J1=1,L_STA
         IF ( SPD_DEL(J1)%STA%NAME == STA_NAM ) ISTA = J1
 410  CONTINUE 
      IF ( ISTA == 0 ) THEN
...
!
      IF ( ELEV < INV_MAP_ISA ( DBLE(SPD_DEL(ISTA)%MAP_ARR(SPD_DEL(ISTA)%ELV%N_EL) ) ) ) THEN
           CALL CLRCH ( STR )
           WRITE ( UNIT=STR(1:12), FMT='(1PD12.4)' ) ELEV/DEG__TO__RAD
           CALL CLRCH ( STR1 )
           WRITE ( UNIT=STR1(1:6), FMT='(F6.3)' ) &
     &             INV_MAP_ISA ( DBLE(SPD_DEL(ISTA)%MAP_ARR(SPD_DEL(ISTA)%ELV%N_EL)) )/DEG__TO__RAD
           CALL CLRCH ( STR2 )
           WRITE ( UNIT=STR2(1:6), FMT='(F6.3)' ) &
     &             INV_MAP_ISA ( DBLE(SPD_DEL(ISTA)%MAP_ARR(1)) )/DEG__TO__RAD
...
!
      ...
      ARGS(1) = MAP_ISA
      ARGS(2) = AZIM
      ARGS(3) = ( MJD_OBS*86400.0D0 + TAI_OBS ) - SPD_DEL(ISTA)%TIM_BEG 
!
! --- ... and dimenstin of an array with B-spline coefficients
!
      DIMS(1) = SPD_DEL(ISTA)%ELV%N_EL
      DIMS(2) = SPD_DEL(ISTA)%AZM%N_AZ
      DIMS(3) = SPD_DEL(ISTA)%N_TIM
!
...
!
      IF ( ABS(ARGS(1) - SPD_DEL(ISTA)%MAP_ARR(1)) < &
     &     EPS_MAR*(SPD_DEL(ISTA)%MAP_ARR(2) - SPD_DEL(ISTA)%MAP_ARR(1)) ) THEN
           INDS(1) = 1
         ELSE IF ( ABS(ARGS(1) - SPD_DEL(ISTA)%MAP_ARR(DIMS(1))) < &
     &             EPS_MAR*(SPD_DEL(ISTA)%MAP_ARR(2) - SPD_DEL(ISTA)%MAP_ARR(1)) ) THEN
           INDS(1) = DIMS(1) - 1
         ELSE 
           INDS(1) = INT ( (ARGS(1) - SPD_DEL(ISTA)%MAP_ARR(1))/ &
     &                     (SPD_DEL(ISTA)%MAP_ARR(2) - SPD_DEL(ISTA)%MAP_ARR(1)) ) &
     &               + 1
      END IF
!
      IF ( ABS(ARGS(2) - SPD_DEL(ISTA)%AZM%AZIM(1) ) < &
     &     EPS_MAR*(SPD_DEL(ISTA)%AZM%AZIM(2) - SPD_DEL(ISTA)%AZM%AZIM(1)) ) THEN
           INDS(2) = 1
        ELSE IF ( ABS(ARGS(2) - SPD_DEL(ISTA)%AZM%AZIM(DIMS(2))) < &
     &            EPS_MAR*(SPD_DEL(ISTA)%AZM%AZIM(2) - SPD_DEL(ISTA)%AZM%AZIM(1)) ) THEN
           INDS(2) = DIMS(2) - 1
        ELSE 
           INDS(2) = INT ( (ARGS(2) - SPD_DEL(ISTA)%AZM%AZIM(1))/ &
     &                     (SPD_DEL(ISTA)%AZM%AZIM(2) - SPD_DEL(ISTA)%AZM%AZIM(1)) ) &
     &               + 1
      END IF
!
      IF ( ABS(ARGS(3) - SPD_DEL(ISTA)%TIM_ARR(1) ) < &
     &     EPS_MAR*(SPD_DEL(ISTA)%TIM_ARR(2) - SPD_DEL(ISTA)%TIM_ARR(1)) ) THEN
           INDS(3) = 1
         ELSE IF ( ABS(ARGS(3) - SPD_DEL(ISTA)%TIM_ARR(DIMS(3))) < &
     &             EPS_MAR*(SPD_DEL(ISTA)%TIM_ARR(2) - SPD_DEL(ISTA)%TIM_ARR(1)) ) THEN
           INDS(3) = DIMS(3) - 1
         ELSE 
           INDS(3) = INT ( (ARGS(3) - SPD_DEL(ISTA)%TIM_ARR(1))/ &
     &                     (SPD_DEL(ISTA)%TIM_ARR(2) - SPD_DEL(ISTA)%TIM_ARR(1)) ) &
     &               + 1
      END IF
!
...
!
      IF ( INDS(2) < 1  .OR.  INDS(2) .GE. DIMS(2) ) THEN
           ...
           CALL CLRCH ( STR1 )
           WRITE ( UNIT=STR1(1:12), FMT='(1PD12.4)' ) SPD_DEL(ISTA)%AZM%AZIM(1)/DEG__TO__RAD
           CALL CLRCH ( STR2 )
           WRITE ( UNIT=STR2(1:12), FMT='(1PD12.4)' ) &
     &             SPD_DEL(ISTA)%AZM%AZIM(SPD_DEL(ISTA)%AZM%N_AZ)/DEG__TO__RAD
...
!
      DO 420 J2=1,SPD_DEL(ISTA)%MOD%N_RFR
         CALL VAL_DER_3D_BSPL4 ( ARGS, SPD__MDEG, DIMS, INDS, &
     &                      SPD_DEL(ISTA)%MAP_ARR, &
     &                      SPD_DEL(ISTA)%AZM%AZIM, &
     &                      SPD_DEL(ISTA)%TIM_ARR, &
     &                      SPD_DEL(ISTA)%DELS(1-SPD__MDEG,1-SPD__MDEG,1-SPD__MDEG,J2), &
...
!
         SPD_ZEN(J2) = VAL_1D_BSPL4 ( ARGS(3), SPD_DEL(ISTA)%N_TIM, &
     &                                SPD__MDEG, INDS(3), &
     &                                SPD_DEL(ISTA)%TIM_ARR, &
     &                                SPD_DEL(ISTA)%ZEN_DEL(1-SPD__MDEG,J2) )
 420  CONTINUE 
!
     ...
     &            SPD_DER(3,SPD__TOT)
      IND = -1
      IF ( SPD_DEL(ISTA)%MF%MF_NAME == SPD__WATS_STR ) THEN
     ...
     &                           SPD_RATE/SPD_ZEN(SPD__WAT)
        ELSE IF ( SPD_DEL(ISTA)%MF%MF_NAME == SPD__TOTS_STR ) THEN
           SPD_DELAY_DER_ZEN = SPD_VAL(SPD__TOT)/SPD_ZEN(SPD__TOT)
           SPD_RATE_DER_ZEN  = SPD_RATE/SPD_ZEN(SPD__WAT)
        ELSE IF ( SPD_DEL(ISTA)%MF%MF_NAME == SPD__NMFW_STR .OR. &
     &            SPD_DEL(ISTA)%MF%MF_NAME == SPD__NMFH_STR .OR. &
     &            SPD_DEL(ISTA)%MF%MF_NAME == SPD__GL_STR        ) THEN
           IF ( ELEV < P2I - EPS_MAR ) THEN
                IND = IXMN8 ( SPD_DEL(ISTA)%MF%L_NOD, SPD_DEL(ISTA)%MF%EL_ARG, ELEV )
                CALL VAL_DER_1D_BSPL ( ELEV, SPD_DEL(ISTA)%MF%L_NOD, &
     &                                            SPD__MDEG, IND, &
     &                                            SPD_DEL(ISTA)%MF%EL_ARG, &
     &                                            SPD_DEL(ISTA)%MF%MF_SPL, &
     &                                            SPD_DELAY_DER_ZEN, &
     &                                            SPD_RATE_DER_ZEN  )
              ELSE
      ...
      END IF
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!   write ( 6, 220 ) SPD_DEL(ISTA)%STA%NAME, ELEV/DEG__TO__RAD, SPD_VAL(SPD__WAT), SPD_ZEN(SPD__WAT), &
!     &              SPD_DELAY_DER_ZEN, IND, SPD_DEL(ISTA)%MF%MF_NAME
