      SUBROUTINE PLOT_REGION_R4 ( IDEV, IPAL, IPRC, NLON, NLAT, ARR_R4, &
     &                           TITLE, UNIT, FILOUT, &
...
! *                                                                      *
! * ### 09-OCT-2012  PLOT_REGION_R4  v2.0 (c) L. Petrov 11-JAN-2016 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE 
...
!
      IF ( VAL_MAX > VAL_MIN ) THEN
           ...
           RVAL_MAX = VAL_MAX
      END IF
      IF ( ( RVAL_MAX - RVAL_MIN) < EPS*RVAL_MIN ) THEN
...
!
      ...
      END IF
      WRITE ( 6, * ) 'RVAL_MIN/RVAL_MAX= ', RVAL_MIN, RVAL_MAX
...
!@           IF ( STR == "VAST" .OR. STR == 'vast' ) THEN
!@                PAP_SIZE = 2.0*PAP_SIZE
!@           END IF
         ELSE IF ( UDEV .EQ. 2 ) THEN
           PAP_SIZE = 270.0
         ELSE IF ( UDEV .EQ. 3 ) THEN
...
!
      DO 430 J3=1,MLON
         ...
         DO 440 J4=1,MLAT
            ...
            IF ( ABS(ARR_R4(IND_LON,IND_LAT)) > VAL_MAX_ALLOWED ) GOTO 440
            IF ( IPRC == 1 ) THEN
...
!
                 IX = INT ( (LAM/360.0)*MLON + 1.E-5 ) + 1
                 IF ( IX == 0  ) IX = MLON
                 IY = INT ( (PHI+90.0)/180.0*(MLAT-1) + 1.E-5 ) + 1
!
                 IF ( IY == 0  ) IY = 1
!
                 VAL_R4 = ARR_R4(IND_LON,IND_LAT)
