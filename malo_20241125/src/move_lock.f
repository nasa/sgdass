      SUBROUTINE MOVE_LOCK ( L_FIL, FINAM, FIL_POSTFIX, IUER )
! ************************************************************************
! *                                                                      *
! *   Routine MOVE_LOCK moves the set of L_FIL filenames FINAM with      *
...
! *                                   case of error.                     *
! *                                                                      *
! *  ### 10-DEC-2002    MOVE_LOCK   v1.2 (c)  L. Petrov 19-OCT-2019 ###  *
! *                                                                      *
! ************************************************************************
      IMPLICIT   NONE
!!      INCLUDE   'spd.i'
...
!
     ...
     &           FILE_WRITE_LOCK*128, FILIN*128, FILOUT*128, STR*128
      LOGICAL*4  LEX
      INTEGER*4  LUN, IOS, ID, IP, IS, STAT_BLOCK(16), &
     &           FD_READ_LOCK, FD_WRITE_LOCK, J1, J2, IER
...
!
     ...
     &                     SPD__LOCK_TIMEOUT, FD_READ_LOCK, FD_WRITE_LOCK, IER )
      IF ( IER .NE. 0 ) THEN
           CALL ERR_LOG ( 4821, IUER, 'MOVE_LOCK', 'Failure to set read '// &
...
!
         ...
         IF ( IP .LE. 1 ) THEN
              CALL CLRCH ( STR )
              CALL INCH  ( J2, STR )
              CALL ERR_LOG ( 4829, IUER, 'MOVE_LOCK', 'Trap of unternal '// &
...
!
                   CONTINUE 
                 ELSE
                   CALL ERR_LOG ( 4830, IUER, 'MOVE_LOCK', 'Error '// &
